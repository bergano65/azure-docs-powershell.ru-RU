...
## NОПИСАНИЕ
Командлет **Get-AzSqlServerAdvisor** получает один или несколько консультантов Azure SQL Server для Azure SQL Server.
...
Эта команда получает список всех рекомендаций для сервера с именем Wi-Runner-Австралия-Восток, который принадлежит к группе ресурсов с именем WIRunnersProd, которая начинается с буквы "d".
