...
## NОПИСАНИЕ
**Disable-AzFrontDoorCustomDomainHttps** отключает HTTPS для настраиваемого домена.

## ИЛЛЮСТРИРУЮТ

### Пример 1: отключение HTTPS для настраиваемого домена с помощью FrontDoorName и ResourceGroupName.
```powershell
...
Отключите HTTPS для собственного домена "frontendpointname1-Custom-XYZ" с FrontDoorName как "frontdoor1" и ResourceGroupName как "resourcegroup1".

### Пример 2: отключение HTTPS для настраиваемого домена с объектом PSFrontendEndpoint.
```powershell
...
Отключите HTTPS для настраиваемого домена с объектом PSFrontendEndpoint.

### Пример 3: отключение HTTPS для настраиваемого домена с ResourceId.
```powershell
...
Отключите HTTPS для собственного домена "frontendpointname1-Custom-XYZ" с ResourceId как $resourceId.
...
### -ResourceId
Идентификатор ресурса конечной точки передней дверцы для отключения HTTPS
