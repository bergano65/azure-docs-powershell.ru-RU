...
После создания шлюза он подключается к ExpressRouteSite с помощью команды New-AzExpressRouteConnection.

После этого соединение будет удалено с помощью имени соединения.
...
То же, что и в примере 1, но теперь он удаляет подключение, используя объект, переданный из командлета Get-AzExpressRouteConnection.
...
### -InputObject
Объект ExpressRouteConenction, который нужно обновить.
...
### -ResourceId
Идентификатор ресурса объекта ExpressRouteConenction, который нужно удалить.
