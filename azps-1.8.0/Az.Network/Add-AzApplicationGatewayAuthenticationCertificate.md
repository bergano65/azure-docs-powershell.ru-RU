...
## NОПИСАНИЕ
Командлет **Add-AzApplicationGatewayAuthenticationCertificate** добавляет сертификат проверки подлинности в шлюз приложений Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Добавление сертификата проверки подлинности в шлюз приложений
```
...
Вторая команда добавляет сертификат проверки подлинности с именем cert01 в шлюз приложений.
Третья команда обновляет шлюз приложения.

## ПАРАМЕТРЫ

### -ApplicationGateway
Указывает имя шлюза приложений, для которого этот командлет добавляет сертификат для проверки подлинности.
...
### -CertificateFile
Указывает путь к сертификату проверки подлинности, который добавляет этот командлет.
...
### -Name (имя)
Указывает имя сертификата, который добавляется этим командлетом в шлюз приложений.
