...
## NОПИСАНИЕ
Командлет **Set-AzIntegrationAccountAssembly** изменяет сборку учетной записи интеграции.

## ИЛЛЮСТРИРУЮТ

### Пример 1: изменение сборки с помощью локального файла
```powershell
...
Изменяет сборку с именем "sampleAssembly" с помощью локального файла, расположенного в пути к файлу, который хранится в разделе "$localAssemblyFilePath".

### Пример 2: изменение сборки с помощью байтовых данных
```powershell
...
Изменяет сборку с именем "sampleAssembly", используя массив байтов, содержащийся в "$assemblyContent".

### Пример 3: изменение сборки с помощью ссылки на контент
```powershell
...
Изменяет сборку с именем "sampleAssembly", используя байтовые данные, расположенные по URL-адресу "$assemblyUrl". Это предлагаемый метод для создания сборок крупного размера.

## ПАРАМЕТРЫ

### -AssemblyData
Данные байт сборки для учетной записи интеграции.
...
### -ContentLink
Общедоступная ссылка на данные сборки учетной записи интеграции.
...
### -Name (имя)
Имя сборки для учетной записи интеграции.
...
### -ResourceId
Идентификатор ресурса сборки учетной записи интеграции.
