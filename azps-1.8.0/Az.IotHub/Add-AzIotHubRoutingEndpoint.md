...
## NОПИСАНИЕ
Добавьте новую конечную точку в центр Интернета вещей. Чтобы узнать, какие конечные точки поддерживаются, ознакомьтесь со статьей https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-devguide-endpoints
...
Добавьте новую конечную точку E2 для типа EventHub в центр Интернета вещей "myiothub".
...
Добавьте новую конечную точку S1 для типа AzureStorageContainer в центр Интернета вещей "myiothub".

## ПАРАМЕТРЫ

### -ConnectionString
Строка подключения конечной точки маршрутизации
...
Группа ресурсов для конечной точки изображение
...
Идентификатор подписки ресурса конечной точки
...
### -Encoding
Выберите формат, в котором вы хотите перенаправлять данные. Вы можете выбрать JSON или AVRO. По умолчанию выбрано значение AVRO.
