...
## КРАТКИй обзор
Переносит группу безопасности сети в стек диспетчера ресурсов Azure.
...
## NОПИСАНИЕ
Командлет **Move-AzureNetworkSecurityGroup** переносит группу безопасности сети в группу ресурсов в стеке диспетчера ресурсов Azure.
...
### -Abort
Указывает, что этот командлет отменяет миграцию сетевой группы безопасности.
...
### -Commit
Указывает на то, что этот командлет запускает миграцию сетевой группы безопасности.
...
Указывает имя группы безопасности сети, на которую переносится этот командлет.
...
### -Подготовка
Указывает на то, что этот командлет подготавливает группу безопасности сети для миграции.
...
### -Проверка
Указывает, что этот командлет проверяет сетевую группу безопасности для миграции.
