...
## КРАТКИй обзор
Переносит таблицу маршрутов в стек диспетчера ресурсов Azure.
...
## NОПИСАНИЕ
Командлет **Move-AzureRouteTable** переносит таблицу маршрутов в группу ресурсов в стеке диспетчера ресурсов Azure.
...
### -Abort
Указывает, что этот командлет отменяет миграцию таблицы маршрутов.
...
### -Commit
Указывает на то, что этот командлет запускает миграцию таблицы маршрутов.
...
### -Подготовка
Указывает на то, что этот командлет подготавливает таблицу маршрутов для миграции.
...
### -RouteTableName
Указывает имя таблицы маршрутов, которая переносится этим командлетом.
...
### -Проверка
Указывает, что этот командлет проверяет таблицу маршрутов для миграции.
