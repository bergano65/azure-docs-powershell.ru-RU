...
## SYNOPSIS
Создание объекта профиля защиты от восстановления сайта.
...
## ОПИСАНИЕ
С **его использованием** создается объект профиля защиты от восстановления сайтов Azure.
Этот cmdlet создает объект **ASRProtectionProfile** для использования с другими.
...
Указывает на то, что профиль защиты позволяет удалять репликируемые объекты.
...
Определяет частоту в часах для согласованных моментальных снимков приложения.
...
Определяет тип проверки подлинности.
Допустимыми значениями для этого параметра являются Сертификат и Kerberos.
...
Указывает имя учетной записи хранилища Azure, на которой будет храниться объект репликации Azure.
...
Определяет ИД подписки Azure для учетной записи хранения.
Этот параметр ссылается на учетную запись, для которой нужно сохранить объект репликации Azure.
...
### -RecoveryPoints
Количество часов для сохранения точек восстановления.
...
Определяет частоту репликации (в секундах). Допустимые значения этого параметра:
...
### -ReplicationMethod
Определяет способ репликации. Допустимые значения этого параметра:
...
### -ReplicationPort
Определяет номер порта, для которого происходит репликация.
...
### -ReplicationProvider
Определяет тип поставщика репликации. Допустимые значения этого параметра: HyperVReplica и HyperVReplicaAzure.
...
Укажите время в течение 24 часов после начала работы.
