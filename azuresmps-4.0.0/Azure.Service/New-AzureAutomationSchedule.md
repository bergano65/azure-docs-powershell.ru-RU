...
Командлет **New-AzureAutomationSchedule** создает расписание в службе автоматизации Microsoft Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание одноразового расписания
```
PS C:\> New-AzureAutomationSchedule -AutomationAccountName "Contoso17" -Name "Schedule01" -StartTime "23:00" -OneTime
```

Следующая команда создает расписание, которое выполняется один раз в текущую дату на 11:00 PM.
...
Следующие команды создают новое расписание, которое запускается в 1:00 PM ежедневно для одного года, начиная с текущего дня.
...
### -ExpiryTime
Указывает время истечения срока действия расписания.
Строка может быть указана, если ее можно преобразовать в допустимый **DateTime**.
...
### -OneTime
Указывает на то, что эта операция создает единовременно разовое расписание.
...
### -StartTime
Указывает время начала расписания.
Строка может быть указана, если ее можно преобразовать в допустимый **DateTime**.
