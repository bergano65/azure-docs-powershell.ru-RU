...
## КРАТКИй обзор
Удаление объектов виртуальной машины.
...
Командлет **Remove-WAPackVM** удаляет объекты виртуальной машины.
...
Вторая команда удаляет виртуальную машину, хранящуюся в $VirtualMachine.
...
### -Force
Указывает, что командлет удаляет виртуальную машину без запроса подтверждения.
...
### -PassThru
Указывает на то, что командлет возвращает логическое значение.
Если операция выполнена успешно, командлет возвращает значение $True.
