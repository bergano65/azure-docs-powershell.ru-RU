...
## КРАТКИй обзор
Удаление тома с устройства StorSimple.
...
## NОПИСАНИЕ
Командлет **Remove-AzureStorSimpleDeviceVolume** удаляет том с устройства StorSimple.
Этот командлет запрашивает подтверждение, если не указан параметр *Force* .

## ИЛЛЮСТРИРУЮТ

### Пример 1: Удаление тома с помощью конвейера
```
...
Эта команда получает том с именем Volume18 на устройстве с именем Contoso63-AppVm, а затем передает этот том в текущий командлет с помощью оператора конвейера.
Текущий командлет запускает задачу, которая удаляет том, а затем возвращает объект **TaskResponse** .
Чтобы просмотреть состояние задачи, используйте командлет **Get-AzureStorSimpleTask** .
В команде не указан параметр *Force* , поэтому командлет запрашивает подтверждение.

### Пример 2: Удаление тома без подтверждения
```
...
Эта команда удаляет том с именем Volume18 с устройства с именем Contoso63-AppVm.
Команда задает параметр *Force* , поэтому командлет не запрашивает подтверждение.

## ПАРАМЕТРЫ

### -Имя_устройства
Указывает имя устройства StorSimple, на котором находится том, который нужно удалить.
...
### -Volume
Задает удаляемый том, как объект **VirtualDisk** .
Чтобы получить объект **VirtualDisk** , используйте командлет **Get-AzureStorSimpleDeviceVolume** .
...
### -Тома
Указывает имя тома, который нужно удалить.
...
### VirtualDisk
Этот командлет принимает либо объект **VirtualDisk** , либо имя тома **VirtualDisk** , который нужно удалить.
