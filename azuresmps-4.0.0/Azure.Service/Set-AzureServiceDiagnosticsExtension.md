...
## КРАТКИй обзор
Включает расширение диагностики Azure на указанных ролях или всех ролях в развернутой службе или в развертывании.
...
## NОПИСАНИЕ
Командлет **Set-AzureServiceDiagnosticsExtension** включает расширение диагностики Azure на указанных ролях или всех ролях в развернутой службе или в развертывании.
...
Задает массив конфигураций для диагностики Azure.
...
Указывает конфигурацию для диагностики Azure.
Вы можете загрузить схему, выполнив следующую команду: 
...
### -Role (роль)
Задает необязательный массив ролей, для которого нужно задать конфигурацию диагностики Azure.
Если этот параметр не указан, конфигурация диагностики применяется как конфигурация по умолчанию для всех ролей.
...
### -ThumbprintAlgorithm
Указывает алгоритм хэширования отпечатка, который используется с отпечатком для идентификации сертификата.
...
### -X509Certificate
Задает сертификат X. 509, который, при указании, автоматически отправляется в облачную службу и используется для шифрования закрытой конфигурации расширения.
