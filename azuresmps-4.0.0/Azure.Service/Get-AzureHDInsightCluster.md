...
Командлет **Get-AzureHDInsightCluster** получает кластеры служб Azure HDInsight для текущей подписки.
Вы можете использовать параметр *Name* для получения определенного кластера.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение кластеров в подписке
```
...
### -Name (имя)
Указывает имя получаемого кластера HDInsight.
