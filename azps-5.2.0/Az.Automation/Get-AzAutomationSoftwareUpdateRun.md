...
## ОПИСАНИЕ
Этот Get-AzAutomationSoftwareUpdateRun возвращает список запусков обновления программного обеспечения. Так как у конфигурации обновления программного обеспечения есть связанное расписание, его можно запускать несколько раз. При каждом запуске расписания создается обновление. Запуск обновления — это агрегат результатов всех запусков компьютера. Вы можете получить запуск для определенной конфигурации обновления программного обеспечения, пройдя параметр SoftwareUpdateConfigurationName. Чтобы получить определенные запуски, необходимо пройти параметр name. Вы также можете перечислить запуск с определенным состоянием, целевую работу с определенной операционной системой или запуск после определенного времени путем передачи соответствующего параметра.

## ПРИМЕРЫ

### Пример 1
В этом примере все обновления запускаются с помощью определенной конфигурации обновления программного обеспечения.
...
### -Id
Ид конфигурации обновления программного обеспечения.
...
Запуск запуска вызван конфигурацией обновления программного обеспечения.
...
Имя конфигурации обновления программного обеспечения, активировали запуск.
...
### -StartTime
Минимальное время начала запуска.
