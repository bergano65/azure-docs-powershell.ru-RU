...
## ОПИСАНИЕ
**Cmdlet Set-AzResourceLock** изменяет блокировку ресурса.
...
Например, чтобы указать базу данных, используйте следующий формат: имя сервера базы данных с ИД ресурса подписки. Чтобы указать группу ресурсов, используйте следующий формат: имя группы ресурсов «ИД `/subscriptions/` `/resourceGroups/` `/providers/Microsoft.Sql/servers/` `/databases/` `/subscriptions/` `/resourceGroups/` подписки»
