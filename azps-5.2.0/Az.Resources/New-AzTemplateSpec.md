...
## ОПИСАНИЕ
Создает новую версию шаблона с указанным содержимым ARM шаблона. Содержимое может быть либо из необработанных строк JSON (с помощью набора параметров **FromJsonStringParameterSet),** либо из указанного файла JSON (с помощью набора параметров **FromJsonFileParameterSet).**  

Если корневая спецификация шаблона не существует, она будет создана вместе с версией Шаблон спецификации. Если спецификация шаблона уже существует с указанным именем, она и указанная версия будут обновлены (все другие существующие версии будут сохранены).
...
Создает новую версию спецификации шаблона "версия 1.0" в шаблоне Spec с именем myTemplateSpec. В указанной версии $templateJson как содержимое шаблона ARM версии.

 **Примечание.** Шаблон ARM в этом примере не имеет никакого никакого ресурса, так как он не содержит фактических ресурсов.
...
Создает новую версию спецификации шаблона "версия 2.0" в шаблоне Spec с именем myTemplateSpec. В качестве содержимого шаблона для указанной версии будет "myTemplateContent.jsARM".
...
### -Force
Не спрашивайте подтверждения при переописи существующей версии.
...
### -Tag
Количество тегов для нового шаблона спецификаций ресурсов.
...
### -TemplateJson
Шаблон Диспетчера ресурсов Azure JSON.
