...
## ОПИСАНИЕ
Для **этого создается группа** управления.
...
Создание новой группы с `DisplayName` установленным и `ParentId` установленным для нее `null` Клиентом будет тот же, что и у родительской `DisplayName` `GroupName` группы.  

### Пример 2. Создание группы управления с отображаемой именем
```
...
В этом случае родительской группой будет клиент, а для группы будет установлено `DisplayName` заданный значение.

### Пример 3. Создание группы управления с родительским и отображаемой именем
```
...
### Пример 4. Создание группы управления с родительским объектом
```
...
### -DisplayName
Отображаемая группа управления
...
### -ParentId
Родительский ИД группы управления
