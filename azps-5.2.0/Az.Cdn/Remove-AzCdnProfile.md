...
## ОПИСАНИЕ
Для удаления профиля сети доставки содержимого (CDN) Azure удаляется **cmdlet Remove-AzCdnProfile.**
...
### -CdnProfile
Определяет профиль, который удаляет этот cmdlet.
...
### -ProfileName
Указывает имя профиля, который удаляет этот cmdlet.
