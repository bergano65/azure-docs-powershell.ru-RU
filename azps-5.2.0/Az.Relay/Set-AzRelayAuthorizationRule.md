...
## ОПИСАНИЕ
Командлет **Set-AzRelayAuthorizationRule** обновляет описание указанного правила авторизации для указанных сущностями relay (Namespace/WcfRelay/HybridConnection).

## ПРИМЕРЫ

### Пример 1.1. Пространство имен с inputObject
```
PS C:\>
...
Добавляет **"Отправить"** из прав на доступ правила авторизации `AuthoRule1` в области `TestNameSpace-Relay1` имен.

### Пример 1.2. Пространство имен с параметром Rights
```
...
Добавляет **"Отправить"** в права на доступ правила авторизации `AuthoRule1` для WcfRelay. `TestWCFRelay1`
...
### Пример 3.1. Гибридное подключение с inputObject
```
...
Добавляет **"Отправить"** в права доступа правила авторизации `AuthoRule1` гибридной `TestHybridConnection` сети.

### Пример 3.2. Гибридное подключение с параметром Rights
```
...
Добавляет **"Отправить"** в права доступа к правилу авторизации `AuthoRule1` гибридной `TestHybridConnection` сети.
...
### -InputObject
Объект Relay AuthorizationRule.
...
### -Rights
Права, например @("Прослушивание";"Отправить";"Управление")
