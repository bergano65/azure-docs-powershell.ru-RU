...
### Пример 2. Включить шифрование в рабочей области Databspaces
```powershell
...
1.
Обновление рабочей области `-PrepareEncryption` (если она не была создана).
1.
Результаты `StorageAccountIdentityPrincipalId` последнего шага.
Предоставить основные разрешения для основной суммы.
1.
Еще раз обновите рабочее пространство, чтобы в нем появилась информация о ключе шифрования.
    - `-EncryptionKeySource`
...
Возможные значения (без учета деления): Default, Microsoft.Keyvault
