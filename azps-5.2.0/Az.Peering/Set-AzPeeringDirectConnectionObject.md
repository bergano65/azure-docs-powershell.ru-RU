...
## ОПИСАНИЕ
Используется в сочетании с Update-AzPeering, это операция в памяти и сохраняется только с `Update-AzPeering` . 

## ПРИМЕРЫ

### Пропускная способность для обновления
```powershell
...
Обновляет пропускную способность первого подключения в объекте пиринга в памяти. 
...
Обновляет адрес пиринга для первого подключения в объекте пиринга в памяти. 

### Обновление службы пиринга
```powershell
...
Обновляет подключение для использования со службой пиринга.
