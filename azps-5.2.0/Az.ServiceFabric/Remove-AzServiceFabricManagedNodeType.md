...
## ОПИСАНИЕ
Удалите тип узла или конкретные узлы для этого типа. Если используется паремтер -NodeName, будут удалены только указанные узлы.
...
Используйте эту задачу с осторожностью, так как это может привести к потере данных, если на узлах запущена рабочая нагрузка с состоянием, или привести к сужением кластера, если после пропадания не хватает узлы с данными.
