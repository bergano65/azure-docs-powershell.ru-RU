...
## SYNOPSIS
Задает свойства ресурсов типа узла или запуск действий повторного запуска в определенных точках типа узла с параметром -Reimage.
...
## ОПИСАНИЕ
Задает свойства ресурсов типа узла или запуск действий повторного запуска в определенных точках типа узла с параметром -Reimage. При повторном перенастройке узлы с тканью службы будут отключены перед повторной сборкой vms и снова включены после их повторного включения. Если это делается для основных типов узлов, может потребоваться некоторое время, так как может потребоваться не переумногание всех узлов одновременно. Используйте -ForceReimage принудительно, даже если сервисный материал не может отключить узлы, но используйте с осторожностью, так как это может привести к потере данных, если на узле работают рабочие нагрузки с состоянием.
...
Обновив количество экземпляров для типа узла.
...
Обновите собственные места размещения узла. При этом будут переописываться старые при размещении, если они есть.
...
Обновив количество экземпляров для типа узла, обновив тип piping.

## PARAMETERS

### -ApplicationEndPort
Порт окончания приложения для диапазона портов.
...
### -Capacity
Теги емкости, примененные к узлам в типе узла в качестве пар ключа и значения, диспетчер ресурсов кластера использует эти теги, чтобы понять, сколько ресурсов имеет узел. При обновлении значения будут переопределяться.
...
### -EpисерalStartPort
Эперитеральный начните порт диапазона портов.
...
### -ForceReimage
Если использовать этот флажок, удаление будет принудительно, даже если сервисная ткань не сможет отключить узлы.
Используйте с осторожностью, так как это может привести к потере данных, если на узле работают рабочие нагрузки с состоянием.
...
### -PlacementProperty
Теги размещения, применяемые к узлам в типе узла в качестве пар ключа и значения, которые могут использоваться для указать, где должны запускаться определенные службы (рабочая нагрузка). При обновлении значения будут переопределяться.
...
### -Reimage
Укажите, нужно ли переустанавлить узлы для типа узла.
