...
Эта команда извлекает секретную папку MySecret из хранилища ключей MyKeyVault и сохраняет ее резервную копию в файл, который автоматически именуется вам, и отображает имя файла.

### Пример 2. Архивная копия секретного файла с указанным именем файла, переописав существующий файл без запроса
```powershell
...
### -Force
Запросит подтверждение перед переописью выходного файла, если он существует.
...
### -Name
Имя секретного, который нужно сделать.
