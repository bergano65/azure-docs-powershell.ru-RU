...
Создайте новый чертеж определения в указанной подписке с помощью определенного словаря параметров и `$blueprintObject` групп ресурсов. Используется удостоверение, назначенное системе. Расположение определяет регион для создания управляемого удостоверения.
