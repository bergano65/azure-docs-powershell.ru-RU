...
## ОПИСАНИЕ
Для базы данных Azure SQL параметры аудита с SQL **Set-AzSqlDataBaseAudit.**
...
### Пример 3. Политика аудита хранилища BLOB-SQL azure с расширенным фильтром с использованием предиката T-SQL.
```powershell
...
### Пример 4. Удаление параметра расширенных фильтров из политики аудита базы данных Azure SQL
```powershell
...
RECEIVE  
REFERENCES  
Общая форма определения действия для аудита: [действие] ON [object] BY [principal] Note that [object] в формате выше может ссылаться на объект, например таблицу, представление или хранимую процедуру, либо всю базу данных или схему. В последних случаях используются формы DATABASE:[dbname] и SCHEMA::[schemaname] соответственно.
Например:  
