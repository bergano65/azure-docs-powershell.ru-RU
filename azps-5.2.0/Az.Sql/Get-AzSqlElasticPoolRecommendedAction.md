...
## ОПИСАНИЕ
Cmdlet **Get-AzSqlElasticPoolRecommendedAction** получает одно или несколько рекомендуемых действий для помощника по SQL Azure.

## ПРИМЕРЫ

### Пример 1. Список всех рекомендуемых действий для помощника
```
...
Эта команда получает список всех рекомендуемых действий помощника CreateIndex, доступных для эластичного пула wiRunnerPool.
...
Эта команда получает рекомендуемое действие IR_ \[ test_schema \] _ \[ test_table_0.0361551 _6C7AE8CC9C87E7FD5893 от помощника \] CreateIndex.

## PARAMETERS

### -AdvisorName
Указывает имя консультанта, для которого этот cmdlet запрашивает рекомендуемые действия.
...
### -ElasticPoolName
Определяет имя эластичного пула, для которого этот cmdlet запрашивает рекомендуемые действия.
...
## ПРИМЕЧАНИЯ
* Ключевые слова: azure, azurerm, arm, resource, management, manager, sql,элестика, mssql, advisor, recommendedaction
