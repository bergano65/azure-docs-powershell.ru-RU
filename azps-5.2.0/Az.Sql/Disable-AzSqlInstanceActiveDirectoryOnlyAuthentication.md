...
## ОПИСАНИЕ
Из-за **cmdlet Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication** отключает только требования проверки подлинности Azure Active Directory (Azure AD) для экземпляра AzureSQL, управляемого в текущей подписке.
...
Эта команда отключает только требования проверки подлинности Azure Active Directory (Azure AD) для управляемого экземпляра AzureSQL ManagedInstance01, связанного с группой ресурсов ResourceGroup01.
...
### -InstanceName
Имя управляемого экземпляра Azure SQL проверки подлинности Azure Active Directory.
