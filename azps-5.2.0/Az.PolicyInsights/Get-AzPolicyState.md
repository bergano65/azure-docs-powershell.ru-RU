...
Возвращает все исторические записи об состояниях политики (включая последние), созданные в последний день для всех ресурсов в рамках подписки в текущем контексте сеанса.
...
### Пример 8. Последние состояния политики определения набора политик в текущей подписке
```powershell
