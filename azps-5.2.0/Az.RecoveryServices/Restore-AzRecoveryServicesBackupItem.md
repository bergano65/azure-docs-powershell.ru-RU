...
Восстановление данных и конфигурации для элемента резервной копии до указанной точки восстановления. Необходимые параметры зависят от типа элемента резервной копии.
...
> [!IMPORTANT]
> По умолчанию резервное копирование VM Azure резервное копирование делает резервные копии всех дисков. Во время включения резервного копирования можно выборочно резервные копии соответствующих дисков с использованием параметров exclusionList или InclusionList. Возможность выборочного восстановления дисков доступна только в том случае, если для одного из них была выборочно размещена их одна из них.
...
Первая команда получает хранилище RecoveryServices и сохраняет его в $vault переменной.
Вторая команда получает элемент резервного копирования и сохраняет его в $BackupItem переменной.
Третья команда получает дату от семи дней назад и сохраняет ее в переменной $StartDate.
Четвертая команда получает текущую дату, а затем сохраняет ее в $EndDate переменной.
Пятая команда получает список точек восстановления для определенного элемента резервной копии, отфильтрованного по $StartDate и $EndDate.
...
Первая команда получает хранилище служб восстановления и сохраняет его в $vault переменной.
Вторая команда получает элемент backup named fileshareitem и сохраняет его в переменной $BackupItem.
...
### Пример 7. Восстановление SQL в VM-объекте Azure в другой целевой VM-платформе для точки восстановления журнала
...
Этот переключатель используется для указания функции "Восстановить как неугодные диски"
...
Папку, в которой нужно восстановить папку в targetFileShareName. Если нужно восстановить его в корневую папку, придате значения целевой папки пустой строке.
...
Группа ресурсов, в которую восстанавливаются управляемые диски. Применимо к резервному копированию VM-дисков
