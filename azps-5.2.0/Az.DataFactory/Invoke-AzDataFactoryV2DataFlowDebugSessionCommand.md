...
## ОПИСАНИЕ
Эта команда выполняет предварительный просмотр данных, предварительный просмотр статистики и выражение для разного потока данных во время сеанса отбивки.
Последовательность команд PowerShell для рабочего процесса отлагки потоков данных должна быть:
1. Start-AzDataFactoryV2DataFlowDebugSession
1. Add-AzDataFactoryV2DataFlowDebugSessionPackage
1. Invoke-AzDataFactoryV2DataFlowDebugSessionCommand (повторите этот шаг для разных команд или целей или повторите шаг 2–3, чтобы изменить файл пакета)
...
В этом примере вызывается команда предварительного просмотра данных для сеанса отладки "fd76cd0d-8b37-4dc0-a370-3f9d43ac686d" в фабрике данных "WiKiADF", а затем преобразует результаты JSON в удобочитаемую строку.
...
### -Command
Команда отбивки потока данных. Optionals are executePreviewQuery, executeStatisticsQuery and executeExpressionQuery
...
### -RowLimits
Ограничение строк для предварительного просмотра потока данных.
...
### -SessionId
ИД сеанса отлагки потоков данных.
...
## ПРИМЕЧАНИЯ
Ключевые слова: azure, azurerm, arm, resource, management, manager, data, factoriesKeywords: azure, azurerm, arm, resource, management, manager, data, factories
