...
## ОПИСАНИЕ
Cmdlet **Get-AzDataFactoryV2ActivityRun** получает сведения о запусках в фабрике данных Azure для указанного запуска конвейера, который произошел в указанный период времени. Кроме того, можно указать фильтры для имени действия, связанного имени службы, который выполнил выполнение, и состояние выполнения.

## ПРИМЕРЫ

### Пример 1. Все действия для запуска конвейера
```
...
Эта команда получает сведения обо всех действиях, которые выполняется в конвейере с помощью ID "f288712d-fb08-4cb8-96ef-82d3b9b30621", которые произошли между "2017-09-01" и "2017-09-30".
...
### -RunStartedAfter
Время запуска (или после запуска) конвейера.
...
### -RunStartedBefore
Время запуска конвейера или до его запуска.
