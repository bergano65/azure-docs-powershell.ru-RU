...
## ОПИСАНИЕ
Для запуска триггера в фабрике данных запускается триггер **Start-AzDataFactoryV2Trigger.** Если триггер находится в состоянии "Остановлено", запускается триггер и в конечном итоге запускается конвейеры на основе его определения. Если триггер уже находится в состоянии "Запущено", этот cmdlet не будет действовать. Если задан параметр Force, то перед запуском триггера не запускается проектлет.
...
Запуск триггера ScheduledTrigger в фабрике данных "WikiADF".
