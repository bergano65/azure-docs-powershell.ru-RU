...
## ОПИСАНИЕ
Обновляет примененные области `Reservation` действия.
...
### -AppliedScope
SubscriptionId, `Reservation` который нужно применить
...
### -InstanceFlexibility
При этом обновляется значение висячивости экземпляра `Reservation` . Если значение не задано, существующее значение не изменяется.
...
### -ReservationOrderId
ИД `ReservationOrder` обновления
