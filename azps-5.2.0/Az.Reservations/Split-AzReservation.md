...
## ОПИСАНИЕ
Разделить `Reservation` 1 на два `Reservation` с помощью указанного распределения количества.
...
Разделение указанного количества `Reservation` на два с соответствующим `Reservation` количеством
...
### -Quantity
С разделителем — запятые для поля "Количество" двух `Reservation`
...
### -Резервирование
Параметр "Pipe object for" (Объект-труба) для `Reservation`
...
### -ReservationOrderId
Id of the `ReservationOrder` that contains the user wants to `Reservation` split
