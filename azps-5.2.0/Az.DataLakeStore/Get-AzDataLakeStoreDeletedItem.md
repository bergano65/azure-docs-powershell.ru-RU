...
Операция может быть длительной, так как для нее может потребоваться поиск миллионов файлов в корзине, и ее можно запустить как задание.
...
### -Count
Количество результатов, которые нужно найти пользователю. Запрос выполняется до тех пор, пока не найдет результаты count или не будет ничего делать в корзине ( в зависимости от того, что произойдет раньше).
