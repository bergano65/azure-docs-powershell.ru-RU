...
### UserOrSystemAssignedEncryption (по умолчанию)
```
...
## ОПИСАНИЕ
Для создания учетной записи Data Lake Store будет создаваться новый **cmdlet AzDataLakeStoreAccount.**
...
Эта команда создает учетную запись Магазина данных с именем ContosoADL для расположения на востоке США.

## PARAMETERS

### -DefaultGroup
Определяет ИД объекта группы AzureActive Directory, который будет по умолчанию владельцем группы для новых файлов и папок.
...
### -DisableEncryption
Означает, что к учетной записи не будет применено шифрование.
...
### -Location
Определяет расположение для учетной записи.
...
### -Name
Указывает имя создаемой учетной записи.
...
### -ResourceGroupName
Имя группы ресурсов, которая содержит учетную запись.
