...
## ОПИСАНИЕ
Cmdlet **Remove-AzDataLakeStoreItemAclEntry** удаляет запись (ACE) из списка управления доступом (ACL) файла или папки в магазине Data Lake Store.

## ПРИМЕРЫ

### Пример 1. Удаление записи пользователя
```
...
Эта команда удаляет пользователя ACE для Facebook Fuller из учетной записи ContosoADL.

### Пример 2. Удаление записи пользователя рекурсивно
```
...
### Пример 3. Удаление разрешений для рекурсивно-рекурсивного ACE с помощью объекта ACL
```
...
Эта команда удаляет из корневого файла ACE пользователя ACE для Facebook Fuller и рекурсивно удаляется из всех его поднаправлений и файлов для учетной записи ContosoADL.
...
### -AceType
Определяет тип ACE, который нужно удалить.
...
### -ACL
Определяет объект ACL, содержащий удаляемые записи.
...
### -По умолчанию
Указывает на то, что эта операция удаляет ACE по умолчанию из указанного значения ACL.
...
### -Id
Определяет ИД объекта пользователя, группы или основной службы AzureActive Directory, для которого требуется удалить ACE.
...
### -Path
Путь к элементу, из которого нужно удалить ACE, начиная с корневого каталога (/).
...
### -Recurse
Указывает, что ACL удаляется рекурсивно для поднаправлений и файлов ребенка.
...
### -ShowProgress
Если он прошел, показывается состояние хода выполнения. Применимо только в случае удаления рекурсивного ACL-удаления.
