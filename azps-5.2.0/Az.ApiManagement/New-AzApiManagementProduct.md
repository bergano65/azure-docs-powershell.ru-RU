...
## ОПИСАНИЕ
Для создания продукта управления API создается новый **azApiManagementProduct.**

## ПРИМЕРЫ

### Пример 1. Создание продукта без подписки
```
...
### Пример 2. Создание продукта, который требует подписки и утверждения
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Требуется подписка и утверждение.
Эта команда задает для периода уведомлений 10 дней.
Длительность подписки составляет один год.

## PARAMETERS

### -ApprovalRequired
Указывает, требуется ли утверждение подписки на продукт.
По умолчанию этот параметр **$False.**
...
### -ProductId
Определяет идентификатор нового продукта.
Если этот параметр не указан, будет создан новый продукт.
...
psdx_paramvalues
- NotPublished
- Опубликовано значение по умолчанию NotPublished.
