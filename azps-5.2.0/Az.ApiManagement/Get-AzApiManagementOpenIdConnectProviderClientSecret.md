...
### Пример 1. Получите секрет клиента поставщика с помощью ИД
```
...
Эта команда является секретом клиента поставщика ID OICProvider01.
...
Идентификатор поставщика OpenID Connect.
Этот параметр является required(обязательно).
