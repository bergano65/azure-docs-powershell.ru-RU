...
## ОПИСАНИЕ
Для удаления из существующего шлюза конфигурация имени хоста **Remove-AzApiManagementGatewayHostnameConfiguration** удаляется.

## ПРИМЕРЫ

### Пример 1. Удаление существующей конфигурации имени шлюза
```powershell
...
Эта команда удаляет существующую конфигурацию имени шлюза и не запросит подтверждение.
...
Идентификатор существующей конфигурации имени шлюза.
Этот параметр является required(обязательно).
...
### -InputObject
Экземпляр PsApiManagementGatewayHostnameConfiguration. Этот параметр является required(обязательно).
...
### -ResourceId
Arm ResourceId шлюзаHostnameConfiguration. Этот параметр является required(обязательно).
