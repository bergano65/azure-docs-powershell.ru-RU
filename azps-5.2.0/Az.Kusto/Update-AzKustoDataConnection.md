...
### UpdateExpandedEventHub (по умолчанию)
```
...
### Пример 1. Обновление существующего подключения к данным EventHub
```powershell
...
С этой командой обновляется существующее подключение к данным EventHub с именем myeventhubdc для базы данных mykustodatabase в кластере testnewkustocluster.

### Пример 2. Обновление существующего подключения к данным EventGrid
```powershell
...
Команда выше обновляет существующее подключение к данным EventGrid "myeventgriddc" для базы данных mykustodatabase в кластере testnewkustocluster.

### Пример 3. Обновление существующего подключения к данным IotHub
```powershell
...
С этой командой обновляется существующее подключение к данным IotHub с именем myiothubdc для базы данных mykustodatabase в кластере testnewkustocluster.

### Пример 4. Обновление существующего подключения к данным EventHub с помощью удостоверения
```powershell
...
### Пример 5. Обновление существующего подключения к данным EventGrid с помощью удостоверения
```powershell
...
### Пример 6. Обновление существующего подключения к данным IotHub с помощью удостоверения
```powershell
...
Имя обработки события типа события хранилища BLOB-типов.
...
### -Сжатие
Центр событий сообщает о сжатии.
...
### -ConsumerGroup
Группа потребительских событий/iot-концентратора.
...
При желании формат данных можно добавить в каждое сообщение.
...
### -EventHubResourceId
ИД ресурса в центре событий, который используется для создания подключения к данным или сетки событий, настроен для отправки событий.
...
### -EventSystemProperty
Системные свойства концентратора событий и iot.
...
### -IgnoreFirstRecord
Если установлено истинное, означает, что при чтении следует игнорировать первую запись каждого файла.
...
### -IotHubResourceId
ИД ресурса концентратора IOT, который будет использоваться для создания подключения к данным.
...
### -MappingRuleName
Правило сопоставления, используемее для передачи данных.
При желании сведения о сопоставлении можно добавить в каждое сообщение.
...
ИД ресурса учетной записи хранения, в которой находятся данные.
...
### -TableName
Таблица, в которой нужно ввести данные.
При желании в каждое сообщение можно добавить сведения из таблицы.
