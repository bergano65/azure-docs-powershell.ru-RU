...
### -JobManagerTask
Определяет задачу диспетчера вакансий.
...
Пакетная служба выполняет задачу "Подготовка задания" на узле компьютеров перед выполнением любых задач этого задания в этом узле.
...
Пакетная служба выполняет задачу "Выпуск задания" на каждом узле компьютеров, где она выполняет любую задачу задания.
...
### -OnAllTasksComplete
Указывает действие, выполненное службой обработки пакетов, если все задачи задачи в задании находятся в завершеном состоянии.
...
### -OnTaskFailure
Указывает действие, необходимое для обработки пакетной службы в случае сбой какой-либо задачи в задаче.
...
Допустимые значения: integers from -1000 to 1000.
