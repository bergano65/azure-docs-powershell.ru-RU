...
### Пример 3. Получить пакетный файл узла, связанный с задачей, и направить его к потоку
```
...
Эта команда получает файл узла Startup\StdOut.txt из узла вычислений, который имеет ID ComputeNode01 в пуле, который содержит пул ID Pool01.
...
Этот cmdlet сохраняет файл в E:\PowerShell\StdOut.txt путь к файлу на локальном компьютере.
...
Первая команда создает поток с помощью New-Object командлета, а затем сохраняет его в $Stream переменной.
Вторая команда получает файл узла с именем StdOut.txt узла вычислений, в пуле которого есть ID ComputeNode01.
Команда направляет содержимое файла в поток в $Stream.
