...
## ОПИСАНИЕ
При **этом** указанный корневой сертификат удаляется из виртуального сетевого шлюза.
...
В этом примере из виртуального сетевого шлюза ContosoVirtualGateway удаляется корневой сертификат клиента с именем ContosoRootCertificate.
...
Затем вторая команда извлекает из строки весь текст $Text кроме первой и последней строк.
...
Результаты должны быть примерно такие же (обратите внимание, что фактический результат будет содержать намного больше строк текста, чем показано в сокращенном примере): ----- BEGIN CERTIFICATE ----- MIIC13FAAXC3671Auij9HGUNEW8343NMJklo0 CVVFAw8w ----- END CERTIFICATE ----- PublicCertData состоит из всех линий между первой строкой (----- BEGIN CERTIFICATE -----) и последней строкой (----- END CERTIFICATE -----) в файле.
