...
## ОПИСАНИЕ
**Cmdlet Set-AzApplicationGatewayRequestRoutingRule** изменяет правило маршрутизации запросов.

## ПРИМЕРЫ

### Пример 1. Обновление правила маршрутки запросов
```
...
Вторая команда изменяет правило маршрутики запроса шлюза приложения на использование back-end HTTP-параметров, задающихся в переменной $Setting, http-прослушивания, указанного в переменной $Listener, и пула адресов с конечными адресами, заданными в переменной $Pool.

## PARAMETERS

### -ApplicationGateway
Определяет объект шлюза приложения, с которым этот cmdlet связывает правило маршрутки запросов.
...
### -BackendAddressPool
Определяет пул адресов шлюза приложения.
...
Определяет ИД пула адресов шлюза приложения.
...
### -BackendHttpSettings
Определяет параметры HTTP шлюза приложения.
...
Определяет back-end HTTP-параметры шлюза приложения.
...
### -HttpListener
Указывает шлюз приложения HTTP-прослушивание.
...
### -HttpListenerId
Определяет ИД HTTP-прослушивание шлюза приложения.
...
### -Name
Указывает имя правила маршрутки запроса, которое изменяет этот cmdlet.
...
### -RuleType
Определяет тип правила маршрутки запросов.
