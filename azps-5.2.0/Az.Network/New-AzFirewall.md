...
В этом примере создается брандмауэр, блок которого блокирует весь трафик (поведение по умолчанию) и в режиме оповещения запущена технология Threat Intel.
...
В этом примере создается брандмауэр, подключенный к виртуальному концентратору "vHub". К брандмауэру будет $fp политики брандмауэра. Этот брандмауэр разрешает и не разрешает трафик на основе правил, упомянутых в политике брандмауэра, $fp. Виртуальный центр и брандмауэр должны быть в одинаковых регионах.
...
Для поддержки сценариев принудительного туннелизации этот брандмауэр будет использовать подсеть AzureFirewallManagementSubnet и общедоступный IP-адрес управления для своего трафика управления.
...
### -Tag
Пары "ключевое значение" в виде hash table. Например:
