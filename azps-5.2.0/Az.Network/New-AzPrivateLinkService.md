...
## ОПИСАНИЕ
Для создания закрытой службы ссылок создается личная служба **AzPrivateLinkService.**

## ПРИМЕРЫ

### Пример 1

В следующем примере создается частная служба ссылок.
...
### -AutoApproval
Подписки на автоматическое утверждение для службы частных ссылок
...
### -EnableProxyProtocol
Включить прокси-протокол для службы частных ссылок.
...
Конфигурация ip-адреса переднего конца
...
### -Видимость
Подписки видимости для службы частных ссылок
