...
## ОПИСАНИЕ
**Cmdlet Set-AzLoadBalancerInboundNatPoolConfig** задает конфигурацию входящий пул NAT для балансиента нагрузки.
...
### -EnableFloatingIP
Настраивает конечную точку виртуальной машины для плавающей возможности IP-адреса, необходимой для настройки группы доступности SQL AlwaysOn. Этот параметр необходим при использовании групп доступности SQL AlwaysOn в SQL сервере. Этот параметр нельзя изменить после создания конечной точки.
...
Время ожидания для неаплихивного подключения TCP. Это значение можно установить от 4 до 30 минут. Значение по умолчанию — 4 минуты. Этот элемент используется только в том случае, если протокол имеет протокол TCP.
