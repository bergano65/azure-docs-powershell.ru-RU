...
## ОПИСАНИЕ
С **его использованием** создается azure ExpressRoutePort.
...
### -BandwidthInGbps
Пропускная способность закупаемого порта в Гбит/с
...
### -Инкапсуляция
Метод инкапсуляции для физических портов.
...
### -Identity
Назначенное пользователем удостоверение для чтения конфигурации MacSec
...
### -Link
Набор физических связей ресурса ExpressRoutePort
...
### -PeeringLocation
Имя расположения пиринга, с которое физически сопожается ExpressRoutePort.
