...
### Пример 2. Настройте ведение журнала потока для указанной NSG-версии и установите для версии ведения журнала потоков 2.
```
PS C:\> $NW = Get-AzNetworkWatcher -ResourceGroupName NetworkWatcherRg -Name NetworkWatcher_westcentralus
PS C:\> $nsg = Get-AzNetworkSecurityGroup -ResourceGroupName NSGRG -Name appNSG
PS C:\> $storageId = "/subscriptions/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/resourceGroups/NSGRG/providers/Microsoft.Storage/storageAccounts/contosostorageacct123"
...
В этом примере настроено ведение журнала потоков в группе безопасности сети (NSG) с заданными журналами версии 2. В ответе мы видим, что в NSG включено ведение журнала потоков, задан формат и политика хранения не настроена. Если регион не поддерживает указанную версию, Network Watcher напишет в нем поддерживаемую по умолчанию версию.
