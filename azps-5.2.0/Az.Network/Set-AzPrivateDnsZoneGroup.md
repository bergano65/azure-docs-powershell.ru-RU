...
## ОПИСАНИЕ
Для группы зон DNS обновляется группа зоны **Set-AzPrivateDnsZoneGroup.**
...
В примере выше группа DNS Zone Group с именем dnsgroup1 обновляется с помощью новой dnsconfig, которая связывается с другой зоной DNS.
