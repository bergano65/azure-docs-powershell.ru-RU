...
## ОПИСАНИЕ
С **помощью cmdlet Get-AzApplicationGatewayBackendAddressPool** можно получить одну или несколько конфигураций пула адресов backend от шлюза приложения.

## ПРИМЕРЫ

### Пример 1. Получить указанный серверный пул серверов
```
...
Вторая команда получает пул конечных адресов, связанный с $AppGw Pool01, и сохраняет его в переменной $BackendPool адресов.

### Пример 2. Получить список серверного пула серверов
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
Первая команда получает шлюз приложения ApplicationGateway01 в группе ресурсов ResourceGroup01 и сохраняет его в переменной $AppGw ресурса.
Вторая команда получает список пулов адресов, связанных с $AppGw, и сохраняет список в переменной $BackendPools адресов.

## PARAMETERS

### -ApplicationGateway
С **помощью cmdlet Get-AzApplicationGatewayBackendAddressPool** можно получить пул адресов с конечными адресами для шлюза приложения.
...
### -Name
Имя пула адресов, который получает этот cmdlet.
