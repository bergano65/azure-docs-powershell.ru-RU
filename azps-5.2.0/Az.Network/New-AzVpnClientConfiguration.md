...
Путь корневого сертификата сервера Radius Server. Это обязательный параметр, который должен быть указан при проверке подлинности EAP-TLS с проверкой подлинности RADIUS. Это полный путь к файлу CER с корневым сертификатом RADIUS, который клиент использует для проверки сервера RADIUS во время проверки подлинности.
