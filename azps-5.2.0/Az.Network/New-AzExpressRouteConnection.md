...
Выше будет создать группу ресурсов, виртуальную сеть, виртуальную сеть, виртуальный концентратор, шлюз ExpressRoute и канал ExpressRoute с частным пирингом в Западной центральной части США в группе ресурсов TestRG в Azure. Созданный шлюз подключается к пирингу через канал ExpressRoute с помощью команды New-AzExpressRouteConnection управления.
...
ИД ресурса пиринга express Route, к которому должно быть создано подключение шлюза Express Route.
...
### -RoutingWeight
Вес для маршрутики пакетов, которая должна быть назначена этому подключению.
