...
## ОПИСАНИЕ
С New-AzDataMigrationTask создается задача переноса данных. Этот cmdlet принимает в качестве входных параметров для enumerator "Тип задачи", "Группа ресурсов Azure", имя связанной службы миграции базы данных Azure и Project. 
...
В этом примере сценарий показывает, как создать задачу переноса данных с именем MyMigrationTask в проекте myDMSProject и службе TestService. 
...
### -MigrationValidation 
Объект ответа задачи по звонку проверки (необязательно, но рекомендуется).
...
### -Wait
Следует ли ждать завершения задачи. Если флажок установлен, проверяется каждые одну секунду, пока задача не завершится, и пользователь вернется к свойствам задачи, для которых можно проверить выходные данные или ошибки. 
```yaml
