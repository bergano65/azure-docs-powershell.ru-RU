...
## ОПИСАНИЕ
С помощью **cmdlet Add-AzVmssSshPublicKey** можно добавить общедоступные ключи, которые можно использовать для подключения к виртуальной машине (VMSS) по безопасной оболочке (SSH).
...
В этом примере к VMSS добавляется открытый ключ SSH.
...
Вторая команда добавляет ключ SSH с указанными ключевыми данными и сохраняет ключ в указанном пути на виртуальной машине.
...
### -KeyData
Указывает данные открытого ключа RSA службы SSH.
