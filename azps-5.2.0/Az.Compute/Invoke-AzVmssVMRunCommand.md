...
## ОПИСАНИЕ
Вызов команды запуска на виртуальной машине (scale Set VM).
...
Вызвать команду запуска RunPowerShellScript, переопределив сценарий sample.ps1 и параметры на ID '0' VM в наборе виртуальной машинной шкалы vmssname в группе ресурсов 'rgname'.
...
### -ScriptPath
Путь к сценарию, который нужно выполнять.  Если задано это значение, сценарий, заданный по умолчанию для команды, переопределит его.
...
Объект VM в наборе виртуальных машин PS.
...
### -VMScaleSetName
Имя набора виртуальной машины (VM).
