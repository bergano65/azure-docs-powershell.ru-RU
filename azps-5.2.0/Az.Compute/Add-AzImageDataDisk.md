...
## ОПИСАНИЕ
Для добавления диска данных к объекту изображения добавляется диск данных **Add-AzImageDataDisk.**
...
Первая команда создает объект изображения, а затем сохраняет его в переменной $imageConfig изображения.
Следующие три команды назначают пути к диску операционной системы и двум дискам данных переменным $osDiskVhdUri, $dataDiskVhdUri 1 и $dataDiskVhdUri 2.
Этот подход подходит только для учитаемости следующих команд:
Следующие три команды добавляют диск операционной системы и два диска данных к изображению, хранямуся в $imageConfig.
URI каждого диска хранится в $osDiskVhdUri, $dataDiskVhdUri 1 и $dataDiskVhdUri 2.
Конечная команда создает изображение с именем ImageName01 в группе ресурсов ResourceGroup01.

## PARAMETERS

### -BlobUri
Указывает ссылку в качестве URI BLOB-части.
...
### -DiskSizeGB
Определяет размер диска в гигабайтах (ГБ).
...
### -Image
Определяет объект локального изображения.
...
### -Lun
Определяет логический номер единицы (LUN).
...
### -SnapshotId
Определяет ИД моментального снимка.
...
### -StorageAccountType
Тип учетной записи хранения диска изображения данных
