...
## ОПИСАНИЕ
Командлет **Add-AzVmssNetworkInterfaceConfiguration** добавляет конфигурацию сетевого интерфейса в набор масштаба виртуальной машины (VMSS).
...
Эта команда добавляет конфигурацию сетевого интерфейса к VMSS.
...
Список IP-адресов DNS-сервера для параметров DNS.
...
Указывает, будет ли ускоренный сетевой интерфейс включен.
...
### -EnableIPForwarding
Указывает, включена ли для этой NIC переадваровка IP-адресов.
...
### -IpConfiguration
Определяет конфигурации IP-адреса интерфейса сети.
...
### -Primary
Указывает, будут ли сетевые интерфейсы, созданные с помощью конфигурации сетевого интерфейса, основным информационным центром сети (NIC) виртуальной машины.
