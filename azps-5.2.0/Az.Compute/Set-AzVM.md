...
## ОПИСАНИЕ
Для виртуальной машины в качестве обобщенного можно пометить cmdlet **Set-AzVM.**
Перед запуском этого cmdlet войдите на виртуальную машину и подготовьте жесткий диск с помощью Sysprep.
...
Эта команда пометит виртуальную машину с именем VirtualMachine07 как обобщенную.
...
### -Обобщено
Указывает на то, что этот cmdlet помещает виртуальную машину как общую.
...
### -Reapply
Чтобы повторно прибавить виртуальную машину.
...
### -Redeploy
Указывает на то, что этот cmdlet вручную перенаправит виртуальную машину на другой хост Azure для устранения проблем.
При перезапуске виртуальной машины она перезапускится, что приводит к потере данных эпозитерального диска.
...
### -Simulateeviction
Указывает на то, что этот cmdlet имитация работы spot virtual machine.
