...
## ОПИСАНИЕ
Cmdlet **Set-AzVMCustomScriptExtension** добавляет настраиваемый сценарий расширения виртуальной машины на виртуальную машину.
Это расширение позволяет запускать собственные сценарии на виртуальной машине.

## ПРИМЕРЫ

### Пример 1. Добавление пользовательского сценария
```powershell
...
Добавляет настраиваемые расширения сценариев на виртуальную машину. (autogenerated)
...
### -Аргумент
Указывает аргументы, которые расширение сценария передает сценарию.
...
### -ContainerName
Имя контейнера хранилища Azure, в котором хранится сценарий.
...
### -FileName
Указывает имя файла сценария. Если файл хранится в хранилище BLOB-файлов Azure, имя файла в этом случае внося в нее значения с чувствительной к делу. Имена файлов, хранимых в хранилище файлов Azure, нечувствительны к делу.
...
### -ForceRerun
Указывает на то, что этот cmdlet приостановит повторение одной и той же конфигурации расширения на виртуальной машине, не устраив и повторно установив расширение.
...
### -Run
Указывает команду, которая будет использовать сценарий.
...
### -SecureExecution
Этот cmdlet позволяет убедиться, что значение параметра *Run* не было возвращено с сервера или возвращено пользователю с помощью API расширения GET.
Значение *"Выполнить"* может содержать секрет или пароли, которые необходимо безопасно передать файлу сценария.
...
### -StorageAccountKey
Ключ для контейнера хранилища Azure.
...
### -StorageAccountName
Указывает имя учетной записи хранения Azure, в которой этот cmdlet хранит сценарий.
...
Чтобы получить версию, запустите Get-AzVMExtensionImage Microsoft.Compute для параметра *PublisherName* и CustomScriptExtension для параметра *Type.*
...
Этот cmdlet добавляет расширение настраиваемого сценария для виртуальной машины, указанное этим параметром.
