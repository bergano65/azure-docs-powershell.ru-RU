...
## ОПИСАНИЕ
Для получения сведений в файле ответа об установке Windows, который не является дополнительным, добавляется **cmdlet Add-AzVmssAdditionalUnattendContent.**
...
Можно использовать только microsoft-Windows-Shell-Setup.
...
### -PassName
Указывает имя передать, к какому содержимому применяется.
Допустимым является только oobeSystem.
