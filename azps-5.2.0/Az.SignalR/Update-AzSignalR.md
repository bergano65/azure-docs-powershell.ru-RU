...
### Обновление определенной службы SignalR.
```powershell
...
### -AllowedOrigin
Разрешенные источникы для службы SignalR. Чтобы разрешить все, используйте "*" и удалите из списка все остальные происхождение. Косые черты не разрешены как часть домена или после TLD
...
### -SKU
SKU службы SignalR.
Значение по умолчанию — "Standard_S1".
...
### -UnitCount
Единица сигнального устройства, значение только от {1, 2, 5, 10, 20, 50, 100}.
