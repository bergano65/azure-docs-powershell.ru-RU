...
Если по-прежнему доступны другие домены, значение в NextLink следует использовать в следующем звонке, чтобы получить следующую страницу доменов.
...
Получает сведения о домене Event Grid \` Domain1 \` в группе ресурсов \` MyResourceGroupName \` с помощью параметра ResourceId.
...
Со списком доменов сетки событий (если они есть) в подписке Azure, удовлетворяющий запросу $odataFilter 20 доменов одновременно. Если доступно больше результатов, $result. NextLink не будет $null. Чтобы получить доступ к следующим страницам доменов, пользователь должен снова позвонить в Get-AzEventGridDomain и использует результат. NextLink, полученные в предыдущем звонке. Звонок должен остановиться в результатах. NextLink становится $null.
...
Это значение получают при первом Get-AzEventGrid, когда для запроса по-прежнему доступно больше ресурсов.
...
Если значение "Верхнее" не указано, возвращается полный список ресурсов.
