...
### Пример 1. Вызов экспорта по именам и области экспорта
```powershell
PS C:\> Invoke-AzCostManagementExecuteExport -ExportName 'TestExport' -Scope 'subscriptions/**********'

```

Вызов экспорта по exportName и области

### Пример 2. Вызов экспорта с помощью inputObject
```powershell
...
Вызов экспорта с помощью inputObject
