...
### -Operator
Оператор, который используется при совпадении с селектором, EqualsAny означает отсутствие селектора (все переменные совпадения указанного типа)
...
### -Selector
Шаблон селектора для совпадения с использованием оператора (если оператор не равно "РавноAny")
...
Например, QueryStringArgNames — это исключение параметров GET, которые соответствуют выборке с заданным оператором.
