...
## ОПИСАНИЕ
Для **этого в учетной** записи хранилища Azure в списке перечислены BLOB-клиенты, которые указаны в этом контейнере.
...
### Пример 2. В контейнере можно получать большой большой объем в контейнере с помощью конвейера
```
...
### Пример 5. Все BLOB-lob-lob в контейнере включают версию BLOB-ветвей
```
...
Эта команда получает один моментальный снимок BLOB-ля с помощью Моментального времени.

## PARAMETERS

### -BLOB
Определяет имя или шаблон имени, который можно использовать для поиска с поддимывными знаками.
Если имя BLOB-ля не задано, будет выдан список всех BLOB-имен в этом контейнере.
Если для этого параметра задано значение, командлет будет перечислять все BLOB-lob с именами, которые соответствуют этому параметру. Этот параметр поддерживает поддиавные знаки в любом месте строки.
...
### -IncludeDeleted
Include Deleted BLOB, по умолчанию он не включает удаленный BLOB-blob.;
