...
## ОПИСАНИЕ
Для отправки содержимого файла в файл в указанной папке будет добавлено содержимое файла в папке **Set-AzStorageFileContent.**
...
Он отфильтровает объекты, которые не являются файлами, а затем передает файлы в ForEach-Object.
...
### Пример 3. Вы можете добавить локальный файл в файл Azure и заявить его свойства (File Attributtes), время создания файла, время последней записи файла в файле Azure.
```
...
### -Force
Указывает на то, что этот cmdlet переописает существующий файл хранилища Azure.
...
Если указать путь к не существует папке, этот cmdlet не будет изменяться и возвратит ошибку.
