...
## SYNOPSIS
Получать сообщения в службу поддержки.
...
## ОПИСАНИЕ
Получает сообщения для билета в службу поддержки. Он получит все сообщения для билета, если не указать какие-либо другие параметры. Вы также можете отфильтровать сообщения по CreatedDate или CommunicationType с помощью параметра Filter. Вот несколько примеров значений фильтра, которые можно указать.
...
### Пример 3. Извлечение первых 2 сообщений для получения сообщения в службу поддержки
```powershell
...
### Пример 4. Извлечение следующих 2 сообщений после пропуска первых 2 сообщений для получения билета в службу поддержки
```powershell
...
### Пример 6. Извлечение всех сообщений, созданных 20 декабря 2019 г. или после них, для получения сообщения в службу поддержки
```powershell
