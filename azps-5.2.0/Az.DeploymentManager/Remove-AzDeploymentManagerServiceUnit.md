...
## ОПИСАНИЕ
**Cmdlet Remove-AzDeploymentManagerServiceUnit** удаляет единицу обслуживания в службе.
...
Эта команда удаляет службу с именем ContosoService1Storage службы ContosoService1 в топологии службы ContosoServiceTopology в contosoResourceGroup.
...
### Пример 3. Удаление единицы обслуживания с помощью объекта единицы обслуживания.
```powershell
PS C:\> Remove-AzDeploymentManagerServiceUnit -InputObject $serviceUnitObject
```

Эта команда удаляет единицу службы, имя которой, имя службы, имя топологии службы и группа ресурсов соответствуют свойствам "Имя Службы", "НазваниеПолуки" и "ИмяГруппы Ресурсов" $serviceUnitObject соответственно.
