...
## ОПИСАНИЕ
С New-AzAttestation создается атестация в указанной группе ресурсов.
...
Создайте новый экземпляр поставщика attestation с именем *pshtest4* с парой тегов и использованием доверия AAD для mastering TEE policy.
...
Создайте новый экземпляр поставщика attestation с именем *pshtest3,* который использует isoladed trust для mastering TEE policy посредством указания набора доверенных ключей подписи с помощью PEM-файла.
...
### -Location
Определяет регион Azure, в котором нужно создать поставщика для проверки. Используйте команду Get-AzResourceProvider с параметром ProviderNamespace, чтобы увидеть ваши варианты.
...
### -Name
Указывает имя созда создаемого экземпляра.
...
Набор надежных ключей подписи для политики выдачи в одном файле сертификата.
...
### -ResourceGroupName
Указывает имя существующей группы ресурсов, в которой нужно создать проверку.
