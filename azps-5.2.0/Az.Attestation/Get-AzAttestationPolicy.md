...
## ОПИСАНИЕ
Этот Get-AzAttestationPolicy получает политику от клиента в Azure Attestation.
...
Получает политику для поставщика attestation provider *pshtest* для Tee type *SgxEnclave.*
...
Получает политику поставщика по умолчанию для проверки attestation из расположения *Uk South* for Tee type *SgxEnclave.*
...
Этот cmdlet получает политику проверки для клиента, указанного этим параметром.
