...
Действия сценариев предоставляют функции, которые используются для установки дополнительного программного обеспечения или изменения конфигурации приложений, которые запускаются на кластере Hadoop с помощью сценариев Windows PowerShell или Bash (для кластеров Windows или Linux соответственно).
На узлах кластера при развертывании кластеров HDInsight выполняется действие сценария, который выполняется после узлов в полной конфигурации HDInsight кластера.
...
Вы можете предоставить каждому кластеру список действий сценариев для работы в указанной последовательности.
...
### -NodeType
Тип узла, для которого нужно выполнить действие сценария.
