...
## SYNOPSIS
Возвращает выходные данные журнала для задания из учетной записи хранения, связанной с указанным кластером.
...
## ОПИСАНИЕ
**Cmdlet Get-AzHDInsightJobOutput** получает результаты журнала для задания из учетной записи хранения, связанной с кластером Azure HDInsight.

## ПРИМЕРЫ

### Пример 1. Поиск выходных данных журнала для задания
```
PS C:\># Cluster info
...
Эта команда получает результаты журнала из кластера hadoop-001.
...
### -DefaultContainer
Указывает имя контейнера, заданное по умолчанию.
...
Указывает ключ учетной записи хранения по умолчанию.
...
### -DisplayOutputType
Определяет тип запрашиваемого результата задания.
...
### -JobId
Определяет ИД задания, результаты которого будут извлекаться.
