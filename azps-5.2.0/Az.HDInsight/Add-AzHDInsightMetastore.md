...
## SYNOPSIS
Добавляет в SQL кластера базу данных, которая будет служить метасемой "Оози" или "Оози".
...
## ОПИСАНИЕ
Cmdlet **Add-AzHDInsightMetastore** добавляет метасхему "Гоголь" или "Oozie" в объект конфигурации HDInsight, созданный New-AzHDInsightClusterConfig-
Метамагага — это база SQL, которая может хранить метаданные для оболя, оози или обоих этих метаданных.

## ПРИМЕРЫ

### Пример 1. Добавление метасхранилища SQL к объекту конфигурации кластера
```
...
Эта команда добавляет метас SQL базы данных в кластер с именем ваш-hadoop-001.

### Пример 2. Добавление настраиваемой базы данных Ambari к объекту конфигурации кластера
```
PS C:\># Primary storage account info
PS C:\> $storageAccountResourceGroupName = "Group"
PS C:\> $storageAccountResourceId = "yourstorageaccountresourceid"
...
Эта команда добавляет настраиваемую базу данных Ambari в кластер с именем ваш-hadoop-002.
...
### -Credential
Определяет учетные данные для базы данных AzureSQL Server.
...
### -DatabaseName
Указывает базу данных в экземпляре AzureSQL Server, который будет использовать для этого метасхемы.
...
### -MetastoreType
Тип метасхемы.
Возможные значения: ОozieMetastore или ОozieMetastore.
...
### -SqlAzureServerName
Указывает экземпляр AzureSQL Server, который будет использовать для этого метасхемы.
