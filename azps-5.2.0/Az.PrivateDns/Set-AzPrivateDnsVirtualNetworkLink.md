...
Это можно сделать с помощью параметра *Overwrite,* который удаляет ссылку независимо от изменений одновременно.
...
Кроме того, можно указать виртуальную сетевую ссылку с помощью объекта **PSPrivateDnsVirtualNetworkLink,** передаваемого через канал или параметр *Link.*
