...
## ОПИСАНИЕ
Для получения зоны частного доменного имени (DNS) от указанной группы ресурсов возвращается cmdlet **Get-AzPrivateDnsZone.**
...
### -Name
Указывает имя зоны private DNS, которая будет получаться.
