...
## ОПИСАНИЕ
Этот Add-AzPrivateDnsRecordConfig добавляет запись частного доменного имени (DNS) к объекту RecordSet. Объект RecordSet является автономным объектом, и изменения в нем не изменяют ответы на частные DNS до тех пор, пока вы не запустите Set-AzPrivateDnsRecordSet, чтобы сохранить изменение в частной службе DNS Microsoft Azure. Записи SOA создаются при создания частной зоны DNS и удаляются при ее удалении. Записи SOA нельзя добавлять и удалять, но можно редактировать. Объект RecordSet можно передать этому cmdlet в качестве параметра или с помощью оператора конвейера.
...
В этом примере запись AAAAA добавляется в существующий набор.
...
В этом примере запись CNAME добавляется в существующий набор записей.
...
В этом примере SRV-запись добавляется в существующий набор.

### Пример 7. Добавление записи TXT в набор
```powershell
...
### -Priority
SRV-запись приоритета, которая нужно добавить.
