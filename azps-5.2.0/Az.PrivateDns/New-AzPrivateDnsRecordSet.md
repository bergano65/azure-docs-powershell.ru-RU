...
Эта команда создает набор записей с именем текста в частной myzone.com. Набор записей имеет тип TXT и имеет срок жизни 1 час (3600 секунд). Она содержит одну частную запись DNS. Чтобы создать набор записей, используя только одну строку pn_PowerShell_short или создать набор записей с несколькими записями, см. пример 1.
...
Эта команда создает набор записей с именем * в частной зоне myzone.com. Это набор записей с подстрокными знаками. Чтобы создать набор записей, используя только одну строку pn_PowerShell_short или создать набор записей с несколькими записями, см. пример 1.
...
### -ZoneName
Зона, в которой создается набор записей (без точки, в которой завершается запись).
