...
Для тегов, условий и действий объекты должны быть созданы заранее и переданы в качестве параметров этого вызова в качестве разделяемой запятой (см. пример ниже).
...
Первая команда похожа на "nop", она задает оповещение с теми же значениями, которые уже содержатся для остальных команд, извлекают правило оповещения, изменяют описание и отключают его, а затем с помощью параметра InputObject сохраняет эти изменения.
