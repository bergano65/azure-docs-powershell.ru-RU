...
## ОПИСАНИЕ
При этом из набора записей удаляется запись DNS системы доменных имен **(AzDnsRecordConfig).**
...
Чтобы удалить запись, все поля для этого типа записи должны в точности соответствовать.
Записи SOA добавлять и удалять нельзя.
Записи SOA создаются автоматически при создания зоны DNS и автоматическом удалении при удалении зоны DNS.
...
В этом примере запись A удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.
...
В этом примере запись AAAA удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.
...
В этом примере запись CNAME удаляется из существующего набора.
Так как набор записей CNAME может содержать не более одной записи, результатом является пустой набор записей.
...
В этом примере запись MX удаляется из существующего набора.
Имя записи "@" означает набор записей на апексе зоны.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.

### Пример 5. Удаление записи NS из набора записей
```
...
В этом примере NS-запись удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.

### Пример 6. Удаление записи PTR из набора записей
```
...
В этом примере запись PTR удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.

### Пример 7. Удаление записи SRV из набора записей
```
...
В этом примере запись SRV удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.

### Пример 8. Удаление записи TXT из набора записей
```
...
В этом примере запись TXT удаляется из существующего набора.
Если это единственная запись в наборе записей, результатом будет пустой набор записей.
Чтобы полностью удалить набор записей, см. видео Remove-AzDnsRecordSet.
...
### -Nsdname
Сервер доменных имен для записи сервера доменных имен.
...
### -Ptrdname
Указывает целевое доменное имя для записи указателя (PTR).
...
### -RecordSet
Определяет объект **RecordSet,** содержащий удаляемую запись.
