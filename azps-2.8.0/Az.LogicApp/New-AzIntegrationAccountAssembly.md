...
## NОПИСАНИЕ
Командлет **Get-AzIntegrationAccountAssembly** создает новую сборку в учетной записи интеграции.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание новой сборки с помощью локального файла
```powershell
...
Создает новую сборку с использованием локального файла, расположенного в пути к файлу, который хранится в разделе "$localAssemblyFilePath".

### Пример 2: создание новой сборки с использованием байтовых данных
```powershell
...
Создает новую сборку, используя массив байтов, содержащийся в "$assemblyContent".

### Пример 3: создание новой сборки с помощью ссылки на контент
```powershell
...
Создает новую сборку с использованием байтовых данных, расположенных на URL-адресе "$assemblyUrl". Это предлагаемый метод для создания сборок крупного размера.
...
### -ParentResourceId
Идентификатор ресурса для учетной записи интеграции.
