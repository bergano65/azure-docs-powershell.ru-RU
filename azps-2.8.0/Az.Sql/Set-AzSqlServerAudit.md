...
## NОПИСАНИЕ
Командлет **Set-AzSqlServerAudit** изменяет параметры аудита для Azure SQL Server.
...
### Пример 3,1: Включение политики аудита хранилища BLOB-объектов для Azure SQL Server с расширенной фильтрацией с помощью предиката T-SQL
```
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Enabled -StorageAccountResourceId "/subscriptions/7fe3301d-31d3-4668-af5e-211a890ba6e3/resourceGroups/resourcegroup01/providers/Microsoft.Storage/storageAccounts/mystorage" -PredicateExpression "statement <> 'select 1'"
```

### Пример 3,2: Удаление параметра расширенной фильтрации из политики аудита на сервере Azure SQL Server
```
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -PredicateExpression ""
```

### Пример 4: Включение политики аудита центра событий для Azure SQL Server
```
...
### Пример 5: отключение политики аудита центра событий для Azure SQL Server
```
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### Пример 6: Включение политики аудита анализа журналов для Azure SQL Server
```
...
### Пример 7: отключение политики аудита для службы анализа журналов для Azure SQL Server
```
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -LogAnalyticsTargetState Disabled
```

### Пример 8: отключение (с помощью конвейера) политики аудита анализа журналов для Azure SQL Server
```
...
### Пример 9: отключение отправки записей аудита для сервера Azure SQL Server в хранилище BLOB-объектов и разрешение их отправки в службу анализа журнала.
```
...
### Пример 10: разрешение отправки записей аудита сервера Azure SQL Server в хранилище больших двоичных объектов, а именно об этом центре событий и в службе анализа журнала.
```
