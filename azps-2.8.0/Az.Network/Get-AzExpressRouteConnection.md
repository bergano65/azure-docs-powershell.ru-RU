...
После создания шлюза он подключается к локальной цепи ExpressRoute с помощью команды New-AzExpressRouteConnection.

Затем он получает подключение, используя имя подключения.
...
Эта команда будет получать все подключения в ExpressRoute "testExpressRoutegw", которые начинаются с "Test".
