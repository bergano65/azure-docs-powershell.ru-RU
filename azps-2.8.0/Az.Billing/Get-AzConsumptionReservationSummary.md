...
## NОПИСАНИЕ
Командлет **Get-AzConsumptionReservationSummary** получает сведения о резервировании для ежедневного или ежемесячного гранка.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение итогов по резервированию с помощью идентификатора заказа на резервирование для ежемесячного детализации
```powershell
...
### Пример 2: получение сводок резервирования с ИД заказа на резервирование и идентификатором резервирования для ежемесячного детализации
```powershell
...
### Пример 3: получение итогов по резервированию с помощью идентификатора заказа на резервирование для ежедневного интервала в указанном диапазоне дат
```powershell
...
### Пример 4: получение сводок резервирования с ИД заказа на резервирование и идентификатором резервирования для ежедневного интервала в указанном диапазоне дат
```powershell
...
### -EndDate
Конечные данные (гггг-мм-дд в формате UTC) сводки резервирований, которые требуются только для ежедневного детализации.
...
### -Зернистость
Временные Гранки сводки резервирования могут быть ежедневно или ежемесячными.
...
### -StartDate
Начальные данные (гггг-мм-дд в формате UTC) сводки резервирований, которые требуются только для ежедневного детализации.
...
### Microsoft. Azure. Commands. потребление. Models. PSReservationSummary
