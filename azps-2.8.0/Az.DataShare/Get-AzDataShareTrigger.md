...
## NОПИСАНИЕ
Командлет **Get-AzDataShareTrigger** получает сведения о триггере "общий доступ к подписке" для общей учетной записи данных.
...
Эта команда получает сведения о триггере AdsTrigger для предоставления общего доступа к подписке AdsShareSubscription в разделе Учетная запись общего доступа к данным WikiAds.
...
### -ResourceId
Идентификатор ресурса триггера.
