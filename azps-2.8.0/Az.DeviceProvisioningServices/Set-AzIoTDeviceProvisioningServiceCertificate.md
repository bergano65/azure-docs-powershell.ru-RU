...
## NОПИСАНИЕ
Для проверки сертификата можно загрузить сертификат проверки, который содержит проверочный код, полученный путем вызова Generate-Verification-Code. Это последний шаг в ходе проверки.
...
Подтвердить владение закрытым ключом "mycertificate".
...
Подтвердить владение закрытым ключом "mycertificate" с помощью конвейера.

## ПАРАМЕТРЫ

### -CertificateName
Имя сертификата службы подготовки устройства IOT
...
### -InputObject
Объект сертификата службы подготовки устройства IoT
...
### -Path
базовое представление 64 сертификата X509. cer или PEM-файла.
...
### -ResourceId
Идентификатор ресурса сертификата службы подготовки устройства IoT
