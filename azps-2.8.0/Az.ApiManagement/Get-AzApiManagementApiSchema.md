...
## NОПИСАНИЕ
Командлет **Get-AzApiManagementApiSchema** получает сведения о схеме API

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение подробных сведений о схеме API для API
```powershell
...
Эта команда получает все схемы API, связанные с API `swagger-petstore-extensive` для определенного контекста ApiManagement.

### Пример 2: получение определенной схемы, связанной с API
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceId /subscriptions/subid/resourceGroups/resourceGroupName/providers/Microsoft.ApiManagement/service/sdktestapim4163
...
Эта команда получает схему API, `5cc9cf67e6ed3b1154e638bd` связанную с API `swagger-petstore-extensive` для определенного контекста ApiManagement.
...
### -ResourceId
Идентификатор ресурса ARM схемы API. Если указано, будет предпринята попытка найти схему API по идентификатору. Этот параметр является обязательным.
