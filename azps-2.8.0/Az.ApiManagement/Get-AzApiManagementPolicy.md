...
## NОПИСАНИЕ
Командлет **Get-AzApiManagementPolicy** получает указанную политику области.
...
### Пример 5: получение политики области клиента в формате RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Эта команда возвращает политику области клиента в формате, отличном от XML-формата.
...
Значением по умолчанию для этого параметра является XML.
