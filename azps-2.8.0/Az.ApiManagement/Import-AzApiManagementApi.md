...
## NОПИСАНИЕ
Командлет **Import-AzApiManagementApi** импортирует API Azure API Management из файла или URL-адреса в языке описания веб-приложений (WADL), языка описания веб-служб (WSDL) или в формате Swagger.
...
### Пример 4: импорт API из ссылки Open API
```powershell
...
Эта команда импортирует API-интерфейс из указанной ссылки на открытую спецификацию 3,0.
...
### -ApiRevision
Идентификатор редакции API. Этот параметр является необязательным. Если этот элемент не указан, импорт будет выполнен на текущую активную версию или в новый API-интерфейс.
...
### -Protocol (протокол)
Протоколы веб-API (HTTP, HTTPS). Протоколы, через которые API станет доступным. Этот параметр является необязательным. При условии, что оно переопределит протоколы, указанные в документе спецификации.
...
### -ServiceUrl
URL веб-службы, предоставляющей API. Этот URL-адрес будет использоваться только для управления API Azure и не будет открыт. Этот параметр является необязательным. Если оно задано, оно переопределяет ServiceUrl, указанный в документе "спецификации".
...
### System. Nullable "1 [[Microsoft. Azure. Commands. ApiManagement. ServiceManagement.. PsApiManagementApiType, Microsoft. Azure. PowerShell. командлеты. ApiManagement. ServiceManagement, Version = 1.0.0.0, Culture = Neutral, PublicKeyToken = NULL]]
