...
## NОПИСАНИЕ
Командлет **Stop-AzStorageBlobCopy** останавливает операцию копирования для указанного большого двоичного объекта назначения.
...
Эта команда останавливает операцию копирования, передавая контейнер на конвейер из **Get-AzStorageContainer**.

### Пример 3: остановка операции копирования с помощью конвейера и Get-AzStorageBlob
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" | Stop-AzStorageBlobCopy -Force
```

Этот пример останавливает операцию копирования, передавая контейнер на конвейер из командлета Get-AzStorageBlob.
...
Вы можете создать объект или использовать командлет Get-AzStorageContainer.
...
Контекст можно создать с помощью командлета New-AzStorageContext.
