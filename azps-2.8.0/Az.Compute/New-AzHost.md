...
Эта команда создает узел заданного SKU в заданной группе узлов и указанном расположении.
...
Указывает, следует ли автоматически заменять узел в случае сбоя. Значение по умолчанию — "истина", если не указано.
...
### -LicenseType
Указывает тип лицензии на программное обеспечение, которая будет применяться к виртуальным машинам, развернутым на узле. Возможные значения: None, Windows_Server_Hybrid и Windows_Server_Perpetual.  Значение по умолчанию — None (нет).
...
### -PlatformFaultDomain
Указывает количество доменов сбоев платформы узла.  Затем минимальное значение равно 0 и максимальное значение равно 2.
