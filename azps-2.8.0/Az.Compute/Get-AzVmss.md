...
Представление "модель" — свойства, заданные пользователем в наборе масштабов виртуальных машин.
Представление экземпляра — это состояние уровня экземпляра для набора масштабов виртуальной машины.
Укажите параметр *InstanceView* , чтобы получить только представление экземпляра для набора шкал виртуальной машины.
...
Так как в команде не указан параметр переключателя *InstanceView* , командлет возвращает представление модели для набора масштабов виртуальных машин.

### Пример 2: получение всех Vmss в группе ресурсов
```
...
Получить все Vmss в группе ресурсов "Group001"

### Пример 3: получение всех Vmss в подписке
```
PS C:\> Get-AzVmss
...
Получить все Vmss в подписке.

### Пример 4: получение всех Vmss с помощью фильтрации
```
...
Получить все Vmss в подписке, которые начинаются с "VMSS00".
...
### -InstanceView
Указывает на то, что этот командлет получает только представление экземпляра для набора масштабов виртуальной машины.
...
### -OSUpgradeHistory
Указывает на то, что этот командлет перечисляет историю обновления операционной системы для набора масштабов виртуальных машин.
