...
## SYNOPSIS
При этом будет удален файл YAML, используемый для настройки конфигурации управления источником, что останавливает его последующие синхронизации.
...
## ОПИСАНИЕ
При этом будет удален файл YAML, используемый для настройки конфигурации управления источником, поэтому в будущем синхронизация будет удалена из повторной настройки источника.

## ПРИМЕРЫ

### Пример 1. Удаление кластера куберсетей по имени
```powershell
...
Эта команда удаляет по имени кластер кюберсетей.

### Пример 2. Удаление кластера куберсетей по объектам
```powershell
...
Эта команда удаляет кластер куберсетей по объектам.
...
INPUTOBJECT <IKubernetesConfigurationIdentity> : Параметр identity
  ...
  - `[ClusterRp <String>]`: RP кластера "Кубберсети" — Microsoft.ContainerService (для кластеров AKS) или Microsoft.Kubernetes (для кластеров OnPrem K8S).
