...
## ОПИСАНИЕ
Обновление артефакта. Существует два способа обновить артефакт: через артефакт JSON в качестве входного файла или путем предоставления вводимых параметров артефакта. Хотя метод JSON не требует предоставления артефакта в виде inline, пользователь должен предоставить тип артефакта с помощью параметра -Type.
...
Обновление артефакта с помощью JSON-файла артефакта.
...
Обновление артефакта с помощью inline parameters.
...
Обновление артефакта с помощью ARM шаблона.
...
### -DependsOn
Список имен артефактов, которые необходимо создать до создания текущего артефакта.
...
### -PolicyDefinitionId
Определение определения определения политики.
...
Возможность передавать параметры в артефакт определения политики.
...
### -ResourceGroupName
Имя группы ресурсов, под которым будет входить артефакт.
...
Расположение файла параметров ARM шаблона на диске.
