...
### Пример 1. Добавление ресурса в коллекцию перемещения.
```powershell
...
Добавление ресурса в коллекцию перемещения в пределах указанной подписки.
...
### -DependsOnOverride
Переопределения зависимостей перемещения ресурсов.
Чтобы создать новую таблицу, см. раздел "ЗАМЕТКИ" для свойств DEPENDSONOVERRIDE и создание hash table.
...
### -ExistingTargetId
Возвращает или задает существующий ARM ИД ресурса.
...
Тип ресурса.
Например, это может быть Microsoft.Compute/virtualMaиes.
...
### -SourceId
Получает или задает исходный ARM ИД ресурса.
...
DEPENDSONOVERRIDE <IMoveResourceDependencyOverride[]>: получает или устанавливает переопределения зависимостей перемещения ресурсов.
  - `[Id <String>]`: получает или задает ARM зависимый ресурс.
  - `[TargetId <String>]`: возвращает или задает ARM ресурса как ресурса MoveResource, так и ARM для зависимого ресурса.
