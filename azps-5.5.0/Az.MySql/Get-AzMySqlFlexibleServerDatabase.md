...
### Пример 1. Получить базу данных MySql по имени ресурса
```powershell
...
Этот cmdlet получает MySql server по имени ресурса.

### Пример 2. Просмотр баз данных MySql по удостоверениям
```powershell
...
Этот cmdlet возвращает mySql server по удостоверениям.

### Пример 3. Список всех баз данных MySql на указанном сервере
```powershell
...
Этот cmdlet lists all the MySql servers in specified the server.
