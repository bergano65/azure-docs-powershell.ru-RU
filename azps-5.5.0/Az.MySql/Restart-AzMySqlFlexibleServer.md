...
### Пример 1. Перезапуск сервера по имени ресурса
```powershell
PS C:\> Restart-AzMySqlFlexibleServer -ResourceGroupName PowershellMySqlTest -Name mysql-test
```

Перезапуск сервера по имени

### Пример 2. Перезапуск сервера по удостоверениям
```powershell
...
Перезапуск сервера по удостоверениям
