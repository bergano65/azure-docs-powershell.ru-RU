...
## ОПИСАНИЕ
Для создания файла или каталога в Filesystem в учетной записи хранилища Azure создается проектлет **New-AzDataLakeGen2Item.**
...
### Пример 1. Создание каталога с заданным разрешением, Propertiessk, свойств и метаданных
```
...
Эта команда создает каталог с указанными разрешениями, Овском, свойствами и метаданными

### Пример 2. Создайте (выкладывайте) файл озера данных из локального источника, и cmdlet работает в фоновом режиме
```
...
Эта команда создает (выкладывайте) файл озера данных из локального источника, и командлет выполняется в фоновом режиме.
...
### -ConcurrentTaskCount
Общий объем одновременной работы с async. Значение по умолчанию — 10.
...
### -Каталог
Указывает на то, что новый элемент является каталогом, а не файлом.
...
### -Force
Если он прошел, новый элемент создается без запроса
...
### -Метаданные
Определяет метаданные для созданного каталога или файла.
...
### -Path
Путь в указанной Filesystem, который нужно создать.
...
Символ (rwxrw-rw-) поддерживается.
...
### -Свойство
Определяет свойства созданного каталога или файла. Поддерживаются такие свойства файла: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5, ContentType.
...
### -Source
Укажите путь к локальному источнику файла, который будет добавлен в файл Datalake Gen2.
...
### -Аsk
При создании нового элемента в родительском каталоге нет стандартного ACL, она ограничивает разрешения для файла или каталога.
Итоговая разрешение предоставляется p & ^u, где p — это разрешение, а вы — sksk.
Символ (rwxrw-rw-) поддерживается.
