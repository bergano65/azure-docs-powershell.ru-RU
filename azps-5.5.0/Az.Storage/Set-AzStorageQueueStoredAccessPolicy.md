...
## ОПИСАНИЕ
**Cmdlet Set-AzStorageQueueStoredAccessPolicy** задает политику хранимого доступа для очереди хранилища Azure.

## ПРИМЕРЫ

### Пример 1. Настройка хранимой политики доступа в очереди с полными разрешениями
```
PS C:\> Set-AzStorageQueueStoredAccessPolicy -Queue "MyQueue" -Policy "Policy07" -Permission arup
```

Эта команда задает политику доступа с именем Policy07 для очереди хранения MyQueue.
...
### -NoStartTime
Указывает на то, что этот cmdlet задает время начала $Null.
