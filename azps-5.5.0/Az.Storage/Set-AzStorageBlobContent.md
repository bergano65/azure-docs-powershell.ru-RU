...
## ОПИСАНИЕ
Для отправки локального файла в большой большой BLOB-проект хранилища Azure с его использованием передается проектлет **Set-AzStorageBlobContent.**
...
Эта команда передает файл с именем PlanningData в BLOB-проект с именем Planning2015.
...
**Cmdlet Set-AzStorageBlobContent** передает файлы в контейнер ContosoUploads.
...
Он содержит метаданные, хранимые в $Metadata, и содержит PremiumPageBlobTier как P10.
...
Эта команда передает файл c:\temp\index.html в контейнер contosouploads с указанными свойствами BLOB-объектов и настроит StandardBlobTier как холодный.
...
### Пример 7. Отправка файла в большой бизнес-бизнес с областью шифрования
```
...
Эта команда передает файл в BLOB-файл с областью шифрования.
...
### -BlobType
Тип BLOB-ля, который будет добавлен этим cmdlet.
...
### -EncryptionScope
Область шифрования, используемая при создании запросов к BLOB-проекту.
...
Подробности в https://docs.microsoft.com/en-us/azure/storage/blobs/storage-blob-storage-tiers
