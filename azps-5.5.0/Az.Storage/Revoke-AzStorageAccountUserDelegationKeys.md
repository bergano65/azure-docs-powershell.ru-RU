...
## ОПИСАНИЕ
Cmdlet **Revoke-AzStorageAccountUserDelegationKeys** отменяет все ключи делегирования пользователей учетной записи хранения, поэтому все маркеры SAS удостоверений учетной записи хранения также будут отозваны.

## ПРИМЕРЫ

### Пример 1. Ото имя всех ключей делегирования пользователей для учетной записи хранения
```powershell
PS C:\>Revoke-AzStorageAccountUserDelegationKeys -ResourceGroupName "myresourcegroup" -AccountName "mystorageaccount"
```

В этом примере все ключи делегирования пользователей учетной записи хранения отозваны, поэтому все маркеры SAS удостоверений, созданные из ключей делегирования пользователей, также будут отозваны.
...
### -PassThru
Как правило, этот $true возвращает результат успешного завершения, но этот параметр заставляет его возвращать значение ($true).
