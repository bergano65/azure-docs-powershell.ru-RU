...
## ОПИСАНИЕ
При **этом каталог** удаляется с его учетной записью.
...
Эта команда удаляет папку ContosoWorkingFolder из папки ContosoShare06.
...
Этот cmdlet удаляет папку, указанную этим параметром.
...
### -PassThru
Означает, что если этот cmdlet успешно, он возвращает значение $True.
Если вы указали этот параметр и не сможете сделать это из-за недопустимого значения параметра _Path,_ будет возвращена ошибка.
...
### -Path
Путь к папке.
Если эта папка пуста, этот cmdlet удалит ее.
Если папка не пустая, этот cmdlet не вносит никаких изменений и возвращает ошибку.
...
Этот cmdlet удаляет папку под файловой папкой, указанной этим параметром.
