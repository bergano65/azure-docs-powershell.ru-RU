...
## ОПИСАНИЕ
В этом списке вы можете выбрать один из химпроиматоров **Get-AzStorageFileHandle.**

## ПРИМЕРЫ

### Пример 1. Рекурсивно перечислить все обрабатывающие файлы в папке, а затем отсортировать их по ClientIp и OpenTime
```
...
Эта команда содержит данные об обработке файлов в папке и сортировать выходные данные по ClientIp, а затем по OpenTime.

### Пример 2. Рекурсивно перечислить первые 2 обработки файлов в каталоге файлов
```
...
В этой команде перечислены первые 2 рекурсивно обрабатываемого файла в каталоге файлов.

### Пример 3. Список 3-го по шестой обработки файла
```
...
Эта команда со списком 3-го по шестой по счету в файле.
...
### -File
Объект CloudFile указал файл в списке "Обработка файлов".
...
### -Share
Объект CloudFileShare указал папку, в которой будут указаны файлы и каталоги.
...
### -IncludeTotalCount
Отчет о количестве объектов в наборе данных (integer), за которым следуют объекты. Если не удается определить общее количество, возвращается "Неизвестное общее количество".
В настоящее время этот параметр не имеет никакого отношения.
...
### -First
Возвращает только первые "n" объектов.
