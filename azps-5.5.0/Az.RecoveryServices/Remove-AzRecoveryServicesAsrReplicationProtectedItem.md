...
### -InputObject
Входной объект для cmdlet: объект asR protecttion protecttion item, соответствующий элементу, защищенного репликацией, для которого репликация должна быть отключена.
...
### -WaitForCompletion
Указывает на то, что перед возвратом cmdlet должен дождаться завершения операции.
