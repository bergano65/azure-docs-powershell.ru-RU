...
Запускает операцию создания элемента, защищенного репликацией, для указанного VmId и возвращает задание ASR, используемую для отслеживания операции (сценарий Azure в Azure). Для отбойной VM-информации, переданной в cmdlet для шифрования, будет использоваться.
...
### -Account
Запустите как учетную запись, которая будет использоваться для установки мобильной службы при необходимости. Должен быть одним из списков запуска в качестве учетных записей в материале ASR.
...
### -ProcessServer
Сервер обработки, который будет использовать для репликации этого компьютера. Для указания серверов конфигурации используйте список серверов процессов в тканье ASR, соответствующей серверу Configuration Server.
...
ИД виртуальной сети Azure, который нужно восстановить на компьютере в случае отсея.
...
Укажите ИД группы расположения близости, который будет использоваться средством отостановки vm в целевом регионе восстановления.
...
### -VMwareToAzure
Параметр переключения для указания реплицируемых элементов является виртуальной машиной или физическим сервером VMWARE, который будет реплицироваться в Azure.
