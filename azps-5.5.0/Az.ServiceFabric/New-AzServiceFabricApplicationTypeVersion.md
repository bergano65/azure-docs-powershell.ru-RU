...
## ОПИСАНИЕ
Этот cmdlet создает новую версию приложения с пакетом, указанным в -PackageUrl. Он должен быть доступен через конечную точку REST диспетчера ресурсов Azure, используемую во время развертывания. Приложение было упаковано и запаковлено с расширением SFPKG. Эта команда создает тип приложения, если он еще не существует.
...
В этом примере создается версия приложения "v1" в типе testAppType. Версия в манифесте приложения, содержалась в пакете, должна иметь ту же версию, что и в -Version.
...
### -DefaultParameter
Укажите значения по умолчанию для параметров приложения в качестве пар ключа и значения.
