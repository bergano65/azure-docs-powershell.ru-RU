...
Первая команда использует Get-AzKeyVault для получения секретного сейфа из хранилища ContosoVault и сохраняет результат в переменной с именем $Vault.
...
Четвертая команда добавляет секрет к службе VMSS, используя секрет сейфа, используя ключ ресурса и сертификат хранилища, $Vault и $CertConfig переменных.
