...
## SYNOPSIS
В ходе операции удаления доменной службы удаляется существующая доменная служба.
...
### Пример 1. Удаление AzADDomain по resourceGroupName и Name
```powershell
PS C:\> Remove-AzADDomainService -ResourceGroupName $env.ResourceGroupName -Name $env.ADdomainName

```

Удаление AzADDomain по resourceGroupName и Name

### Пример 2. Удаление AzADDomain с помощью InputObject
```powershell
...
Удаление AzADDomain с помощью InputObject
