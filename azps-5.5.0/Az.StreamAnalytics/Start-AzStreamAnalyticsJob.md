...
Эта команда запускает задание StreamingJob и указывает, что поток события вывода должен начинаться с timestamp 2014-07-03T01:00Z.
...
### -OutputStartTime
Время начала вывода.
Это значение является отметкой времени в формате ISO-8601, которая указывает на отправную точку потока выходных событий, или $Null, чтобы указать, что поток событий вывода начнется при запуске задания потоковой передачи.
Это свойство должно иметь значение, если *режим OutputStartMode* имеет значение CustomTime.
