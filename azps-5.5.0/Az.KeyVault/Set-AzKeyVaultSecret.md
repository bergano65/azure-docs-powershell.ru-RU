...
### Пример 1. Изменение значения секретного с помощью стандартных атрибутов
```powershell
...
### -SecretValue
Указывает значение секрета как объекта **SecureString.** Чтобы получить объект **SecureString,** используйте **cmdlet ConvertTo-SecureString.** Для получения дополнительных сведений введите `Get-Help
ConvertTo-SecureString` .
