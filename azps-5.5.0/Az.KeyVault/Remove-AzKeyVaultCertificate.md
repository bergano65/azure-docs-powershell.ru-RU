...
## ОПИСАНИЕ
**Cmdlet Remove-AzKeyVaultCertificate** удаляет сертификат из хранилища ключа.
...
Эта команда удаляет сертификат SelfSigned01 из хранилища ключей ContosoKV01.
Эта команда определяет параметр *Force.*
Таким образом, он не будет подсказок на подтверждение.

### Пример 2. Окончательное удаление удаленного сертификата из хранилища ключей
```powershell
PS C:\> Remove-AzKeyVaultCertificate -VaultName 'Contoso' -Name 'MyCert' -InRemovedState
```

Эта команда окончательно удаляет сертификат "MyCert" из хранилища ключей Contoso.
Для выполнения этого cmdlet необходимо разрешение на очистку, которое должно быть ранее и явно предоставлено пользователю в этом хранилище ключей.
...
### -InRemovedState
В случае навсегда удаляет ранее удаленный сертификат.
...
### -Name
Указывает имя сертификата, который этот cmdlet удаляет из хранилища ключей.
Этот cmdlet конструирует полное доменное имя (FQDN) сертификата на основе имени, указанного этим параметром, имени хранилища ключей и текущей среды.
...
### -VaultName
Указывает имя хранилища ключей, из которого этот cmdlet удаляет сертификат.
