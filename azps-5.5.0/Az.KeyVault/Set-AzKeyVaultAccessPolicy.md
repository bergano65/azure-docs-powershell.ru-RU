...
Первая команда предоставляет пользователю разрешения в Azure Active Directory на выполнение операций с ключами и секретами с ключным хранилищем PattiFuller@contoso.com Contoso03Vault. В *результате с помощью параметра PassThru* обновленный объект возвращается этим cmdlet.
Вторая команда изменяет разрешения, предоставленные в первой команде, чтобы теперь можно было не только устанавливать и удалять секрет, но и получать PattiFuller@contoso.com секрет. Разрешения для ключевых операций остаются без изменений после этой команды.
Конечная команда еще больше изменяет существующие разрешения на удаление всех PattiFuller@contoso.com разрешений для ключевых операций. Разрешения на секретные операции остаются без изменений после этой команды.
...
Эта команда предоставляет права доступа к секретам указанного имени пользователя, а также разрешения на получение, список и настройка разрешений для указанного имени пользователя.

### Пример 5. Включить секретную информацию, извлекаемую поставщиком ресурсов Microsoft.Compute из ключевых сейфов
```powershell
...
Затем с помощью результата этой команды вы Set-AzKeyVaultAccessPolicy разрешения для хранилища ключей **(myownvault)** для группы 2. В этом примере группы с именем "группа2" многут в одной командной строке.
...
Определяет разрешения на операции определения SaS и управляемые учетные записи хранения, которые необходимо предоставить пользователю или основной службе.
Допустимые значения для этого параметра:
- все
- Получить
- список
