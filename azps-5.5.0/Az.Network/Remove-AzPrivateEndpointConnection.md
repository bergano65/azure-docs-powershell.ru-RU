...
## ОПИСАНИЕ
Для удаления подключения к закрытой конечной точке удаляется **cmdlet Remove-AzPrivateEndpointConnection.**
...
В этом примере удаляется частное подключение конечной точки MyPrivateEndpointConnection1
...
### -ServiceName
Имя службы, к которую относится частное подключение конечной точки.
