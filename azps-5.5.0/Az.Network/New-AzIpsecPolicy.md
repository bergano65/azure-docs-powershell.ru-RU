...
## ОПИСАНИЕ
С New-AzIpsecPolicy создается предложение политики IPSec, которое будет использоваться для подключения к виртуальному сетевому шлюзу.
...
Создание политики IPSec, которая будет использоваться для нового подключения виртуального сетевого шлюза.
...
### -DhGroup
Группы DH, используемые в IKE, этапе 1 для первоначального SA
...
### -IkeEncryption
Алгоритм шифрования IKE (этап 1 IKE)
...
### -IkeIntegrity
Алгоритм целостности IKE (этап 1 IKE)
...
### -IpsecEncryption
Алгоритм шифрования IPSec (этап 2 IKE)
...
### -IpsecIntegrity
Алгоритм целостности IPSec (этап 2 IKE)
...
### -PfsGroup
Группы DH, используемые в этапе 2 IKE для нового ребенка SA
...
### -SADataSizeKilobytes
Связь безопасности IPSec (также называется быстрым режимом или этапом 2 SA) в КБ
...
### -SALifeTimeSeconds
Срок службы IPSec Security Association (также называется быстрым режимом или этапом 2 SA) в секундах
