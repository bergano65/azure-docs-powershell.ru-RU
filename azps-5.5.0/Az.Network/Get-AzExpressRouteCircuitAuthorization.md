...
Эти команды представляют собой вариант команд, используемых в примере 1. Однако в этом случае сведения возвращаются только для тех авторов, которые доступны для использования (то есть для авторизации, не назначенной виртуальной сети). Для этого сведения о авторизации канала возвращаются в команде 2 и сдаются **командлету Where-Object** по каналу.
