...
Эта команда создает новый канал авторизации с именем ContosoCircuitAuthorization, а затем сохраняет этот объект в переменной $Authorization. Важно сохранить объект в переменной: хотя **New-AzExpressRouteCircuitAuthorization** может создать авторизацию каналов, она не может добавить эту авторизацию в маршрут схемы. Вместо этого переменная $Authorization используется New-AzExpressRouteCircuit при создании нового схемы ExpressRoute.
