...
Когда создается виртуальный сетевой шлюз, для него настраивается набор стандартных политик vpn ipsec. Если пользователь на сайте хочет использовать настраиваемую политику ipsec для подключения к VPN-шлюзу, сначала необходимо настроить эту политику ipsec на VPN Gateway. **Set-AzVpnClientIpsecParameter** позволяет это сделать.
