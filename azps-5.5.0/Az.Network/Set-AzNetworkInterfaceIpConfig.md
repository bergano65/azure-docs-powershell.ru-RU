...
Первые две команды получают виртуальную сеть подсети под названием myvnet и подсеть подсети mysubnet и хранят ее в переменных, $vnet и $subnet соответственно. Третья команда получает сетевой интерфейс nic1, связанный с конфигурацией IP,которая должна быть обновлена. Третья команда задает частный IP-адрес для основного IP-адреса конфигурации IPconfig1 в качестве 10.0.0.11. Наконец, последняя команда обновляет сетевой интерфейс, обеспечивая успешное внесение изменений.
...
Четвертая команда получает сетевой интерфейс nic1, связанный с конфигурацией IP,которая должна быть обновлена. При Set-AzNetworkInterfaceIpConfig ipconfig1 основного IP-адреса конфигурации ipconfig1 устанавливается 10.0.0.11 и создается связь с извлеченной группой безопасности приложения.
Наконец, последняя команда обновляет сетевой интерфейс, обеспечивая успешное внесение изменений.
...
### -Name
Задает имя конфигурации IP-адреса сети, для которой этот cmdlet sets.
