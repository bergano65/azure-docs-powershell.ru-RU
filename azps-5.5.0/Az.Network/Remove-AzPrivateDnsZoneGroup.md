...
## ОПИСАНИЕ
Для удаления группы зон DNS удаляется **cmdlet Remove-AzPrivateDnsZoneGroup.** 
...
В примере выше удаляется группа DNS zone grup named dnsgroup1 из конечной точки test-pr-endpoint.
