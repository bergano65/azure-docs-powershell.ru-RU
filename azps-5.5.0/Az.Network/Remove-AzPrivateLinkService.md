...
## ОПИСАНИЕ
Для **удаления частной службы ссылок удаляется cmdlet Remove-AzPrivateLinkService.**
...
В этом примере удаляется частная служба с именем TestPrivateLinkService.
...
### -ResourceGroupName
Имя группы ресурсов для закрытой службы ссылок.
