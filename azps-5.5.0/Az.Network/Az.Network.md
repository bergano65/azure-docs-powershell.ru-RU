...
Добавляет конфигурацию исходящие правила в балансировую нагрузку.
...
Создает ресурс virtual Hub Route Table, который является частью VirtualHub.
...
Отключите подключенные vpn-клиентские подключения с помощью заданного виртуального сетевого шлюза.
...
Возвращает ресурс бастов или ресурсы бастов.
...
Получает конфигурацию пиринга для каналов ExpressRoute.
...
Получает группу правил брандмауэра Azure для сбора правил политики
...
Получает конфигурацию конфигурации для балансиров нагрузки.
...
Возвращает список информационных ресурсов тегов службы.
...
Переход от VM к следующему переходу.
...
Получает существующую структуру VpnServerConfiguration, указывающую на подключение к сайту.
...
Создает ресурс-базовую подпись.

### [New-AzContainerNicConfig](New-AzContainerNicConfig.md)
Создает объект конфигурации контейнера сетевого интерфейса.
...
Создает набор правил брандмауэра.
...
Создание конфигурации сервера внешнего радиуса
...
Командлет New-AzVirtualHubVnetConnection создает ресурс HubVirtualNetworkConnection, который приравняет виртуальную сеть к виртуальному концентратору Azure.
...
Удаляет сопоставления URL-адресов с пулом серверов серверов.
...
Начинается операция захвата пакетов с подключением виртуального сетевого шлюза.
