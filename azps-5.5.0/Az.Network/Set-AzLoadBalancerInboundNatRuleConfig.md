...
## ОПИСАНИЕ
Cmdlet **Set-AzLoadBalancerInboundNatRuleConfig** задает конфигурацию правила перевода сетевых адресов (NAT) для балансиров нагрузки Azure.

## ПРИМЕРЫ

### Пример 1. Изменение конфигурации входящие правила NAT для балансиров нагрузки
```powershell
...
Вторая команда передает балансиру нагрузки в $slb add-AzLoadBalancerInboundNatRuleConfig, который добавляет в него конфигурацию правила NAT для входящие проектов.
Третья команда передает балансиру нагрузки в **set-AzLoadBalancerInboundNatRuleConfig,** который сохраняет и обновляет конфигурацию входящие правила NAT.
Обратите внимание, что конфигурация правила была настроена без включения плавающего IP-адреса, который был включен предыдущей командой.
...
Этот cmdlet задает конфигурацию входящие правила NAT для балансироса нагрузки, указанного этим параметром.
...
### -Name
Указывает имя конфигурации входящие правила NAT.
...
### -Protocol
Определяет протокол, который соответствует конфигурации входящие правила NAT.
