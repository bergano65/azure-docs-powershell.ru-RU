...
В этом примере создается группа ресурсов, виртуальная сеть WAN, виртуальный концентратор, масштабируемый VPN-шлюз в Центральной ЧАСТИ США, а затем сразу удаляется. Чтобы скрыть запрос при удалении виртуального шлюза, используйте флаг -Force.
...
В этом примере создается группа ресурсов, виртуальная сеть WAN, виртуальный концентратор, масштабируемый VPN-шлюз в Центральной ЧАСТИ США, а затем сразу удаляется. Это удаление происходит с помощью piping powershell, в котором используется объект VpnGateway, Get-AzVpnGateway команды.
