...
## ОПИСАНИЕ
Для Set-AzNetworkWatcherConnectionMonitor ресурса обновляется подключение.

## ПРИМЕРЫ

### Пример 1. Обновление монитора подключения
```
...
В этом примере мы обновляем существующий монитор подключения, изменяя destinationAddress и добавляя теги.
...
### -SourceResourceId
Код ресурса, используемого в качестве источника монитора подключения.
