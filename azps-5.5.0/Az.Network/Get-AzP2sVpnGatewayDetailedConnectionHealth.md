...
С помощью **cmdlet get-AzP2sVpnGatewayDetailedConnectionHealth** можно получить подробные сведения о текущих точках подключения к сайтам из P2SVpnGateway. Клиент может скачать сведения о состоянии здоровья с url-адреса, который прошел через SAS. При этом будут отсвечены сведения о каждой точке подключения к сайту с именами пользователей, ветвями, в которых были выделены IP-адреса и т. д.
