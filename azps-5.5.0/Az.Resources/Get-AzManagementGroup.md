...
С помощью `Expand` флажка можно перемещаться по массиву и получать сведения `Children` о каждом ребенке. Например, `Children[0]` можно предоставить сведения о группе с отображаемой именем. `TestGroup1DisplayName`
