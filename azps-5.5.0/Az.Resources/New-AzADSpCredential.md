...
## ОПИСАНИЕ
Этот New-AzADSpCredential можно использовать для добавления новых учетных данных или для ската для основной службы.
Основной счет службы определен по имени объекта или основные службы.
...
Предоставленный общедоступный сертификат X509 (myapp.cer) с кодом Base64 добавляется к существующему основному службе с помощью его spN.
...
Значение даты окончания по умолчанию составляет один год от сегодняшнего дня.
Для учетных данных "асимметричного" типа это должно быть задано в день, когда допустим сертификат X509 или раньше нее.
...
Имя (SPN) основной службы, к имени которого нужно добавить учетные данные.
...
Объект-служба, к который нужно добавить учетные данные.
...
Для "асимметричных" учетных данных этого типа необходимо установить в день, начиная с даты действия сертификата X509, или после нее.
