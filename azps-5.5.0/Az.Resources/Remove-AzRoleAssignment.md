...
Область назначения может быть указана и, если не указана, по умолчанию для области подписки, т. е. оно попытается удалить назначение указанному основному заданию и роли в области подписки.
...
Удаляет первый объект назначения роли, извлеченный из командлета Get-AzRoleAssignment роли.
...
### -ObjectId
ОбъектНый объект Azure AD пользователя, группы или службы.
...
Необходимо использовать в сочетании с параметрами ResourceGroupName, ResourceType и (необязательно)ParentResource, чтобы создать иерархическую область в виде относительного URI, определяемого ресурсом, и удаления назначения в этой области.
