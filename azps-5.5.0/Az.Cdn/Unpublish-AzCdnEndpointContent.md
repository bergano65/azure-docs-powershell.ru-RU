...
## ОПИСАНИЕ
Для очистки содержимого из конечной точки cdn сети доставки содержимого Azure будет очистка содержимого из-за неопубликованного контента **AzCdnEndpointContent.**
...
### -CdnEndpoint
Указывает конечную точку, которую очищает этот cmdlet.
...
### -EndpointName
Указывает имя конечной точки, которую очищает этот cmdlet.
...
### -PurgeContent
Определяет массив относительных путей для содержимого на сервере-источнике, который очищает этот cmdlet.
