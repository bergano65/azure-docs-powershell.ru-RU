...
### EventhubPropertiesSet (по умолчанию)
```
...
## ОПИСАНИЕ
С New-AzEventHub создается новый центр событий Azure.
Чтобы создать Е eventhub со свойствами описания захвата, выполните следующие действия (примеры 2). 

## ПРИМЕРЫ

### Пример 1. Создание нового события в EventHub
```powershell
PS C:\> New-AzEventHub -ResourceGroupName MyResourceGroupName -NamespaceName MyNamespaceName -Name MyEventHubName
```

Создает центр событий с названием MyEventHubName с 3-дневной периодом хранения сообщений и двумя разделами в расположении WestUS с группой ресурсов \` \` \` \` \` MyResourceGroupName. \`

### Пример 2. Обновление Eventhub с помощью captureDescription
```powershell
...
Создает центр событий с названием MyEventHubName с 3-дневной периодом хранения сообщений, 2 разделами и свойствами CaptureDescription в расположении WestUS и группой ресурсов \` \` \` \` \` MyResourceGroupName. \`
...
Хранение сообщений в Eventhub в днях
