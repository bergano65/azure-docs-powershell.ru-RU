...
Возвращает группу потребителей \` MyConsumerGroupName в Центре событий MyEventHubName, который существует в пространствах имен MyNamespaceName с группой ресурсов \` \` \` \` \` \` MyResourceGroupName. \`
...
### -MaxCount
Определите максимальное число групп ConsumerGroups, которые нужно вернуть.
