...
## ОПИСАНИЕ
Этот Get-AzSqlInstanceOperation получает сведения об операциях с SQL управляемым экземпляром. Вы можете просмотреть все операции с управляемым экземпляром или просмотреть конкретную операцию, укав ее имя.

## ПРИМЕРЫ

### Пример 1. Получить все операции экземпляра
```powershell
...
Эта команда возвращает все операции, которые SQL управляемым экземпляром.

### Пример 2. Получить определенную операцию
```powershell
...
Эта команда получает операцию с именем 5870c6d8-6703-4b27-8ae4-687b4ca7caea в SQL управляемый экземпляр.

### Пример 3. Использование ид ресурса операции
```powershell
...
Эта команда получает операцию с id '/subscriptions/a8c9a924-06c0-4bde-9788-e7b1370969e1/resourceGroups/ps3753/providers Microsoft.Sql/managedInstances/ps3698/operations/5870c6d8-6703-4b27-8ae4-687b4ca7caea'.
...
### -ResourceId
Идентификатор ресурса для операции управляемого экземпляра.
