...
## ОПИСАНИЕ
С Remove-AzSqlElasticJobTarget целевой ресурс удаляется из целевой группы.

## ПРИМЕРЫ

### Пример 1. Удаление целевого сервера
```powershell
...
### Пример 2. Удаление целевого сайта базы данных
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
### Пример 3. Удаление целевого пула в качестве эластичного пула
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
### Пример 4. Удаление целевого показателя карты в области «Карта»
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
Удаляет целевую группу (серверную, эластичный пул, базу данных и карту shard) из целевой группы.

## PARAMETERS

### -AgentName
SQL имя агента базы данных.
...
### -AgentServerName
SQL имя сервера агента базы данных.
...
### -ElasticPoolName
Целевое имя целевого пула в эластичном пуле
...
Обновление имени учетных данных
