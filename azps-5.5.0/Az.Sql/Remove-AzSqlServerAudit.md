...
## ОПИСАНИЕ
С **его** SQL Azure удаляются параметры аудита для сервера Azure SQL.
Укажите *параметры ResourceGroupName* *и ServerName,* чтобы определить сервер.

## ПРИМЕРЫ

### Пример 1. Удаление параметров аудита сервера Azure SQL
```
PS C:\>Remove-AzSqlServerAudit -ResourceGroupName "resourcegroup01" -ServerName "server01"
```

### Пример 2. Удаление через канал параметров аудита сервера Azure SQL
```
