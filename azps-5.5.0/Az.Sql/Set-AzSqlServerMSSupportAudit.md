...
### Пример 1. В enable the BLOB-хранилище Microsoft support operations auditing policy of an Azure SQL server
```powershell
...
### Пример 3. В том случае, если центр событий поддерживает операции аудита сервера Azure SQL Майкрософт
```powershell
...
### Пример 4. Отключение центра поддержки событий, в ходе работы с политикой аудита сервера Azure SQL Майкрософт
```powershell
...
### Пример 7. Отключение (через конвейер) аналитики журналов корпорацией Майкрософт для операций аудита сервера Azure SQL Server
```powershell
