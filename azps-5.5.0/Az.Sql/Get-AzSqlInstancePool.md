...
## ОПИСАНИЕ
**Cmdlet Get-AzSqlInstancePool** возвращает сведения об одном или SQL Azure.
Укажите имя пула экземпляров, чтобы увидеть сведения только для этого пула экземпляров.

## ПРИМЕРЫ

### Пример 1. Все пулы экземпляров для подписки клиента
```powershell
...
### Пример 2. Объединение всех пулов экземпляров в группу ресурсов
```powershell
...
### Пример 3. Просмотр сведений о пуле экземпляров
```powershell
...
Эта команда получает сведения о экземпляре пула экземпляровPool0.

### Пример 4. Получите сведения о пуле экземпляров с помощью ИД ресурса пула экземпляров
```powershell
...
Эта команда получает сведения о пуле экземпляров с идентификатором ресурса.
