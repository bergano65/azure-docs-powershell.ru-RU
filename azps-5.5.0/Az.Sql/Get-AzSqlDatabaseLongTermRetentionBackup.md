...
## ОПИСАНИЕ
Чтобы получить все резервные копии хранения для местоположения, сервера или базы данных, можно получить все долгосрочные резервные копии с **cmdlet Get-AzSqlDataBaseLongTermRetentionBackup.**

## ПРИМЕРЫ

### Пример 1. Получить все резервные копии для расположения
```powershell
...
Эта команда получает все долгосрочные резервные копии хранения для всех баз данных (которые могут быть живы или удалены) в northeurope, группа ресурсов будет настроена только в том случае, если сервер находится в реальном времени.

### Пример 2. Получить все резервные копии для расположения в группе ресурсов
```powershell
...
Эта команда получает все долгосрочные резервные копии хранения для всех баз данных (которые могут быть живы или удалены) в группе ресурсов в northeurope.

### Пример 3. Получите резервную копию хранения в течение определенного времени
```powershell
...
### Пример 4. Получите все резервные копии хранения в долгосрочной перспективе для базы данных
```powershell
...
Эта команда получает все долгосрочные резервные копии хранения для базы данных01

### Пример 5. Получите долгосрочное резервное копирование хранения с помощью фильтрации
```powershell
...
Эта команда получает все резервные копии с именем, которое начинается с "601061b7"
...
### -DatabaseName
Имя базы данных Azure SQL, из которой создана резервная копия.
...
### -DatabaseState
Состояние базы данных, резервные копии которой нужно найти: "Живые", "Удаленные" или "Все".
...
### -InputObject
Объект базы данных, для которой нужно получить резервные копии.
...
### -Location
Расположение исходных серверов резервных копий.
...
Нужно ли получать только последние резервные копии для базы данных.
...
### -ResourceId
ИД ресурса базы данных, для которой нужно получить резервные копии.
...
### -ServerName
Имя службы Azure SQL Server под резервными копиями.
