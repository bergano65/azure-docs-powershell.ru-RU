...
### RemoveIFGDefaultSet (по умолчанию)
```
...
## ОПИСАНИЕ
Эта команда удаляет группу отбойных экземпляров с указанным именем, оставляя все базы данных без изменений. Конечная точка будет отрегистрирована из DNS.
...
Удалите группу отбойных экземпляров.
...
### -InputObject
Удаляемая группа отбойных экземпляров
...
### -Name
Имя группы отбойных экземпляров, которые нужно удалить.
...
### -PassThru
Указывает, следует ли пройти через вывод выполнения с помощью cmdlet.
...
### -ResourceId
ИД ресурса группы отбойных экземпляров, который требуется удалить.
