...
## ОПИСАНИЕ
Этот Set-AzSqlInstanceTransparentDataEncryptionProtector задает защиту TDE для SQL управляемого экземпляра. При изменении типа защиты TDE он будет повернут.

## ПРИМЕРЫ

### Пример 1. Задайте для типа защиты прозрачного шифрования данных (TDE) тип ServiceManaged
```powershell
...
Эта команда обновляет тип защиты TDE управляемого экземпляра на Service Managed.

### Пример 2. Выбор прозрачного типа защиты шифрования данных в хранилище ключей Azure
```powershell
...
Эта команда обновляет указанный управляемый экземпляр, чтобы использовать ключ ключа управляемого экземпляра с ID ' в качестве защиты https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 TDE.

### Пример 3. Выбор типа прозрачного защищенного ключа шифрования данных в хранилище ключа Azure с помощью объекта управляемого экземпляра
```powershell
...
### Пример 4. Выбор типа прозрачного защищенного ключа шифрования данных в хранилище ключа Azure с помощью ид ресурса
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### Пример 5. Выбор прозрачного типа защиты шифрования данных в хранилище ключей Azure с помощью piping
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### -Type
Прозрачный защита от шифрования данных в базе данных Azure Sql.
