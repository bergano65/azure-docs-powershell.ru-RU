...
## ОПИСАНИЕ
Для обновления свойств задачи в службе Azure Batch обновляется **cmdlet Set-AzBatchTask.**
Используйте Get-AzBatchTask для получения объекта **PSCloudTask.**
...
Первая команда получает задачу с помощью **Get-AzBatchTask,** а затем сохраняет ее в $Task переменной.
Следующие две команды изменяют ограничения задачи в $Task.
Конечная команда обновляет пакетную службу в соответствие с локальным объектом в $Task.
...
### -Задача
Определяет **PSCloudTask,** для которого этот cmdlet обновляет пакетную службу.
