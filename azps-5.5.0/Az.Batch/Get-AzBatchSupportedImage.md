...
## ОПИСАНИЕ
Для получения поддерживаемых изображений виртуальных машин, доступных в учетной записи Azure Batch, поддерживается cmdlet **Get-AzBatchSupportedImage.**
Укажите учетную запись с помощью *параметра BatchContext.*

## ПРИМЕРЫ

### Пример 1. Получить все доступные поддерживаемые изображения
...
Вторая команда получает все поддерживаемые изображения для этой учетной записи Пакета.

## PARAMETERS

### -BatchContext
Экземпляр BatchAccountContext, который используется при взаимодействии со службой batch.
Если для Get-AzBatchAccount BatchAccountContext используется Get-AzBatchAccount, при взаимодействии со службой batchy будет использоваться проверка подлинности Azure Active Directory.
...
### -Filter
Предложение фильтра OData для поддерживаемых изображений.
Если не указать фильтр, этот cmdlet возвращает все изображения, поддерживаемые учетной записью batch.
...
### -MaxCount
Указывает максимальное количество поддерживаемых изображений, которые нужно вернуть.
