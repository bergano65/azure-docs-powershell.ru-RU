...
## ОПИСАНИЕ
С **использованием cmdlet Remove-AzBatchPool** удаляется указанный пул пакетов Azure.
Вам будет предложено подтвердить запрос, если вы не используете параметр *Force.*

## ПРИМЕРЫ

### Пример 1. Удаление пакетного пула по ИД пула
```
PS C:\>Remove-AzBatchPool -Id "MyPool" -BatchContext $Context
```

Эта команда удаляет пул с помощью ID MyPool.
Перед удалением пользователю будет предложено подтвердить операцию.

### Пример 2. Принудительное удаление всех пакетных пулов
```
PS C:\>Get-AzBatchPool -BatchContext $Context | Remove-AzBatchPool -Force -BatchContext $Context
```

Эта команда удаляет все пакетные пулы.
Так как параметр *Force* присутствует, запрос подтверждения подавляется.
...
### -Id
Определяет ИД пула, который нужно удалить.
