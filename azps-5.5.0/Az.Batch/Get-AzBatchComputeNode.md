...
## ОПИСАНИЕ
Для получения узлов обработки пакетных вычислений Azure из пула получается **cmdlet Get-AzBatchComputeNode.**
Укажите параметр *PoolID* или *Pool.*
Укажите *параметр "ИД",* чтобы получить один узел вычислений.
Укажите *параметр Filter,* чтобы получить узлы компьютеров, которые соответствуют фильтру OData.

## ПРИМЕРЫ

### Пример 1. Получить узел вычислений по ИД
```
...
Эта команда получает узел вычислений, который содержит телевизор с ИД-2316545714_1-20150725t213220z из пула, который содержит ID Pool06.
Используйте Get-AzBatchAccountKey для назначения контекста переменной $Context.

### Пример 2. Получить из пула все неавтовые узлы для компьютеров
```
...
Эта команда возвращает все узлы неательных компьютеров, содержащиеся в пуле с пулом ID Pool06.
Команда определяет состояние неа установленного состояния с помощью *параметра Filter.*

### Пример 3. Объединение всех узлов компьютеров в указанный пул
```
...
Эта команда возвращает пул с пулом ID Pool07 с помощью Get-AzBatchPool командлета.
Эта команда передает пул текущему командлету с помощью оператора конвейера.
Он получает все узлы компьютеров из этого пула.
...
Этот командлет возвращает вычислительные узлы, которые соответствуют фильтру, указанному этим параметром.
Если не указать фильтр, этот cmdlet возвращает все узлы компьютеров для пула.
...
### -Id
Определяет код узла вычислений, который этот cmdlet получает из пула.
...
### -MaxCount
Указывает максимальное количество возвращаемых узлов компьютеров.
...
### -Pool
Пул (объект **PSCloudPool),** содержащий узлы для вычислений.
Чтобы получить объект **PSCloudPool,** воспользуйтесь Get-AzBatchPool.
...
### -PoolId
Определяет ИД пула, который содержит узлы для компьютеров.
