...
## ОПИСАНИЕ
С **его учетной записью в** текущей подписке получается учетная запись Azure Batch. Вы можете использовать параметр *AccountName* для получения одной учетной записи или параметр *ResourceGroupName* для получения учетных записей в этой группе ресурсов.
