...
## ОПИСАНИЕ
Для указанной учетной записи с помощью cmdlet **Get-AzBatchPoolUsageMetric** метрики использования, совокупные по пулу за отдельные интервалы времени.
...
Эта команда указывает строку фильтра, определяемую пулом, и использует то же $Context, что и в предыдущем примере.
...
Допустимым свойством является **poolId со** строковой строкой.
