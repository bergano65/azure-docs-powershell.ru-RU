...
## ОПИСАНИЕ
С **помощью cmdlet Get-AzBatchPool** можно получить пулы Azure Batch в учетной записи Batch, заданной с *параметром BatchContext.*
Параметр *Id* можно использовать для получения одного пула или с помощью параметра *Filter* для получения пулов, которые соответствуют фильтру OData.

## ПРИМЕРЫ

### Пример 1. Получить пул по ИД
```
...
Эта команда получает пул с ID MyPool.

### Пример 2. Получить все пулы с помощью фильтра OData
```
...
Эта команда возвращает пулы, ИД которых начинаются с моего, с помощью *параметра Filter.*
...
### -Filter
Указывает предложение фильтра OData, используемее при запросе пулов.
Если не указать фильтр, возвращаются все пулы в учетной записи Batch, заданной с параметром *BatchContext.*
...
### -Id
Определяет ИД пула, который нужно получить.
