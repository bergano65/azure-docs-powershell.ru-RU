...
## ОПИСАНИЕ
Для узла компьютеров в пуле виртуальных машин, основанном на инфраструктуре, командылет **Get-AzBatchRemoteLoginSetting** получают параметры удаленного логина.
...
Первая команда получает контекст пакетной учетной записи с ключами доступа для вашей подписки, а затем сохраняет ее в переменной $Context доступа.
