...
## ОПИСАНИЕ
Этот Get-AzAttestationPolicySigners получает доверенных подписателей политики от клиента в Azure Attestation.
...
Получает доверенных подписателей для поставщика attestation Provider *pshtest* в группе ресурсов *psh-test-rg.*  Обратите внимание, что для этого поставщика проверки нет доверенных подписок.
...
Подписывает доверенных подписателей для поставщика attestation Provider *pshtest2* в группе ресурсов *psh-test-rg.*  Обратите внимание на то, что для этого поставщика услуг attestation имеется один надежный подписыватель.
...
Получает доверенных подписателей для поставщика по умолчанию для проверки attestation в Центре *обслуживания по месту США.*  Обратите внимание, что надежных подписателей для поставщика по умолчанию для проверки attestation нет.
...
### -DefaultProvider
Указывает, что это запрос к поставщику проверки по умолчанию.
...
### -Location
Определяет расположение поставщика для проверки по умолчанию.
