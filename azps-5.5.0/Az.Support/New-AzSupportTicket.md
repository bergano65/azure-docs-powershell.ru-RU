...
## SYNOPSIS
Создается билет в службу поддержки.
...
## ОПИСАНИЕ
С его можно создать вопрос о выставлении счета, управлении подписками, квоте или технических вопросах. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для определения службы Azure и соответствующих классификаций проблем, для которых требуется получить поддержку. Необходимо указать следующие параметры: 
...
Для создания объекта CustomerContactDetail можно использовать New-AzSupportContactProfileObject-помощника.

Поставщики облачных решений могут создать билет в службу поддержки для подписок клиента, войдя в клиент клиента и указав его номер домашнего клиента с помощью параметра *CSPHomeTenantId.*

__Для технических билетов:__

Чтобы указать имя ресурса, укажите ARM ресурса с помощью параметра *TechnicalTicketResourceId.* См. пример ниже. 

__Для билетов на квоту:__

Чтобы запросить увеличение квоты для ядер  **VM Compute,** пакетной базы **данных,** SQL и хранилища **SQL,** уделять дополнительные сведения *объекту QuotaTicketDetail.* Объект QuotaTicketDetail состоит из 3 свойств, как описано ниже. Для получения подробной документации [щелкните здесь.](https://aka.ms/supportrpquotarequestpayload)
...
Подробные сведения о том, как построить грузку для различных типов квот, можно [найти здесь](https://aka.ms/supportrpquotarequestpayload)

## ПРИМЕРЫ

### Пример 1. Создание билета в службу поддержки по вопросам вы выставления счета или управления подписками. Используйте Get-AzSupportService и Get-AzSupportProblemClassification, чтобы получить правильныеGUID для классификации проблем с выставлением счета или управлением подписками, для которой нужно запросить поддержку 
```powershell
...
### Пример 2. Создайте билет в службу технической поддержки для ресурса Virtual Machine для Windows. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации проблем виртуальной машины для Windows, для которой требуется запросить поддержку 
```powershell
...
### Пример 3. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для ядер виртуальной машины для определенного семейства VM. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUIDs для классификации проблемных Ядер VM в quota Compute.
```powershell
...
### Пример 4. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для ядер с низким приоритетом для учетной записи с низким приоритетом. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблем с пакетом квот.
```powershell
...
### Пример 5. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту Ядра VM для определенной семьи VM для учетной записи пакета. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблем с пакетом квот.
```powershell
...
### Пример 6. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту пулов для учетной записи пакета. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблемных пакетов квот.
```powershell
...
### Пример 7. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту активных заданий и расписания для учетной записи пакета. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблем с пакетом квот.
```powershell
...
### Пример 8. Создайте билет в службу поддержки по квоте, чтобы увеличить количество пакетных учетных записей для подписки. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблем с пакетом квот.
```powershell
...
### Пример 9. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту DTUs для SQL базы данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильных GUIDs для классификации SQL управления базой данных.
```powershell
...
### Пример 10. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для серверов SQL базы данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильных GUIDs для классификации SQL управления базой данных.
```powershell
...
### Пример 11. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту DTUs для SQL хранилища данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации SQL хранилища дат.
```powershell
...
### Пример 12. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для серверов SQL хранилища данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильных GUID для классификации SQL хранилища данных.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### Пример 13. Создайте билет в службу поддержки с квотой, чтобы увеличить квоту для ядер с низким приоритетом для службы машинного обучения. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильных GUID для классификации проблем службы машинного обучения.
```powershell
...
### Пример 14. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту ядер VM для конкретной службы VM Для машинного обучения. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации проблем службы машинного обучения по квоте.
```powershell
...
### Пример 15. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для azure SQL Управляемый экземпляр. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильных GUIDs для классификации SQL проблемы со службой управляемых экземпляров.
```powershell
...
### Пример 16. Создайте билет в службу поддержки, указав параметры контактов клиента вместо объекта CustomerContactDetail. 
```powershell
...
### Пример 17. Создание запроса в службу поддержки с запросом на 24 x 7 ответов от Azure. 
```powershell
...
### Пример 18. Создайте билет в службу поддержки от имени клиента, если вы поставщик облачных решений. CSP сначала должен войти в клиент, а затем войти в клиент клиента, как показано в примере ниже. После этого они должны использовать параметр -CSPHomeTenantId, чтобы указать ид домашнего клиента во время создания билета в службу поддержки.  
```powershell
...
### -CSPHomeTenantId
Это номер домашнего клиента, который пользователь поставщика облачных решений пытается создать для своего клиента в службу поддержки.
...
Это должен быть допустимый код для одного из поддерживаемых языков, перечисленных в этом https://azure.microsoft.com/en-us/support/faq/ списке.
...
### -Name
Имя билета в службу поддержки, который создает этот cmdlet.
...
### -ProblemClassificationId
Каждая служба Azure имеет собственный набор категорий проблем, который соответствует типу проблемы.
Этот параметр является ARM ресурса ProblemClassification.
...
### -QuotaTicketDetail
Дополнительные сведения о квоте для получения поддержки.
...
### -Require24X7Response
Указывает, требуется ли для этого запроса в службу поддержки ответ в 24:7 из Azure.
...
### -Severity
Серьезность билета в службу поддержки.
Это означает срочность дела, которая, в свою очередь, определяет время ответа в соответствии с соглашением об уровне обслуживания плана технической поддержки Azure.
...
Это ид ресурса ресурса службы Azure (например, ресурс виртуальной машины или ресурс HDInsight), для которого создается билет в службу поддержки.
...
### -Title
Название билета в службу поддержки.
