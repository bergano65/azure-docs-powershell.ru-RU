...
## SYNOPSIS
Обновив network ACL службы SignalR.
...
## ОПИСАНИЕ
Обновление сетевого ACL службы SignalR, включая действие по умолчанию и сетевые acls для подключения к общедоступным и частным сетям.

## ПРИМЕРЫ

### Разрешить RESTAPI,ClientConnection для общедоступных сетей и установить для действия по умолчанию запрет
```powershell
...
### Разрешение клиентского подключения и подключения к серверу для подключения к закрытой конечной точке
```powershell
...
### Запретить клиентскую подключение как для общедоступных сетей, так и для частных конечных точек.
```powershell
...
### -DefaultAction
Действие по умолчанию сетевых ALS SignalR, разрешать или от запретить. Он решает, следует ли запретить ALS сети или разрешить ее действие. Например, если по умолчанию разрешается действие, имеет значение только запрет на ALS.
...
### -PrivateEndpointName
Имена частных конечных точек, которые требуется обновить
...
### -PublicNetwork
Обновление ALS общедоступных сетей
