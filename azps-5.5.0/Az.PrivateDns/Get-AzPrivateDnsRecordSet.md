...
## ОПИСАНИЕ
Этот Get-AzPrivateDnsRecordSet получает набор записей частного доменного имени (DNS) с указанным именем и типом в указанной частной зоне. Если не указать параметры Name или RecordType, этот cmdlet возвращает все наборы записей указанного типа в закрытой зоне. Если для параметра RecordType задан параметр RecordType, но не параметр Name, этот cmdlet возвращает все наборы записей указанного типа записи. С помощью оператора конвейера можно передать объект PSPrivateDnsZone этому cmdlet или передать объект PSPrivateDnsZone в качестве параметра зоны. Кроме того, можно указать зону и группу ресурсов по имени. Вы также можете указать частную зону, указав ее ИД ресурса.
...
Эта команда получает набор записей типа A "www" в указанной группе ресурсов и закрытой зоне, а затем сохраняет его в $RecordSet переменной. Так как параметры Name и RecordType заданы, возвращается только один объект RecordSet.
...
Эта команда получает массив всех наборов записей типа A в частной зоне myzone.com в группе ресурсов MyResourceGroup, а затем сохраняет их в переменной $RecordSets.

### Пример 3. Получить все наборы записей в закрытой зоне
```powershell
...
Эта команда получает массив всех наборов записей в частной зоне myzone.com в группе ресурсов MyResourceGroup, а затем сохраняет их в переменной $RecordSets ресурса.

### Пример 4. Получить все наборы записей в частной зоне с помощью объекта PSPrivateDnsZone
```powershell
...
Этот пример эквивалентен примеру 3 выше. На этот раз частная зона задана с использованием объекта закрытой зоны.
...
### -Zone
Объект DnsZone, представляющий зону, в которой создается набор записей.
