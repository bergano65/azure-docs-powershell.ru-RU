...
## ОПИСАНИЕ
Командлет **New-AzPrivateDnsVirtualNetworkLink** создает виртуальную сеть DNS в указанной группе ресурсов и закрытой зоне. Необходимо указать уникальное имя ссылки для параметра *Name,* иначе при этом будет возвращена ошибка. Параметр Confirm  и переменная $ConfirmPreference Windows PowerShell можно использовать для управления запросом на подтверждение с помощью cmdlet.

## ПРИМЕРЫ

### Пример 1. Создание ссылки на частную виртуальную сеть DNS
```
...
Эта команда создает виртуальную сетевую связь, связанную с частной зоной DNS с именем myzone.com и виртуальной сетью "myvirtualnetwork" (которая уже была создана в группе ресурсов) в указанной группе ресурсов, а затем сохраняет ее в переменной $Link.
...
### -EnableRegistration
Параметр переключения, который представляет, включена ли ссылка на регистрацию.
...
### -Name
Указывает имя создаемой виртуальной сетевой ссылки.
...
### -RemoteVirtualNetworkId
ИД ресурса виртуальной сети в другом клиенте.
...
### -ResourceGroupName
Определяет группу ресурсов, в которой нужно создать связь.
...
### -VirtualNetwork
Виртуальный сетевой объект, связанный со ссылкой.
...
### -VirtualNetworkId
ИД ресурса виртуальной сети, связанной со ссылкой.
...
### -ZoneName
Указывает имя зоны, которая будет связана с виртуальной сетевой связью.
