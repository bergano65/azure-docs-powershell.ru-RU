...
## ОПИСАНИЕ
С **помощью cmdlet Add-AzMetricAlertRule** можно добавить или обновить правило оповещения на основе метрических показателей.
...
### -MetricName
Указывает имя метрик, для которого отслеживается правило.
...
### -TargetResourceId
Определяет ИД ресурса, который отслеживается правилом. ПРИМЕЧАНИЕ. Это свойство невозможно обновить для существующего правила оповещения.
...
Указывает оператор агрегирования, который будет применяться к периоду времени при оценке правила.
