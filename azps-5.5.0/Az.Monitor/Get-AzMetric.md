...
## ОПИСАНИЕ
С **помощью cmdlet Get-AzMetric** можно получить метрические значения для указанного ресурса.

## ПРИМЕРЫ

### Пример 1. Метрическая система получения обобщенных результатов
```
...
Эта команда получает метрические значения для website3 с временем, большим количеством минут.

### Пример 2. Метрическая система подробных выходных данных
```
...
Эта команда получает метрические значения для веб-сайта3 с временем, большим количеством минут.
Результат будет подробным.

### Пример 3. Подробные выходные данные для указанной метрик
```
...
Эта команда получает подробные выходные данные для метрике "Запросы".

### Пример 4. Обобщение результатов для указанной метрик с помощью указанного фильтра измерения
```
...
Эта команда получает итоговые результаты для метрик PageViews с указанным фильтром измерения и типом агрегирования.
...
### -DetailedOutput
Указывает на то, что этот cmdlet выводит подробные данные.
По умолчанию выходные данные обобщены.
...
### -MetricFilter
Фильтр метрических измерений для метрики запроса.
...
### -MetricNamespace
Указывает пространство имен метрики, для которого нужно запрашивать метрики.
...
### -OrderBy
Определяет агрегат, который будет применяться для сортировки результатов, и направление сортировки (пример: sum asc).
...
### -ResultType
Тип возвращаемого результата (метаданные или данные).
...
### -TimeGrain
Определяет time grain of the metric as a **TimeSpan** object in the format hh:mm:ss.
...
### -Top
Указывает максимальное количество записей, которые нужно извлечь (по умолчанию:10), заданное с помощью $filter.
...
Дополнительные сведения о поддерживаемых показателях можно найти по https://docs.microsoft.com/en-us/azure/azure-monitor/platform/metrics-supported
