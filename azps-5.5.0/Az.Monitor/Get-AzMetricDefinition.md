...
## ОПИСАНИЕ
Для **этого с помощью cmdlet Get-AzMetricDefinition** будут метрические определения.

## ПРИМЕРЫ

### Пример 1. Получить метрические определения для ресурса
```
...
### Пример 2. Получите метрические определения с подробными выходными сведениями
```
...
Эта команда получает определения метрик для website2.
Результат будет подробным.

### Пример 3. Определение метрик по имени
```
...
### -MetricNamespace
Указывает пространство имен метрик, для которого нужно запрашивать определения метрик.
