...
## ОПИСАНИЕ
Узлы DSC (DSC) APS получает от автоматизации Azure для получения узлы **Get-AzAutomationDsnode.**
...
Эта команда получает метаданные для всех узлов DSC в учетной записи автоматизации Contoso17, которые сопостаются с конфигурацией узла DSC, которая была сгенерирована конфигурацией DSC ContosoConfiguration.
...
### -ResourceGroupName
Указывает имя группы ресурсов, в которой этот cmdlet получает узлы DSC.
