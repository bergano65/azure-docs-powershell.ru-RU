...
## ОПИСАНИЕ
С New-AzDataMigrationTask создается задача переноса данных. Этот cmdlet принимает в качестве входных параметров для enumerator "Тип задачи", "Группа ресурсов Azure", имени связанной службы миграции базы данных Azure и Project. 
...
В этом примере показано, как создать задачу переноса данных с именем MyMigrationTask в проекте myDMSProject и службе TestService. 
...
### -Wait
Следует ли ждать завершения задачи. Если флажок установлен, проверяется каждые одну секунду до завершения задачи и возвращается к свойствам задачи, для которых можно проверить выходные данные или ошибки. 
```yaml
