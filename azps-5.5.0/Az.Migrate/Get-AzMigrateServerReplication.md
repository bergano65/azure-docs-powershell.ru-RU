...
## SYNOPSIS
Извлекает сведения о сервере репликации.
...
### Пример 1. Подробные сведения по id
```powershell
...
### Пример 2. Список всех проектов по id.
```powershell
...
### Пример 2. Список всех проектов по имени.
```powershell
...
### -DiscoveredMachineId
Определяет ИД компьютера найденного сервера.
...
### -InputObject
Определяет машинный объект сервера репликации.
...
### -MachineName
Указывает отображаемого имя компьютера репликации.
...
### -ProjectName
Указывает проект Azure Migrate в текущей подписке.
...
INPUTOBJECT: <IMigrationItem> определяет машинный объект сервера репликации.
