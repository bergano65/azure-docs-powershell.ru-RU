...
## SYNOPSIS
Создает новое удостоверение назначенного пользователя или обновляет существующее удостоверение.
...
## ОПИСАНИЕ
Для **создания нового идентификатора User AssignedIdentity new-AzUserAssignedIdentity** будет создаваться новое удостоверение, назначенное пользователю. При его использования с уже существующим удостоверением оно обновлялось.
Чтобы добавить теги Диспетчера ресурсов Azure в удостоверение, воспользуйтесь Set-AzResource командлетом.

## ПРИМЕРЫ

### Пример 1
В этом примере создается новое назначенное пользователем удостоверение с именем **ID1** в группе **ресурсов PSRG** в расположении группы ресурсов.
...
### Пример 2
В этом примере создается новое удостоверение, назначенное пользователю, с именем **ID1** в группе **ресурсов PSRG** в западной части региона.
...
### -Location
Имя региона Azure, в котором нужно создать удостоверение.
...
### -Tag
Теги Диспетчера ресурсов Azure, связанные с удостоверением.
