...
### Пример 1. Создание рабочей области "Datab ветвей"
```powershell
...
Эта команда создает рабочее пространство Datab ветвей.

### Пример 2. Создание рабочей области Datab в настраиваемой виртуальной сети
```powershell
...
Эта команда создает рабочее пространство Datab в группе ресурсов с настроенной виртуальной сетью.

### Пример 3. Создание рабочей области Datab ветвь шифрования
```powershell
...
Эта команда создает рабочее пространство Datab ветвей и настраивает ее для подготовки к шифрованию.
Дополнительные параметры шифрования Update-AzDatabricksWorkspace примерах этой программы.
...
### -EnableNoPublicIP
Значение, которое будет использоваться для этого поля.
...
Имя группы управляемых ресурсов.
...
### -PrepareEncryption
Подготовьте рабочее пространство для шифрования.
Включает управляемое удостоверение для учетной записи управляемого хранилища.
...
### -PrivateSubnetName
Имя частной подсети в виртуальной сети.
...
### -PublicSubnetName
Имя открытой подсети в виртуальной сети.
...
Boolean indicating whether the DBFS root file system will be enabled with secondary layer of encryption with platform managed keys for data at rest.
...
### -VirtualNetworkId
Это ИД виртуальной сети, в которой должен быть создан кластер datab в сети.
...
### Microsoft.Azure.PowerShell.Cmdlets.Datab models.Models.Api20180401.IWorkspace
