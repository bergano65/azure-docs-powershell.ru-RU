...
## ОПИСАНИЕ
Командлет **New-AzApiManagementCustomHostnameConfiguration** является командлетом-помощником, который создает экземпляр **PsApiManagementCustomHostNameConfiguration.**
...
Эта команда создает и инициализирует экземпляр **PsApiManagementCustomHostNameConfiguration** для портала. Затем создается новая служба ApiManagement с настраиваемой конфигурацией имени пользователя.

### Пример 2. Создание и инициализация экземпляра PsApiManagementCustomHostNameConfiguration с помощью секретного ресурса KeyVault
```powershell
...
### -KeyVaultId
KeyVaultId секрету, храняму настраиваемый SSL-сертификат.
