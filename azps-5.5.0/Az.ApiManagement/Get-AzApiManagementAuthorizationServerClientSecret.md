...
## ОПИСАНИЕ
Cmdlet **Get-AzApiManagementAuthorizationServerClientSecret** получает секрет клиента сервера авторизации управления API Azure.

## ПРИМЕРЫ

### Пример 1. Получите указанный секрет клиента авторизации по id
```
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementAuthorizationServerClientSecret -Context $ApiMgmtContext -ServerId "0123456789"
```

Эта команда получает указанный секрет сервера авторизации.
...
Если он указан, будет пытаться найти сервер авторизации по идентификатору.
Этот параметр является required(обязательно).
...
Если он указан, будет указан сервер авторизации по идентификатору.
