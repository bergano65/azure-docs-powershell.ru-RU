...
### SchemaDocumentInline (по умолчанию)
```
...
## ОПИСАНИЕ
Создает новую схему API.

## ПРИМЕРЫ

### Пример 1. Создание схемы для Swagger Petstore Extensive API
```powershell
...
Cmdlet **New-AzApiManagementApiSchema** создает или обновляет схему `swagger-petstore-extensive` aPI.

## PARAMETERS

### -ApiId
Идентификатор api. Этот параметр является required(обязательно).
...
ContentType схемы api. Этот параметр является required(обязательно).
