...
## ОПИСАНИЕ
Для получения указанной подписки (или всех подписок), если она не указана, с ее выполнения можно получить указанный **cmdlet Get-AzApiManagementSubscription.**
Ключи не включаются в сведения о результатах. Чтобы получить ключи, используйте **Get-AzApiManagementSubscriptionKey.**

## ПРИМЕРЫ

### Пример 1. Получить все подписки
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
PS C:\>Get-AzApiManagementSubscription -Context $apimContext
```

Эта команда возвращает все подписки.

### Пример 2. Получить подписку с указанным ИД
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
PS C:\>Get-AzApiManagementSubscription -Context $apimContext -SubscriptionId "0123456789"
```

Эта команда получает подписку по ИД.

### Пример 3. Получить все подписки для пользователя
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
PS C:\>Get-AzApiManagementSubscription -Context $apimContext -UserId "777"
```

Эта команда возвращает подписки пользователя.

### Пример 4. Получить все подписки на продукт
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
PS C:\>Get-AzApiManagementSubscription -Context $apimContext -ProductId "999"
```

Эта команда возвращает все подписки на продукт.

### Пример 5. Получить все подписки для области
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
Эта команда возвращает все подписки, настроенные для глобальной области действия API.

### Пример 6. Получить все подписки для продукта и области действия пользователя
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### -ProductId
Определяет идентификатор продукта.
Если он задан, этот cmdlet находит все подписки по идентификатору продукта.
...
### -Scope
Идентификатор области. Область действия подписки, будь то область API /apis/{apiId}, область продуктов /products/{productId} или глобальная область API /apis или глобальная область /.
...
Если он задан, этот cmdlet находит подписку по идентификатору.
...
Если он задан, этот cmdlet находит все подписки по идентификатору пользователя.
