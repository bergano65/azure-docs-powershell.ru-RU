...
## ОПИСАНИЕ
Для создания подписки будет создаваться новый **cmdlet AzApiManagementSubscription.**
...
### Пример 3. Создание подписки для области продуктов
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
### -AllowTracing
Флаг, который определяет, можно ли включить трассировку на уровне подписки. Это необязательный параметр, который по умолчанию $null.
...
### -ProductId
Определяет ИД продукта, на который нужно подписаться.
