...
## ОПИСАНИЕ
Для создания API создается операция **New-AzApiManagementOperation.**
...
### Пример 2. Создание операции управления API со сведениями о запросах и ответах
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
В этом примере создается операция управления API со сведениями о запросах и ответах.

## PARAMETERS

### -ApiId
Определяет идентификатор операции управления API.
...
### -ApiRevision
Идентификатор изменения API. Этот параметр является необязательным. Если не указано, операция будет привязана к активной редакции API.
...
### -Method
Метод HTTP для новой операции управления API.
...
### -Name
Отображает имя новой операции управления API.
...
### -Request
Подробные сведения об управлении API.
...
### -Ответы
Определяет массив возможных ответов на операции управления API.
...
### -TemplateParameters
Указывает массив параметров, определенный в *параметре URLTemplate.*
Если этот параметр не указан, будет сгенерировано значение по умолчанию на основе *URLTemplate.*
