...
### -Account
Имя учетной записи Data Lake Analytics, на которую будет отправлено задание.
...
### -PipelineId
ИД, который указывает на отправку задания, является частью набора повторяющихся заданий и также связан с каналом заданий.
...
### -PipelineName
Необязательное удобное имя для конвейера, связанного с этим заданием.
...
### -RecurrenceName
Необязательное удобное имя для корреляции повторения между заданиями.
...
### -Runtime
При желании за задание можно установить версию времени запуска. Если это не так, используется стандартное время запуска.
