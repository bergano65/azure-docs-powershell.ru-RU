...
Вторая команда использует Get-Credential для сбора проверки подлинности Windows (имя пользователя и пароль), а затем сохраняет этот **объект PSCredential** в переменной $Credential Windows.
Для получения дополнительных сведений введите `Get-Help Get-Credential` .
Третья команда создает зашифрованное значение для объекта, который хранится в $Value и $Credential для указанного фабрики данных, шлюза, группы ресурсов и связанного типа службы.
...
Этот cmdlet зашифрует учетные данные, которые вы здесь указали.
...
Если файловая система локализована на компьютере шлюза, используйте Локализованную или локализованную службу файлов, а если файловая система находится на сервере, который отличается от компьютера шлюза, используйте имя \\ \\ сервера.
