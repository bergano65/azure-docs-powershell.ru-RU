...
## SYNOPSIS
Удалите подключенный кластер, удалите отслеживаемый ресурс в Диспетчере ресурсов Azure (ARM).
...
### Пример 1. Удаление подключенных куберсетей
```powershell
PS C:\> Remove-AzConnectedKubernetes -ResourceGroupName azureps-manual-test -ClusterName ps-connaks-t01

```

Эта команда удаляет подключенные куберсети

### Пример 2. Удаление подключенных куберсетей с помощью объекта
```powershell
...
Эта команда удаляет подключенные к объекту куберсети
