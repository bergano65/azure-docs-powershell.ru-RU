...
Если другие темы домена по-прежнему доступны, значение в NextLink следует использовать в следующем звонке, чтобы получить следующую страницу тем домена.
...
Gets the details of Event Grid domain topic \` DomainTopic1 \` under Event Grid \` domain1 in resource group \` \` MyResourceGroupName. \`
...
Перечислите все темы домена "Сетка событий" в домене Event Grid Domain1 в группе ресурсов \` \` \` MyResourceGroupName без разбиения на разделы (все результаты возвращаются одним \` кадром).
