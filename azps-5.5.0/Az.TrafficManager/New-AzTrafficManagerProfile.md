...
## ОПИСАНИЕ
Для создания профиля Azure Traffic Manager будет создаваться новый **cmdlet AzTrafficManagerProfile.**
...
Список ожидаемых диапазонов кода состояния HTTP для запросов в отношении проверки в отношении проверки.
...
Количество последовательных сбойных проверок, которые диспетчер трафика проверял перед объявлением конечной точки в этом профиле "Ухудшения" после следующей повторной проверки. Значение по умолчанию — 3.
...
### -Name
Указывает имя профиля Диспетчера трафика, который создает этот cmdlet.
