...
### ScheduledAlertRule (по умолчанию)
```
...
## ОПИСАНИЕ
Для создания правила оповещения **New-AzSentinelAlertRule** в указанной рабочей области создается правило аналитического оповещения.
Необходимо указать один из трех параметров: *"План",*"Запланировано" или *"MicrosoftSecurityIncidentCreation",* чтобы указать тип созда создаемого правила оповещения.   У каждого типа есть разные параметры.
...
В этом примере создается тип **AlertRule** типа *"Да",* основанный на шаблоне для *advanced Multistage Attack Detection,* и он сохраняется в переменной $AlertRule.
...
В этом примере создается предупреждение типа *MicrosoftSecurityIncidentCreation,* основанное на шаблоне "Создать инциденты" на основе оповещений Azure "Центр безопасности *для IoT",* и оно сохраняется в $AlertRule безопасности. 
...
В этом примере создается  запланированный тип **DataConnector,** который затем сохраняется $AlertRule ненадегим.
...
### -DisplayName
Отображаемая имя правила оповещения.
...
Отображаемая фильтрация имен в правиле оповещения.
...
### -DisplayNamesFilter
Фильтр отображаемой области имен в правиле оповещения.
...
### -QueryFrequency
Частота запроса правил оповещения.
...
### -QueryPeriod
Период запроса правил оповещения.
...
### -SeveritiesFilter
Фильтр серьезности правила оповещения.
...
### -SuppressionDuration
Длительность подавляции правил оповещения.
...
### -SuppressionEnabled
Включено подавления правил оповещения.
...
### -TriggerOperator
Оператор триггера правила оповещения.
...
### -TriggerThreshold
Пороговое значение триггера правила оповещения.
