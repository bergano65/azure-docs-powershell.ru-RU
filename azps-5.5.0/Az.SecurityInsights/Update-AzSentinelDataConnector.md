...
## SYNOPSIS
Обновление соединитела данных.

## СИНТАКСИС

### DataConnectorId (по умолчанию)
```
...
## ОПИСАНИЕ
В этом рабочей области соединитель **Update-AzSentinelDataConnector** обновляет соединитель данных.
Объект **DataConnector** можно передать в качестве параметра или с помощью оператора конвейера. Кроме того, можно указать необходимые параметры.
...
Команда получает соединители данных по *DataConnectorId* и задает состояние *"Отключение* оповещений".   Все остальные свойства остаются одинаковыми.
