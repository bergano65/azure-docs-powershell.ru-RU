...
## ОПИСАНИЕ
Для обновления правила оповещения в указанной рабочей области обновляется **cmdlet Update-AzSentinelAlertRule.**
Можно использовать -InputObject или -ResourceId или -AlertId.  Вы можете обновить один или несколько дополнительных партераров.
...
В этом примере обновление параметра **AlertRule** для параметра *Disabled* и переименования *disabled-AlertRuleDisplayName.*  Все остальные свойства остаются одинаковыми.
...
В этом примере обновление параметра **AlertRule** с помощью параметра InputObject, который он *отключил.*  Все остальные свойства останутся без сухую.
