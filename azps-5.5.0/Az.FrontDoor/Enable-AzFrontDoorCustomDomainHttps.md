...
Включить HTTPS для пользовательского домена frontendpointname1-custom-xyz, который является частью front door "frontdoor1" в группе ресурсов "resourcegroup1", используя сертификат Front Door Managed.

### Пример 2. Включить HTTPS для пользовательского домена с помощью FrontDoorName и ResourceGroupName с помощью собственного сертификата в хранилище ключей.
```powershell
