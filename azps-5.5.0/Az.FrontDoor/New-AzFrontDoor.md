...
### ByFieldsWithBackendPoolsSettingParameterSet (по умолчанию)
```
...
## ОПИСАНИЕ
Для создания нового балансиента загрузки Azure Front Door в указанной группе ресурсов в рамках текущей подписки создается новый остаток на счете в **AzFrontDoor.**

## ПРИМЕРЫ

### Пример 1. Создание переднего входа на основе заданных параметров.
```powershell
...
Создание передней двери на основе заданных параметров.
...
Следует ли отключить проверку имени сертификата по HTTPS-запросам во все пулы backend. Никакие действия не влияют на запросы, не относясь к HTTPS-запросам.
...
### -EnabledState
EnabledState балансира загрузки front Door.
