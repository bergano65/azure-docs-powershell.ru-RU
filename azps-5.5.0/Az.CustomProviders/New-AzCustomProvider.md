...
Чтобы построить таблицу, см. раздел "ЗАМЕТКИ" для свойств ДЕЙСТВИЯ и создание таблицы с hash.
...
ACTION <ICustomRpActionRouteDefinition[]>: список действий, которые реализует поставщик ресурсов.
  - `Endpoint <String>`: URI конечной точки определения маршрутов, к который настраиваемый поставщик ресурсов будет запрашивать прокси-сервер. Это может быть в виде плоского URI (например, ' ') или может указывать маршрут по маршруту https://testendpoint/ (например, ' https://testendpoint/{requestPath} ').
