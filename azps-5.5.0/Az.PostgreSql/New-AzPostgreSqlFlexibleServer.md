...
### Пример 1. Создание нового гибкого сервера PostgreSql с аргументами
```powershell
...
### Пример 2. Создание нового гибкого сервера PostgreSql с настройкой по умолчанию
```powershell
...
Этот cmdlet создает гибкий сервер PostgreSql со значениями параметров по умолчанию и создает сервер в новой виртуальной сети и делегирует серверу подсеть.
По умолчанию местоположение имеет значение West US 2, SKU — Standard_D2s_v3, SKU — GeneralPurpose, а размер хранилища — 128GiB.
...
### Пример 3. Создание нового гибкого сервера PostgreSql с виртуальной сетью
```powershell
PS C:\> $Vnet = 'vnetname'
...
Этот cmdlet создает гибкий сервер PostgreSql с ИД vnet или именем vnet, предоставленным пользователем.
Если виртуальная сеть не существует, ее создает cmdlet.

### Пример 4. Создание нового гибкого сервера PostgreSql с именем виртуальной сети и подсети
```powershell
...
Этот cmdlet создает гибкий сервер PostgreSql с именем vnet, именем подсети, префиксом vnet и префиксом подсети.
Если виртуальная сеть и подсеть не существуют, будет создаться ее с клавиатуры.

### Пример 7. Создание нового гибкого сервера PostgreSql с общедоступным доступом ко всем IP-данным
```powershell
...
Этот cmdlet создает гибкий сервер PostgreSql, открытый для всех IP-адресов.

### Пример 8. Создание нового гибкого сервера PostgreSql с брандмауэром
```powershell
...
Этот cmdlet создает гибкий сервер PostgreSql, открытый по указанным IP-адресам.
...
Обратите внимание, что подсеть будет делегирована службе Microsoft.DBforPostgreSQL/flexibleServers.
...
Имя должно начинаться с буквы или числа, заканчивается буквой, числом или подчеркиваменем и может содержать только буквы, цифры, подчеркиваия, цыпцы или дефис.
