...
## ОПИСАНИЕ
Для учетных записей Edge Storage, доступных на устройстве Edge, можно получить учетные записи **Get-AzDataBoxEdgeStorageAccount.** Вы можете указать имя в качестве параметра в cmdlet, чтобы получить сведения об определенной учетной записи edge Storage.
