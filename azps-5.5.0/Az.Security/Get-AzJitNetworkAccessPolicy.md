...
Политика определяет, какие порты, IP-адреса протоколов и источников могут запрашивать подключение к VM-порту, и максимальную продолжительность перед автоматическим закрытием подключения.
