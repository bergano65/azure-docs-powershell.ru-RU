...
## ОПИСАНИЕ
Создает новое задание или обновляет существующее задание в указанной подписке.

## ПРИМЕРЫ

### Пример 1. Создание задания ImportExport
```powershell
...
Эти cmdlets создают задание ImportExport.
...
### -CancelRequested
Указывает, был ли отправлен запрос на отмену задания.
...
### -ClientTenantId
ИД клиента, который запрашивает клиент.
...
### -DiagnosticsPath
Виртуальный каталог BLOB-файлов, в котором будут храниться журналы копирования и резервные копии файлов манифеста диска (если они включены).
...
### -DriveList
Список из десяти дисков, которые входят в состав задания.
Список дисков является элементом, требуемого для задания импорта. оно не задано для заданий экспорта.
Чтобы создать таблицу, см. раздел "ЗАМЕТКИ" для свойств DRIVELIST и создание hash table.
...
Относительный URI для BLOB-блоков, которые содержат список путей blob-блоков или префиксов путей BLOB-блоков, как определено выше, начиная с имени контейнера.
Если BLOB-проект находится в корневом контейнере, URI должен начинаться с $root.
...
Путь к BLOB-пакету, который указывает на большой большой блок блока, содержащий список имен BLOB-блоков, которые не были экспортированы из-за нехватки места на диске.
Если экспорт всех BLOB-элементов был успешно завершен, этот элемент не включается в ответ.
...
### -Location
Указывает поддерживаемую службу Azure, в которой должно быть создано задание.
...
### -PercentComplete
Общий процент завершения задания.
...
### -ProvisioningState
Определяет состояние подготовка задания.
...
### -StorageAccountId
Идентификатор ресурса учетной записи хранения, в который будут импортироваться или экспортироваться данные.
...
### -Tag
Определяет теги, которые будут назначены задаче.
...
DriveLIST <IDriveStatus[]>: список из десяти дисков, которые входят в состав этой работы. Список дисков является элементом, требуемого для задания импорта. оно не задано для заданий экспорта.
  - `[BitLockerKey <String>]`. Ключ BitLocker, используемый для шифрования диска.
  - `[BytesSucceeded <Int64?>]`: Переданные для диска bytes успешно перенесены.
  ...
  - `[ManifestHash <String>]`: Зашифрованный с кодом MD5(с кодом Base16) hash файла манифеста на диске.
