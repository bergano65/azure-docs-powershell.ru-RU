...
## SYNOPSIS
Создайте объект ''Список драйверов'' для ImportExport.
...
### Пример 1. Создание нового списка дисков для задания ImportExport
```powershell
...
Эти cmdlets create a new DriveList for ImportExport job.
...
### -BytesSucceeded
Переданные для диска bytes успешно перенесены.
...
### -CopyStatus
Подробное состояние процесса передачи данных.
Это поле не возвращается в ответе, пока диск не находится в состоянии переноса.
...
### -DriveHeaderHash
Hash value the drive header (Hash value).
...
### -DriveId
Серийный номер диска без пробелов.
...
### -ErrorLogUri
URI, который указывает на BLOB-проект, содержащий журнал ошибок для операции передачи данных.
...
### -ManifestHash
С кодируемым кодом MD5 базовой кодировки файла манифеста на диске.
...
### -ManifestUri
URI, который указывает на BLOB-проект, содержащий файл манифеста диска.
...
### -PercentComplete
Процентное завершения для диска.
...
### -VerboseLogUri
URI, который указывает на BLOB-проект, содержащий подробный журнал для операции передачи данных.
