...
Параметры сценария U-SQL имеются перед основным содержимым сценария, например DECLARE @Department string = "Sales"; DECLARE @NumRecords int = 1000; DECLARE @StartDateTime DateTime = new DateTime(2017, 12, 6, 0, 0, 0, 0);
...
### -RecurrenceId
ИД, который указывает на отправку задания, является частью набора повторяющихся заданий с тем же ид.
...
### -ScriptParameter
Параметры сценария для этого задания, в качестве словаря имен параметров (строка) со значениями (любое сочетание byte, sbyte, int, uint (или uint32), long, ulong (или uint64), float, double, decimal, short (или int16), ushort (или uint16), char, string, DateTime, bool, Guid или byte[]).
