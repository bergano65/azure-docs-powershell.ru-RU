...
Один или несколько запусков дляреза могут быть вызваны искомой частью или если вы повторно запускаете его из-за сбоев.
Время начала для среза.
Чтобы получить время начала среза, воспользуйтесь Get-AzDataFactorySlice.
Например, чтобы получить запуск для следующего среза, используйте время начала 2015-04-02T20:00:00.
