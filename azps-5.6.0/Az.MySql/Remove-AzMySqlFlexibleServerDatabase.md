...
### Пример 1. Удаление базы данных MySql по имени
```powershell
PS C:\> Remove-AzMySqlFlexibleServerDatabase -Name databasetest -ResourceGroupName PowershellMySqlTest -ServerName mysql-test
```

Этот cmdlet удаляет базу данных MySql по имени.

### Пример 2. Удаление базы данных MySql по удостоверениям
```powershell
...
Эти cmdlets remove MySql database by identity.
