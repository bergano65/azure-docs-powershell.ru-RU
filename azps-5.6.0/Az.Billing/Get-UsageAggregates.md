...
## ОПИСАНИЕ
С **помощью cmdlet Get-UsageAggregates** можно получить агрегированные данные об использовании подписки Azure по следующим свойствам: 
...
При этом мы рекомендуем следующую ссылку в ответе на страницу.
...
Хотя запрос был запросчитано по времени, с помощью cmdlet можно агрегировать данные ответов по времени использования ресурсов.
...
Если $False, служба собирает результаты на стороне сервера и поэтому возвращает меньше групп.
