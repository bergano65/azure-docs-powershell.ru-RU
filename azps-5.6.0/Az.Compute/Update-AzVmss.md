...
## ОПИСАНИЕ
При **настройке** набора виртуальных машин (VMSS) состояние набора виртуальных машин (VMSS) обновляется до состояния локального объекта VMSS.
...
### -ImageUri
Определяет URI BLOB-адреса для изображения пользователя.
...
### -LicenseType
Укажите тип лицензии, который будет выводить на себя.
...
### -PlanPromotionCode
Определяет рекламный код плана.
...
Настраиваемая продолжительность удаления виртуальной машины (в минутах) может утверждать событие "Завершить запланированный", прежде чем событие будет утверждено автоматически (по времени).
...
Этот объект виртуальной машины содержит обновленное состояние VMSS.
