...
Создайте версию изображения коллекции с шифрованием образа диска.
...
### -DataDiskImage
Изображения на диске данных.   Например: @{Source = @{Id=<source_id>}; Lun=1; SizeInGB = 100; HostCaching = "ReadOnly" }
...
### -OSDiskImage
Изображение диска ОС, например @{Source = @{Id=<source_id>}; SizeInGB = 100; HostCaching = "ReadOnly" }
...
Дата окончания жизненного срока версии коллекции изображений.
...
Если он установлен, виртуальные машины, развернутые из последней версии определения изображения, не будут использовать эту версию изображения.
...
### -ReplicaCount
Количество копий версии изображения, созда необходимой для каждого региона.
...
### -SourceImageId
Код изображения, на котором будет создана версия изображения.
...
### -StorageAccountType
Определяет тип учетной записи хранения, которая будет использоваться для хранения изображения. Это свойство не является updatable (updatable). Доступны значения Standard_LRS, Standard_ZRS и Premium_LRS.
...
### -TargetRegion
Целевые области, в которые будет реплицирована версия изображения.
