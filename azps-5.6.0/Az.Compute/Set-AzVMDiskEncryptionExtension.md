...
Windows: параметр **VolumeType** может быть опущен, в этом случае операция по умолчанию — для всех. если параметр VolumeType присутствует для виртуальной машины Windows, необходимо установить для него параметр All или OS.
...
В этом примере включается шифрование на VM-устройствах без указания учетных данных AD.
...
### Пример 6. Включить шифрование с помощью ИД клиента Azure AD, thumbprint-эскиза клиента и ключа шифрования диска с переносом с помощью ключа шифрования
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
#The KeyVault must have enabledForDiskEncryption property set on it
...
В этом примере используются препечатка "Код клиента Azure AD" и "сертификат клиента", чтобы включить шифрование на VM-модели, и перенос ключа шифрования диска с помощью ключа шифрования ключа.
...
Этот Add-AzVMSecret можно использовать для развертывания сертификата на виртуальной машине в Azure.
...
### -ExtensionType
Тип расширения. Укажите этот параметр, чтобы переопречить стандартное значение AzureDiskEncryption для VMs Windows и AzureDiskEncryptionForLinux для VMs linux.
...
### -KeyEncryptionKeyUrl
Определяет URL-адрес ключа шифрования ключа, который используется для переноса и размывания ключа шифрования виртуальной машины.
...
### -TypeHandlerVersion
Определяет версию расширения шифрования.
