...
### -DiskIOPSReadWrite
Определяет, Read-Write IOPS для управляемого диска. Следует использовать только в том случае, если StorageAccountType UltraSSD_LRS. Если не указано, значение по умолчанию будет назначено на основе размера дискового размера ГБ.
...
### -DiskMBpsReadWrite
Указывает пропускную способность (МБ в секунду) для управляемого диска. Следует использовать только в том случае, если StorageAccountType UltraSSD_LRS. Если не указано, значение по умолчанию будет назначено на основе размера дискового размера ГБ.
