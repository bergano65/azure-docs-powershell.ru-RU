...
Определяет, следует ли автоматически заменять хост в случае сбоя. Значение по умолчанию имеет значение ИСТИНА, если не предоставлено.
...
### -LicenseType
Указывает тип лицензии на программное обеспечение, который будет применяться к ВМ-машин, развернутых на размещении. Возможные значения: Нет, Windows_Server_Hybrid и Windows_Server_Perpetual.  Значение по умолчанию — "Нет".
...
### -PlatformFaultDomain
Определяет количество домена неисправности платформы для хоста.  Минимальное значение — 0, максимальное — 2.
