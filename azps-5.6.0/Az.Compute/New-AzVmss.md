...
### Пример 1. Создайте VMSS с помощью **`SimpleParameterSet`**
```powershell
$vmssName = <VMSSNAME>
...
### Пример 2. Создайте VMSS с помощью **`DefaultParameterSet`**
```powershell
# Common
$LOC = "WestUs";
$RGName = "rgkyvms";
...
* Шестая команда использует командлет **New-AzVirtualNetwork** для создания виртуальной сети и сохраняет результат в переменной с $VNet.
...
* Для создания и получения информации с этого общераспозитного IP-адреса используется частью **New-AzPublicIpAddress** и **Get-AzureRmPublicIpAddress.**
...
* Для создания конфигурации правила балансиров нагрузки в 14-й команде используется **new-AzLoadBalancerRuleConfig** и результат сохраняется в переменной $LBRule.
...
* 16-я команда использует **командлет Get-AzLoadBalancer** для получения сведений о балансире нагрузки, созданном с помощью пятнадцатой команды, и сохраняет их в переменной с именем $ExpectedLb.
* В семнадцатой команде используется командлет **New-AzVmssIPConfig** для создания конфигурации IP-конфигурации VMSS, которая сохраняет данные в переменной с именем $IPCfg.
...
### -BackendPort
Номера портов, используемые средством балансиирования нагрузки "Масштаб", для связи с ВМ-данными в масштабе набора.  Если значения не указаны, порты 3389 и 5985 будут использоваться для VMS Windows, а порт 22 — для linux VMs.
...
### -MaxPrice
Максимальная цена выставить счет для набора масштабов виртуальных машин с низким приоритетом.
...
Используется для создания набора "Масштаб" в одной группе размещения (по умолчанию используется несколько групп).
...
### -SubnetAddressPrefix
Префикс адреса подсети, используемой этим набором scaleSet. Параметры подсети по умолчанию (192.168.1.0/24) будут применены, если значение не предоставлено.
...
### -VirtualNetworkName
Имя виртуальной сети, используемой для этого набора масштаба.  Если значение не задается, создается новая виртуальная сеть с тем же именем, что и у набора.
