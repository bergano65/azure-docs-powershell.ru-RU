...
## ОПИСАНИЕ
Для набора виртуальных машин (VMSS) свойства профиля хранилища задает **cmdlet Set-AzVmssStorageProfile.**

## ПРИМЕРЫ

### Пример 1. Настройка свойств профиля хранилища для VMSS
```
...
### -DataDisk
Определяет объект диска данных.
...
### -DiffDiskSetting
Определяет другие параметры диска операционной системы.
...
### -OsDiskCaching
Определяет режим кэшации диска операционной системы. Допустимыми значениями для этого параметра являются:
- ReadOnly
- Значением по умолчанию readWrite является ReadWrite.
Если изменить значение кэшинга, будет перезапущена виртуальная машина.
...
### -OsDiskCreateOption
Определяет, как этот cmdlet создает виртуальные машины VMSS.
Допустимыми значениями для этого параметра являются:
- Attach : Это значение используется при создании виртуальной машины VMSS на специализированном диске. 
- FromImage: это значение используется при использовании изображения для создания виртуальной машины VMSS.
Если вы используете изображение платформы, вы также будете использовать *параметр imageReference.*
...
### -OsDiskOsType
Определяет тип операционной системы на диске.
Это необходимо только для сценариев использования изображений пользователей, а не для изображений платформы.
...
Чтобы получить объект, используйте New-AzVmssConfig объекта.
...
### Microsoft.Azure.Management.Compute.Models.VirtualMa modelseScaleSetDataDisk[]
