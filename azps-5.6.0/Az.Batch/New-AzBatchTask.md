...
Для этой команды используется контекст, сохраненный в $Context.
...
Чтобы получить объект **PSCloudJob,** используйте Get-AzBatchJob.
...
Для задач с несколькими экземплярами файлы загружаются только с узла компьютеров, на котором выполняется основная задача.
