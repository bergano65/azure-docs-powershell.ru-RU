...
## ОПИСАНИЕ
Для **выполнения пакетного** задания с его использованием получается пакетные задачи Azure.
Задание можно указать с помощью *параметров JobId* и *JobId.*
Чтобы получить одну задачу, укажите параметр *"ИД".*
Вы можете указать параметр *Filter,* чтобы получить задачи, которые соответствуют фильтру OData.
...
Если не указать фильтр, этот cmdlet возвратит все задачи для учетной записи или задания пакета.
