...
## ОПИСАНИЕ
Для создания пакета в учетной записи Azure создается пакет приложения **New-AzBatchApplicationPackage.**

## ПРИМЕРЫ

### Пример 1. Установка пакета приложения в пакетную учетную запись
```
...
Эта команда создает и активирует версию 1.0 приложения Litware, а также передает содержимое litware.1.0.zip пакета приложения.

## PARAMETERS

### -AccountName
Имя учетной записи пакета, в которую этот cmdlet добавляет пакет приложения.
...
### -ActivateOnly
Указывает на то, что этот cmdlet активирует пакет приложения, который уже был добавлен.
...
### -FilePath
Указывает, что файл будет добавлен как двоичный файл пакета приложения.
...
### -Format
Формат двоичного файла пакета приложения.
