...
## ОПИСАНИЕ
Для учетной записи Batch, заданной параметром *BatchAccountContext,* с помощью cmdlet **Get-AzBatchJob** можно получить задания Azure Batch.
С помощью *параметра Id* можно получить одно задание.
С помощью параметра *Filter* можно получить задания, которые соответствуют фильтру OData.
Если у вас есть код расписания задания или **экземпляр PSCloudJobSchedule,** этот cmdlet возвращает только задания для этого расписания.

## ПРИМЕРЫ

### Пример 1. Пакетное задание по ИД
```
...
Эта команда получает задание, у которое есть задание "ИД01".
Используйте Get-AzBatchAccountKey для назначения контекста переменной $Context.

### Пример 2. Поиск всех активных заданий для расписания заданий
```
...
Эта команда возвращает активные задания для расписания заданий с расписанием заданий ID JobSchedule27.

### Пример 3. Получает все задания в соответствии с расписанием заданий с помощью конвейера
```
...
Эта команда получает расписание задания с расписанием и расписанием задания с помощью Get-AzBatchJobSchedule календаря.
Эта команда передает расписание задания текущему командлету с помощью оператора конвейера.
Команда получает все задания для этого расписания заданий.
...
### -Filter
Указывает предложение фильтра OData для заданий.
Если не указать фильтр, этот cmdlet возвращает все задания для учетной записи пакета или расписания заданий.
...
### -JobSchedule
Указывает объект **PSCloudJobSchedule,** который представляет расписание заданий, которое содержит задания.
Чтобы получить объект **PSCloudJobSchedule,** воспользуйтесь Get-AzBatchJobSchedule..
...
### -JobScheduleId
Определяет ИД расписания заданий, который содержит задания.
...
### -MaxCount
Указывает максимальное количество возвращаемых заданий.
