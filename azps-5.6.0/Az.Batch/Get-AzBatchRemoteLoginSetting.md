...
## SYNOPSIS
Возвращает удаленные параметры для узла компьютеров.
...
Команда получает параметры удаленного логотипа для каждого узла вычислений.
...
### -ComputeNode
Определяет узел вычислений в качестве объекта **PSComputeNode,** для которого этот cmdlet получает параметры удаленного логона.
