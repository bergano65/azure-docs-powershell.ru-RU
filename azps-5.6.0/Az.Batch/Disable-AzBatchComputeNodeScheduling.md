...
Выполнение задач, остановленных таким способом, не будет перепланено. 
...
Эта ссылка на объект хранится в переменной с именем $context.
Вторая команда в примере использует эту ссылку на объект и **Get-AzBatchComputeNode** для получения коллекции всех узлов вычислений, найденных в Pool06.
...
Определяет, как этот cmdlet работает с задачами, которые в данный момент работают на узле компьютера, где планирование отключено.
...
- RetainedData.
В настоящее время выполнение задач будет завершено, а срок хранения данных истечет до отключения планирования задач на узле компьютеров.
Новые задачи не будут запланированы на этом узле.
...
### -Id
Определяет ИД узла компьютеров, для которой отключено планирование задач.
