...
## ОПИСАНИЕ
С Remove-AzPrivateDnsRecordConfig- и DNS-записи из набора записей удаляются. Объект RecordSet является автономным объектом, и изменения в нем не изменяют ответы на частные DNS до тех пор, пока вы не запустите Set-AzPrivateDnsRecordSet, чтобы сохранить изменение в частной службе DNS Microsoft Azure. Чтобы удалить запись, все поля для этого типа записи должны в точности соответствовать. Записи SOA добавлять и удалять нельзя. Записи SOA автоматически создаются при создания частной зоны DNS и автоматическом удалении при ее удалении. Объект RecordSet можно передать этому cmdlet в качестве параметра или с помощью оператора конвейера.

## ПРИМЕРЫ

### Пример 1. Удаление записи A из набора записей
```powershell
...
В этом примере запись A удаляется из существующего набора. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

### Пример 2. Удаление записи AAAA из набора записей
```powershell
...
В этом примере запись AAAA удаляется из существующего набора. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

### Пример 3. Удаление записи CNAME из набора записей
```powershell
...
В этом примере запись CNAME удаляется из существующего набора. Так как набор записей CNAME может содержать не более одной записи, результатом является пустой набор записей.

### Пример 4. Удаление записи MX из набора записей
```powershell
...
В этом примере запись MX удаляется из существующего набора. Имя записи "@" означает набор записей на апексе зоны. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

### Пример 5. Удаление записи PTR из набора записей
```powershell
...
В этом примере запись PTR удаляется из существующего набора. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

### Пример 6. Удаление записи SRV из набора записей
```powershell
...
В этом примере запись SRV удаляется из существующего набора. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

### Пример 7. Удаление записи TXT из набора записей
```powershell
...
В этом примере запись TXT удаляется из существующего набора. Если это единственная запись в наборе записей, результатом будет пустой набор записей. Чтобы полностью удалить набор записей, см. видео Remove-AzPrivateDnsRecordSet.

## PARAMETERS

### -Cname
Каноническое имя удаляемой записи CNAME.
...
### -Exchange
Сервер обмена электронной почтой записи MX, который нужно удалить.
Не должно быть относительно имени зоны.
Не должно быть точки, заканчиваемой точкой
...
### -Ipv6Address
IPv6-адрес записи AAAA, который нужно удалить.
...
### -Port
Номер порта для удаляемой записи SRV.
...
### -Preference
Значение параметра записи MX, который нужно удалить.
...
### -Priority
Значение приоритета записи SRV, которая нужно удалить.
...
### -Ptrdname
Целевой хост записи PTR, который нужно удалить.
Не должно быть относительно имени зоны.
Не должно быть точки, заканчиваемой точкой
...
### -RecordSet
Набор записей, из которого нужно удалить запись.
...
### -Target
Целевой хост записи SRV, который нужно удалить.
Не должно быть относительно имени зоны.
Не должно быть точки, заканчиваемой точкой
...
### -Значение
Текстовое значение записи TXT, которая нужно удалить.
...
### -Weight
Значение веса записи SRV, которая нужно удалить.
