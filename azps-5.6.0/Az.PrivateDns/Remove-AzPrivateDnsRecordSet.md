...
## ОПИСАНИЕ
Этот Remove-AzPrivateDnsRecordSet удаляет указанный набор записей из указанной зоны. Записи SOA, автоматически созданные в закрытой зоне, удалить нельзя. Передать объект RecordSet этому cmdlet можно с помощью оператора конвейера, параметра или resourceId. Чтобы определить запись, заданную по имени и типу, без использования объекта RecordSet, необходимо передать эту зону в качестве объекта PSPrivateDnsZone с помощью оператора конвейера или параметра либо указать параметры ZoneName и ResourceGroupName. Параметр Confirm и переменная $ConfirmPreference Windows PowerShell можно использовать для управления запросом на подтверждение с помощью cmdlet. При указании набора записей с использованием объекта RecordSet набор записей не удаляется, если он был изменен в частных DNS Azure после извлечения локального объекта RecordSet. Это обеспечивает защиту от одновременного изменения. Вы можете скрыть это с помощью параметра Overwrite, который удаляет набор записей независимо от одновременного изменения.
