...
### Пример 1. Создание экземпляра монитора SAP по строке для HANA
```powershell
...
Эта команда создает экземпляр монитора SAP по строке для HANA.

### Пример 2. Создание экземпляра монитора SAP с хранилищем ключа для HANA
```powershell
...
Эта команда создает экземпляр монитора SAP в хранилище ключей для HANA.

### Пример 3. Создание экземпляра монитора SAP по словарю для PrometheusHaCluster
```powershell
...
Эта команда создает экземпляр монитора SAP по словарю для PrometheusHaCluster.

### Пример 4. Создание экземпляра монитора SAP по словарю для PrometheusOS
```powershell
...
Эта команда создает экземпляр монитора SAP по словарю для PrometheusOS.

### Пример 5. Создание экземпляра монитора SAP в словаре для MsSqlServer
```powershell
...
Эта команда создает экземпляр монитора SAP для словаря MsSqlServer.

### Пример 6. Создание экземпляра монитора SAP в словаре для SapHana
```powershell
...
Эта команда создает экземпляр монитора SAP для словаря SapHana.
...
### -HanaDatabaseName
Имя экземпляра базы данных SAP HANA.
...
ИД ресурса хранилища ключей, который содержит учетные данные HANA.
...
Секретный идентификатор секретного сейфа, который содержит учетные данные HANA.
...
Поддерживаемые значения: "СапХана".
