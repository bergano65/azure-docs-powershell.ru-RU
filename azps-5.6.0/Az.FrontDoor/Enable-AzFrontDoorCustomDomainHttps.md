...
## ОПИСАНИЕ
**Enable-AzFrontDoorCustomDomainHttps** включает HTTPS для пользовательского домена.

## ПРИМЕРЫ

### Пример 1. Включить HTTPS для пользовательского домена с помощью FrontDoorName и ResourceGroupName с помощью сертификата Front Door Managed.
```powershell
...
Включить HTTPS для пользовательского домена frontendpointname1-custom-xyz, который является частью передней двери "frontdoor1" в группе ресурсов "resourcegroup1", используя сертификат, управляемый front Door.

### Пример 2. Включить HTTPS для пользовательского домена с помощью FrontDoorName и ResourceGroupName, используя собственный сертификат в хранилище ключей.
```powershell
...
### Пример 3. Включить HTTPS для пользовательского домена с объектом PSFrontendEndPoint с помощью сертификата Front Door Managed.
```powershell
...
Включить HTTPS для пользовательского домена с объектом PSFrontendEndpoint с помощью сертификата Front Door Managed.

### Пример 4. Включить HTTPS для пользовательского домена с помощью ИД ресурса с помощью сертификата Front Door Managed.
```powershell
...
Включить HTTPS для пользовательского домена frontendpointname1-custom-xyz с помощью $resourceId ресурса с помощью сертификата Front Door Managed.
...
### -InputObject
Объект конечной точки Frontend, который нужно обновить.
...
### -ResourceId
ИД ресурса конечной точки front Door, чтобы включить https
...
### -VaultId
Удостоверение ключа, содержащее SSL-сертификат
