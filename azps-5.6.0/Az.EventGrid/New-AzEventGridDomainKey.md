...
Повторно сгенерировать ключ, соответствующий key1'\ домена Event Grid Domain1 в группе ресурсов \' \` \` \` MyResourceGroupName. \`
...
Повторно сгенерировать ключ, соответствующий key2'\ домена Event Grid Domain1 в группе ресурсов \' \` \` \` MyResourceGroupName, используя полный код \` ресурса.
...
### -Name
Имя ключа, который требуется сгенерировать повторно.
