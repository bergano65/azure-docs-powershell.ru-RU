...
Чтобы создать контейнер Windows Server на сервере AKS, при его создании необходимо указать как минимум четыре следующих параметра: значение и значение (и должно `NetworkPlugin` `NodeVmSetType` `azure` быть) `VirtualMachineScaleSets` и соответственно.
...
Следует ли включить авто scaler
...
### -GenerateSshKey
Создание SSH-файла в папку {HOME}/.ssh/id_rsa.
...
### -LoadBalancerSku
SKU балансира нагрузки для управляемого кластера.
...
### -NodeVmSetType
AgentPoolType представляет типы пула агентов. Возможные значения: 'VirtualMachineScaleSets', 'AvailabilitySet'
...
Пароль администратора для VMs Windows должен иметь длину не менее 12 символов, содержащих не менее одного нижнего знака, например, одного и одного специального `[a-z]` `[A-Z]` `[!@#$%^&*()]` знака.
