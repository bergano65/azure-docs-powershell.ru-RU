...
## ОПИСАНИЕ
Для запуска кэша HPC в Azure запускается **cmdlet Start-AzHpcCache.**
...
### -Force
Указывает на то, что при этом не будет предложено подтвердить запрос. По умолчанию этот cmdlet запросит подтверждение запуска кэша.
...
### -ResourceGroupName
Имя группы ресурсов, с которой вы хотите начать кэш.
