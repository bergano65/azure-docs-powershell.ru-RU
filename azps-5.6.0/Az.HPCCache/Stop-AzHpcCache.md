...
## ОПИСАНИЕ
При **этом кэш HPC Azure** останавливается.
...
### -Force
Указывает на то, что при этом не будет предложено подтвердить запрос. По умолчанию этот cmdlet запросит подтверждение остановки кэша.
...
### -ResourceGroupName
Имя группы ресурсов, для которой нужно прекратить кэш.
