...
## ОПИСАНИЕ
Для создания объекта config asR NIC создается объект config **AsR** NIC, содержащий сведения о отсесе и проверке отседания. Если какие-либо сведения не передаются, соответствующие значения выбираются из элемента, защищенного репликацией, чтобы эти значения не обновлялись до значения NULL.
...
Ip-адрес для восстановления NIC.
...
Определяет ID NSG, связанный с отбойной проверкой NIC.
...
### -TfoVMNetworkId
Определяет ID виртуальной сети отбойной проверки.
