...
## ОПИСАНИЕ
**Cmdlet Reset-AzRedisCache** перезапускет узлы экземпляра Azure Redis Cache.
...
Эта команда перезапускет оба узла для кэша RedisCache06.
...
### -PassThru
Указывает на то, что этот cmdlet возвращает boolean that indicates whether the operation succeeds.)
...
### -ShardId
Определяет код shard, который будет перезапущен этим cmdlet для кэша премиум-класса с включенной кластеризаем.
...
## ПРИМЕЧАНИЯ
* Ключевые слова: azure, azurerm, arm, resource, management, manager, redis, cache, webapp, website
