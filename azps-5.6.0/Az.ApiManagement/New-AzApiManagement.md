...
## ОПИСАНИЕ
С **его использованием** создается развертывание управления API в службе управления API Azure.
...
Эта команда определяет адрес администратора и организации.
Эта команда не указывает параметр *SKU.*
Таким образом, для этого используется значение по умолчанию для разработчика.

### Пример 2. Создание службы уровня "Стандартный" с тремя единицами
```powershell
PS C:\>New-AzApiManagement -ResourceGroupName "ContosoGroup02" -Name "ContosoApi" -Location "Central US" -Organization "Contoso" -AdminEmail "admin@contoso.com" -Sku Standard -Capacity 3
```

Эта команда создает службу управления API стандартного уровня, которая имеет три единицы.

### Пример 3. Создание службы уровня потребления
```powershell
...
Эта команда создает службу управления API уровня потребления с включенным сертификатом клиента в Западной Европе.

### Пример 4. Создание службы управления API для внешней виртуальной сети
```powershell
PS C:\> $virtualNetwork = New-AzApiManagementVirtualNetwork -Location "West US" -SubnetResourceId "/subscriptions/a8ff56dc-3bc7-4174-b1e8-3726ab15d0e2/resourceGroups/ContosoGroup/providers/Microsoft.Network/virtualNetworks/westUsVirtualNetwork/subnets/backendSubnet"
PS C:\> New-AzApiManagement -ResourceGroupName "ContosoGroup" -Location "West US" -Name "ContosoApi" -Organization Contoso -AdminEmail admin@contoso.com -VirtualNetwork $virtualNetwork -VpnType "External" -Sku "Premium"
```

Эта команда создает службу управления API премиум-уровня в виртуальной подсети Azure, в которой есть конечная точка внешнего шлюза с эталонным регионом в западной части США.

### Пример 5. Создание службы управления API для внутренней виртуальной сети
```powershell
PS C:\> $virtualNetwork = New-AzApiManagementVirtualNetwork -Location "West US" -SubnetResourceId "/subscriptions/a8ff56dc-3bc7-4174-b1e8-3726ab15d0e2/resourceGroups/ContosoGroup/providers/Microsoft.Network/virtualNetworks/westUsVirtualNetwork/subnets/backendSubnet"
PS C:\> New-AzApiManagement -ResourceGroupName "ContosoGroup" -Location "West US" -Name "ContosoApi" -Organization "Contoso" -AdminEmail "admin@contoso.com" -VirtualNetwork $virtualNetwork -VpnType "Internal" -Sku "Premium"
```

Эта команда создает службу управления API премиум-уровня в виртуальной подсети Azure с конечной точкой внутреннего шлюза с эталонным регионом в западной части США.

### Пример 6. Создание службы управления API и включить протокол TLS 1.0
```powershell
...
Эта команда создает стандартную службу управления API SKU и включает TLS 1.0 в клиенте Frontend для шлюза ApiManagement и обратного клиента между шлюзом ApiManagement и Backend.

## PARAMETERS

### -AdditionalRegions
Дополнительные регионы развертывания управления Azure API.
...
### -AdminEmail
Указывает почтовый адрес всех уведомлений, отправляемых системой управления API.
...
### -Capacity
Определяет емкость SKU службы управления API Azure.
...
Настраиваемые конфигурации имен хостов. Значение по умолчанию — $null. При $null будет установлено имя по умолчанию.
...
### -EnableClientCertificate
Флаг предназначен только для службы ApiManagement Service для потребления. При этом для каждого запроса шлюзу будет выдан сертификат клиента. Это также позволяет проверить подлинность сертификата в политике на шлюзе.
...
### -Name
Указывает имя развертывания управления API.
...
### -Организация
Указывает имя организации.
Для управления API этот адрес используется на портале разработчика в уведомлениях по электронной почте.
...
### -ResourceGroupName
Имя группы ресурсов, для которой этот cmdlet создает развертывание управления API.
...
### -SKU
Определяет уровень службы управления API.
Допустимые значения: 
- Разработчик 
- Стандартный 
- Premium по умолчанию — "Разработчик".
...
### -SslSetting
Параметр SSL службы ApiManagement. Значение по умолчанию — $null
...
Сертификаты, выданные внутренним ЦС, для установки в службе. Значение по умолчанию — $null.
...
Назначьте этим серверам удостоверения пользователей для использования с ключевыми службами управления, например Azure KeyVault.
...
### -VirtualNetwork
Виртуальная конфигурация сети для master Azure API Management deployment region.
...
### -VpnType
Виртуальный тип сети развертывания ApiManagement. Допустимые значения 
- "Нет" (значение по умолчанию). ApiManagement не является частью любой виртуальной сети")
- "Внешняя" (развертывание ApiManagement настроено в виртуальной сети с конечной точкой для подключения к Интернету)
- "Внутренняя" (развертывание ApiManagement настроено в виртуальной сети, в которой есть конечная точка интрасети)
