...
## ОПИСАНИЕ
Подробные **сведения о схеме API получает cmdlet Get-AzApiManagementApiSchema**

## ПРИМЕРЫ

### Пример 1. Подробные сведения о схеме API
```powershell
...
Эта команда получает все схемы API, связанные с Api для конкретного контекста `swagger-petstore-extensive` ApiManagement.

### Пример 2. Получить определенную схему, связанную с API
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceId /subscriptions/subid/resourceGroups/resourceGroupName/providers/Microsoft.ApiManagement/service/sdktestapim4163
...
Эта команда возвращает схему API, связанную с Api для конкретного контекста `5cc9cf67e6ed3b1154e638bd` `swagger-petstore-extensive` ApiManagement.
...
### -ResourceId
Идентификатор ресурса Arm схемы Api. Если он указан, будет пытаться найти схему api по идентификатору. Этот параметр является required(обязательно).
