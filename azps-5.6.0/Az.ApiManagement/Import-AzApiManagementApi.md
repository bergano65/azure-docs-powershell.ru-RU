...
### Пример 4. Импорт API из связи с Open API
```powershell
...
### -ApiRevision
Идентификатор изменения API. Этот параметр является необязательным. Если он не задан, импорт будет внесен в активную версию или новый api.
...
### -ApiType
Этот параметр является необязательным и имеет значение по умолчанию http. Параметр Soap применяется только при импорте WSDL и создает API-заполушка SOAP.
...
### -Protocol
Протоколы Веб-API (http, https). Протоколы, для которых доступен API. Этот параметр является необязательным. При этом протоколы, указанные в документе спецификаций, будут переопределяться.
...
### -WsdlServiceName
Локальное имя службы WSDL, которая будет импортироваться. Должен иметь длину от 1 до 400 знаков. Этот параметр является необязательным и требуется только для импорта Wsdl. Значение по умолчанию — $null.
