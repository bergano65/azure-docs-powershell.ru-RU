...
## ОПИСАНИЕ
Заданную политику области **получает cmdlet Get-AzApiManagementPolicy.**

## ПРИМЕРЫ

### Пример 1. Получить политику на уровне клиента
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -SaveAs "C:\contoso\policies\tenantpolicy.xml"
```

Эта команда получает политику на уровне клиента и сохраняет ее в файл с именем tenantpolicy.xml.

### Пример 2. Просмотр политики области продуктов
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementPolicy -Context $apimContext -ProductId "0123456789"
```

Эта команда получает политику области продуктов.

### Пример 3. Получить политику области API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
### Пример 4. Просмотр политики области действия
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
### Пример 5. Просмотр политики области клиента в формате RawXml
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Эта команда получает политику на область клиента в escape-формате, который не является XML-с помощью XML.

## PARAMETERS

### -ApiId
Определяет идентификатор существующего API.
Если этот параметр задан, будет возвращена политика области API.
...
### -ApiRevision
Идентификатор изменения API. Этот параметр является необязательным. Если не указано, политика будет извлечена из активного изменения api.
...
### -Format
Формат политики управления API.
По умолчанию этот параметр имеет значение XML.
...
Если для параметра *ApiId* задан этот параметр, возвращается политика области операций.
...
Если этот параметр задан, будет возвращена политика области продуктов.
...
### -SaveAs
Путь к файлу, в который нужно сохранить результат.
Если этот параметр не указан, результат будет отсве речь идет о sting.
