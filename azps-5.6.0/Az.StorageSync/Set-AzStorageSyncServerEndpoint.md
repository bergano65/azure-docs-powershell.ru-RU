...
В этом примере выполняются два действия: задана новая политика уровня облака для указанной конечной точки сервера, которая переводит сервер на уровень всех файлов, которые не были доступны за последние 30 дней, а также отключает автономный режим передачи данных, который изначально был включен на этой конечной точке сервера при его создании. Автономный перенос данных используется для обеспечения связи со службами массовой миграции, такими как поле данных Azure.
