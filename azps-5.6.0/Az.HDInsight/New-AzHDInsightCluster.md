...
### Пример 10. Создание кластера Azure HDInsight, который обеспечивает изоляцию компьютера.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### -AmbariDatabase
Получает или задает базу данных для амгари.
...
Получает или задает sku выделенных хостов для вычисления изоляции.
...
### -EnableComputeIsolation
Позволяет вычислить изоляцию через HDInsight.
...
### -OozieMetastore
Указывает, SQL базу данных Oozie для хранения метаданных.
Вы также можете использовать Add-AzHDInsightMetastore управления.
