...
## SYNOPSIS
Получите службы, для которых доступна поддержка. 
...
### Пример 1. Все службы, доступные для поддержки
```powershell
...
### Пример 2. Подробные сведения об отдельной службе по id доступны для поддержки
```powershell
