...
## SYNOPSIS
Получить классификации проблем для указанной службы.
...
## ОПИСАНИЕ
Возвращает текущий список классификации проблем для службы Azure. GUID классификации службы и проблемы можно использовать для создания нового билета в службу поддержки через New-AzSupportTicket.

Всегда используйте полученные программным путемGUID классификации службы и проблем. Этот метод гарантирует, что у вас есть самый последний набор служб иGUID-интерфейсов классификации проблем для создания билетов в службу поддержки.

## ПРИМЕРЫ

### Пример 1. Получите все параметры classificaitons для службы, используя параметр service id.
```powershell
...
### Пример 2. Получить все классификации проблем для службы с использованием родительского объекта службы
```powershell
...
### Пример 3. Просмотр сведений об одной проблемной классификации по id по объекту-службе piping
```powershell
...
### -ServiceObject
Объект службы, для которого извлекаются классификации проблем.
