...
INPUTOBJECT <IServerAutoGenerated> : сервер для строки подключения
  ...
  - `[AdministratorLoginPassword <SecureString>]`. Пароль администратора (требуется для создания сервера).
  ...
  - `[SkuTier <SkuTier?>]`— уровень определенного SKU, например " Скайп Скайп".
