...
## ОПИСАНИЕ
Для создания ключа в хранилище ключей Azure или импорта ключа в хранилище ключей будет добавлена клавиша **Add-AzKeyVaultKey.**
...
При создании или импорте ключа с тем же именем, что и у существующего ключа в хранилище ключей, исходный ключ обновляется с помощью значений, которые вы указываете для нового ключа. Вы можете получить доступ к предыдущим значениям, используя URI для конкретной версии ключа. Чтобы узнать о ключевых версиях и [](http://go.microsoft.com/fwlink/?linkid=518560) структуре URI, см. информацию о ключах и секретах в документации KEY Vault REST API.
Примечание. Чтобы импортировать ключ из собственного модуля безопасности оборудования, необходимо сначала создать пакет BYOK (файл с расширением byok file name) с помощью инструмента "Хранилище ключей Azure BYOK". Дополнительные сведения см. в сведениях о том, как создавать и [HSM-Protected клавиш для хранилища ключей Azure.](http://go.microsoft.com/fwlink/?LinkId=522252)
...
Чтобы импортировать ключ из собственного модуля безопасности оборудования, необходимо сначала создать пакет BYOK (файл с расширением .byok) с помощью средства BYOK key Vault Azure.
Дополнительные сведения см. в сведениях о том, как создавать и [HSM-Protected клавиш для хранилища ключей Azure.](http://go.microsoft.com/fwlink/?LinkId=522252)
...
Первая команда преобразует строку в безопасную строку с помощью командлета **ConvertTo-SecureString,** а затем сохраняет ее в $Password переменной. Для получения дополнительных сведений введите `Get-Help
...
Первая команда преобразует строку в безопасную строку с помощью командлета **ConvertTo-SecureString,** а затем сохраняет ее в $Password переменной.
Вторая команда создает объект **даты** и времени с помощью командлета **Get-Date,** а затем сохраняет его в $Expires переменной.
Третья команда создает переменную $tags для заметок высокой важности и ИТ-информации.
...
Создает ключ (ключ обмена ключом). КЕК должен быть ключом RSA-HSM, который имеет только операцию импорта. Только key Vault Premium SKU поддерживает клавиши RSA-HSM.
Дополнительные сведения https://docs.microsoft.com/azure/key-vault/keys/hsm-protected-keys

## PARAMETERS

### -CurveName
Определяет имя кривой в шифровании с многотипной кривой. Это значение допустимо, если KeyType — EC.
...
Примечание. Чтобы использовать HSM в качестве назначения, необходимо иметь ключ сейф, который поддерживает HSMs. Дополнительные сведения об уровнях обслуживания и возможностях хранилища ключей Azure см. на веб-сайте цены [на ключ хранилища Azure.](http://go.microsoft.com/fwlink/?linkid=512521)
Этот параметр требуется при создании нового ключа. Если ключ импортируется с помощью *параметра KeyFilePath,* этот параметр является необязательным:
- Если этот параметр не указан, а этот cmdlet импортирует ключ с расширением byok, он будет импортироваться как ключ, защищенный HSM. Этот ключ нельзя импортировать как защищенный программным обеспечением ключ.
...
### -Disable
Указывает на то, что для добавляемого ключа за установлено начальное состояние отключения. Любая попытка использовать ключ будет неудачной. Используйте этот параметр, если вы предварительно добавили ключи, которые предполагается включить позже.
...
### -KeyFilePassword
Пароль импортируемого файла в качестве объекта **SecureString.** Чтобы получить объект **SecureString,** воспользуйтесь **cmdlet ConvertTo-SecureString.** Для получения дополнительных сведений введите `Get-Help ConvertTo-SecureString` . Этот пароль необходимо указать для импорта файла с расширением PFX.
...
Допустимыми значениями для этого параметра являются разделенные запятыми списки основных операций, определяемые спецификацией [JSON Web Key (JWK):](http://go.microsoft.com/fwlink/?LinkID=613300)
- шифровать
- расшифровать
- wrapKey
- unw уберегатьKey
