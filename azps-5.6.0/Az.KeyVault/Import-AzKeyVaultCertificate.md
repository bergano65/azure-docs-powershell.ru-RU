...
- Используется `Add-AzKeyVaultCertificate` для создания запроса подписи сертификата и его отправки в орган сертификации. См. https://docs.microsoft.com/azure/key-vault/certificates/create-certificate-signing-request
...
Первая команда использует ConvertTo-SecureString для создания защищенного пароля, а затем сохраняет его в $Password переменной.
...
### -Name
Указывает имя сертификата. Этот cmdlet конструируют полное доменное имя (FQDN) сертификата из имени ключа хранилища, выбранной среды и имени сертификата.
...
[Создание и объединение CSR в хранилище ключей](https://docs.microsoft.com/azure/key-vault/certificates/create-certificate-signing-request)