...
Если есть несколько версий ключа, все версии включаются в резервную копию.
Так как скачаное содержимое зашифровано, его невозможно использовать за пределами хранилища ключей Azure.
Вы можете восстановить подпапавную клавишу в любое хранилище ключей в подписке, из которую они были переначаты.
...
Используйте Restore-AzKeyVaultKey **Backup-AzKeyVaultKey,** чтобы извлечь ключ в зашифрованном формате, а затем используйте Restore-AzKeyVaultKey и укажите хранилище ключа во втором регионе.
...
### Пример 3. Архивная копия ранее извлеченного ключа на указанное имя файла, переописав файл без запроса.
```powershell
