...
## ОПИСАНИЕ
Полностью резервное копирование управляемой HSM-системы в учетную запись хранения.
Используется `Restore-AzKeyVault` для восстановления резервной копии.
...
Он создает папку (обычно она называется) в контейнере хранилища, сохраняет резервную копию в этой папке и выводит `mhsm-{name}-{timestamp}` URI папки.
