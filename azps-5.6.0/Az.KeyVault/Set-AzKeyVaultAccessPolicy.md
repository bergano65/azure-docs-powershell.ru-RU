...
Следующие каталоги должны быть в одном каталоге Azure:
...
Итоговая команда еще больше изменяет существующие разрешения на удаление всех PattiFuller@contoso.com разрешений для ключевых операций. Разрешения для секретных операций остаются без изменений после этой команды.
...
При запуске этой команды укажите имя своего сейфа ключа, но необходимо указать параметр *ServicePrincipalName* с GUID **00000012-0000-0000-c000-000000000000** и указать разрешения в примере.
