...
### -FileEvent
Переключение этого параметра для настройки триггера FileEvent
...
### -Расписание
Периодическая частота, при которой требуется поднять событие timer. Укажите расписание в днях (от 1 до 365), часах (от 1 до 23) или минутах (от 1 до 59).
...
### -StartTime
Время дня, которое приводит к действительному триггеру. Для вычисления расписания указывается время (до секунд). Если часовой пояс не указан, время будет указано в часовом поясе устройства. Значение всегда возвращается в качестве времени в UTC.
