...
Третья команда изменяет конфигурацию RDP-правила на "Запретить". Однако при этом правило переопишется и задает только параметры, которые передаются Set-AzNetworkSecurityRuleConfig функции.   ПРИМЕЧАНИЕ. Изменить один атрибут не существует
...
### -Name
Задает имя конфигурации правила безопасности сети, заданного этим набором cmdlet.
...
Объект **NetworkSecurityGroup,** содержащий заданную конфигурацию правила безопасности сети.
...
- Подстановавный знак (*), который соответствует любому IP-адресу, можно также использовать такие теги, как VirtualNetwork, AzureLoadBalancer и Internet.
