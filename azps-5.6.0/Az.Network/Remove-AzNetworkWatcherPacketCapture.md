...
## ОПИСАНИЕ
Ресурс Remove-AzNetworkWatcherPacketCapture удаляется. Рекомендуется позвонить в Stop-AzNetworkWatcherPacketCapture, прежде чем вызывать Remove-AzNetworkWatcherPacketCapture. Если во время сеанса захвата пакетов Remove-AzNetworkWatcherPacketCapture это называется, это может быть не так. Если сеанс остановлен до удаления, CAP-файл, содержащий данные захвата, не удаляется. 
...
## ПРИМЕЧАНИЯ
Ключевые слова: azure, azurerm, arm, resource, management, manager, network, network, networker, network watcher, packet, capture, traffic, remove
