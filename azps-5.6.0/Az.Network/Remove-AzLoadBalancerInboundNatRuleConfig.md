...
## ОПИСАНИЕ
Для балансиров нагрузки Azure конфигурация правила перевода сетевых адресов (NAT) **remove-AzLoadBalancerInboundNatRuleConfig.**

## ПРИМЕРЫ

### 1. Удаление входящий правила NAT из балансиров нагрузки Azure
```
...
Первая команда загружает уже существующий баланс нагрузки под названием "mylb" и сохраняет его в переменной $load балансе. Вторая команда удаляет правило NAT, связанное с этим балансировом нагрузки.
...
### -LoadBalancer
Определяет объект **LoadBalancer,** который содержит конфигурацию правила NAT для входящий сообщений, удаляемую этим cmdlet.
...
### -Name
Указывает имя конфигурации правила NAT, которую удаляет этот cmdlet.
