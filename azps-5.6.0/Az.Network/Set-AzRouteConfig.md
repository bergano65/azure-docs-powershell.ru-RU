...
## ОПИСАНИЕ
Для **таблицы маршрутов cmdlet Set-AzRouteConfig** обновляет конфигурацию маршрутов.
...
Текущий cmdlet изменяет маршрут Route02, а затем передает результат в **cmdlet Set-AzRouteTable,** который обновляет таблицу с учетом внесенных изменений.
...
### -Name
Указывает имя маршрута, который изменяет этот cmdlet.
...
Виртуальный частный сетевой шлюз Azureserver-to-server. 
- VnetLocal.
...
### -RouteTable
Определяет таблицу маршрутов, с которой связан этот маршрут.
