...
## ОПИСАНИЕ
С **помощью cmdlet Update-AzVpnGateway** можно обновить VPN-шлюз в масштабируемом масштабе.  
VPN-шлюз Azure — это программное обеспечение, определяемое подключением сайта к подключениям к сайтам в VirtualHub. Масштаб этого шлюза будет меняться в зависимости от единицы, указанной пользователем. Подключение можно настроить из филиала или сайта, который называется VPN-сайтом, к масштабируемой шлюзу. Каждое подключение состоит из 2 Active-Active туннельов.
...
После создания шлюза с помощью Update-AzVpnGateway его можно обновить до трехблоков.
...
После создания шлюза с помощью Set-AzVpnGateway BgpPeeringAddress.
...
### -BgpPeeringAddress
Адреса пиринга BGP для этого Bgpsettings VpnGateway.
...
### -ResourceId
ИД ресурсов Azure для VpnGateway, который нужно изменить.
