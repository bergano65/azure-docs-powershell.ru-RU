...
Выше будет создать группу ресурсов, виртуальную сеть, виртуальную сеть, виртуальный концентратор на западной части США в группе ресурсов testRG в Azure. После этого в виртуальном центре будет создан шлюз ExpressRoute с 2-масштабными единицами.

Созданный шлюз подключается к expressRouteSite с помощью команды New-AzExpressRouteConnection входа.
...
То же, что и в примере 1, но теперь соединение, используемого в качестве канала Get-AzExpressRouteConnection, удаляется с помощью канала "Get-AzExpressRouteConnection".
