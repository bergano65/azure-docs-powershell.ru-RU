...
### ByPublicIpAddressByVirtualNetwork (по умолчанию)
```
...
## ОПИСАНИЕ
Создает ресурс-базовую подпись. Для этого потребуется общедоступный IP-адрес и VirtualNetwork. В этой службе VirtualNetwork должна быть подсеть с именем AzureBastionSubnet. Для создания PUBIC IP-адреса необходимо использовать стандартный SKU.
...
### -PublicIpAddress
Объект public IP address for bastion (Объект общего IP-адреса).
...
### -PublicIpAddressId
Общедоступный IP-адрес ИД ресурса Azure для использования в условиях использования.
...
### -PublicIpAddressName
Имя ресурса для использования общедоступных IP-адресов.
...
Имя группы ресурсов для общедоступных IP-адресов.
...
### -ResourceGroupName
Название группы ресурсов, для которой нужно создать подпись.
...
### -VirtualNetwork
Объект виртуальной сети для набок.
...
### -VirtualNetworkId
ИД ресурсов виртуальной сети Azure для использования в условиях баз данных.
...
### -VirtualNetworkName
Имя виртуального сетевого ресурса для названия подмосковного.
...
Имя группы ресурсов виртуальной сети для использования в названии.
