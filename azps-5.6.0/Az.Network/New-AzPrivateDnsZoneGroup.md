...
## ОПИСАНИЕ
С **помощью cmdlet New-AzPrivateDnsZoneGroup** можно создать новую частную группу зон DNS.
...
После создания закрытой конечной точки над примером создается группа зоны `test-pr-endpoint` DNS в этой частной конечной точке.
...
Набор личных конфигураций зоны DNS закрытой группы зон DNS.
