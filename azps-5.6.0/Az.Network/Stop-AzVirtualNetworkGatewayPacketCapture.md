...
## ОПИСАНИЕ
Остановка операции захвата пакетов на виртуальном сетевом шлюзе и отправка результата для заданного контейнера хранилища SasUrl.
...
### -InputObject
Объект виртуального сетевого шлюза, в котором нужно начать захват пакетов.
...
### -Name
Имя виртуального сетевого шлюза, в котором нужно начать захват пакетов.
...
### -ResourceId
Azure resource ID of the VirtualNetworkGateway where packet capture to be started.
...
### -SasUrl
URL-адрес SAS для захвата пакетов на виртуальном сетевом шлюзе.
