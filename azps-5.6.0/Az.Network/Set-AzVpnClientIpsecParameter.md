...
Когда создается виртуальный сетевой шлюз, для него настраивается набор стандартных политик vpn ipsec. Если пользователь на сайте хочет использовать настраиваемую политику ipsec для подключения к VPN-шлюзу, сначала необходимо настроить эту политику IPSEC для ШЛЮЗА VPN. **Set-AzVpnClientIpsecParameter** позволяет это сделать.
