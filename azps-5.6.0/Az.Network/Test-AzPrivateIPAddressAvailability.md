...
Этот cmdlet возвращает список доступных частных IP-адресов, если запрашивается частный IP-адрес.
...
Эта команда получает виртуальную сеть и использует оператор конвейера для ее переадресации **Test-AzPrivateIPAddressAvailability,** проверяя доступность указанного закрытого IP-адреса.
