...
## ОПИСАНИЕ
Для удаления переднего порта из шлюза приложения Azure удаляется клиентский порт **Remove-AzApplicationGatewayFrontendPort.**

## ПРИМЕРЫ

### Пример: удаление переднего порта из шлюза приложения
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
PS C:\> Remove-AzApplicationGatewayFrontendPort -ApplicationGateway $AppGw -Name "FrontEndPort02"
```

Первая команда получает шлюз приложения ApplicationGateway01, который принадлежит к группе ресурсов ResourceGroup01, и сохраняет шлюз в $AppGw переменной.
Вторая команда удаляет порт FrontEndPort02 из шлюза приложения.

## PARAMETERS

### -ApplicationGateway
Указывает шлюз приложения, из которого нужно удалить передний порт.
...
### -Name
Указывает имя порта переднего входа, который нужно удалить.
