...
## ОПИСАНИЕ
Для удаления маршрутов из таблицы маршрутов Azure удаляется **cmdlet Remove-AzRouteConfig.**
...
Текущий cmdlet удаляет маршрут Route02, а его результат передается в **cmdlet Set-AzRouteTable,** который обновляет таблицу с учетом изменений.
Таблица больше не содержит маршрут с именем Route02.
...
### -Name
Указывает имя маршрута, который удаляет этот cmdlet.
...
### -RouteTable
Определяет таблицу маршрутов, которая содержит маршрут, который удаляет этот cmdlet.
