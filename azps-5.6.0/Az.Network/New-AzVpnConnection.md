...
### ByVpnGatewayNameByVpnSiteObject (по умолчанию)
```
...
### -ParentObject
Родительская vpnGateway для этого подключения.
...
### -ParentResourceId
ИД ресурса родительского VPNGateway для этого подключения.
...
### -VpnSite
Удаленный VPN-сайт, к которому подключено это виртуальное сетевое подключение концентратора.
...
Список vpnSiteLinkConnections, которые есть у этого VPNConnection.
