...
## SYNOPSIS
Добавляет доверенный корневой сертификат к шлюзу приложения.
...
## ОПИСАНИЕ
**Cmdlet Add-AzApplicationGatewayTrustedRootCertificate** добавляет доверенный корневой сертификат к шлюзу приложения Azure.
...
Вторая команда добавляет в шлюз приложения новый доверенный корневой сертификат, который берет путь к корневому сертификату в качестве входных данных.
Третья команда создает новый параметр серверной серверной почты http с использованием надежного корневого сертификата для проверки сертификата сервера серверов серверов.
Четвертая команда обновляет шлюз приложения.
