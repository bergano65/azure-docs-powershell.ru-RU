...
## ОПИСАНИЕ
Cmdlet **Add-AzExpressRouteCircuitConnectionConfig** добавляет конфигурацию подключения к частному пирингу для схемы ExpressRoute. Это позволяет пиринг двух каналов Express Route в разных регионах или подписках. Обратите внимание, что после запуска **Add-AzExpressRouteCircuitConnectionConfig** необходимо вызвать Set-AzExpressRouteCircuit для активации конфигурации.

## ПРИМЕРЫ

### Пример 1. Добавление ресурса подключения к каналу ExpressRoute
```
...
### Пример 2. Добавление конфигурации подключения к каналу с помощью Piping в существующем канале ExpressRoute
```
$circuit_peer = Get-AzExpressRouteCircuit -Name $peeringCircuitName -ResourceGroupName $rg
$addressSpace = '60.0.0.0/29'
...
### -AddressPrefix
Минимальное /29 адресное пространство для создания VxLan-каналов между каналами express route для IPv4-каналов.
...
```
### -AddressPrefixType
Эта определяет семейство адресов, к которой принадлежит префикс адреса.
