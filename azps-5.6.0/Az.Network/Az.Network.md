...
Добавляет сертификат проверки подлинности в шлюз приложения.
...
Получает доверенный корневой сертификат с определенным именем из шлюза приложения.
...
Возвращает конфигурацию исходящие правила в балансе нагрузки.
...
Получает частную группу зоны DNS
...
Возвращает определение политики конечных точек службы.
...
Получает ресурс Azure VpnSite по имени ИЛИ перечисляет все VPNSites в resourceGroup или SubscriptionId. 
...
Создает объект удостоверения для шлюза приложения. При этом будет удержана ссылка на удостоверение пользователя, назначенное пользователю.
...
Создает коллекцию сетевых правил брандмауэра Azure.
...
Создайте или обновйте ресурс журнала потоков для указанной группы безопасности сети.
...
Создание ресурса для захвата пакетов и начало сеанса захвата пакетов на VM-
...
Удаляет пиринг перекрестного подключения ExpressRoute.
...
Удаляет определение политики конечной точки службы.
...
Удаляет виртуальнуюwan-wan Azure.
...
Обновляет конфигурацию ip-адреса переднего ip-адреса для балансиров нагрузки.
...
Обновляет определение политики конечной точки службы.
...
Настраивает виртуальный пиринг сети.
...
Обновление виртуальной WAN Azure.
