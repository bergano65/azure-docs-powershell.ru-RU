...
## ОПИСАНИЕ
Этот Set-AzContext задает сведения для проверки подлинности для cmdlets, которые вы запустите в текущем сеансе.
...
### -Подписка
Имя или ид подписки, для которого должен быть установлен контекст. У этого параметра есть псевдонимы к -SubscriptionName и -SubscriptionId, поэтому для ясности при указании имени и ид можно использовать любой из них вместо -Subscription.
