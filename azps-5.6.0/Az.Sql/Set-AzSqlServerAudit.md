...
## ОПИСАНИЕ
Чтобы изменить параметры аудита для azure SQL Server, можно настроить параметры аудита для сервера **Set-AzSqlServerAudit.**
Чтобы использовать этот cmdlet, используйте *параметры ResourceGroupName* и *ServerName* для идентификации сервера.
...
### Пример 3. Политика аудита хранилища BLOB-SQL Azure с расширенными фильтрами с использованием предика T-SQL.
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Enabled -StorageAccountResourceId "/subscriptions/7fe3301d-31d3-4668-af5e-211a890ba6e3/resourceGroups/resourcegroup01/providers/Microsoft.Storage/storageAccounts/mystorage" -PredicateExpression "statement <> 'select 1'"
```

### Пример 4. Удаление параметра advanced filtering из политики аудита сервера Azure SQL Server
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -PredicateExpression ""
```

### Пример 5. Политика аудита концентратора событий для сервера Azure SQL
```powershell
...
### Пример 6. Отключение политики аудита концентратора событий для сервера Azure SQL
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### Пример 7. Политика аудита аналитики журнала для сервера Azure SQL
```powershell
...
### Пример 8. Отключение политики аудита аналитики журнала для сервера Azure SQL
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -LogAnalyticsTargetState Disabled
```

### Пример 9. Отключение (через конвейер) политики аудита аналитики журналов для сервера Azure SQL.
```powershell
...
### Пример 10. Отключать отправку записей аудита сервера Azure SQL для хранения BLOB-приложений и включить отправку их для аналитики журналов.
```powershell
...
### Пример 11. Отправка записей аудита на сервере Azure SQL для хранения BLOB-сайтов, центра событий и аналитики журналов.
```powershell
