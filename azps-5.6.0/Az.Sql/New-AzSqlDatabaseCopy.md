...
## ОПИСАНИЕ
С **помощью cmdlet New-AzSqlDataCopy** создается копия базы данных Azure SQL, в которой используются моментальные снимки данных в текущий момент времени. Используйте этот вместо Start-AzSqlDatabaseCopy для создания однолетней копии базы данных. Этот cmdlet возвращает объект **Database** копии.
...
Создает копию базы данных SQL, в которой в данный момент используется моментальный снимок. (autogenerated)
...
Задает имя группы ресурсов Azure, для которой нужно назначить копию.
...
### -DatabaseName
Указывает имя базы данных SQL, которая будет копироваться.
