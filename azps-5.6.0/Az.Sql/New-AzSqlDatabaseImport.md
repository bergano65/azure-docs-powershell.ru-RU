...
## ОПИСАНИЕ
Для импорта файла bacpac из учетной записи хранилища Azure в новую базу данных Azure импортируется SQL **AzSqlDataImport.**
Запрос на состояние импорта базы данных может быть отправлен для получения сведений о состоянии для этого запроса.

## ПРИМЕРЫ

### Пример 1. Создание запроса на импорт для файла BACPAC
```
...
Эта команда создает запрос на импорт .bacpac в новую базу данных.
...
Этот параметр по умолчанию SQL, если тип проверки подлинности не задан.
Допустимыми значениями для этого параметра являются:
- SQL.
SQL проверки подлинности.
Задайте для параметров *AdministratorLogin* и *AdministratorLoginPassword* имя SQL и пароль администратора. 
- ADPassword.
Проверка подлинности Azure Active Directory.
*Задайте для administratorLogin* и *AdministratorLoginPassword* имя пользователя и пароль администратора Azure Active Directory.
...
Максимальный размер импортируемой базы данных.
...
### -Edition
Выпуск новой базы данных, в который нужно импортироваться.
...
Указывает имя цели службы, которая будет назначаться базе данных Azure SQL.
...
ИД ресурса учетной записи хранения для создания личной ссылки
