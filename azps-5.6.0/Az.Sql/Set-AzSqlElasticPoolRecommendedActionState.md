...
## ОПИСАНИЕ
Состояние **cmdletState Set-AzSqlElasticPoolRecommendedActionState** обновляется в состоянии рекомендуемого SQL Azure SQL.
Этот cmdlet применяет рекомендуемое действие, возвращается в состояние "Отменено" или "Удаляется" в зависимости от нового состояния.

## ПРИМЕРЫ

### Пример 1. Обновление состояния рекомендуемого действия на "Ожидание ожидания"
```
...
Эта команда обновляет состояние рекомендуемого пула эластичности с именем IR_ \[ test_schema \] _ \[ test_table_0.0361551 \] _6C7AE8CC9C87E7FD5893 ожиданию.
...
Указывает имя рекомендуемого действия, для которого этот cmdlet обновляет состояние.
...
### -State
Определяет значение, до которого этот cmdlet обновляет рекомендуемое состояние действия.
