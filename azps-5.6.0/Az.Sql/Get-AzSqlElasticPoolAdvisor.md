...
## ОПИСАНИЕ
Чтобы получить один или несколько помощников по SQL Azure для эластичного пула Azure, можно получить SQL **AzSqlElasticPoolAdvisor.**

## ПРИМЕРЫ

### Пример 1. Список всех консультантов для указанного эластичного пула
```
...
Команда получает список всех консультантов для эластичного пула с именем WiRunnerPool.

### Пример 2. Получить одного помощника для указанного эластичного пула
```
...
Эта команда получает помощник CreateIndex для эластичного пула WiRunnerPool.
...
Эта команда получает всех консультантов по эластичности пула с рекомендуемыми действиями, включенными в ответ.

### Пример 4. Набери одного помощника, включив в ответ рекомендуемые действия
```
...
### Пример 5. Список всех консультантов для указанного эластичного пула с помощью фильтрации
```
...
Эта команда возвращает список всех консультантов для эластичного пула WiRunnerPool, который начинается с буквы "d".
...
### -ElasticPoolName
Определяет имя эластичного пула, для которого этот cmdlet запрашивает помощника.
...
Указывает на то, что в ответе помощника указаны рекомендуемые действия помощника.
...
### -ResourceGroupName
Указывает имя группы ресурсов сервера, который содержит этот эластичный пул.
...
### -ServerName
Указывает имя сервера, на который находится эластичный пул.
