...
## ОПИСАНИЕ
Для помощника по azure SQL Server одно или несколько рекомендуемых действий получает SQL Server **Get-AzSqlServerRecommendedAction.**

## ПРИМЕРЫ

### Пример 1. Получить список всех рекомендуемых действий для определенного консультанта
```
...
Эта команда получает список всех рекомендуемых действий помощника по SQL Server CreateIndex для сервера Wi-runner-australia-east.
...
### -AdvisorName
Указывает имя помощника, для которого этот cmdlet запрашивает действия.
...
### -ResourceGroupName
Указывает имя группы ресурсов сервера, который содержит этот сервер.
...
### -ServerName
Имя сервера, на который принадлежит консультант.
...
## ПРИМЕЧАНИЯ
* Ключевые слова: azure, azurerm, arm, resource, management, manager, sql, server, mssql, advisor, recommendedaction
