...
Вы можете создать эластичную SQL, задав *параметр "ЭластичнаяPoolName"* в существующем пуле эластичности.
...
Первая команда получает базу SQL Database01, а затем сохраняет ее в переменной $Database.
Вторая команда восстанавливает базу данных в $Database из указанной заданной период резервной копии в базу данных SQL "ВосстановленнаяБаза" в эластичном пуле под названием "эластичный пул01".
...
С помощью Get-AzSqlDatabaseGeoBackup можно получить гео-избыточное резервное копирование.
