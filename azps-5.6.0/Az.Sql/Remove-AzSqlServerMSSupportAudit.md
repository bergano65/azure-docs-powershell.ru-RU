...
## ОПИСАНИЕ
С **его** SQL microsoft удаляются параметры аудита операций поддержки Microsoft.
Чтобы использовать этот cmdlet, используйте *параметры ResourceGroupName* и *ServerName* для идентификации сервера.

## ПРИМЕРЫ

### Пример 1. Удаление параметров аудита операций поддержки Майкрософт для сервера Azure SQL
```powershell
PS C:\>Remove-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
```

### Пример 2. Удаление (через канал) параметров аудита для сервера Azure SQL Майкрософт
```
