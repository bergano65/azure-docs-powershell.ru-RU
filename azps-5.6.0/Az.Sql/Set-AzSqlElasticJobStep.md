...
## ОПИСАНИЕ
Новый Set-AzSqlElasticJobStep обновляет шаг задания

## ПРИМЕРЫ

### Пример 1. Обновление целевой группы шага задания
```powershell
...
### Пример 2. Обновляет сценарий T-SQL задания в шаге задания
```powershell
PS C:\> $jobStep = Get-AzSqlElasticJobStep -ResourceGroupName rg -ServerName elasticjobserver -AgentName agent -JobName job1 -StepName step1
...
Начальная retry interval seconds
...
Максимальное время повторного интервала
...
ИД ресурса выходной базы данных
...
### -RemoveOutput
Флажок, который показывает, нужно ли удалять выходные данные
...
Множитель backoff интервала повторно
...
### -StepId
Текст пошаговая пошаговая пошаговая по
...
### Microsoft.Azure.Commands.Sql.AsticJobs.Model.AzureSqlElasticJobStepModel
