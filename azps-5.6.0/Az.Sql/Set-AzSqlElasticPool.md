...
## ОПИСАНИЕ
**Cmdlet Set-AzSqlElasticPool** задает свойства эластичного пула в базе данных Azure SQL. Этот cmdlet может изменять eDTUs для каждого пула *(Dtu),* максимальный размер хранилища для пула *(StorageMB),* максимальное число eDTUs для базы данных *(DatabaseDtuMax)* и минимальное значение eDTUs на базу данных *(DatabaseDtuMin).*
...
### -StorageMB
Ограничение хранилища в мегабайтах для эластичного пула. Дополнительные сведения см. в New-AzSqlElasticPool.
...
### -Теги
Указывает словарь пар "ключ-значение", который этот cmdlet связывает с эластичным пулом в виде hash table. Например: `@{key0="value0";"key 1"=$null;key2="value2"}`
...
### -VCore
Общее общее количество vcore для Sql Azure 2016: "Эластичный пул Sql".
