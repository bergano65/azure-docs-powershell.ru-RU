...
## ОПИСАНИЕ
Для базы данных Azure SQL помощник по базе данных Azure вы получаете один или несколько помощников по SQL Azure **AzSqlDataBase.**

## ПРИМЕРЫ

### Пример 1. Список всех консультантов для указанной базы данных
```
...
Эта команда возвращает список всех консультантов для базы данных WiRunner, которая принадлежит к серверу Wi-run-australia-east.

### Пример 2. Получить одного помощника для указанной базы данных
```
...
Эта команда получает помощник CreateIndex для базы данных с именем WIRunner.

### Пример 3. Укаймь всех консультантов, в ответ на которые они порекомендуют
```
...
Эта команда получает всех консультантов для базы данных WiRunner с рекомендуемыми действиями, включенными в ответ.
Так как для команды используется параметр *ExpandRecommendedActions,* командлет получает рекомендуемые действия вместе с ответом.
...
Эта команда получает помощник CreateIndex из базы данных WiRunner со своими рекомендуемами действиями, включенными в ответ.
Так как для команды используется параметр *ExpandRecommendedActions,* командлет получает рекомендуемые действия вместе с ответом.
...
Эта команда возвращает список всех консультантов для базы данных WiRunner, которая принадлежит к серверу Wi-run-australia-east и начинается с буквы "d".
...
### -DatabaseName
Имя базы данных, для которой этот cmdlet запрашивает помощника.
...
Указывает на то, что этот cmdlet получает рекомендуемые действия вместе с ответом.
...
### -ServerName
Указывает имя сервера, на который входит база данных.
...
## ПРИМЕЧАНИЯ
* Ключевые слова: azure, azurerm, arm, resource, management, manager, sql, database, mssql, advisor
