...
## ОПИСАНИЕ
**Cmdlet Set-AzSqlDatabaseAudit** изменяет параметры аудита базы данных Azure SQL.
...
### Пример 1. Политика аудита хранилища BLOB-SQL Azure
```powershell
...
### Пример 2. Отключение политики аудита хранилища BLOB-SQL Azure
```powershell
...
### Пример 3. Применение политики аудита хранилища BLOB-SQL для базы данных Azure с использованием SQL предиката T-SQL.
```powershell
...
### Пример 4. Удаление фильтрации из политики аудита базы данных Azure SQL
```powershell
...
### Пример 5. Политика аудита концентратора событий для базы данных Azure SQL
```powershell
...
### Пример 6. Отключение политики аудита центра событий в базе данных Azure SQL
```powershell
...
### Пример 7. Политика аудита аналитики журнала для базы данных Azure SQL
```powershell
...
### Пример 8. Отключение политики аудита аналитики журналов в базе данных Azure SQL
```powershell
...
### Пример 9. Отключение (через конвейер) политики аудита аналитики журналов для базы данных Azure SQL.
```powershell
...
### Пример 10. Отключать отправку записей аудита базы данных Azure SQL в хранилище BLOB-приложений и включить отправку их в аналитику журнала.
```powershell
...
### Пример 11. Отправка записей аудита базы данных Azure SQL в хранилище BLOB-приложений, центр событий и аналитику журналов.
```powershell
...
RECEIVE  
REFERENCES  
Общая форма определения действия для аудита: [действие] ON [object] BY [principal] Обратите внимание, что [object] в формате выше может ссылаться на объект, например таблицу, представление или хранимую процедуру, либо всю базу данных или схему. В последних случаях используются формы DATABASE:[dbname] и SCHEMA::[schemaname] соответственно.
Например:  
Select on dbo.myTable by public  
SELECT в database::myDatabase по общедоступным  
SELECT on SCHEMA::mySchema by public  
Дополнительные сведения https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions см. в .
...
Это сочетание также настроено по умолчанию. Эти группы охватывают все SQL и хранимые процедуры, выполненные в базе данных, и не должны использоваться вместе с другими группами, так как это приведет к дублированию журналов аудита.
Дополнительные сведения https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-action-groups см. в .
...
### -EventHubTargetState
Указывает, является ли центр событий местом назначения для записей аудита.
...
Указывает, является ли аналитика журнала местом назначения для записей аудита.
...
### -PassThru
Указывает, нужно ли выводить политику аудита в конце выполнения cmdlet.
...
### -StorageKeyType
Определяет, какой ключ доступа к хранилищу нужно использовать.
...
### -WorkspaceResourceId
ИД рабочей области (ИД ресурса рабочей области журнала аналитики журнала) для рабочей области "Аналитика журналов", в которую вы хотите отправлять журналы аудита. Пример: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
