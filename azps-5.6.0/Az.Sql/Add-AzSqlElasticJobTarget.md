...
## ОПИСАНИЕ
С Add-AzSqlElasticJobTarget добавляет целевой ресурс в целевую группу.

## ПРИМЕРЫ

### Пример 1. Добавление целевого сервера
```powershell
...
### Пример 2. Добавление целевого сайта базы данных
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
### Пример 3. Добавление целевого пула в качестве эластичного пула
```powershell
...
### Пример 4. Добавление целевой карты в качестве карты
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
Добавляет целевую группу (серверную, эластичный пул, базу данных и карту shard) в целевую группу.
