...
## ОПИСАНИЕ
Для экземпляра AzureSQL **Managed** Instance в текущей подписке, который является неактивным, для azure Active Directory (Azure AD) отключены только требования к проверке подлинности для экземпляра, управляемого AzureSQL.
