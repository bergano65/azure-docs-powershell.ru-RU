...
## ОПИСАНИЕ
С помощью **cmdlet New-AzMetricAlertRuleV2Criteria** создается объект локальных критериев метрики, который будет использоваться в качестве входного для параметра [Add-AzMetricAlertRuleV2,](https://docs.microsoft.com/powershell/module/az.monitor/add-azmetricalertrulev2) который создает новое правило метрических оповещений.

## ПРИМЕРЫ

### Пример 1. Создание простого критерия метрических оповещений
...
Эта команда создает простые критерии метрических оповещений, которые можно использовать в правиле метрических оповещений.

### Пример 2. Создание динамических метрических критериев оповещения
...
Эта команда создает динамические метрические условия оповещения, которые можно использовать в правиле метрических оповещений.

### Пример 3. Создание более сложных критериев метрических оповещений
...
Этот набор команд создает более сложные критерии метрических оповещений, которые включают выбор измерений.

### Пример 4. Создание критериев доступности веб-тестов
...
Эта команда создает условия доступности веб-тест, которые можно использовать в правиле метрических оповещений.

## PARAMETERS

### -ApplicationInsightsId
ИД ресурса Application Insights.
...
### -DynamicThreshold
Параметр переключения для использования динамического порогового типа
...
Общее количество проверяемого балла
...
### -FailedLocationCount
Минимальное количество сбойных мест для оповещения.
...
Позволяет создать правило оповещения для настраиваемой метрике, которая еще не излучается, из-за чего проверка метрики пропускается.
...
Конфиденциальность условий правил
...
### -TimeAggregation
Операция агрегирования, которая использовалась для сужения нескольких метрических значений по интервалу окна
...
### -ViolationCount
Минимальное количество нарушений в выбранном окне времени lookback, необходимое для оповещения
...
### -WebTest
Параметр переключения для использования типа условия доступности
...
### -WebTestId
Веб-тестовый Id Application Insights.
