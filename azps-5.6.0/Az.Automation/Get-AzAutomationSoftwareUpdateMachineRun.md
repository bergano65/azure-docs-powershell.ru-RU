...
## ОПИСАНИЕ
Этот cmdlet возвращает список запусков компьютера. При каждом запуске обновления программного обеспечения запускаются компьютеры для каждого конечного компьютера, на который она будет запускаться. Чтобы запустить определенный компьютер, передите параметр ID. Вы можете перечислить все запускы компьютера, все запускы на конкретном компьютере и все с определенным состоянием, указав соответствующие параметры.
