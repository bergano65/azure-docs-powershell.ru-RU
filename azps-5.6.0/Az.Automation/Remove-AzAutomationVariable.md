...
## ОПИСАНИЕ
При **этом из автоматизации** Azure удаляется переменная.
...
Эта команда удаляет переменную с именем StringVariable22 в учетной записи автоматизации Contoso17.
...
Указывает имя учетной записи автоматизации, содержаной переменную, удаляемую этим cmdlet.
...
### -Name
Указывает имя переменной, удаляемой этим cmdlet.
...
### -ResourceGroupName
Определяет группу ресурсов, для которой этот cmdlet удаляет переменную.
