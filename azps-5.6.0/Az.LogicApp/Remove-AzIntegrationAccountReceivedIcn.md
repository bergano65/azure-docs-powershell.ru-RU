...
## ОПИСАНИЕ
Этот cmdlet предназначен для использования в сценариях аварийного восстановления для удаления полученного номера управления обмена данными из учетной записи интеграции, чтобы соединитель B2B мог повторно обработать сообщение при включенной функции обнаружения повторяюх номеров.
В редких случаях полученный номер обмена обмена может быть зарезервирован незадолго до аварийного Скайп и до того, как соединитель B2B отклоняет этот обмен как ошибочный.
В таких случаях может потребоваться снова повторить обработку на сайте восстановления того же обмена данными после исправления его полезной нагрузки.
...
Попытка получить полученный номер обмена данными X12, содержимое которого не имеет допустимого формата.
Удаляет полученный номер управления обмена X12.
Подтверждает, что полученный номер управления обмена данными X12 был удален при попытке получить его еще раз.
...
Попытка получить полученный номер обмена данными Edifact, содержимое которого не имеет допустимого формата.
Удаляет полученный номер управления обмена Edifact.
Подтверждает, что полученный номер управления обмена данными Edifact удален путем попытки получить его еще раз.
...
### -AgreementType
Тип соглашения об интеграции с учетной записью.
