...
**Cmdlet Get-AzLogicAppTriggerHistory** получает историю триггеров в приложении логики в функции "Приложения логики".
Этот cmdlet возвращает объект **WorkflowTriggerHistory.**
Укажите приложение логики, группу ресурсов и триггер.
...
### Пример 1. Получить историю триггеров приложения логики
...
Эта команда получает определенный историю триггера приложения логики для триггера в приложении логики LogicApp03.

### Пример 2. Получить триггеры приложения логики
...
Эта команда возвращает истории рабочего процесса для триггера в приложении логики LogicApp07.

### Пример 3. Получить весь историю триггера приложения логики
...
Эта команда получает весь историю триггера рабочего процесса для триггера в приложении логики LogicApp08, следуя команде NextPageLink.
...
Эта команда получает первые две страницы истории триггера рабочего процесса для триггера в приложении логики LogicApp09, следуя следующей команде NextPageLink и ограничив размер результатов двумя страницами.
...
Определяет имя истории, которую получает этот cmdlet.
...
Имя приложения логики, для которого этот cmdlet получает историю триггеров.
...
Имя группы ресурсов, в которой этот cmdlet получает историю.
...
Указывает имя триггера, для которого этот cmdlet получает историю.
