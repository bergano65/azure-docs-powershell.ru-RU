...
## ОПИСАНИЕ
Для **этого создается** конечная точка сети доставки содержимого (CDN) Azure.
...
### -CdnProfile
Определяет объект профиля CDN, к которому добавляется конечная точка.
...
Указывает массив типов контента для сжатия от edge node до клиента.
...
### -GeoFilters
Список геофильтров, которые применяются к этой конечной точке.
...
### -httpPort
Указывает номер порта HTTP на сервере-источнике.
...
### -HttpsPort
Указывает номер порта HTTPS на сервере-источнике.
...
### -IsHttpAllowed
Указывает, включает ли конечная точка трафик HTTP.
...
### -IsHttpsAllowed
Указывает, включает ли конечная точка трафик HTTPS.
...
### -Location
Определяет расположение ресурса конечной точки.
...
### -OptimizationType
Указывает оптимизацию, установленную этой конечной точкой.
...
### -OriginHostHeader
Определяет начало хоста источника для конечной точки.
...
### -OriginHostName
Указывает имя хоста сервера-источника.
...
### -OriginName
Указывает имя ресурса сервера-источника.
...
### -СкайпPath
Путь к участкам динамического ускорения сайта.
...
Определяет поведение конечной точки CDN, если строка запроса находится в URL-адресе запроса.
...
### -ResourceGroupName
Имя группы ресурсов, к которой относится эта конечная точка.
...
### -Tag
Теги, которые нужно связать с конечной точкой CDN Azure.
