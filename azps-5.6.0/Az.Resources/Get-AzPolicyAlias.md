...
## SYNOPSIS
Get-AzPolicyAlias извлекает и выводит типы ресурсов поставщика Azure, которые определяются псевдонимами и соответствуют заданным значениям параметров. Если параметры не заданы, будут выданы все типы ресурсов поставщика, содержащие псевдоним.
Переключатель -ListAvailable изменяет это поведение, перечисляя все совпадающие типы ресурсов, включая те, у которых нет псевдонимов.
...
## ОПИСАНИЕ
Для **получения псевдонимов** политик возвращается список псевдонимов политик.
Псевдонимы политик используются политикой Azure для ссылки на свойства типа ресурса.
Параметры ограничивают элементы в списке путем совпадения различных свойств типа ресурса или его псевдонимов.
Заданное значение совпадает, если целевая строка содержит его с учетом неоценяемого случая.
...
Список всех типов ресурсов поставщика, включая те, которые не указаны псевдонимами.
...
Список всех типов ресурсов поставщика, пространство имен которых соответствует "компьютеру" и содержит псевдоним.
...
Список всех типов ресурсов поставщика, тип которых соответствует виртуальному и содержит псевдоним.
...
Список всех типов ресурсов поставщиков, тип которых соответствует "виртуальным", включая те, которые не указаны псевдонимами.
...
В этой списке перечислены все типы ресурсов поставщика, пространство имен которых соответствует типу "компьютер" и "тип ресурса", "виртуальный" и содержащий псевдоним.
Примечание. -NamespaceMatch и -ResourceTypeMatch предоставляют эксклюзивные совпадения, а остальные — включительно.
...
В этой ссылке перечислены все типы ресурсов поставщика, которые содержат псевдоним, совпадающий с виртуальным или сетевым.
...
Список всех типов ресурсов поставщика с версией альфа-api или псевдонимом с версией альфа-api.
...
### -ApiVersion
В этом случае указывается используемая версия API поставщика ресурсов. Если он не задан, версия API будет автоматически определена как наиболее последняя из доступных.
...
### -ApiVersionMatch
Включает в выходные элементы, типы ресурсов или псевдонимы которых имеют совпадающие версии api.
...
### -ListAvailable
Включает в выходные элементы, совпадающие с псевдонимами и без них.
...
### -LocationMatch
Включает в выходные элементы, типы ресурсов которых имеют совпадающие расположения.
...
### -PathMatch
Включает в выходные элементы с псевдонимами, содержащими путь, который соответствует этому значению.
...
### -ResourceTypeMatch
Ограничивает выходные данные элементами, тип ресурсов которых соответствует этому значению.
...
* Чтобы развернуть псевдонимы или любое другое свойство, перетастите выходной канал в нужное `select -ExpandProperty <property>` время. Например: `Get-AzPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* Дополнительные свойства доступны в выходных данных и могут отображаться путем перенакаки выходных `Format-List` данных. Например: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
