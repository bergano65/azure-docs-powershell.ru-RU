...
### Пример 1. Создание приложения с функцией PowerShell для дополнительных функций в Центральной части США.
```powershell
...
Эта команда создает приложение с дополнительными функциями PowerShell в Центральной части США.

### Пример 2. Создание приложения с функцией PowerShell, которое будет иметь план обслуживания.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Эта команда создает приложение для работы с функцией PowerShell, которое будет иметь план обслуживания.

### Пример 3. Создание приложения с функцией с помощью личного изображения ACR.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Эта команда создает приложение для работы с функцией с помощью личного изображения ACR.

## PARAMETERS

### -ApplicationInsightsKey
Ключ инструментов для app Insights, который нужно добавить.
...
### -AppSetting
"Параметры приложения для функций".
...
Отключать создание ресурса анализа приложений во время создания приложения с функцией.
...
### -DockerImageName
Только для Linux.
Container image name from Docker Registry, например publisher/image-name:tag.
...
Имя пользователя и пароль контейнера реестра.
Требуется для личных реестров.
...
Допустимыми значениями для этого параметра являются: - SystemAssigned - UserAssigned
...
### -OSType
ОС, в которая будет вмещаться приложение функций.
