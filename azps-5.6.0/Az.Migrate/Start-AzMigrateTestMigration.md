...
## SYNOPSIS
Запускает тестовую миграцию для сервера репликации.
...
## ОПИСАНИЕ
С Start-AzMigrateTestMigration- инициируется тестовая миграция для сервера репликации.
...
### -InputObject
Указывает сервер репликации, для которого нужно инициировать тестовую миграцию.
...
### -TestNetworkID
Обновляет виртуальный сетевой id в рамках подписки Azure, используемой для тестовой миграции.
...
INPUTOBJECT: сервер репликации, для которого нужно <IMigrationItem> инициировать тестовую миграцию. Объект сервера можно извлечь с помощью Get-AzMigrateServerReplication..
