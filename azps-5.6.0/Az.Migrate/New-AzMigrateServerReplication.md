...
### Пример 1. При только на диске ОС
```powershell
...
Это происходит в ситуации, когда нужно защитить только один диск.
...
Это происходит в ситуации, когда нужно защитить несколько дисков.
...
### -DiskEncryptionSetID
Определяет используемый набор encyption диска.
...
### -DiskToInclude
Определяет диски на сервере-источнике, которые нужно включить для репликации.
Чтобы создать таблицу, см. раздел "Заметки" для свойств DISKTOINCLUDE и создание hash table.
...
### -InputObject
Определяет обнаруженный сервер, который нужно перенести.
Объект сервера можно извлечь с помощью Get-AzMigrateServer..
...
### -LicenseType
Указывает, применимо ли гибридное преимущество Azure к переносимому серверу-источнику.
...
### -MachineId
Определяет машинный ИД обнаруженного сервера, который нужно перенести.
...
### -OSDiskID
Определяет диск операционной системы для сервера-источника, который нужно перенести.
...
### -PerformAutoResync
Указывает, будет ли репликация восстановлена автоматически в случае потери отслеживания изменений для исходных серверов в режиме репликации.
...
Набор доступности, используемый для создания VM, определяет набор доступности, который будет использоваться для создания VM.
...
### -TargetNetworkId
Определяет виртуальный сетевой id в рамках подписки Azure, в которую требуется перенести сервер.
...
Определяет id группы ресурсов в рамках подписки Azure, в которую требуется перенести сервер.
...
### -TargetSubnetName
Имя подсети в пределах назначения Virtual Netowk, в который требуется перенести сервер.
...
### -TargetVMName
Указывает имя созда создаемого azure VM.
...
### -TargetVMSize
Указывает SKU созда создаемого VM Azure.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: определяет диски на сервере-источнике, которые нужно включить для репликации.
  - `DiskId <String>`: ИД диска.
  - `IsOSDisk <String>`: значение, указывающее, является ли диск ОС диском.
  - `LogStorageAccountId <String>`: учетная запись хранения журнала, ARM ИД.
  - `LogStorageAccountSasSecretName <String>`: key vault secret name of the log storage account.
  - `[DiskEncryptionSetId <String>]`: DiskEncryptionSet ARM ID.
  - `[DiskType <DiskAccountType?>]`Тип диска.

INPUTOBJECT: <IVMwareMachine> определяет обнаруженный сервер, который нужно перенести. Объект сервера можно извлечь с помощью Get-AzMigrateServer.
