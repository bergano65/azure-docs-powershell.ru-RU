...
### Пример 3. Группа ресурсов, проект и имя задания
```powershell
...
INPUTOBJECT: <IJob> определяет объект задания сервера репликации.
  ...
  - `[AllowedAction <String[]>]`Действие "Разрешено".
  - `[CustomDetailAffectedObjectDetail <IJobDetailsAffectedObjectDetails>]`: затронутые свойства объекта, такие как исходный сервер, источник облака, целевой сервер, целевое облако и т. д., на основе сведений о объекте рабочего процесса.
  ...
  - `[Task <IAsrTask[]>]`Задачи.
    - `[AllowedAction <String[]>]`. Состояние или действия, применимые к данной задаче.
    ...
    - `[StartTime <DateTime?>]`: время начала.
    - `[State <String>]`: Штат. Это одно из этих значений: NotStarted, InProgress, Succeeded, Failed, Cancelled, Suspended или Other.
