...
## ОПИСАНИЕ
Для создания контейнера BLOB-lob-хранилища создается проектлет **New-AzRmStorageContainer.**

## ПРИМЕРЫ

### Пример 1. Создание контейнера BLOB-хранилищ с именем учетной записи хранения и именем контейнера с метаданными
```
PS C:\>New-AzRmStorageContainer -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount" -ContainerName "myContainer" -Metadata @{tag0="value0";tag1="value1";tag2="value2"}
```

Эта команда создает контейнер BLOB-хранилищ с именем учетной записи хранения и именем контейнера с метаданными.

### Пример 2. Создание контейнера BLOB-объектов хранилища с объектом учетной записи хранения и именем контейнера с общедоступным доступом в качестве BLOB-объекта
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -AccountName "myStorageAccount"
PS C:\>New-AzRmStorageContainer -StorageAccount $accountObject -ContainerName "myContainer" -PublicAccess Blob
```

Эта команда создает контейнер BLOB-объектов хранилища с объектом учетной записи хранения и именем контейнера с общедоступным доступом в качестве BLOB-объектов.

### Пример 3. Создание контейнера хранилища с параметром EncryptionScope
```
...
Эта команда создает контейнер хранилища с де defalt encryptionScope и блокирует переопределения области шифрования, заданной по умолчанию.
Затем откажите свойства связанного контейнера.

## PARAMETERS

### -DefaultEncryptionScope
Используется по умолчанию для всех записей с заданной областью шифрования.
