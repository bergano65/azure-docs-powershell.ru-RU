...
## ОПИСАНИЕ
**Cmdlet Set-AzStorageObjectReplicationPolicy** создает или обновляет указанную политику репликации объектов в учетной записи хранения.

## ПРИМЕРЫ

### Пример 1. Настройка политики репликации объектов для учетной записи назначения и источника.
```
...
Эта команда задает политику репликации объектов для учетной записи назначения и источника.
Сначала создайте 2 правила политики репликации объектов и установите политику с 2 правилами для учетной записи назначения. Затем за установите политику репликации объектов из учетной записи назначения в учетную запись-источник.
...
### -DestinationAccount
DestinationAccount политики репликации объектов.
...
### -InputObject
Объект политики репликации объектов, который нужно установить для указанной учетной записи.
...
### -PolicyId
ИД политики репликации объектов. Это должен быть GUID или значение по умолчанию.
Если не ввести значение policyId, будет использовать значение по умолчанию, то есть создать новую политику, и ее ИД будет возвращен в созданную политику.
...
### -SourceAccount
SourceAccount политики репликации объектов.
