...
Эта команда задает для учетной записи хранения тип Standard_RAGRS.
...
Эта команда задает пользовательский домен для учетной записи хранения.
...
Она задает пользовательский домен и теги для учетной записи хранения.
...
### Пример 10. Обновление учетной записи хранения с помощью проверки подлинности Azure Files AAD DS.
```
...
Команда обновляет учетную запись хранилища, в результате чего включается проверка подлинности доменной службы Azure Files Active Directory, а затем показан параметр проверки подлинности на основе удостоверения файла.

### Пример 12: Set MinimumTlsVersion, AllowBlobPublicAccess и AllowSharedKeyAccess
```
...
Команда задает MinimumTlsVersion, AllowBlobPublicAccess и AllowSharedKeyAccess, а затем показывает три свойства учетной записи. 
...
Указывает, разрешено ли учетной записи хранения запрашивать разрешение с помощью ключа доступа к учетной записи с помощью общего ключа. Если это так, все запросы, включая подписи общего доступа, должны быть авторизованны в Azure Active Directory (Azure AD). Значение по умолчанию равно NULL, что эквивалентно значению ИСТИНА.
...
### -KeyvaultEncryption
Указывает, следует ли использовать ключ KeyVault (Майкрософт) для ключей шифрования при использовании шифрования службы хранилища. Если параметр KeyName, KeyVersion и KeyVaultUri задан, параметр KeySource будет иметь значение Microsoft.Keyvault независимо от того, задан ли этот параметр. 
...
Указывает на то, должны ли публиковаться конечные точки для хранения маршрутов в Интернете.
...
Указывает на то, должны ли публиковаться конечные точки хранилища маршрутов Майкрософт.
...
### -ResourceGroupName
Имя группы ресурсов, в которой нужно изменить учетную запись хранения.
...
- Standard_ZRS — избыточное место в зоне.
...
- Standard_RAGZRS — чтение доступа к гео-избыточной зоне— избыточное хранилище.
