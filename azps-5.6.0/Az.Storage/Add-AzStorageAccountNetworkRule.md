...
## ОПИСАНИЕ
Командлет **Add-AzStorageAccountNetworkRule** добавляет IpRules или VirtualNetworkRules к свойству NetworkRule учетной записи хранения.

## ПРИМЕРЫ

### Пример 1. Добавление нескольких IpRules с IPAddressOrRange
```
...
### Пример 2. Добавление значения VirtualNetworkRule с помощью VirtualNetworkResourceID
```
...
### Пример 4. Добавление нескольких объектов IpRule с помощью JSON
```
PS C:\>Add-AzStorageAccountNetworkRule -ResourceGroupName "myResourceGroup" -Name "mystorageaccount" -IPRule (@{IPAddressOrRange="10.0.0.0/24";Action="allow"},@{IPAddressOrRange="28.2.0.0/16";Action="allow"})
```

Эта команда добавляет несколько объектов IpRule, вводимых с помощью JSON.

### Пример 5. Добавление правила доступа к ресурсам
```
...
Эта команда добавляет правило доступа к ресурсам с TenantId и ResourceId.

### Пример 6. Добавление всех правил доступа к ресурсам из одной учетной записи хранения в другую
```
...
Эта команда получает все правила доступа к ресурсам из одной учетной записи хранения и добавляет их в другую учетную запись хранения.
...
### -IPAddressOrRange
Массив IpAddressOrRange, добавьте IpRules со входным значением IpAddressOrRange и действием по умолчанию Allow to NetworkRule Property.
...
### -IPRule
Массив объектов IpRule, добавляемого в свойство NetworkRule.
...
### -ResourceAccessRule
NetworkRule ResourceAccessRules учетной записи хранения.
...
### -ResourceGroupName
Имя группы ресурсов содержит учетную запись хранения.
...
Массив VirtualNetworkResourceId добавит VirtualNetworkRule со входными данными VirtualNetworkResourceId и действием по умолчанию Allow to NetworkRule Property.
