...
## ОПИСАНИЕ
Для типа службы хранилища Azure с использованием cmdlet **Get-AzStorageCORSRule** задаются правила совместного использования ресурсов (CORS) в разных источниках.

## ПРИМЕРЫ

### Пример 1. Получить правила CORS для службы BLOB-lob
```
PS C:\>Get-AzStorageCORSRule -ServiceType Blob
```

Эта команда получает правила CORS для типа службы BLOB-типов.
...
### -ServiceType
Указывает тип службы хранилища Azure, для которой этот cmdlet получает правила CORS.
