...
## ОПИСАНИЕ
Для **содержимого BLOB-blob-содержимого** с помощью простого язык SQL (SQL) применяется простая язык SQL(SQL) и подмножество запросов данных сохраняется в локальном файле.

## ПРИМЕРЫ

### Пример 1. Запрос BLOB-lob
```powershell
...
Эта команда запрашивает У BLOB-файл succssfully с входными данными config в качестве CSV-файла, а выводит config в качестве json и сохраняет выходные данные в локальный файл ,c:\resultfile.js"on".

### Пример 2. Запрос на моментальный снимок BLOB-клиента
```powershell
...
Эта команда сначала получает объект BLOB-объектов для моментального снимка BLOB-объектов, а затем запрашивает его моментальный снимок и возвращает результат ошибки запроса.
...
### -ConcurrentTaskCount
Общий объем одновременной работы с async.
...
### -Force
Принудительно переопишем существующий файл.
...
Конфигурация, используемая для обработки входного текста запроса. Создайте объект конфигурации с помощью New-AzStorageBlobQueryConfig.
...
### -PassThru
Возвращает, успешно ли запрашивается указанный BLOB-проект.
...
### -QueryString
Строка запроса. Дополнительные сведения см. в: https://docs.microsoft.com/azure/storage/blobs/query-acceleration-sql-reference
...
### -ResultFile
Путь к локальному файлу для сохранения результата запроса.
