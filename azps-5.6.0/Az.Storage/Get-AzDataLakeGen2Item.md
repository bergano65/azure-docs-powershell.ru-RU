...
## ОПИСАНИЕ
Для получения сведений о файле или каталоге в Filesystem учетной записи хранилища Azure можно получить сведения о файле или каталоге **Get-AzDataLakeGen2Item.**
...
### Пример 1. Получите каталог от Filesystem и откажите подробные сведения
```
...
Эта команда получает каталог из Filesystem и показывает подробные сведения.

### Пример 2. Получить файл от Filesystem
```
...
Эта команда получает сведения о файле от Filesystem. 
...
Не укажите этот параметр, чтобы получить корневой каталог Filesystem.
