...
## ОПИСАНИЕ
Команда **Get-AzSynapsePipelineRun** возвращает сведения о запуске для указанного конвейера. Если задан PipelineRunId, в нем будут указаны сведения о запуске с этим ИД. Если значение PipelineRunId не указано, в нем будут показаны сведения обо всех запусках для каналов, которые произошли между значениями RunStartedAfter и RunStartedBefore.
