...
## КРАТКИй обзор
Получение определенного обновления на месте обновления.
...
### Пример 1: получение всех обновлений
```powershell
PS C:\> Get-AzsUpdate
...
При отсутствии каких бы то ни было параметров, Get-AzsUpdate выведет список всех обновлений, которые могут быть обнаружены штампом
...
Будет получать все обновления, соответствующие указанному имени.

### Пример 3: получение всех обновлений по расположению
```powershell
...
Будут извлечены все обновления в указанном расположении.
В настоящее время поддерживается только одно расположение, так что это эквивалентно Get-AzsUpdate
...
### Microsoft. Azure. PowerShell. командлеты. UpdateAdmin. Models. Api20160501. IUpdate
