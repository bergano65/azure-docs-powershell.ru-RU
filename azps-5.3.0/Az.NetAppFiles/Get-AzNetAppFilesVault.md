...
## ОПИСАНИЕ
Для получения списка резервных копий учетных записей ANF возвращается список **cmdlet Get-AzNetAppFilesVault.**
...
Эта команда получает список резервных копий для учетной записи Azure NetappFiles (ANF) "MyAnfAccount".
...
### -AccountObject
Учетная запись для нового объекта резервной копии
