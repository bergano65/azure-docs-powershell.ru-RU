...
## ОПИСАНИЕ
Для создания правила оповещения **New-AzSentinelAlertRule** в указанной рабочей области создается правило аналитики.
...
В этом примере создается тип **AlertRule** типа *"По",* основанный на шаблоне для *advanced Multistage Attack Detection,* и он сохраняется в переменной $AlertRule.
...
В этом примере создается оповещение типа *MicrosoftSecurityIncidentCreation,* основанное на шаблоне "Создать инциденты" на основе оповещений *Azure "Интернет-система* безопасности", и оно сохраняется в $AlertRule безопасности. 
