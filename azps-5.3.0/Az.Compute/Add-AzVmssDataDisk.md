...
## ОПИСАНИЕ
**Cmdlet Add-AzVmssDataDisk** добавляет диск данных в экземпляр набора виртуальных машин (VMSS).
...
### -CreateOption
Определяет параметр создания диска.
...
### -DiskIOPSReadWrite
Определяет, Read-Write IOPS для управляемого диска. Следует использовать только в том случае, если StorageAccountType UltraSSD_LRS. Если не указано иное, значение по умолчанию будет назначено на основе размера дискового размера ГБ.
...
### -DiskMBpsReadWrite
Указывает пропускную способность (в МБ в секунду) для управляемого диска. Следует использовать только в том случае, если StorageAccountType UltraSSD_LRS. Если не указано иное, значение по умолчанию будет назначено на основе размера дискового размера ГБ.
...
### -StorageAccountType
Определяет тип учетной записи хранения диска.
