...
## ОПИСАНИЕ
Для **удаления расширения из** набора виртуальных машин (VMSS) удаляется расширение.
...
Эта команда удаляет расширение VMSS и обновляет их после изменения.

### Пример 2. Удаление экземпляра из VMSS
```
PS C:\> $vmss = Get-AzVmss -ResourceGroupName $RGName -VMScaleSetName $vmssName 
PS C:\> Remove-AzVmssExtension -ResourceGroupName "Group002" -VirtualMachineScaleSet $vmss -Id $extensionId
```

Эта команда удаляет из VMSS ID расширения, который принадлежит группе ресурсов "Группа002".
...
### -Id
Определяет код расширения, которое этот cmdlet удаляет из VMSS.
...
### -Name
Указывает имя расширения, которое этот cmdlet удаляет из VMSS.
...
Определяет VMSS, из которых нужно удалить расширение.
