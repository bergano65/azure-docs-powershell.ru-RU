...
## ОПИСАНИЕ
**Cmdlet Set-AzVMOSDisk** задает свойства диска операционной системы на виртуальной машине.
...
### Пример 2. Задает свойства на виртуальной машине из изображения пользователя
```
PS C:\> $AvailabilitySet = Get-AzAvailabilitySet -ResourceGroupName "ResourceGroup11" -Name "AvailabilitySet13" 
...
Первая команда получает набор доступности с именем AvailabilitySet13 в группе ресурсов ResourceGroup11 и сохраняет этот объект в переменной $AvailabilitySet ресурса.
...
В случае обобщенного изображения пользователя необходимо указать параметр *SourceImageUri* и параметры *Windows* или *Linux,* чтобы указать для платформы Azure расположение и тип VHD-диска операционной системы, а не с помощью cmdlet **Set-AzVMSourceImage.**
