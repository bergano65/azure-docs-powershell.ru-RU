...
## ОПИСАНИЕ
Cmdlet **Set-AzDataLakeAnalyticsCatalogAclEntry** добавляет или изменяет запись (ACE) в списке управления доступом (ACL) элемента каталога в data Lake Analytics.
...
Эта команда изменяет ACE каталога дляНси Фукера на разрешение на чтение.

### Пример 2. Изменение разрешений пользователя для базы данных
```powershell
...
Эта команда изменяет ACE базы данных дляНси Фукера на разрешения на чтение.
...
Эта команда изменяет ACE каталога, чтобы другие люди получили разрешения на чтение.
...
### Пример 5. Изменение разрешений владельца каталога
```powershell
...
Эта команда задает для учетной записи разрешение на чтение.

### Пример 6. Изменение разрешений владельца базы данных
```powershell
...
Эта команда задает для владельца базы данных разрешение на чтение.
...
### -Group
Установите запись aCL каталога для группы.
...
### -GroupOwner
Установите запись aCL каталога для владельца группы.
...
### -ItemType
Определяет тип элементов каталога или каталога. Допустимые значения этого параметра:
- Каталог
- База данных
...
### -ObjectId
Удостоверение пользователя, который нужно установить.
...
### -Other
Установите запись ACL каталога для другого.
...
### -User
Установите запись aCL каталога для пользователя.
...
### -UserOwner
Установите запись aCL каталога для владельца пользователя.
