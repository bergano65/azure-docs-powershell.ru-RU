...
## SYNOPSIS
Позволяет получить конвейер работы или конвейеры анализа для data Lake Analytics.
...
## ОПИСАНИЕ
**Get-AzDataLakeAnalyticsJobPipeline** получает определенный конвейер аналитики azure Data Lake Analytics или список конвейеров.

## ПРИМЕРЫ

### Пример 1. Получить указанный канал
```
PS C:\>Get-AzDataLakeAnalyticsJobPipeline -Account "contosoadla" -PipelineId 83cb7ad2-3523-4b82-b909-d478b0d8aea3
```

Эта команда получает указанный канал с ид '83cb7ad2-3523-4b82-b909-d478b0d8aea3' в учетной записи contosoadla.

### Пример 2. Получить список всех каналов в учетной записи
```
PS C:\>Get-AzDataLakeAnalyticsJobPipeline -AccountName "contosoadla"
```

Эта команда получает список всех каналов в учетной записи contosoadla.

## PARAMETERS

### -Account
Имя учетной записи Data Lake Analytics, под которой нужно получить канал задания.
...
### -SubmittedAfter
Необязательный фильтр, который возвращает конвейеры задания, отправленные только после указанного времени.
...
### -SubmittedBefore
Необязательный фильтр, который возвращает конвейеры задания, только отправленные до указанного времени.
