...
## SYNOPSIS
Получите билеты в службу поддержки.
...
## ОПИСАНИЕ
Получает список билетов в службу поддержки. Он получит все билеты в службу поддержки, если не указать параметры. Вы также можете фильтровать билеты в службу поддержки по status или CreatedDate с помощью параметра Filter. Вот несколько примеров значений фильтра, которые можно указать.
...
### Пример 4. Первые 2 билета в службу поддержки, отфильтрованные по состояниям
```powershell
