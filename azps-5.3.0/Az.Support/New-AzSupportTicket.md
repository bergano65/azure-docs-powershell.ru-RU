...
### Пример 2. Создайте билет в службу технической поддержки для ресурса Virtual Machine для Windows. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID-интерфейсов для виртуальной машины для Windows, для которых требуется запросить поддержку 
```powershell
...
### Пример 4. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для ядер с низким приоритетом для учетной записи с низким приоритетом. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблемных пакетов квот.
```powershell
...
### Пример 5. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту Ядра VM для определенной семьи VM для учетной записи пакета. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблемных пакетов квот.
```powershell
...
### Пример 6. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту пула для учетной записи пакета. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для получения правильныхGUID для классификации проблемных пакетов квот.
```powershell
...
### Пример 9. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту DTUs для SQL базы данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации SQL управления базой данных.
```powershell
...
### Пример 10. Создайте билет в службу поддержки по квоте, чтобы увеличить квоту для серверов SQL базы данных. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации SQL управления базой данных.
```powershell
...
### Пример 13. Создайте билет в службу поддержки с квотой, чтобы увеличить квоту для ядер с низким приоритетом для службы машинного обучения. Используйте Get-AzSupportService и Get-AzSupportProblemClassification для извлечения правильныхGUID для классификации проблем службы машинного обучения по квоте.
```powershell
...
### Пример 18. Создайте билет в службу поддержки от имени клиента, если вы поставщик облачных решений. CSP сначала должен войти в клиент, а затем войти в клиент клиента, как показано в примере ниже. После этого они должны использовать параметр -CSPHomeTenantId, чтобы указать его id для домашнего клиента во время создания билета в службу поддержки.  
```powershell
