...
## ОПИСАНИЕ
Для **отключения пакета Azure отключается cmdlet Disable-AzBatchJob.**
После того как задание будет в которое в включиться, могут выполняться новые задачи.
Отключенные задания не запускают новые задачи.
Вы можете включить отключенную работу позже.

## ПРИМЕРЫ

### Пример 1. Отключение задания пакета
```
PS C:\>Disable-AzBatchJob -Id "Job-000001" -DisableJobOption "Terminate" -BatchContext $Context
```

Эта команда отключает задание с ИД-000001.
Команда завершает активные задачи для задания.
...
### -DisableJobOption
Определяет, что делать с активными задачами, связанными с заданием, которое отключает этот cmdlet.
Допустимые значения:
- Повторное
- Завершить
- Подождите
...
### -Id
Определяет код задания, которое этот cmdlet отключает.
