...
## SYNOPSIS
Получает количество пакетных узлов на каждого состояния узла, сгруппировали по id пула.
...
## ОПИСАНИЕ
С Get-AzBatchPoolNodeCount позволяет клиентам возвращать количество узлов в каждого состояния узла, сгруппировали по пулу. Возможные состояния узла: создание, неактуальный, выход ИзPool, автономный, предустановленный, перезагрузка, перезагрузка, запуск, запуск, startTaskFailed, unknown, unusable и ожиданиеForStartTask. Для фильтрации только пула с указанным ид пула для этого параметра принимается параметр PoolId или Pool. 
...
Узел списка подсчитываются в каждой области узла для пулов в контексте текущей пакетной учетной записи.
...
Показывать количество узлов в состоянии узла для заданного пула.
...
### -MaxCount
Указывает максимальное количество пулов, которые нужно вернуть.
...
### -Pool
Определяет **PSCloudPool,** для которого нужно подсчитать количество узлов.
...
### -PoolId
Id пула, для которого требуется подсчитать количество узлов.
