...
## ОПИСАНИЕ
**Cmdlet Get-AzBatchCertificate** получает сертификаты в учетной записи Azure Batch, задаваемой параметром *BatchContext.*
Чтобы получить определенный сертификат, укажите параметры *ThumbprintAlgorithm* и *Thumbprint.*
Укажите *параметр Filter,* чтобы получить сертификаты, которые соответствуют фильтру OData.

## ПРИМЕРЫ

### Пример 1. Получить сертификат по thumbprint
```
...
Эта команда получает один сертификат с указанным эскизом.
Алгоритм печати сертификата имеет алгоритм sha1.

### Пример 2. Получить отфильтрованные сертификаты
```
...
Эта команда получает все сертификаты в активном состоянии из учетной записи пакета.
Параметр *Filter* определяет состояние.
...
Если этот параметр задан, этот командлет получает сертификаты, которые соответствуют фильтру.
...
### -MaxCount
Указывает максимальное количество сертификатов, которые нужно вернуть.
...
### -Thumbprint
Определяет эскиз сертификата, который получает этот cmdlet.
...
### -ThumbprintAlgorithm
Алгоритм, используемый для получения *параметра Thumbprint.*
В настоящее время допустимым значением является только sha1.
