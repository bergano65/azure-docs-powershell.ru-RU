...
## ОПИСАНИЕ
При **этом операция копирования** в указанный BLOB-проект прекращается.

## ПРИМЕРЫ

### Пример 1. Остановка копирования по имени
```
...
### Пример 2. Остановка копирования с помощью конвейера
```
PS C:\>Get-AzStorageContainer container* | Stop-AzStorageBlobCopy -Blob "BlobName"
```

Эта команда останавливает операцию копирования, передавая контейнер в конвейере из **Get-AzStorageContainer.**

### Пример 3. Остановка копирования с помощью конвейера и Get-AzStorageBlob
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" | Stop-AzStorageBlobCopy -Force
```

В этом примере операция копирования прекращается, передавая контейнер в конвейере из Get-AzStorageBlob-Get-AzStorageBlob.
...
Вы можете создать объект или воспользоваться Get-AzStorageContainer.
...
Вы можете создать контекст с помощью New-AzStorageContext управления.
...
### -Force
Остановка текущей задачи копирования для указанного BLOB-задача без запроса на подтверждение.
