...
## ОПИСАНИЕ
Для **устройства Edge Data Box** с его учетной записью удаляется связанная учетная запись edge Storage. Вы можете указать имя учетной записи edge Storage, которая будет удалена в качестве параметра в cmdlet.
