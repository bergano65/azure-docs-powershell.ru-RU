...
### Пример 1. Получите приложение функции по имени и удалите его.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Remove-AzFunctionApp -Force
```

Эта команда получает приложение функции по имени и удаляет его.

### Пример 2. Удаление приложения функции по имени.
```powershell
PS C:\> Remove-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Эта команда удаляет приложение функции по имени.
...
### -Force
Этот cmdlet удаляет приложение функции без запроса на подтверждение.
