...
## SYNOPSIS
Создание метаданных экземпляра службы.
...
### Пример 1. Создание новой службы Azure healthcareapis fhir с именем MyService в группе ресурсов MyResourceGroup в расположении westus2 с пропускной способностью предложения "1sdb" = 400
```powershell
...
### Пример 2. Создание новой службы Azure healthcareapis fhir с именем MyService в группе ресурсов MyResourceGroup в расположении westus2 с пропускной способностью предложения "400" и ключом ключа хранилища "https:// \<my-keyvault> \<my-key> .vault.azure.net/keys/"
```powershell
...
Используйте * для любого заглавия.
...
Укажите URL-адреса сайтов-источников, которые могут получать доступ к этому API, или используйте " *", чтобы разрешить доступ с любого сайта.
...
### -ThsOfferThroughput
HealthcareApis Fhir ServiceHirsOfferThroughput.
...
### -ManagedIdentity
Используете управляемое удостоверение?
