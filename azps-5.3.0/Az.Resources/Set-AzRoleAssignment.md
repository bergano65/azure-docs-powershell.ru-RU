...
## SYNOPSIS
Обновив существующее назначение роли.

## СИНТАКСИС

### RoleAssignmentParameterSet (по умолчанию)
```
...
## ОПИСАНИЕ
Используйте команду New-AzRoleAssignment, чтобы изменить существующее задание.  
Описания могут быть любой допустимой строкой, используемой для офертации друг друга.  
если за условием настроена версия условия, необходимо также настроить ее, но если вы обновляете условие, это необязательно.
Версию условия можно обновить с 1.0 до 2.0, но нельзя вернуть к более новой версии. Будьте осторожны, так как число 2,0 не является ретроспективным с числом 1,0.
...
Обновление существующего назначения роли путем изменения объекта
...
Обновление существующего назначения роли с помощью файла
...
### -InputFile
Имя файла, содержащее одно определение роли.
...
### -PassThru
Отображает обновленное назначение роли, если задано
