...
## ОПИСАНИЕ
С **помощью cmdlet Set-AzDataFactoryV2** создается фабрика данных с указанным именем и расположением группы ресурсов.
...
### Пример 2. Создание фабрики данных с подробными сведениями о конфигурации повторного использования с использованием существующего объекта.
```
...
Эта команда создает фабрику данных с именем WikiADF в группе ресурсов ADF в расположении EastUS с конфигурацией управления источником в Azure DevOps.

### Пример 3. Создание фабрики данных с подробными сведениями конфигурации повторного изменения GitHub с использованием нового заводского объекта.
```
...
Эта команда создает фабрику данных с именем WikiADF в группе ресурсов ADF в расположении На Востоке с конфигурацией управления исходным источником GitHub.

## PARAMETERS

### -AccountName
Имя учетной записи для настройки repo.
...
### -CollaborationBranch
Ветвь совместной работы для настройки repo.
...
Словарь, содержащий глобальные параметры фабрики данных.
...
### -HostName
Имя нового хоста для конфигурации перезаполнить GitHub.
...
### -LastCommitId
Последний id commit for repo configuration .id for repo configuration (Последний id commit for repo configuration).
...
### -Location
В этом регионе создается фабрика данных.
...
### -ProjectName
Имя проекта Azure DevOps для конфигурации переоценки.
...
### -RepositoryName
Имя репозитория для конфигурации репозитория.
...
### -ResourceId
ИД ресурсов Azure в фабрике данных V2.
...
### -RootFolder
Корневая папка для настройки repo.
...
### -TenantId
Конфигурация повторной настройки клиента для Azure DevOps.
