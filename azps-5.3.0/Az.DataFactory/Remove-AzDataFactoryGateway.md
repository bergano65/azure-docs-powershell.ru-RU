...
## ОПИСАНИЕ
Для удаления указанного шлюза из фабрики данных Azure удаляется **cmdlet Remove-AzDataFactoryGateway.**
...
Эта команда удаляет шлюз ContosoGateway из фабрики данных с именем WikiADF.

## PARAMETERS

### -DataFactory
Указывает объект **PSDataFactory.**
Этот cmdlet удаляет шлюз из фабрики данных, указанной этим параметром.
...
### -Force
Указывает на то, что этот cmdlet удаляет шлюз без запроса подтверждения.
...
Этот cmdlet removes a gateway that belongs to the group that this parameter specifies.
