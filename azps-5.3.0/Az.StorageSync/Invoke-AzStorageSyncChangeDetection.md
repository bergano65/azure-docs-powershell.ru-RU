...
> [!Note]  
> С Invoke-AzStorageSyncChangeDetection-файлами Azure не будут обнаружены следующие изменения:
> - Удаленные файлы. 
> - Файлы, которые перемещаются из папки "Поделиться".
...
В этом примере для набора файлов, которые известны вызываемой команде, будет запускаться обнаружение изменений. Необходимо, чтобы синхронизация файлов Azure также выявляла и синхронизировала эти изменения.
