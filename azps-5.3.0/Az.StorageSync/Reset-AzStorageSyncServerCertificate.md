...
## ОПИСАНИЕ
Эта команда позволит свернуть сертификат сервера синхронизации хранилища, используемый для описания удостоверения сервера, в службу синхронизации хранилища. Этот вариант предназначен для использования в сценариях устранения неполадок. При вызове этой команды сертификат сервера заменяется. Обновление службы синхронизации хранилища, зарегистрированной на этом сервере, путем отправки открытой части ключа. Так как создается новый сертификат, срок его действия также обновляется. Эту команду также можно использовать для обновления просроченного сертификата. Это может произойти, если сервер работает в автономном режиме в течение длительного времени.
