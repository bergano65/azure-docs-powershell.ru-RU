...
## ОПИСАНИЕ
Командлет Invoke-AzDataMigrationCommand создает новую задачу, которая будет запускаться для существующей задачи миграции.
...
В примерах выше Invoke-AzDataMigrationCommand командлет Invoke-AzDataMigrationCommand для создания команды для существующей службы, проекта и задачи.
...
### -TaskName
Имя задачи, для которого будет выполниться команда.
...
```
### -ObjectName
Имя объекта базы данных, с которого будет запускаться команда.
