...
## ОПИСАНИЕ
Настраивает существующую учетную запись хранилища Azure с ключным хранилищем для управления ключами учетной записи хранения. Учетная запись хранения должна уже существовать. Ключи хранения никогда не 2010 2013 не аются для вызываемого устройства.
Key Vault auto regenerates and switches the active key based on the period. Обзор [этой функции см. в powerShell](https://docs.microsoft.com/azure/key-vault/key-vault-overview-storage-keys-powershell) для учетной записи управляемого хранилища key Vault Azure.
...
Задает учетную запись хранения с ключным хранилищем, чтобы управлять ключами из хранилища ключей. Активный набор клавиш имеет значение "ключ1". Этот ключ будет использоваться для создания токенов sas. Key Vault will regenerate 'key2' after the period from the time of this command and set it as the active key. Этот процесс автоматической обработки будет продолжаться между ключами 1 и 2 с пропажами в 90 дней.
...
Задает классическую учетную запись хранения с ключным хранилищем, чтобы управлять ключами из хранилища ключей. Для активного ключа замерен первичный ключ. Этот ключ будет использоваться для создания токенов sas. Key Vault will regenerate 'Secondary' key after the period from the time of this command and set it as the active key. Этот процесс автоматической обработки будет продолжаться между первичным и дополнительным с пропажами в 90 дней.
...
Клавиша автоматического повторного сгенерации. Если это так, то неактивный ключ учетной записи хранения автоматически сгенерируется и становится новым активным ключом после расчетного периода. Если этот ложь, ключи управляемой учетной записи хранения не сгенерированы автоматически.
...
### -СкайпПериод
Период засеки. Если включено автоматическое сгенерированное ключ, это значение определяет период времени, после которого неактивные ключи будут сгенерированы автоматически и становятся активным ключом.
