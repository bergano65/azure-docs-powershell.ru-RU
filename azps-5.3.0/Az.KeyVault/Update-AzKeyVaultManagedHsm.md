...
## ОПИСАНИЕ
Этот cmdlet обновляет состояние службы управления HSM Azure.

## ПРИМЕРЫ

### Пример 1. Обновление управляемого HSM напрямую
```powershell
...
Обновляет теги управляемого HSM в `$hsmName` группе `$resourceGroupName` ресурсов.

### Пример 2. Обновление управляемого HSM с помощью piping
```powershell
...
Обновляет теги управляемого HSM с использованием синтаксиса piping.
