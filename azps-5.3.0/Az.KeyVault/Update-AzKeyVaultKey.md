...
## ОПИСАНИЕ
При нажатии на **клавиши Update-AzKeyVaultKey** редактируемые атрибуты ключа обновляются в хранилище ключей.

## ПРИМЕРЫ

### Пример 1. Изменение ключа, чтобы включить его, и настройка даты окончания срока действия и тегов
```powershell
...
Первая команда создает объект **даты и времени** с помощью командлета **Get-Date.** Этот объект определяет время на два года в будущем. Эта команда сохраняет эту дату в переменной $Expires.
Для получения дополнительных сведений введите `Get-Help Get-Date` .
Вторая команда создает переменную для хранения значений тегов высокой важности и бухгалтерии.
Конечная команда изменяет ключ с именем ITSoftware. Эта команда включает ключ, задает время окончания срока его действия, сохраненное в $Expires, и устанавливает теги, которые хранятся в $Tags.
...
Эта команда удаляет все теги для определенной версии ключа ITSoftware.
...
### -Enable
Значение true включает ключ, а ложь отключает ключ.
Если он не задан, существующее состояние (включено или отключено) не изменяется.
...
### -Expires
Время окончания срока действия ключа в UTC.
Если он не задан, срок действия ключа не изменяется.
...
### -KeyOps
Операции, которые можно выполнять с помощью клавиши.
Если он не задан, существующие операции ключа остаются без изменений.
...
### -NotBefore
Время в UTC, до которого нельзя использовать ключ.
Если он не задан, существующий атрибут NotBefore ключа не изменяется.
...
Если этот ключ задан, возвращает обновленный объект пакета ключей.
...
### -Tag
В этом элементе есть теги ключа.
Если он не задан, существующие теги ключа остаются без изменений.
