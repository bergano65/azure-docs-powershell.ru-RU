...
Передает компьютер на компьютер, на который вы подключены.

### Пример 2. Подключение удаленного компьютера к компьютеру в качестве подключенного устройства
```powershell
...
Имя пользователя используется по умолчанию.
