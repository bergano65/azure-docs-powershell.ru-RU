...
Создает подписку для указанной темы `SB-TopicSubscription-Example1` "Автобус `SB-Topic_exampl1` обслуживания".

## PARAMETERS

### -AutoDeleteOnIdle
Указывает интервал неа [установленного](https://msdn.microsoft.com/library/system.timespan.aspx) времени ожидания, после которого подписка будет автоматически удалена. Минимальная длительность — 5 минут.
...
Значение, которое указывает на то, есть ли в подписке неявная буква в исключениях при оценке фильтра.
