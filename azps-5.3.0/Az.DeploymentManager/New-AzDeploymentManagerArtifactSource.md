...
Вы можете изменить возвращенный объект локально, а затем применить изменения к источнику артефакта с помощью Set-AzDeploymentManagerArtifactSource..

Этот cmdlet возвращает объект ArtifactSource, который имеет код Ресурса, на который можно ссылаться в New-AzDeploymentManagerServiceTopology, чтобы ссылаться на артефакты, необходимые для ресурса ServiceUnit, файлы шаблона и параметров.
...
### -АртефактRoot
Необязательный смещение каталога под контейнером хранилища артефактов.
