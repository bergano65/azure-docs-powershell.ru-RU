...
Создает новую подписку на событие EventSubscription1 для группы ресурсов \` \` \` MyResourceGroupName с конечной точкой \` webhook. `https://requestb.in/19qlscd1` В этой подписке на события используются фильтры по умолчанию.
...
Создание подписки на \` событие EventSubscription1 для выбранной в данный момент подписки Azure с указанным центром событий в качестве места \` назначения событий. В этой подписке на события используются фильтры по умолчанию.
