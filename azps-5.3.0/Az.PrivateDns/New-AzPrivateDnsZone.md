...
## ОПИСАНИЕ
Для создания новой зоны частного доменного имени (DNS) в указанной группе ресурсов будет указана новая зона **AzPrivateDnsZone.** Необходимо указать уникальное имя зоны DNS для параметра *Name,* иначе при этом будет возвращена ошибка. После создания зоны создайте в New-AzPrivateDnsRecordSet наборы записей с помощью New-AzPrivateDnsRecordSet.
...
### -Name
Указывает имя частного DNS-зоны, который нужно создать.
