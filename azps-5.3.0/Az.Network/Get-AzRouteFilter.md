...
## ОПИСАНИЕ
Для **получения фильтра маршрутов возвращается cmdlet Get-AzRouteFilter.**
...
Эта команда получает фильтр маршрутов с именем RouteFilter01, который принадлежит к группе ресурсов ResourceGroup01.
...
Эта команда получает все фильтры маршрутов, которые начинаются с "Маршрутизатор".
...
### -ExpandResource
Ссылка на ресурсы, которая должна быть расширена.
