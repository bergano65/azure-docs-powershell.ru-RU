...
Когда создается виртуальный сетевой шлюз, он устанавливает для шлюза набор стандартных политик VPN IPSEC. Если пользователь на сайте хочет использовать настраиваемую политику ipsec для подключения к VPN-шлюзу, сначала необходимо настроить эту политику ipsec на VPN Gateway. **Set-AzVpnClientIpsecParameter** позволяет это сделать.
