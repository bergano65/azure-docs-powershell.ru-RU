...
## SYNOPSIS
Остановка операции захвата пакетов для VPN-подключения
...
## ОПИСАНИЕ
Остановка операции захвата пакетов через VPN-подключение и отправка результата по заданным sasUrl контейнера хранения.
...
### -InputObject
Объект VPN-подключения, в котором нужно начать захват пакетов.
...
### -Name
Имя VPN-подключения, в котором нужно начать захват пакетов.
...
### -ResourceId
Azure resource ID of the VpnConnection where packet capture to be started.
...
### -SasUrl
URL-адрес SAS для остановки захвата пакетов в Vpn.
