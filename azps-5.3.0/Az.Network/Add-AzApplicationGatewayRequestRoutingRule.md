...
## ОПИСАНИЕ
Cmdlet **Add-AzApplicationGatewayRequestRoutingRule** добавляет правило маршрутизации запросов к шлюзу приложения.

## ПРИМЕРЫ

### Пример 1. Добавление правила маршрутки запроса к шлюзу приложения
```
...
Вторая команда добавляет правило маршрутки запроса к шлюзу приложения.

## PARAMETERS

### -ApplicationGateway
Указывает шлюз приложения, к которому этот cmdlet добавляет правило маршрутки запросов.
...
### -BackendAddressPool
Определяет объект пула адресов шлюза приложения.
...
### -BackendHttpSettings
Определяет объект параметров HTTP для шлюза приложения.
...
Определяет дополнительный http-параметры ИД шлюза приложения.
...
### -HttpListener
Определяет объект HTTP-прослушивание шлюза приложения.
...
### -HttpListenerId
Определяет ИД HTTP-адреса шлюза приложения.
...
### -Name
Указывает имя правила маршрутки запроса, которое добавляет этот cmdlet.
...
### -UrlPathMapId
Определяет url-адрес карты пути для правила маршрутки.
