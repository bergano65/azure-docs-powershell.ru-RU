...
## ОПИСАНИЕ
Этот Get-AzNetworkWatcherConnectionMonitorReport возвращает отчет о последних состояниях подключения.

## ПРИМЕРЫ

### Пример 1. Получить последний снимок подключения монитора подключения по имени в указанном расположении
```
...
В этом примере мы запрашиваем последние состояния подключения указанного монитора подключения.
...
### -ResourceId
ИД ресурса монитора подключения.
...
## ПРИМЕЧАНИЯ
Ключевые слова: azure, azurerm, arm, resource, connectivity, management, manager, network, networking, network watcher, network watcher, connection monitor
