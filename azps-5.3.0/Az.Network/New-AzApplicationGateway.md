...
### IdentityByUserAssignedIdid (по умолчанию)
```
...
У слушателя есть передней порт, IP-адрес переднего уровня, протокол (HTTP или HTTPS) и имя SSL-сертификата (при настройке offload SSL).
...
В следующем примере создается шлюз приложения, сначала создается группа ресурсов и виртуальная сеть, а также:
...
Шестая команда создает конфигурацию переднего IP-адреса с $PublicIp, именует ее FrontEndPortConfig01 и сохраняет ее в переменной $FrontEndIpConfig.
