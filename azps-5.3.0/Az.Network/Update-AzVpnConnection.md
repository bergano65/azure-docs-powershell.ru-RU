...
## ОПИСАНИЕ
С **помощью cmdlet Update-AzVpnConnection** обновляется VPN-подключение.  
VPN-подключение создает подключение IPsec, которое подключает VPN-шлюз к удаленной ветви клиента, представленной в Azure как VPN-сайт.
...
После этого подключение будет обновлено с использованием команды Set-AzVpnConnection IpSecPolicy.
...
Соединение будет обновлено, чтобы получить новый общий ключ с помощью конструкции безопасной строки.
...
### -EnableBgp
Включить BGP для этого подключения
...
### -SharedKey
Общий ключ, необходимый для этого подключения.
...
Используйте локальный IP-адрес Azure в качестве источника при иниции подключения.
...
Для этого подключения используйте селекторы трафика на основе политики.
...
Список vpnSiteLinkConnections, необходимых для этого VPNConnection.
