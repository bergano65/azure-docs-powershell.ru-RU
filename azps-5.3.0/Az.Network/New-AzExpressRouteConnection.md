...
## ОПИСАНИЕ
Создает подключение ExpressRoute между локальной схемой BGP expressRoute и шлюзом ExpressRoute внутри виртуального концентратора.
...
Выше мы создадим группу ресурсов, виртуальную сеть, виртуальную сеть, виртуальный концентратор, шлюз ExpressRoute и канал ExpressRoute с частным пирингом в Западной центральной части США в группе ресурсов TestRG в Azure. Созданный шлюз подключается к пирингу через канал ExpressRoute с помощью команды New-AzExpressRouteConnection управления.
...
### -AuthorizationKey
Ключ, полученный от владельца схемы ExpressRoute для создания подключения к шлюзу в другой подписке.
...
Включить безопасность Интернета для этого подключения шлюза ExpressRoute
...
ИД ресурса пиринга express Route, к которому должно быть создано подключение к шлюзу Express Route.
...
Родительская компания ExpressRouteGateway для этого подключения.
...
### -ParentResourceId
ИД ресурса родительского ресурса ExpressRouteGateway для этого подключения.
...
### -RoutingWeight
Вес маршрутов пакетов, который необходимо на должно быть назначено этому подключению.
