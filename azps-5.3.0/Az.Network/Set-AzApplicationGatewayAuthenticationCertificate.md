...
## ОПИСАНИЕ
**Cmdlet Set-AzApplicationGatewayAuthenticationCertificate** обновляет сертификат проверки подлинности для шлюза приложения Azure.

## ПРИМЕРЫ

### Пример 1. Обновление сертификата проверки подлинности
```
...
Вторая команда обновляет сертификат проверки подлинности с именем cert01 в шлюзе приложения.
Третья команда обновляет шлюз приложения.

## PARAMETERS

### -ApplicationGateway
Указывает имя шлюза приложения, для которого этот cmdlet обновляет сертификат проверки подлинности.
...
### -CertificateFile
Путь к файлу сертификата проверки подлинности, с помощью которого этот cmdlet обновляет сертификат.
...
### -Name
Указывает имя сертификата проверки подлинности, который обновляется этим cmdlet.
