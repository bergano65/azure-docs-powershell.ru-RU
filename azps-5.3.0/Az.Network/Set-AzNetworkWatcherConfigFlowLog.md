...
В этом примере мы настроим состояние ведения журнала потоков для группы безопасности сети. В ответе мы видим, что в NSG включено ведение журнала потоков, формат по умолчанию, а политика хранения не задана.
...
Возвращает или задает интервал (в минутах), который определяет, как часто служба tas должна делать аналитические данные. Поддерживаемые значения — 10 и 60 минут.
