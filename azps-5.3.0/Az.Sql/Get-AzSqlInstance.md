...
## SYNOPSIS
Возвращает сведения о экземпляре управляемой базы SQL Azure.
...
## ОПИСАНИЕ
**Cmdlet Get-AzSqlInstance** возвращает сведения об одном или SQL управляемых экземплярах Azure.
Укажите имя экземпляра, чтобы увидеть сведения только для этого экземпляра.

## ПРИМЕРЫ

### Пример 1. Все экземпляры, которые назначены группе ресурсов
```powershell
...
Эта команда получает сведения обо всех экземплярах, которые назначены группе ресурсов ResourceGroup01.

### Пример 2. Просмотр сведений о экземпляре
```powershell
...
Эта команда получает сведения об экземпляре ManagedInstance1.

### Пример 3. Получить все экземпляры, которые назначены группе ресурсов, с помощью фильтрации
```powershell
...
Эта команда получает сведения обо всех экземплярах, которые назначены группе ресурсов ResourceGroup01, которые начинаются с managedInstance.

### Пример 4. Все экземпляры в пуле экземпляров
```powershell
...
### Пример 5. Получить все экземпляры в пуле экземпляров с помощью объекта пула экземпляров
```powershell
...
### Пример 6. Получить все экземпляры в пуле экземпляров с помощью идентификатора ресурса пула экземпляров
```powershell
...
### Пример 7. Использование идентификатора ресурса для управляемого экземпляра
```powershell
