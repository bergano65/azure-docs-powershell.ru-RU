...
## ОПИСАНИЕ
Для **этого** с его SQL azure получаются избыточные резервные копии из экземпляра, управляемого базой данных Azure SQL.

## ПРИМЕРЫ

### Пример 1. Получите в экземпляре все избыточные резервные копии базы данных
```
...
Эта команда получает все избыточные резервные копии базы данных для экземпляра ManagedInstance1.

### Пример 2. Резервное копирование избыточной базы данных по имени в управляемом экземпляре
```
...
Эта команда получает избыточное резервное копирование базы данных с именем ManagedDatabase1 из экземпляра ManagedInstance1.

### Пример 3. Получите все избыточные резервные копии базы данных в экземпляре с помощью фильтрации
```
...
Эта команда получает все избыточные резервные копии базы данных в экземпляре ManagedInstance1, которые начинаются с "ManagedDatabase".
