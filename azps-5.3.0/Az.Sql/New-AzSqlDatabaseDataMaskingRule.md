...
Укажите *таблицу TableName* и *ColumnName,* чтобы указать целевой объект для правила и параметр *MaskingFunction,* чтобы определить маскировать данные.
Если *maskingFunction* имеет значение "Число" или "Текст", для параметров *NumberFrom* и NumberTo можно задать параметры NumberFrom и *NumberTo,* а для маскирования текста — *параметры PrefixSize,* *ReplacementString* и *SuffixSize.*
...
### -ReplacementString
Определяет количество символов в конце текста, которые не маскирует.
Укажите этот параметр только в том случае, если для параметра *MaskingFunction* задано значение Text.
