...
### RemoveBackupDefault (по умолчанию)
```
...
## ОПИСАНИЕ
Указанный резервная копия удаляется с учетом **cmdlet Remove-AzSqlDatabaseLongTermRetentionBackup.**

## ПРИМЕРЫ

### Пример 1. Удаление одной резервной копии с помощью группы ресурсов
```powershell
...
Удаляет резервную копию с именем 601061b7-d10b-46e0-bf77-a2bfb16a6add;131655666550000000

### Пример 2. Удаление одной резервной копии без группы ресурсов
```powershell
...
### Пример 3. Удаление всех резервных копий для расположения
```powershell
...
Эта команда удаляет все долгосрочные резервные копии хранения для местоположения northeurope.
...
### -InputObject
Объект долгосрочной резервной копии базы данных, который нужно удалить.
...
### -ResourceId
Код ресурса базы данных, из которой нужно удалить резервную копию данных в течение долгого времени.
...
### -ServerName
Имя azure SQL Server под резервной копией.
