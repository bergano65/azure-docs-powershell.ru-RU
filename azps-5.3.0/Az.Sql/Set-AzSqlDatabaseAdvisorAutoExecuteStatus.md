...
## ОПИСАНИЕ
Cmdlet **Set-AzSqlDatabaseAdvisorAutoExecuteStatus** изменяет свойство автоматического выполнения для помощника по базе данных Azure SQL Azure.
В настоящее время этот cmdlet поддерживает значения Enabled, Disabled и Default.
...
Эта команда изменяет состояние автоматического выполнения помощника CreateIndex на Enabled.

## PARAMETERS

### -AdvisorName
Указывает имя консультанта, состояние которого изменяется этим cmdlet.
...
### -AutoExecuteStatus
Указывает значение для состояния.
...
### -DatabaseName
Указывает имя базы данных, для которой этот cmdlet изменяет состояние.
