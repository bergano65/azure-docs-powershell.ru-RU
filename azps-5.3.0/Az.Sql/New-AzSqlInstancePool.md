...
## ОПИСАНИЕ
С его использованием создается пул экземпляров Azure SQL **AzSqlInstancePool.**
...
Эта команда создает новый пул экземпляров Azure SQL Azure.
...
### -ComputeGeneration
Генерация компьютеров для пула экземпляров.
...
### -LicenseType
Определяет тип лицензии.
Возможные значения: BasePrice (со скидкой AHB) и LicenseIncluded (без скидки AHB).
...
### -Location
Расположение для создания пула экземпляров.
...
### -SubnetId
ИД подсети, который используется для создания пула экземпляров.
