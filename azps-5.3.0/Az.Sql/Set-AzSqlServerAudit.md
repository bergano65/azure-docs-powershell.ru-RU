...
## ОПИСАНИЕ
Чтобы изменить параметры аудита для azure SQL Server, можно изменить параметры аудита для сервера **Set-AzSqlServerAudit.**
...
### Пример 4. Удаление параметра расширенных фильтров из политики аудита сервера Azure SQL
```powershell
...
### Пример 8. Отключение политики аудита аналитики журналов для сервера Azure SQL
```powershell
PS C:\>Set-AzSqlServerAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -LogAnalyticsTargetState Disabled
```

### Пример 9. Политика аудита аналитики журнала для сервера Azure SQL отключена (по конвейеру)
```powershell
