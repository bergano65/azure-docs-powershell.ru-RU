...
## ОПИСАНИЕ
С **помощью cmdlet Get-AzAutoscaleSetting** можно получить все параметры автомасштабы, связанные с группой ресурсов или заданным параметром авто шкалы.

## ПРИМЕРЫ

### Пример 1. Настройка параметров автомасштаби
```
...
Эта команда получает параметры авто шкалы, которые назначены группе ресурсов По умолчанию (Веб-восток).

### Пример 2. Настройка авто шкалы по имени
```
...
Эта команда получает параметр авто шкалы с именем DefaultServerFarm-Default-Web-EastUS.
...
### -DetailedOutput
Указывает на то, что в выходных данных этой операции отображаются полные сведения.
...
### -Name
Указывает имя параметра, который нужно получить.
