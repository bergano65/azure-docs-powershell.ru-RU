...
Определяет набор доступности, который будет использоваться для создания VM-решения.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: определяет диски на сервере-источнике, которые нужно включить для репликации.
  ...
  - `LogStorageAccountSasSecretName <String>`: ключевое имя секретного сейфа учетной записи хранения журнала.
