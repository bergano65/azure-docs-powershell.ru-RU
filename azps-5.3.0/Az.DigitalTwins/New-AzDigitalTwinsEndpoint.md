...
### Пример 1. Создание azDigitalTwinsEndpoint для Ервутуба
```powershell
...
Создание azDigitalTwinsEndpoint для Eventhub с помощью connectionStringPrimaryKey

### Пример 2. Создание azDigitalTwinsEndpoint для EventGrid
```powershell
...
Создание azDigitalTwinsEndpoint для Eventhub по TopicEndpoint и accessKey1

### Пример 3. Создание azDigitalTwinsEndpoint для ServiceBus
```powershell
...
Создание AzDigitalTwinsEndpoint для ServicBus с помощью PrimaryConnectionString
...
### -DeadLetterSecret
Немерявная секретная буква в хранилище.
Во время чтения он будет заме же читаем.
...
### -EndpointDescription
Ресурс конечной точки DigitalTwinsInstance.
Чтобы построить таблицу, см. раздел "ЗАМЕТКИ" для свойств ENDPOINTDESCRIPTION и создание hash table.
...
### -EndpointType
Тип конечной точки цифровых технологий
...
ENDPOINTDESCRIPTION : ресурс конечной точки <IDigitalTwinsEndpointResource> DigitalTwinsInstance.
  - `EndpointType <EndpointType>`: Тип конечной точки DigitalАs
  - `[DeadLetterSecret <String>]`: секрет хранилища с письмами. Во время чтения он будет заме же читаем.
