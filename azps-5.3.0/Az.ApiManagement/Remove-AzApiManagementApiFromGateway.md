...
## ОПИСАНИЕ
**Cmdlet Remove-AzApiManagementApiFromGateway** прикрепит API к шлюзу.

## ПРИМЕРЫ

### Пример 1. Удаление API из шлюза
```
...
### -ApiId
Идентификатор существующих API, которые нужно удалить из шлюза.
...
### -GatewayId
Идентификатор существующего шлюза для удаления API.
Этот параметр является required(обязательно).
