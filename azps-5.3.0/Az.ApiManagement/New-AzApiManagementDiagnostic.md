...
## ОПИСАНИЕ
С помощью cmdlet **New-AzApiManagementDiagnostic** создается диагностическая сущность в глобальной или определенной области API.

## ПРИМЕРЫ

### Пример 1. Создание диагностики новой глобальной области
```powershell
...
В этом примере создается диагностическая сущность в глобальной области.

### Пример 2. Создание диагностики в области Api
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
В примере выше создается диагностика для API для регистрации в журнале header и `httpbin` 100 bytes of `azuremonitor` Body.
...
Если задан этот задан, будет настроена политика области API.
...
### -DiagnosticId
Идентификатор сущности диагностики.
Этот параметр является необязательным.
