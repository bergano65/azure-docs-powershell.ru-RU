...
## ОПИСАНИЕ
Для существующего шлюза создается имяузла **AzApiManagementGatewayHostnameConfiguration.**

## ПРИМЕРЫ

### Пример 1. Создание конфигурации имени пользователя для существующего шлюза
```powershell
...
Эта команда создает конфигурацию имени хоста h01 для шлюза g01.

## PARAMETERS

### -CertificateResourceId
Идентификатор ресурса для существующего идентификатора сертификата. Этот параметр является required(обязательно).
...
Идентификатор новой конфиденциации имен хостов шлюза.
...
Пометить, чтобы принудительно применить NegotiateClientCertificate.
Этот параметр является необязательным.
