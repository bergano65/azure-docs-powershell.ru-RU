...
### ImportFromLocalFile (по умолчанию)
```
...
## ОПИСАНИЕ
Для **импорта AzApiManagementApi** импортируется API управления Azure API из файла или URL-адреса на языке описания веб-приложения (WADL), языке описания веб-служб (WSDL) или формате Swagger.
...
### Пример 2. Импорт API из Swagger-файла
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Эта команда импортирует API из указанного Swagger-файла.

### Пример 3. Импорт API из ссылки WADL
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
### Пример 4. Импорт API из связи с Открытым API
```powershell
...
Эта команда импортирует API из указанной связи спецификации Open 3.0.

### Пример 5. Импорт API из ссылки Open Api в набор ApiVersion
...
Эта команда импортирует API из указанного документа спецификации Open 3.0 и создает новый ApiVersion.

## PARAMETERS

### -ApiId
Определяет ИД импортируемого API.
...
### -ApiRevision
Идентификатор изменения API. Этот параметр является необязательным. Если он не задан, импорт будет сделан на текущий активный момент изменения или новый api.
...
### -ApiType
Этот параметр является необязательным и имеет значение по умолчанию http. Параметр Soap применяется только при импорте WSDL и создает API-пасс SOAP.
...
### -Path
Указывает путь веб-API как последнюю часть url-адреса API.
Этот URL-адрес используется пользователями API для отправки запросов в веб-службу.
Должен иметь длину от 1 до 400 знаков.
...
### -Protocol
Протоколы Web API (http, https). Протоколы, для которых доступен API. Этот параметр является необязательным. При этом протоколы, указанные в документе спецификаций, будут переопределяться.
...
### -ServiceUrl
URL-адрес веб-службы, предлагая API. Этот URL-адрес будет использоваться только службой управления API Azure и не будет разоснана. Этот параметр является необязательным. При условии, что он переопределит Параметр ServiceUrl, указанный в документе "Спецификации".
...
### -WsdlEndpointName
Локальное имя импортируемой конечной точки WSDL (порта). Должен иметь длину от 1 до 400 знаков. Этот параметр является необязательным и требуется только для импорта Wsdl. Значение по умолчанию — $null.
...
### -WsdlServiceName
Локальное имя службы WSDL, которое нужно импортировать. Должен иметь длину от 1 до 400 знаков. Этот параметр является необязательным и требуется только для импорта WSDL. Значение по умолчанию — $null.
