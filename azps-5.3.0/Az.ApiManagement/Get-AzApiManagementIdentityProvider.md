...
## ОПИСАНИЕ
Получите сведения о конфигурации поставщика удостоверений.
ClientSecret не включается в сведения о результатах. Чтобы получить секрет клиента, используйте **Get-AzApiManagementIdentityProviderClientSecret.**

## ПРИМЕРЫ

### Пример 1. Получить всех поставщиков удостоверений
...
Получите все настройки поставщика удостоверений для службы.

### Пример 2. Получить поставщика удостоверений типа AAD
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Получает конфигурацию поставщика удостоверений Azure Active Directory.

### Пример 3. Получить поставщика удостоверений типа AAD B2C
```powershell
