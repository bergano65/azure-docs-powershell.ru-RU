...
Инструкции по установке Azure PowerShell на других платформах см. в статье [Установка Azure PowerShell в ОС macOS или Linux](install-azurermps-maclinux.md).
...
Чтобы установить Azure PowerShell версии 6.0 и новее, на компьютере под управлением Windows должна быть установлена версия PowerShell, не ниже 5.0. Чтобы проверить установленную на своем компьютере версию PowerShell, выполните следующую команду:
...
По умолчанию коллекция PowerShell не выступает доверенным репозиторием для PowerShellGet. При первом использовании PSGallery отображается следующее сообщение:
