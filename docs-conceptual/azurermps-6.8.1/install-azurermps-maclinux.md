...
> [!IMPORTANT]
> Модуль `AzureRM`, описанный в других статьях, не предназначен для .NET Core и не будет работать с PowerShell Core. Модули `Az` содержат псевдонимы команд для всех командлетов `AzureRM`, поэтому можно пользоваться всей документацией для `AzureRM`.
