...
Последняя версия модуля Az позволяет работать в Azure PowerShell с помощью PowerShell, начиная с версии 6.x, на всех платформах, включая Windows, macOS и Linux. В Windows он также поддерживает работу с PowerShell 5.1.
