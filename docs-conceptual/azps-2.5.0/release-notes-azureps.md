...
* Обновлены следующие команды для функции: Флаг PrivateEndpointNetworkPolicies/PrivateLinkServiceNetworkPolicies в подсети в виртуальной сети.
    - Обновлены командлеты New-AzVirtualNetworkSubnetConfig/Set-AzVirtualNetworkSubnetConfig/Add-AzVirtualNetworkSubnetConfig.
        - Добавлен необязательный параметр -PrivateEndpointNetworkPoliciesFlag, который указывает, что включение или отключение приводят к применению политик сети на частной конечной точке в этой подсети.
        - Добавлен необязательный параметр -PrivateLinkServiceNetworkPoliciesFlag, который указывает, что включение или отключение приводят к применению политик сети в службе приватного канала в этой подсети.
