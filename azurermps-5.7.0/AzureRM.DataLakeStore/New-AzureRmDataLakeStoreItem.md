...
## NОПИСАНИЕ
Командлет **New-AzureRmDataLakeStoreItem** создает новый файл или папку в Data Lake Store.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание нового файла и новой папки
```
...
### -Папка
Указывает на то, что эта операция создает папку.
...
### -Force
Указывает на то, что эта операция может перезаписать целевой элемент, если он уже существует.
...
### -Path
Указывает путь к Data Lake Store для создаваемого элемента, начиная с корневого каталога (/).
...
### -Value (значение)
Указывает содержимое, которое будет добавлено к созданному элементу.
...
### подстрок
Полный путь к созданному файлу или папке.
