...
## NОПИСАНИЕ
Проверяет сертификат, выдавая сертификат проверки, который содержит проверочный код, полученный командлетом Get-AzureRmIotHubCertificateVerificationCode. Это последний шаг в ходе проверки.
...
Проверяет владение закрытым ключом MyCertificate. 
