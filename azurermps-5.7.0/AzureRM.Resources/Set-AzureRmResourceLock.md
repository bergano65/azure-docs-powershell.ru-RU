...
## NОПИСАНИЕ
Командлет **Set-AzureRmResourceLock** изменяет блокировку ресурса.
...
Эта команда обновляет Примечание для блокировки с именем ContosoSiteLock.
...
### -LockId
Указывает идентификатор блокировки, которую изменяет этот командлет.
...
### -LockName
Указывает имя блокировки, измененной этим командлетом.
...
### -ResourceType
Указывает тип ресурса, к которому применяется блокировка.
...
`/subscriptions/`Идентификатор подписки имя `/resourceGroups/` группы ресурсов имя `/providers/Microsoft.Sql/servers/` сервера `/databases/` базы данных

Чтобы указать группу ресурсов, используйте следующий формат: 
