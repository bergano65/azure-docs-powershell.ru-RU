...
## NОПИСАНИЕ
Командлет **Get-AzureRmLogicAppTriggerHistory** получает историю триггеров в приложении логики в компоненте "логические приложения".
Этот командлет возвращает объект **WorkflowTriggerHistory** .
Укажите логическое приложение, группу ресурсов и триггер.
...
### Пример 1: получение истории триггеров для приложения логики
```
...
Эта команда получает журнал триггеров для определенного логики приложения для триггера в приложении Logic с именем LogicApp03.

### Пример 2: получение журналов триггеров для приложения логики
```
...
Эта команда возвращает журналы триггеров рабочих процессов для триггера в приложении Logic с именем LogicApp07.
...
### -HistoryName
Указывает имя журнала, который получает этот командлет.
...
### -Name (имя)
Указывает имя логического приложения, для которого этот командлет получает журнал триггеров.
...
### -ResourceGroupName
Указывает имя группы ресурсов, в которой этот командлет получает историю.
...
### -TriggerName
Указывает имя триггера, для которого этот командлет получает историю.
