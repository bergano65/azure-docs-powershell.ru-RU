...
## КРАТКИй обзор
Возвращает ключи API для учетной записи. Эти ключи являются механизмом проверки подлинности, используемым при последующих звонках на службы расположения Azure.
...
## NОПИСАНИЕ
Командлет **Get-AzureRmLocationBasedServicesAccountKey** получает ключи API для учетной записи служб, основанной на местоположении.

Учетная запись служб на основе расположения имеет два ключа API: Primary и Secondary.
Ключи обеспечивают взаимодействие с конечной точкой учетной записи служб на основе расположения.

Используйте [New-AzureRmLocationBasedServicesAccountKey](New-AzureRmLocationBasedServicesAccountKey.md) для повторного создания ключа.
...
Возвращает ключи для указанной учетной записи служб на основе расположения.
