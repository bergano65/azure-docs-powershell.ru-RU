...
## NОПИСАНИЕ
Командлет **New-AzureRmLocationBasedServicesAccountKey** заново создает ключ API для учетной записи служб на основе расположения.
...
Повторно создает основной ключ API для учетной записи Моя учетная запись в группе ресурсов MyResourceGroup.
...
Повторно создает вторичный ключ API для указанной учетной записи служб на основе расположения.
...
### -InputObject
Учетная запись служб на основе расположения, [полученная от Get-AzureRmLocationBasedServicesAccount](Get-AzureRmLocationBasedServicesAccount.md).
...
### -KeyName
Ключ учетной записи служб на основе расположения.
...
### -Name (имя)
Имя учетной записи служб на основе местоположения.
...
### -ResourceId
ResourceId (учетная запись) служб на основе местоположения.
```yaml
Type: String
