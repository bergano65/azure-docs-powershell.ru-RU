...
## NОПИСАНИЕ
Командлет **Remove-AzureRmCdnProfile** удаляет профиль сети доставки содержимого (CDN) для Azure.
...
### -CdnProfile
Указывает профиль, который удаляет этот командлет.
...
### -Имя_профиля
Указывает имя профиля, который удаляется этим командлетом.
