...
## NОПИСАНИЕ
Командлет **Get-AzureRmSqlServerRecommendedAction** получает одно или несколько рекомендованных действий для консультанта SQL Server Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение списка всех рекомендуемых действий для определенного помощника
```
...
Эта команда возвращает список всех рекомендованных действий для SQL Server Advisor с именем CreateIndex, доступных для сервера с именем Wi-Runner-Австралия-Восток.
...
Эта команда получает рекомендуемое действие с именем IR_ \[ test_schema \] _ \[ test_table_0 .0361551 \] _6C7AE8CC9C87E7FD5893 из помощника с именем CreateIndex.

## ПАРАМЕТРЫ

### -AdvisorName
Указывает имя консультанта, для которого этот командлет запрашивает действия.
...
### -ResourceGroupName
Указывает имя группы ресурсов сервера, содержащего этот сервер.
...
### -ИмяСервера
Указывает имя сервера, которому принадлежит помощник.
