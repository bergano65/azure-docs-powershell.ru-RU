...
## NОПИСАНИЕ
Командлет **Remove-AzureRmVirtualNetwork** удаляет виртуальную сеть Azure.

## ИЛЛЮСТРИРУЮТ

### 1: создание и удаление виртуальной сети
```
...
Этот пример создает виртуальную сеть в группе ресурсов, а затем немедленно удаляет ее. Чтобы отключить вывод запроса при удалении виртуальной сети, используйте флаг-force.
...
### -Name (имя)
Указывает имя виртуальной сети, которую этот командлет удаляет.
...
### -ResourceGroupName
Указывает имя группы ресурсов, которая содержит виртуальную сеть, которую удаляет этот командлет.
