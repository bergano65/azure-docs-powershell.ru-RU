...
## NОПИСАНИЕ
Командлет Get-AzureRmNetworkWatcherConnectionMonitorReport возвращает отчет о последних состояниях подключения.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Получение последнего снимка подключения монитора подключений по имени в указанном расположении
```
...
В этом примере мы получаем запросы о последних состояниях подключения для указанного монитора подключений.
...
### -ResourceId
Идентификатор ресурса для монитора соединений.
