...
## NОПИСАНИЕ
Командлет **Get-AzureBatchCertificate** получает сертификаты из пакетной учетной записи Azure, которую указывает параметр *BatchContext* .
Чтобы получить определенный сертификат, укажите параметры *ThumbprintAlgorithm* и *Thumbprint* .
Укажите параметр *фильтра* , чтобы получить сертификаты, соответствующие фильтру Open Data Protocol (OData).

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение сертификата по отпечатку
```
...
Эта команда возвращает один сертификат с указанным отпечатком.
Алгоритм отпечатка сертификата — SHA1.

### Пример 2: получение отфильтрованных сертификатов
```
...
Эта команда возвращает все сертификаты в активном состоянии из пакетной учетной записи.
Параметр *Filter* задает состояние.
...
Если указать этот параметр, этот командлет получает сертификаты, соответствующие фильтру.
...
### -MaxCount
Задает максимальное количество возвращаемых сертификатов.
