...
## NОПИСАНИЕ
Командлет **Enable-AzureBatchTask** повторно активирует задачу.
Если в задаче исчерпано количество повторных попыток, этот командлет, тем не менее, позволяет ему выполняться.

## ИЛЛЮСТРИРУЮТ

### Пример 1: повторная активация задачи
```
PS C:\>Enable-AzureBatchTask -JobId "Job7" -Id "Task2" -BatchContext $Context
```

Эта команда повторно активирует Task2 задачи в задании Job7.

### Пример 2: повторная активация задачи с помощью конвейера
```
PS C:\>Get-AzureBatchTask -JobId "Job8" -Id "Task3" -BatchContext $Context | Enable-AzureBatchTask -BatchContext $Context
```

Эта команда возвращает пакетную задачу с ИДЕНТИФИКАТОРом Task3 в задании с ИД Job8 с помощью командлета Get-AzureBatchTask.
Команда передает эту задачу текущему командлету с помощью оператора конвейера.
Эта команда активирует эту задачу повторно.
...
### -ID
Указывает идентификатор задачи, которую нужно повторно активировать.
...
### -Задача
Указывает задачу, которую повторно активировал этот командлет.
