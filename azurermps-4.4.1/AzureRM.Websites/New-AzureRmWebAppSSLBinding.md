...
### Пример 2: отправка сертификата и привязка его к веб-приложению
```
...
Пример 2 также привязывает сертификат E3A38EBA60CAA1C162785A2E1C44A15AD450199C3 к веб-приложению с именем ContosoWebApp.
Однако в этом случае сертификат еще не был загружен в Azure.
Из-за этого параметр *CertificateFilePath* используется для указания локальной копии сертификата. PFX-файл.
Этот сертификат будет отправлен в Azure, а затем будут созданы новые привязки SSL.
