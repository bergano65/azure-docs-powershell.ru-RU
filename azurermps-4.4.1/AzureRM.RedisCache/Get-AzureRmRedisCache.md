...
## NОПИСАНИЕ
Командлет **Get-AzureRmRedisCache** получает указанный кэш Azure Redis.
Если вы укажете параметр "нет", эта операция возвращает каждый кэш Redis для текущей подписки.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение кэша Redis по имени
```
...
Эта команда получает кэш Redis с именем myexists.

### Пример 2: получение каждого кэша Redis в группе ресурсов
```
...
Эта команда возвращает каждый кэш Redis в указанной группе ресурсов.

### Пример 3: получение всех Redis кэша в текущей подписке
```
...
Эта команда возвращает каждый кэш Redis в текущей подписке.

## ПАРАМЕТРЫ

### -Name (имя)
Указывает имя кэша Redis, который требуется получить.
Использование с параметром *ResourceGroupName* .
...
### -ResourceGroupName
Указывает имя группы ресурсов, содержащей кэш Redis, который требуется получить.

Если указан только параметр *ResourceGroupName* , эта операция возвращает каждый кэш Redis в указанной группе ресурсов.
...
Возвращает массив объектов **RedisCacheAttributes** .
