...
## КРАТКИй обзор
Задает настраиваемую конфигурацию hostname для прокси-сервера или портала службы управления API.
...
### Установка из предоставленного экземпляра PsApiManagement
```
...
## NОПИСАНИЕ
Командлет **Set-AzureRmApiManagementHostnames** применяет настраиваемую конфигурацию hostname для прокси-сервера или портала службы управления API.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Настройка конфигурации настраиваемого hostname для прокси-сервера и портала
```
...
Эта команда задает настраиваемое имя HostName Configuration для прокси-сервера и портала.

### Пример 2: Настройка настраиваемого имени узла для прокси-сервера и портала
```
...
В этом примере осуществляется настройка настраиваемого имени узла для прокси-сервера и портала.
Вам нужно импортировать соответствующие сертификаты и затем применить пользовательские имена узлов.

## ПАРАМЕТРЫ

### -ApiManagement
Указывает экземпляр **PsApiManagement** , который этот командлет получает для параметров *PortalHostnameConfiguration* и *ProxyHostnameConfiguration* .
...
Указывает конфигурацию узла настраиваемого портала.
Передача $null командлету задает имя узла по умолчанию.
...
Указывает конфигурацию имени пользователя прокси-сервера.
Передача $null задает имя узла по умолчанию.
...
### -ResourceGroupName
Указывает имя группы ресурсов, в которой находится экземпляр управления API.
