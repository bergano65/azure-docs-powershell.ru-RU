...
### Пример 1: Создание связи с настраиваемым поставщиком
```powershell
...
Создание связи с настраиваемым поставщиком. соответствующие целевые provioder должны быть правильно настроены для маршрута "Associations"
...
Область может быть любым допустимым экземпляром ресурса в оставшейся части.
Например, используйте "/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/Microsoft.Compute/virtualMachines/{vm-name}" для ресурсов виртуальной машины.
...
### -TargetResourceId
Экземпляр ресурса ОСТАВШЕЙся целевого ресурса для этой связи.
