...
## NОПИСАНИЕ
Командлет Get-AzRecoveryServicesBackupRPMountScript загружает сценарий, который подключает тома точки восстановления на компьютере, на котором она выполняется.
...
При запуске сценария файлы точки восстановления будут подключены $rp \[ 0\]

### Пример 2

Загружает сценарий для присоединения всех файлов точки восстановления. (автоматически сформированные)
...
### -Path
Место, где должен быть загружен файл, в случае восстановления файла. Если не указан путь, файл сценария будет загружен в текущий каталог.
...
### Microsoft. Azure. Commands. RecoveryServices. Backup. командлеты. Models. RPMountScriptDetails
