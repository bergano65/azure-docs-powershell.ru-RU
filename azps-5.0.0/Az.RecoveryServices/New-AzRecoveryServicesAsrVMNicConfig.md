...
## КРАТКИй обзор
Создание конфигурации сетевого адаптера ASR, содержащей сведения о конфигурации, связанные с отработкой отказа и тестированием.
...
## NОПИСАНИЕ
Командлет **New-AzRecoveryServicesAsrVMNicConfig** создает объект конфигурации сетевого адаптера ASR, содержащий сведения об отработке отказа и тест, связанные с отработкой отказа. В случае если любая информация не передается, соответствующие значения выбираются из элемента, защищенного репликацией, чтобы избежать их обновления до значения NULL.
...
Создает объект ASRVmNicConfig с сетевыми параметрами faiover, настроенными для сетевого адаптера, и тестами. Любое свойство, не переданное выше, извлекается из переданного защищенного элемента.
...
Создает объект ASRVmNicConfig с параметрами "проверить сетевые faiover", настроенными для переименования NIC. Любое свойство, не переданное выше, извлекается из переданного защищенного элемента.


### Пример 3

Создание конфигурации сетевого адаптера ASR, содержащей сведения о конфигурации, связанные с отработкой отказа и тестированием. (автоматически сформированные)
...
Указывает, включена ли сеть ускорения для восстановления сетевого адаптера.
...
Указывает, включена ли сеть ускоренной проверки поработки отказа для тестового сетевого адаптера.
...
Указывает идентификаторы пулов серверных адресов для сетевого адаптера восстановления.
...
Указывает идентификатор NSG, связанного с сетевым адаптером восстановления.
...
### -RecoveryNicName
Указывает имя сетевого адаптера восстановления.
...
Указывает IP-адрес сетевого адаптера восстановления.
...
Указывает идентификатор общедоступного IP-адреса, связанного с сетевым адаптером восстановления.
...
Указывает имя подсети восстановления.
...
Укажите элемент, защищенный репликацией ASR.
...
### -ReuseExistingNic
Указывает, можно ли использовать существующую сетевую карту во время отработки отказа.
...
Указывает идентификатор NSG, связанного с сетевым адаптером тестовой отработки отказа.
...
### -TfoNicName
Указывает имя сетевого адаптера тестовой отработки отказа.
...
Указывает имя группы ресурсов "сетевая карта для проверки отработки отказа".
...
Указывает IP-адрес тестового сетевого адаптера для отработки отказа.
...
Указывает идентификатор общедоступного IP-адреса, связанного с сетевой адаптером тестового отработки отказа.
...
### -TfoReuseExistingNic
Указывает, можно ли использовать существующую сетевую карту во время тестовой отработки отказа.
...
### -TfoVMNetworkId
Указывает идентификатор тестовой виртуальной сети для отработки отказа.
...
### -TfoVMSubnetName
Указывает имя подсети тестовой отработки отказа.
