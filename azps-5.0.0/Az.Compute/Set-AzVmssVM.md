...
## NОПИСАНИЕ
Командлет **Set-AzVmssVM** изменяет состояние экземпляра набора масштабов виртуальных машин (VMSS).

## ИЛЛЮСТРИРУЮТ

### Пример 1

Изменяет состояние экземпляра VMSS. (автоматически сформированные)
...
### -PerformMaintenance
Указывает на то, что этот командлет выполняет обслуживание виртуальной машины в VMSS.
...
Указывает на то, что этот командлет повторно развертывает виртуальную машину в VMSS.
...
### -SimulateEviction
Указывает на то, что этот командлет имитирует вытеснение плашечной виртуальной машины в наборе масштабирования ВИРТУАЛЬНЫХ машин.
Вытеснение будет происходить в течение 30 минут при вызове API.
