...
## NОПИСАНИЕ
Командлет **Get-AzConsumptionMarketplace** возвращает Marketplaces для подписки.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение сведений об рынках
```powershell
...
### Пример 2: получение сведений об использовании Marketplace с диапазоном дат
```powershell
...
### Пример 3: получение сведений об использовании BillingPeriodName на рынке
```powershell
...
### Пример 4: получение сведений об использовании Marketplace с фильтром InstanceName
```powershell
...
### -BillingPeriodName
Название определенного расчетного периода, который нужно связать с магазином рынка.
...
### -EndDate
Конечная дата (в формате UTC) Marketplaces для фильтрации.
...
### -InstanceId
Идентификатор экземпляра Marketplaces для фильтрации.
...
### -InstanceName
Имя экземпляра Marketplaces для фильтрации.
...
### -ResourceGroup
Группа ресурсов для "рынки", которые нужно отфильтровать.
...
### -StartDate
Начальная дата (в формате UTC) партнеров, которые нужно отфильтровать.
...
### Microsoft. Azure. Commands. потребление. Models. PSMarketplace
