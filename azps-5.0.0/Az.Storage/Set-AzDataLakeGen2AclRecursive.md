...
## КРАТКИй обзор
Рекурсивно настройте ACL по указанному пути. 
...
## NОПИСАНИЕ
Командлет **Set-AzDataLakeGen2AclRecursive** задает список ACL для заданных путей. Входной ACL полностью заменит исходный список ACL.

## ИЛЛЮСТРИРУЮТ

### Пример 1: рекурсивный набор ACL для directiry
```
...
Эта команда сначала создает объект ACL с 3 записями ACL, а затем рекурсивно задает список ACL для каталога.

### Пример 2: рекурсивный набор ACL для корневого directiry файловой системы
```
...
Эта команда сначала рекурсивно задает список ACL для корневого каталога, а затем возобновите его с помощью ContinuationToken после того, как пользователь исправит неудачный файл.

### Пример 3: настройка рекурсивного блока ACL по блокам
```
$token = $null
...
Этот сценарий задает список ACL rescursively в блоке каталогов с помощью блока, в котором размер блока равен BatchSize * MaxBatchCount. Размер блока — 200 в этом сценарии.

### Пример 4: рекурсивный набор ACL для каталога и ContinueOnFailure, а затем возобновление из сбоев по одному
```
...
Эта команда сначала рекурсивно задает список ACL для каталога с ContinueOnFailure, а некоторые элементы произошел сбой, после чего возобновите неудачные элементы по одному.
