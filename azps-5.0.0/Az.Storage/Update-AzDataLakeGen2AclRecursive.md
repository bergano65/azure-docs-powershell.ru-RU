...
## КРАТКИй обзор
Рекурсивно обновите список ACL по указанному пути. 
...
## NОПИСАНИЕ
Командлет **Update-AzDataLakeGen2AclRecursive** обновляет ACL на заданном пути рекурсивно. Список ACL ввода будет объединять исходную таблицу управления доступом: Если элемент ACL с одинаковым AccessControlType/EntityId/DefaultScope существует, разрешение на обновление; в противном случае добавьте новую запись списка ACL.

## ИЛЛЮСТРИРУЮТ

### Пример 1: рекурсивное обновление ACL для корневого directiry файловой системы
```
...
### Пример 2: рекурсивное обновление ACL в каталоге и возобновление после сбоя в ContinuationToken
```
...
Эта команда сначала рекурсивно обновляет ACL для каталога, а затем возобновляется с ContinuationToken после того, как пользователь исправит неудачный файл.

### Пример 3: обновление рекурсивного блока ACL по блокам
```
...
Этот сценарий будет обновлять ACL rescursively в блоке каталога блоком, где размер блока равен BatchSize * MaxBatchCount. Размер блока — 5000 в этом сценарии.

### Пример 4: обновление списка управления доступом (ACL) в каталоге и ContinueOnFailure, затем возобновление из сбоев по одному
```
...
Эта команда сначала рекурсивно обновляет ACL на каталог с ContinueOnFailure, и некоторые элементы не произошел, а затем возобновляются элементы с ошибками по одному.
