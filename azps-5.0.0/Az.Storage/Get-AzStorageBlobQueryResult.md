...
## NОПИСАНИЕ
Командлет **Get-AzStorageBlobQueryResult** применяет простой оператор структурированного запроса (SQL) к содержимому BLOB-объектов и сохраняет запрашиваемое подмножество данных в локальном файле.

## ИЛЛЮСТРИРУЮТ

### Пример 1: запрос больших двоичных объектов
```powershell
...
Эта команда запрашивает большой двоичный объект succsssfully с параметром input config as CSV, а выходной файл конфигурации — в формате JSON и сохраняет выходные данные в локальный файл "c:\resultfile.json".

### Пример 2: запрос моментального снимка большого двоичного объекта
```powershell
...
Эта команда сначала получает объект BLOB для моментального снимка BLOB, затем запрашивает снимок большого двоичного объекта и отображает результат ошибки запроса включения.
...
Конфигурация, используемая для обработки ввода текста запроса. Создайте объект конфигурации с помощью New-AzStorageBlobQueryConfig.
...
Конфигурация, используемая для обработки текста вывода запроса. Создайте объект конфигурации с помощью New-AzStorageBlobQueryConfig.
...
### -PassThru
Возвращает значение, указывающее, успешно ли запрашивается указанный двоичный объект.
...
### -QueryString
Строке запроса ознакомьтесь с дополнительными сведениями в следующих случаях: https://docs.microsoft.com/en-us/azure/storage/blobs/query-acceleration-sql-reference
...
### -ResultFile
Локальный путь к файлу для сохранения результата запроса.
...
### Azure. Storage. BLOB. специализированные. BlobBaseClient
