...
## КРАТКИй обзор
Получение всех обнаруженных серверов в проекте миграции.
...
Получение всех серверов в проекте для миграции.
...
Получение сервера в проекте для миграции по имени.
Name — это уникальный объект для сервера.

### Пример 3: список в управляющем устройстве
```powershell
...
Список всех серверов в проекте для устройства.

### Пример 4: получение управляющего устройства
```powershell
...
Получение сервера для устройства в проекте.
Name — это уникальный объект для сервера.

### Пример 5: список и фильтрация по отображаемому имени
```powershell
...
Перечислите серверы в проекте миграции и отфильтруйте ответы с помощью отображаемого имени.

### Пример 6: список в управляющем устройстве и фильтрация по отображаемому имени
```powershell
...
Выводит список серверов для управляющего устройства в проекте миграции и отфильтруйте ответы с помощью отображаемого имени.

## ПАРАМЕТРЫ

### -ApplianceName
Указывает имя управляющего устройства.
Это внутреннее сопоставление с сайтом.
...
### -DisplayName
Указывает отображаемое имя компьютера VMware.
...
Это внутреннее имя.
Для пользователей используйте отображаемое имя.
...
### Microsoft. Azure. PowerShell. командлеты. remigrate. Models. Api202001. IVMwareMachine
