...
### Пример 1: получение сведений по идентификатору
```powershell
...
### Пример 2: список всех в Project по идентификатору.
```powershell
...
### Пример 2: список всех в Project по имени.
```powershell
...
### -DiscoveredMachineId
Указывает ИД компьютера обнаруженного сервера.
...
### -InputObject
Указывает объект Machine сервера репликации.
...
### -ProjectName
Указывает проект миграции Azure в текущей подписке.
...
### Microsoft. Azure. PowerShell. командлеты. remigrate. Models. Api20180110. IMigrationItem
...
INPUTOBJECT <IMigrationItem> : определяет объект компьютера, на котором находится сервер репликации.
