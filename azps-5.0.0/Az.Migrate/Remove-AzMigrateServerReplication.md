...
## КРАТКИй обзор
Останавливает репликацию для перенесенного сервера.
...
### Пример 1: удаление по идентификатору.
```powershell
...
Повторная синхронизация по идентификатору.
...
### -ForceRemove
Указывает, нужно ли принудительно удалять репликацию.
...
### -TargetObjectID
Указывает сервер replcating, для которого необходимо отключить replicatio.
