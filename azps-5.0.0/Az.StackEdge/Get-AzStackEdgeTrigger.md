...
## NОПИСАНИЕ
Командлет **Get-AzStackEdgeTriger** получает триггеры для устройства. Вы можете указать имя в качестве параметра в командлете, чтобы получить подробные сведения о конкретных триггерах.
