...
## КРАТКИй обзор
Действие "вызвать отладку" в сеансе отладки потока данных.
...
## NОПИСАНИЕ
Эта команда выполняет предварительный просмотр предварительной версии данных/просмотра статистики/выражения для другого потока потока данных в сеансе отладки.
Ниже приведена последовательность команд PowerShell для рабочего процесса отладки потока данных.
1. Start-AzDataFactoryV2DataFlowDebugSession
1. Add-AzDataFactoryV2DataFlowDebugSessionPackage
1. Invoke-AzDataFactoryV2DataFlowDebugSessionCommand (повторите эти действия для разных команд и целевых объектов или повторите шаг 2-3, чтобы изменить файл пакета).
...
Этот пример вызывает команду Предварительный просмотр данных для сеанса отладки "fd76cd0d-8b37-4DC0-A370-3f9d43ac686d" в фабрике данных "WiKiADF", а затем преобразует выходные данные JSON в читаемый текст.
...
### -Столбцы
Список столбцов для предварительного просмотра статистики потока данных.
...
### -Command
Команда отладки потока данных. Варианты выбора — executePreviewQuery, executeStatisticsQuery и executeExpressionQuery
...
### -Expression
Выражение для предварительного просмотра выражения потока данных.
...
### -RowLimits
Предельное количество строк для предварительного просмотра данных потока данных.
...
### -StreamName
Имя потока потока данных для отладки.
...
## Пуск
Ключевые слова: Azure, azurerm, ARM, Resource, Management, Manager, Data, factoriesKeywords: Azure, azurerm, ARM, Resource, Management, Manager, Data, фабрика
