...
## КРАТКИй обзор
API для регистрации нового компьютера и, таким образом, для создания отслеживаемого ресурса на ARM
...
### Пример 1: встроенный компьютер в качестве подключенного компьютера
```powershell
...
Интегрированный компьютер, на котором установлен подключенный компьютер.

### Пример 2: подключение удаленного компьютера к подключенному устройству
```powershell
...
Настольная плата на удаленном компьютере в качестве подключенного устройства с помощью удаленного взаимодействия PowerShell.
Примечание. в настоящее время поддерживается только Windows в качестве целевого объекта.
...
### -Location
Расположение для созданного ConnectedMachine.
...
Имя узла используется по умолчанию.
...
### -Прокси
URI для прокси-сервера, который нужно использовать.
...
### -PSSession
Если указан, команда, на которой на Azure выводятся компьютеры, будет выполняться в каждом сеансе PSSession.
Примечание. Эта возможность работает только в Windows.
...
### -ResourceGroupName
Имя группы ресурсов, в которую вы хотите добавить компьютер.
...
### -SubscriptionId
Идентификатор подписки, в которую вы хотите добавить компьютер.
