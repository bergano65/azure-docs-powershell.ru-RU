...
## NОПИСАНИЕ
Командлет New-AzAnalysisServicesServer создает новый сервер служб Analysis Services.
...
Создание сервера с именем TestServer в Azure Region Запад — US и в группе ресурсов testresourcegroup. Уровень SKU для сервера будет равен S1.

### Пример 2

Создает новый сервер служб Analysis Services. (автоматически сформированные)
...
### -GatewayResourceId
Идентификатор ресурса шлюза для связи с сервером анализа
...
### -SKU
Имя SKU для сервера.
Поддерживаются значения 0 ", 1", "2", "4", "8", "8V2", "9v2", "", ",", ", а для стандартного уровня. "B1", "B2" для базового уровня и 1 "для уровня разработки.
