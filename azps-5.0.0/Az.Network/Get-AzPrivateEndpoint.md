...
## NОПИСАНИЕ
Командлет **Get-AzPrivateEndpoint** получает одну или несколько закрытых конечных точек.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение частной конечной точки
```powershell
...
Эта команда получает частную конечную точку с именем MyPrivateEndpoint1 в группе ресурсов TestResourceGroup

### Пример 2: список всех закрытых конечных точек в ResourceGroup
```powershell
...
Эта команда получает все закрытые конечные точки в группе ресурсов TestResourceGroup
