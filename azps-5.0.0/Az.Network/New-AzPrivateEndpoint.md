...
Командлет **New-AzPrivateEndpoint** создает частную конечную точку.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание частной конечной точки

В следующем примере создается частная конечная точка с определенным ИДЕНТИФИКАТОРом службы частной ссылки в указанной подсети виртуальной сети.
...
Использование запроса вручную для установления соединения.
...
Не запрашивать подтверждение на перезапись ресурса.
...
Задает расположение для закрытой конечной точки. Для определения допустимых значений для этого параметра используйте [Get-AzLocation](/powershell/module/az.resources/get-azlocation) .
...
Идентификатор ресурса для подключения закрытой конечной точки.
