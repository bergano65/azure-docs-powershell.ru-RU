...
## NОПИСАНИЕ
Командлет **Set-AzApplicationGatewayUrlPathMapConfig** задает конфигурацию для массива сопоставлений URL-путей с серверным пулом.
...
Первая команда получает шлюз приложения с именем appGwName и сохраняет результат в переменной $appgw.
Вторая команда обновляет сопоставление URL-пути с именем map01 на шлюзе приложений.
