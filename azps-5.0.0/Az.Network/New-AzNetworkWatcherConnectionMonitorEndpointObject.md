...
Name (имя): workspaceEndpoint тип: MMAWorkspaceMachine ResourceId:/subscriptions/00000000-0000-0000-0000-000000000000/resourcegroups/myresourceGroup/providers/Microsoft.OperationalInsights/workspaces/myworkspace адрес: scope: {"include": [{"адрес": "WIN-P0HGNDO2S1B"}]}
...
### -AzureSubnet
Переключатель конечной точки подсети Azure.
...
### -AzureVM
Коммутатор конечной точки виртуальной машины Azure.
...
### -AzureVNet
Коммутатор конечной точки Azure vnet.
...
### -CoverageLevel
Тестовое покрытие для конечной точки.
Поддерживаются значения по умолчанию, Low, BelowAverage, Average, AboveAvergae, Full.
...
### -ExcludeItem
Список элементов, которые необходимо исключить из области конечной точки.
...
### -ExternalAddress
Ключ конечной точки внешнего адреса.
...
### -IncludeItem
Список элементов, которые необходимо включить в область endpont.
...
### -MMAWorkspaceMachine
Переключатель конечной точки компьютера рабочей области MMA.
...
### -MMAWorkspaceNetwork
Переключатель конечной точки сети MMA рабочей области.
