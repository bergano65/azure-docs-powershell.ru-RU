...
```
Описанные выше команды получит RoutingConfiguration уже существующий AzVHubRoute и добавить статический маршрут для соединения. Кроме того, если вы планируете создать новое подключение с использованием статического маршрута, ознакомьтесь с разрешениями в [статье](New-AzRoutingConfiguration.md) пример 1.
## ПАРАМЕТРЫ
