...
## NОПИСАНИЕ
Командлет **Get-AzSynapseSqlPool** получает сведения о пуле SQL Azure Synapse Analytics.
...
Эта команда получает все пулы SQL в рабочей области.
...
Эта команда получает пул SQL в разделе Рабочая область ContosoWorkspace с именем ContosoSqlPool.
...
Эта команда получает доступ ко всем пулам SQL в рабочей области с помощью конвейера.
...
Эта команда получает пул SQL с указанным ИДЕНТИФИКАТОРом ресурса.
...
### -ResourceId
Идентификатор ресурса пула SQL Synapse.
...
### -Version
[Эта функция доступна только для некоторых подписок (Предварительная версия). Версия пула Synapse SQL. Например, 2 или 3.
