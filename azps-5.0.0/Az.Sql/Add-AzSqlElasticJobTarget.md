...
## NОПИСАНИЕ
Командлет Add-AzSqlElasticJobTarget добавляет целевой ресурс в целевую группу.
...
### Пример 2: Добавление целевого объекта базы данных
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
### Пример 3: Добавление целевого объекта эластичного пула
```powershell
...
### Пример 4: Добавление цели карты сегментов
```powershell
PS C:\> $tg = Get-AzSqlElasticJobTargetGroup -ResourceGroupName rg -ServerName elasticjobserver -Name tg1
...
Добавляет конечный объект (сервер, пул эластичных БД, базу данных и карту сегментов) в целевую группу
