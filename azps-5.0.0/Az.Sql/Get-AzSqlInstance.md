...
## NОПИСАНИЕ
Командлет **Get-AzSqlInstance** возвращает сведения об одном или нескольких экземплярах, УПРАВЛЯЕМЫХ Azure SQL.
Укажите имя экземпляра, чтобы просмотреть сведения только для этого экземпляра.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение всех экземпляров, назначенных группе ресурсов
```powershell
...
Эта команда получает сведения обо всех экземплярах, назначенных группе ресурсов ResourceGroup01.

### Пример 2: получение сведений о экземпляре
```powershell
...
Эта команда получает сведения о экземпляре с именем managedInstance1.

### Пример 3: получение всех экземпляров, назначенных группе ресурсов с помощью фильтрации
```powershell
...
Эта команда получает сведения обо всех экземплярах, назначенных группе ресурсов ResourceGroup01, которая начинается с "managedInstance".

### Пример 4: получение всех экземпляров в пуле экземпляров
```powershell
...
Эта команда получает сведения обо всех экземплярах в пуле экземпляров "instancePool0".

### Пример 5: получение всех экземпляров в пуле экземпляров с помощью объекта пула экземпляров
```powershell
...
### Пример 6: получение всех экземпляров в пуле экземпляров с помощью идентификатора ресурса пула экземпляров
```powershell
...
### Пример 7: получение управляемого экземпляра с помощью его идентификатора ресурса
```powershell
...
### -ResourceId
Идентификатор ресурса управляемого экземпляра.
