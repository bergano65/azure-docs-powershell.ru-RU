...
## NОПИСАНИЕ
Командлет **Get-AzSqlDatabaseLongTermRetentionBackup** получает все долгосрочные резервные копии хранения для местоположения, сервера или базы данных или для получения определенной длительной резервной копии хранения.
...
Эта команда получает все долгосрочные резервные копии хранения для всех баз данных (которые могут быть неактивны или удалены) в northeurope, так как группа ресурсов будет задана только в том случае, если сервер является активным.

### Пример 2: получение всех резервных копий для расположения в группе ресурсов
```powershell
...
Эта команда получает все долгосрочные резервные копии хранения для всех баз данных (которые могут быть активны или удалены) в группе ресурсов в northeurope.

### Пример 3: получение определенной длительной резервной копии хранения
```powershell
...
### Пример 4: получение всех долгосрочных резервных копий хранения для базы данных
```powershell
...
### Пример 5: получение долгосрочных резервных копий с помощью фильтрации
```powershell
...
Эта команда получает все резервные копии с именем, начинающимся с "601061b7"
