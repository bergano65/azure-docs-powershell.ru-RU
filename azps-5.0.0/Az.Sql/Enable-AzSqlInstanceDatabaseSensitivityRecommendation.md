...
## NОПИСАНИЕ
Командлет Enable-AzSqlInstanceDatabaseSensitivityRecommendation включает рекомендации по отметкам для столбцов в базе данных экземпляра SQL, управляемой службой Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Включите рекомендации по чувствительности для определенного столбца в базе данных экземпляра SQL, управляемой с помощью Azure.
```powershell
...
### Пример 2: Включите рекомендации по отметкам для определенного столбца в базе данных экземпляра SQL с управляемой службой Azure с помощью конвейера.
```powershell
