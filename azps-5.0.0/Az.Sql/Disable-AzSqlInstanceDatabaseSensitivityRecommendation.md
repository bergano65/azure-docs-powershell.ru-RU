...
## NОПИСАНИЕ
Командлет Disable-AzSqlInstanceDatabaseSensitivityRecommendation отключает (отправку) рекомендации по пометкам столбцов в базе данных SQL, управляемой с помощью Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: отключение рекомендаций по отметкам для заданного столбца в базе данных экземпляра SQL, управляемой с помощью Azure.
```powershell
...
### Пример 2. Отключите рекомендации по отметкам для столбцов, которые содержат рекомендации по пометкам в базе данных экземпляра Azure SQL Managed.
```powershell
...
### Пример 3. Отключите рекомендации по отметкам для определенного столбца в базе данных экземпляра SQL с управляемым управлением Azure с помощью конвейера.
```powershell
