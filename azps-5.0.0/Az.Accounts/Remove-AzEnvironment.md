...
## NОПИСАНИЕ
Командлет Remove-AzEnvironment удаляет конечные точки и сведения о метаданных для подключения к определенному экземпляру Azure.
...
В этом примере показано, как создать среду с помощью Add-AzEnvironment, а затем удалить среду с помощью Remove-AzEnvironment.
