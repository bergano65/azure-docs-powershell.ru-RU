...
## NОПИСАНИЕ
Командлет **Invoke-AzResourceAction** вызывает действие с указанным ресурсом Azure.
Чтобы получить список поддерживаемых действий, воспользуйтесь средством "Обозреватель ресурсов Azure".

## ИЛЛЮСТРИРУЮТ

### Пример 1: вызов для запуска виртуальной машины с ResourceId
...
### Пример 2: Invoke poweroffing ВМ с ResourceName
...
Эта команда останавливает виртуальную машину с помощью {ResourceId}.
Команда задает параметр *Force* , поэтому она не запрашивает подтверждение.

### Пример 3: Invoke регистрация поставщика ресурсов с ИД ресурсов
...
Эта команда регистрирует поставщик ресурсов "Microsoft. Network".
Команда задает параметр *Force* , поэтому она не запрашивает подтверждение.
...
### -Parameters
Задает параметры, которые являются хэш-таблицами для действия, вызванного этим командлетом.
...
### -ResourceId
Указывает полный ИД ресурса ресурса, в котором этот командлет вызывает действие.
...
### -ResourceName
Указывает имя ресурса ресурса, который этот командлет вызывает действие.
