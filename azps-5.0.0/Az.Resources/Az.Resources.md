---
Module Name: Az.Resources
Module Guid: ab3ca893-26fe-44b0-bd3c-8933df144d7b
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.resources
Help Version: 5.5.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
ms.openlocfilehash: 25460ff58e2c3f2a493638cc7011e352d551280e
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/27/2020
ms.locfileid: "94316071"
---
# Модуль AZ. Resources
## Nописание
В этом разделе отображаются разделы справки по командлетам диспетчера ресурсов Azure.

## Командлеты AZ. Resources
### [Add-AzADGroupMember](Add-AzADGroupMember.md)
Добавляет пользователя в существующую группу AD.

### [Export-AzResourceGroup](Export-AzResourceGroup.md)
Захватывает группу ресурсов в качестве шаблона и сохраняет ее в файле.

### [Get-AzADAppCredential](Get-AzADAppCredential.md)
Извлекает список учетных данных, связанных с приложением.

### [Get-AzADApplication](Get-AzADApplication.md)
Список существующих приложений Azure Active Directory.

### [Get-AzADGroup](Get-AzADGroup.md)
Фильтрует группы Active Directory.

### [Get-AzADGroupMember](Get-AzADGroupMember.md)
Список участников группы БАННЕРов в текущем клиенте.

### [Get-AzADServicePrincipal](Get-AzADServicePrincipal.md)
Фильтрует субъектов-служб Active Directory.

### [Get-AzADSpCredential](Get-AzADSpCredential.md)
Извлекает список учетных данных, связанных с субъектом-службой.

### [Get-AzADUser](Get-AzADUser.md)
Фильтрует пользователей Active Directory.

### [Get-AzDenyAssignment](Get-AzDenyAssignment.md)
Список запрета заданий в заданной области в Azure RBAC.
По умолчанию выводится список всех запрещенных заданий в выбранной подписке Azure.
Используйте соответствующие параметры для списка отказа от назначения определенному пользователю или для включения в список запрещенных назначений для определенной группы или ресурса.

### [Get-AzDeployment](Get-AzDeployment.md)
Получить развертывание

### [Get-AzDeploymentOperation](Get-AzDeploymentOperation.md)
Операция получения развертывания

### [Get-AzDeploymentScript](Get-AzDeploymentScript.md)
Возвращает или перечисляет сценарии развертывания.

### [Get-AzDeploymentScriptLog](Get-AzDeploymentScriptLog.md)
Получает журнал выполнения скрипта развертывания.

### [Get-AzDeploymentWhatIfResult](Get-AzDeploymentWhatIfResult.md)
Получает шаблон ARM What-If результат развертывания в области подписки. 

### [Get-AzLocation](Get-AzLocation.md)
Получает все расположения и поддерживаемые поставщики ресурсов для каждого местоположения.

### [Get-AzManagedApplication](Get-AzManagedApplication.md)
Получает управляемые приложения

### [Get-AzManagedApplicationDefinition](Get-AzManagedApplicationDefinition.md)
Получение определений управляемых приложений

### [Get-AzManagementGroup](Get-AzManagementGroup.md)
Получает группы управления (-ов)

### [Get-AzManagementGroupDeployment](Get-AzManagementGroupDeployment.md)
Получение развертывания в группе "Управление"

### [Get-AzManagementGroupDeploymentOperation](Get-AzManagementGroupDeploymentOperation.md)
Операция получения развертывания для группы управления

### [Get-AzManagementGroupDeploymentWhatIfResult](Get-AzManagementGroupDeploymentWhatIfResult.md)
Получает шаблон ARM What-If результат развертывания в области группы управления. 

### [Get-AzPolicyAlias](Get-AzPolicyAlias.md)
Get-AzPolicyAlias извлекает и выводит типы ресурсов поставщика Azure, которые имеют определенные псевдонимы и соответствуют указанным значениям параметров. Если не указаны никакие параметры, будут выведены все типы ресурсов поставщика, которые содержат псевдоним.
Параметр-ListAvailable изменяет это поведение, передавая список всех соответствующих типов ресурсов, включая те, у которых нет псевдонимов.

### [Get-AzPolicyAssignment](Get-AzPolicyAssignment.md)
Получение назначений политики.

### [Get-AzPolicyDefinition](Get-AzPolicyDefinition.md)
Получение определений политики.

### [Get-AzPolicySetDefinition](Get-AzPolicySetDefinition.md)
Получение определений политики.

### [Get-AzProviderFeature](Get-AzProviderFeature.md)
Получение сведений о возможностях поставщика Azure.

### [Get-AzProviderOperation](Get-AzProviderOperation.md)
Возвращает операции для поставщика ресурсов Azure, защищаемые с помощью Azure RBAC.

### [Get-AzResource](Get-AzResource.md)
Возвращает ресурсы.

### [Get-AzResourceGroup](Get-AzResourceGroup.md)
Возвращает группы ресурсов.

### [Get-AzResourceGroupDeployment](Get-AzResourceGroupDeployment.md)
Возвращает развертывания в группе ресурсов.

### [Get-AzResourceGroupDeploymentOperation](Get-AzResourceGroupDeploymentOperation.md)
Получает операцию развертывания группы ресурсов

### [Get-AzResourceGroupDeploymentWhatIfResult](Get-AzResourceGroupDeploymentWhatIfResult.md)
Получает шаблон ARM What-If результат развертывания в области группы ресурсов. 

### [Get-AzResourceLock](Get-AzResourceLock.md)
Возвращает блокировку ресурса.

### [Get-AzResourceProvider](Get-AzResourceProvider.md)
Возвращает поставщик ресурсов.

### [Get-AzRoleAssignment](Get-AzRoleAssignment.md)
Выводит список назначений ролей Azure RBAC в указанной области.
По умолчанию в списке перечислены все назначения ролей в выбранной подписке Azure.
Применяйте соответствующие параметры для списка назначений определенного пользователя или перечислите назначения в определенной группе ресурсов или ресурсе.

### [Get-AzRoleDefinition](Get-AzRoleDefinition.md)
Список всех ролей Azure RBAC, доступных для назначения.

### [Get-AzTag](Get-AzTag.md)
Получает предопределенные Теги Azure | Возвращает весь набор тегов на ресурсе или в подписке.

### [Get-AzTenantDeployment](Get-AzTenantDeployment.md)
Получить развертывание в области клиента

### [Get-AzTenantDeploymentOperation](Get-AzTenantDeploymentOperation.md)
Операция получения развертывания для развертывания в области клиента

### [Get-AzTenantDeploymentWhatIfResult](Get-AzTenantDeploymentWhatIfResult.md)
Получает шаблон ARM What-If результат развертывания в области клиента. 

### [Invoke-AzResourceAction](Invoke-AzResourceAction.md)
Вызывает действие с ресурсом.

### [Move-AzResource](Move-AzResource.md)
Перемещает ресурс в другую группу ресурсов или подписку.

### [New-AzADAppCredential](New-AzADAppCredential.md)
Добавляет учетные данные в существующее приложение.

### [New-AzADApplication](New-AzADApplication.md)
Создание нового приложения Azure Active Directory.

### [New-AzADGroup](New-AzADGroup.md)
Создание новой группы Active Directory.

### [New-AzADServicePrincipal](New-AzADServicePrincipal.md)
Создание нового субъекта-службы Azure Active Directory.

### [New-AzADSpCredential](New-AzADSpCredential.md)
Добавление учетных данных к существующему субъекту-службе.

### [New-AzADUser](New-AzADUser.md)
Создание нового пользователя Active Directory.

### [New-AzDeployment](New-AzDeployment.md)
Создание развертывания

### [New-AzManagedApplication](New-AzManagedApplication.md)
Создание приложения, управляемого Azure.

### [New-AzManagedApplicationDefinition](New-AzManagedApplicationDefinition.md)
Создание определения управляемого приложения.

### [New-AzManagementGroup](New-AzManagementGroup.md)
Создание группы управления

### [New-AzManagementGroupDeployment](New-AzManagementGroupDeployment.md)
Создание развертывания в группе "Управление"

### [New-AzManagementGroupSubscription](New-AzManagementGroupSubscription.md)
Добавляет подписку в группу управления.

### [New-AzPolicyAssignment](New-AzPolicyAssignment.md)
Создание назначения политики.

### [New-AzPolicyDefinition](New-AzPolicyDefinition.md)
Создание определения политики.

### [New-AzPolicySetDefinition](New-AzPolicySetDefinition.md)
Создание определения множества политик.

### [New-AzResource](New-AzResource.md)
Создание ресурса.

### [New-AzResourceGroup](New-AzResourceGroup.md)
Создание группы ресурсов Azure.

### [New-AzResourceGroupDeployment](New-AzResourceGroupDeployment.md)
Добавляет развертывание Azure в группу ресурсов.

### [New-AzResourceLock](New-AzResourceLock.md)
Создание блокировки ресурса.

### [New-AzRoleAssignment](New-AzRoleAssignment.md)
Назначает указанную роль RBAC указанному участнику в заданной области.

### [New-AzRoleDefinition](New-AzRoleDefinition.md)
Создание настраиваемой роли в Azure RBAC.
Укажите в качестве входных данных либо файл определения роли в формате JSON, либо объект PSRoleDefinition.
Сначала используйте команду Get-AzRoleDefinition, чтобы создать объект определения базовой роли.
Затем измените его свойства в соответствии с требованиями.
Наконец, используйте эту команду, чтобы создать настраиваемую роль с помощью определения роли.

### [New-AzTag](New-AzTag.md)
Создание предопределенного тега Azure или добавление значений в существующий тег | Создает или обновляет весь набор тегов на ресурсе или подписке.

### [New-AzTenantDeployment](New-AzTenantDeployment.md)
Создание развертывания в области клиента

### [Регистрация — AzProviderFeature](Register-AzProviderFeature.md)
Регистрация компонента поставщика Azure в учетной записи.

### [Регистрация — AzResourceProvider](Register-AzResourceProvider.md)
Регистрация поставщика ресурсов.

### [Remove-AzADAppCredential](Remove-AzADAppCredential.md)
Удаление учетных данных из приложения.

### [Remove-AzADApplication](Remove-AzADApplication.md)
Удаляет приложение Azure Active Directory.

### [Remove-AzADGroup](Remove-AzADGroup.md)
Удаление группы Active Directory.

### [Remove-AzADGroupMember](Remove-AzADGroupMember.md)
Удаление пользователя из группы AD.

### [Remove-AzADServicePrincipal](Remove-AzADServicePrincipal.md)
Удаление субъекта-службы Azure Active Directory.

### [Remove-AzADSpCredential](Remove-AzADSpCredential.md)
Удаление учетных данных из субъекта-службы.

### [Remove-AzADUser](Remove-AzADUser.md)
Удаляет пользователя Active Directory.

### [Remove-AzDeployment](Remove-AzDeployment.md)
Удаление развертывания и связанных с ней операций

### [Remove-AzDeploymentScript](Remove-AzDeploymentScript.md)
Удаление сценария развертывания и связанных с ним ресурсов.

### [Remove-AzManagedApplication](Remove-AzManagedApplication.md)
Удаляет управляемое приложение

### [Remove-AzManagedApplicationDefinition](Remove-AzManagedApplicationDefinition.md)
Удаляет определение управляемого приложения

### [Remove-AzManagementGroup](Remove-AzManagementGroup.md)
Удаление группы управления

### [Remove-AzManagementGroupDeployment](Remove-AzManagementGroupDeployment.md)
Удаляет развертывание в группе управления и любых связанных с ней операций.

### [Remove-AzManagementGroupSubscription](Remove-AzManagementGroupSubscription.md)
Удаляет подписку из группы управления.

### [Remove-AzPolicyAssignment](Remove-AzPolicyAssignment.md)
Удаление назначения политики.

### [Remove-AzPolicyDefinition](Remove-AzPolicyDefinition.md)
Удаляет определение политики.

### [Remove-AzPolicySetDefinition](Remove-AzPolicySetDefinition.md)
Удаление определения политики.

### [Remove-AzResource](Remove-AzResource.md)
Удаляет ресурс.

### [Remove-AzResourceGroup](Remove-AzResourceGroup.md)
Удаляет группу ресурсов.

### [Remove-AzResourceGroupDeployment](Remove-AzResourceGroupDeployment.md)
Удаляет развертывание группы ресурсов и связанные с ней операции.

### [Remove-AzResourceLock](Remove-AzResourceLock.md)
Удаление блокировки ресурса.

### [Remove-AzRoleAssignment](Remove-AzRoleAssignment.md)
Удаляет назначение роли указанному участнику, которому назначена определенная роль в определенной области.

### [Remove-AzRoleDefinition](Remove-AzRoleDefinition.md)
Удаляет настраиваемую роль в Azure RBAC.
Роль, которую нужно удалить, задается с помощью свойства ID роли.
Удаление завершится сбоем, если для настраиваемой роли были заданы существующие назначения ролей.

### [Remove-AzTag](Remove-AzTag.md)
Удаление предопределенных тегов или значений Azure | Удаляет весь набор тегов на ресурсе или подписке.

### [Remove-AzTenantDeployment](Remove-AzTenantDeployment.md)
Удаляет развертывание в области клиента и любые связанные операции.

### [Сохранить — AzDeploymentScriptLog](Save-AzDeploymentScriptLog.md)
Сохранение журнала выполнения сценария развертывания на диске.

### [Сохранить — AzDeploymentTemplate](Save-AzDeploymentTemplate.md)
Сохранение шаблона развертывания в файле.

### [Сохранить — AzManagementGroupDeploymentTemplate](Save-AzManagementGroupDeploymentTemplate.md)
Сохранение шаблона развертывания в файле.

### [Сохранить — AzResourceGroupDeploymentTemplate](Save-AzResourceGroupDeploymentTemplate.md)
Сохранение шаблона развертывания группы ресурсов в файле.

### [Сохранить — AzTenantDeploymentTemplate](Save-AzTenantDeploymentTemplate.md)
Сохранение шаблона развертывания в файле.

### [Set-AzManagedApplication](Set-AzManagedApplication.md)
Обновляет управляемое приложение

### [Set-AzManagedApplicationDefinition](Set-AzManagedApplicationDefinition.md)
Обновляет определение управляемого приложения

### [Set-AzPolicyAssignment](Set-AzPolicyAssignment.md)
Изменение назначения политики.

### [Set-AzPolicyDefinition](Set-AzPolicyDefinition.md)
Изменяет определение политики.

### [Set-AzPolicySetDefinition](Set-AzPolicySetDefinition.md)
Изменение определения Set политики

### [Set-AzResource](Set-AzResource.md)
Изменяет ресурс.

### [Set-AzResourceGroup](Set-AzResourceGroup.md)
Изменяет группу ресурсов.

### [Set-AzResourceLock](Set-AzResourceLock.md)
Изменяет блокировку ресурса.

### [Set-AzRoleAssignment](Set-AzRoleAssignment.md)
Обновите назначение роли.

### [Set-AzRoleDefinition](Set-AzRoleDefinition.md)
Изменяет настраиваемую роль в Azure RBAC.
Укажите измененное определение роли либо в виде JSON-файла, либо в виде PSRoleDefinition.
Сначала используйте команду Get-AzRoleDefinition, чтобы получить настраиваемую роль, которую вы хотите изменить.
Затем измените свойства, которые вы хотите изменить.
Наконец, сохраните определение роли с помощью этой команды.

### [Остановить-AzDeployment](Stop-AzDeployment.md)
Отмена выполняющегося развертывания

### [Остановить-AzManagementGroupDeployment](Stop-AzManagementGroupDeployment.md)
Отмена выполняющегося развертывания в группе "Управление"

### [Остановить-AzResourceGroupDeployment](Stop-AzResourceGroupDeployment.md)
Отменяет развертывание группы ресурсов.

### [Остановить-AzTenantDeployment](Stop-AzTenantDeployment.md)
Отмена выполняющегося развертывания в области клиента

### [Test-AzDeployment](Test-AzDeployment.md)
Проверка развертывания.

### [Test-AzManagementGroupDeployment](Test-AzManagementGroupDeployment.md)
Проверка развертывания в группе "Управление".

### [Test-AzResourceGroupDeployment](Test-AzResourceGroupDeployment.md)
Проверяет развертывание группы ресурсов.

### [Test-AzTenantDeployment](Test-AzTenantDeployment.md)
Проверяет развертывание в области клиента.

### [Отмена регистрации — AzProviderFeature](Unregister-AzProviderFeature.md)
Отмена регистрации функции поставщика Azure в учетной записи.

### [Отмена регистрации — AzResourceProvider](Unregister-AzResourceProvider.md)
Отмена регистрации поставщика ресурсов.

### [Update-AzADApplication](Update-AzADApplication.md)
Обновляет существующее приложение Azure Active Directory.

### [Update-AzADServicePrincipal](Update-AzADServicePrincipal.md)
Обновление существующего субъекта-службы Azure Active Directory.

### [Update-AzADUser](Update-AzADUser.md)
Обновление существующего пользователя Active Directory.

### [Update-AzManagementGroup](Update-AzManagementGroup.md)
Обновляет группу управления

### [Update-AzTag](Update-AzTag.md)
Выборочно обновляет набор тегов на ресурсе или в подписке.
