...
## КРАТКИй обзор
Обновите список ACL сети службы сигнальных служб.
...
## NОПИСАНИЕ
Обновите список ACL сети службы сигнальных служб, включая действие по умолчанию и сетевые ACL для общедоступного и частного подключения.

## ИЛЛЮСТРИРУЮТ

### Разрешить RESTAPI, ClientConnection для общедоступной сети и установить действие по умолчанию "запретить"
```powershell
...
### Разрешение подключения клиента и подключения сервера к частной конечной точке
```powershell
...
### Запрет подключения клиента как к общедоступной сети, так и к частной конечной точке
```powershell
...
### -DefaultAction
Действие по умолчанию списков ACL сети SignalR, разрешено или запрещено. Вы решите, нужно ли отклонять сетевые ACL или разрешить использование ACL сети. Например, если разрешено действие по умолчанию, то это не относится к отклонениям ACL.
...
### -Deny
Отказано в доступе к сетевым ACL
...
### -PrivateEndpointName
Имена (-ов) закрытых конечных точек, которые нужно обновить
...
### -PublicNetwork
Обновление списков ACL общедоступной сети
...
### -ResourceGroupName
Имя группы ресурсов.
Значение по умолчанию будет использоваться, если оно не указано.
