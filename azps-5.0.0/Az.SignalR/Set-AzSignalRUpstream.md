...
### Установка двух упорядоченных шаблонов восходящего потока
```powershell
...
Следующий JSON представляет наборы реальных шаблонов. 
...
### Удаление всех параметров восходящего потока
```powershell
...
### -Clear
Удалите все параметры восходящего потока.
...
### -Template
Элементы шаблона (-ов) для параметров восходящего потока.
Обязательный ключ: UrlTemplate.
Необязательные клавиши: HubPattern, EventPattern, CategoryPattern.
Пример использования синтаксиса splatting для передачи параметра Templates: @ {UrlTemplate = ' http://host-connections1.com ', HubPattern = ' chat '; EventPattern = "Broadcast"}, @ {UrlTemplate = ' http://host-connections2.com '}
...
[Использование splatting для передачи параметров командам в PowerShell](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_splatting?view=powershell-7)
