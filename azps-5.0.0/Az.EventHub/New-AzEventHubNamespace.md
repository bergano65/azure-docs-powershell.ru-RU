...
## NОПИСАНИЕ
Командлет New-AzEventHubNamespace создает новое пространство имен концентраторов событий типа.
...
### Пример 3: разрешенное пространство имен Kafka
```powershell
...
Создает пространство имен концентраторов \` событий \` , MyNamespaceName в указанном географическом расположении \` MyLocation \` , в группе ресурсов \` MyResourceGroupName \` с Kafka и AutoInflate.

### Пример 4: разрешенное пространство имен ZoneRedundant
```powershell
...
### Пример 5: создание пространства имен с помощью оснастки «Управление удостоверениями» в кластере 
```powershell
...
### -ClusterARMId
Идентификатор ARM кластера, в котором нужно создать пространство имен
...
### -ZoneRedundant
Включение и отключение избыточности зоны для пространства имен
