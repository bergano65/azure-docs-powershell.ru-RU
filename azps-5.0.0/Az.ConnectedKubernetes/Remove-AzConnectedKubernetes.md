...
### Пример 1: удаление подключенного kubernetes
```powershell
PS C:\> Remove-AzConnectedKubernetes -ResourceGroupName azureps-manual-test -ClusterName ps-connaks-t01

```

Эта команда удаляет подключенный kubernetes

### Пример 2: удаление подключенного kubernetes по объекту
```powershell
...
Эта команда удаляет подключенный kubernetes по объекту.
