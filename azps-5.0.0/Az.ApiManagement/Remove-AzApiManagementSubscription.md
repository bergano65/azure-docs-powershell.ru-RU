...
## NОПИСАНИЕ
Командлет **Remove-AzApiManagementSubscription** удаляет существующую подписку.
...
### -InputObject
Экземпляр PsApiManagementSubscription. Этот параметр является обязательным.
...
### -ResourceId
ИД ResourceId для подписки. Этот параметр является обязательным.
