...
## NОПИСАНИЕ
Командлет **New-AzApiManagementGatewayHostnameConfiguration** создает имя узла configuratin для существующего шлюза.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание конфигурации hostname для существующего шлюза
```powershell
...
Эта команда создает конфигурацию hostname "H01" для шлюза "G01".

## ПАРАМЕТРЫ

### -CertificateResourceId
Идентификатор ресурса для существующего идентификатора сертификата. Этот параметр является обязательным.
...
Идентификатор нового hostname (имя узла) confiuration.
...
Пометка для принудительного применения NegotiateClientCertificate.
Этот параметр является необязательным.
