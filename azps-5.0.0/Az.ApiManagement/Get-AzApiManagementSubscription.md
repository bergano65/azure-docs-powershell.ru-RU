...
## NОПИСАНИЕ
Командлет **Get-AzApiManagementSubscription** получает указанную подписку или все подписки, если подписка не указана.
Ключи не будут включены в данные результата. Чтобы получить доступ к клавишам, используйте **Get-AzApiManagementSubscriptionKey**.
...
### Пример 5: получение всех подписок для области
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
Эта команда получает все подписки, настроенные для глобальной области API.

### Пример 6: получение всех подписок для продукта и области пользователя
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### -Scope
Идентификатор области. Область действия подписки, независимо от того, является ли она областью API/apis/{apiId} или областью продукта/products/{productId} или Global API Scope/APIs или Global Scope/.
