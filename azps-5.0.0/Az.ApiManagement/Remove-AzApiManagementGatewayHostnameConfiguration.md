...
### ContextParameterSetName (по умолчанию)
```
...
## NОПИСАНИЕ
Командлет **Remove-AzApiManagementGatewayHostnameConfiguration** удаляет конфигурацию hostname из существующего шлюза.

## ИЛЛЮСТРИРУЮТ

### Пример 1: удаление существующей конфигурации hostname Gateway
```powershell
...
Эта команда удаляет существующую конфигурацию hostname Gateway и не запрашивает подтверждение у пользователя.
...
Идентификатор существующей конфигурации hostname Gateway.
Этот параметр является обязательным.
...
### -GatewayId
Идентификатор существующего шлюза.
Этот параметр является обязательным.
...
### -InputObject
Экземпляр PsApiManagementGatewayHostnameConfiguration. Этот параметр является обязательным.
...
### -ResourceId
ИД ResourceId для GatewayHostnameConfiguration. Этот параметр является обязательным.
