...
## NОПИСАНИЕ
Командлет **New-AzApiManagementDiagnostic** создает диагностический объект либо в глобальной области, либо в определенной области API.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание новой диагностики глобальной области
```powershell
...
В этом примере создается объект диагностики в глобальной области.
...
В примере выше создается диагностика для API `httpbin` для записи заголовка и 100 байт тела в `azuremonitor` средство ведения журнала.
...
Если задано значение, будет задана политика области API.
...
### -DiagnosticId
Идентификатор объекта диагностики.
Этот параметр является необязательным.
