...
### Пример 4: получение API управления с помощью GatewayId
```
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementApi -Context $ApiMgmtContext -GatewayId "g01"
```

Эта команда возвращает API для указанного GatewayId.
...
### -GatewayId
Если указано, попытается получить все API шлюза.
