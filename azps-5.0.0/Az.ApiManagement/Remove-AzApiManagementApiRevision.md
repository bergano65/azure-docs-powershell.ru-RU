...
## NОПИСАНИЕ
Командлет **Remove-AzApiManagementApiRevision** Удаляет определенную редакцию API.
...
Эта команда удаляет `2` редакцию API `echo-api` из службы управления API.

### Пример 2

Эта команда удаляет 2-API Echo API из службы управления API. (автоматически сформированные)
...
### -ApiRevision
Идентификатор редакции API. Этот параметр является обязательным.
