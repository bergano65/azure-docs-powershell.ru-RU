...
## NОПИСАНИЕ
Командлет **Get-AzApiManagementOpenIdConnectProvider** получает поставщиков OpenID Connect в средстве управления API Azure.
ClientSecret не будет включаться в данные результата. Чтобы получить секрет клиента, используйте **Get-AzApiManagementOpenIdConnectProviderClientSecret**.
...
Эта команда получает поставщик с ИДЕНТИФИКАТОРом OICProvider01.
