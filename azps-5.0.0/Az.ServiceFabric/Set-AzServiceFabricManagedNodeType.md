...
## КРАТКИй обзор
Задает свойства ресурса типа Node или выполняет действия по переобразу для определенного NDES с параметром-Reimage.
...
## NОПИСАНИЕ
Задает свойства ресурса типа Node или выполняет действия по переобразу для определенного NDES с параметром-Reimage. В операции reimgae узлы структуры служб будут отключены, прежде чем reimaging виртуальные машины и снова включить их снова после возврата. Если это делается для основных типов узлов, это может занять некоторое время, так как это может привести к тому, что вы не перемещаете сразу все узлы. Функция-ForceReimage принудительно выполняет операцию, даже если фабрика служб не может отключить узлы, но используется с осторожностью, так как это может привести к потере данных, если на узле выполняются нагрузки с отслеживанием состояния.
...
Обновите количество экземпляров типа узла.
...
Обновите properites размещения для типа узла. Это приведет к перезаписи более ранних properites размещения, если таковые имеются.
...
Переобразировать узлы 0 и 3 на типе узла.
...
Обновите количество экземпляров типа узла с помощью конвейера.

## ПАРАМЕТРЫ

### -ApplicationEndPort
Конечный порт приложения диапазона портов.
...
Порт запуска приложения в диапазоне портов.
...
### -Capacity
Теги производственных мощностей, примененные к узлам в типе узла в виде пар "ключ-значение", диспетчер ресурсов кластера использует эти теги для понимания того, сколько ресурсов имеет узел. После обновления текущие значения будут переопределены.
...
### -EphemeralEndPort
Эфемерный конечный порт диапазона портов.
...
### -ForceReimage
Использование этого флага приведет к принудительному удалению, даже если фабрика служб не может отключить узлы.
Используйте с осторожностью, так как это может привести к потере данных, если на узле запущены рабочие нагрузки с отслеживанием состояния.
...
### -PlacementProperty
Теги размещения применяются к узлам в типе узла в виде пар "ключ-значение", которые можно использовать, чтобы указать, где должны выполняться определенные службы (рабочей нагрузки). После обновления текущие значения будут переопределены.
...
Укажите, как переобразировать узлы на типе узла.
...
### -ResourceId
Идентификатор ресурса типа Node
