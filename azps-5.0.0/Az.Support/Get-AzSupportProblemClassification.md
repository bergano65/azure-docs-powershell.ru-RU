...
## КРАТКИй обзор
Получение классификаций проблем для указанной службы.
...
### Пример 1: получение всех проблем classificaitons для службы с помощью параметра идентификатора службы.
```powershell
...
### Пример 2: получение всех проблем classificaitons для службы с помощью родительского объекта-службы
```powershell
...
### Пример 3: получение подробных сведений об одной проблеме, classificaiton по идентификатору объекта обслуживания трубопроводов
```powershell
...
### -ServiceId
Идентификатор службы, для которой извлекаются все классификации проблем.
...
### -ServiceObject
Объект обслуживания, для которого извлекаются классификации проблем.
