...
## КРАТКИй обзор
Импорт изображения из общедоступного раздела или реестра Azure в реестр контейнера Azure.
...
Импорт busybox в запись контроля доступа. Обратите внимание 
* "Library/" необходимо добавить перед исходным изображением. "busybox: latest" => "Library/busybox: latest"
* Учетные данные необходимы, если исходный реестр недоступен для общего доступа
...
Импортируйте BusyBox из исходной контроля доступа на целевую запись контроля доступа. Обратите внимание 
* Учетные данные необходимы, если пользователь администратора для исходного реестра отключен.
...
### Режим
В случае принудительного применения все существующие целевые теги будут переписаны.
Если значение "непринудительно", любые существующие целевые теги не смогут выполнить операцию перед началом копирования.
...
### -SourceImage
Имя репозитория исходного изображения.

Укажите изображение в репозитории ("Hello-World").
В этом случае будет использоваться тег "последние".
...
Идентификатор ресурса для реестра исходного контейнера Azure.
...
### -TargetTag
Список строк тега "репозиторий" формы \[ \] .
Если указан тег, будет использоваться источник (или "последние", если исходный тег также не указан).
...
Список строк с именами репозитория, для которых требуется скопировать только манифест.
Тег не будет создан.
