...
## NОПИСАНИЕ
Создание регистрации устройства в службе подготовки устройств центра для Azure IoT.
...
Создание регистрации с типом аттестации SymmetricKey
...
Создайте регистрацию с помощью аттестации доверенного платформенного модуля.
...
Создание регистрации с типом аттестации X509
...
Создайте регистрацию с типом подтверждения SymmetricKey и начальным состоянием двойной.
...
### -EndorsementKey
Ключ подтверждения доверенного платформенного модуля для устройства доверенного платформенного модуля.
...
### -PrimaryCAName
Имя сертификата основного корневого ЦС.
Если требуется подтверждение с сертификатом корневого ЦС, необходимо указать имя корневого ЦС.
...
### -PrimaryCertificate
Путь к файлу, содержащему основной сертификат.
...
### -PrimaryKey
Основной симметричный ключ доступа, хранящийся в формате Base64.
...
### -SecondaryCAName
Имя сертификата вторичного корневого ЦС.
Если требуется подтверждение с сертификатом корневого ЦС, необходимо указать имя корневого ЦС.
...
Путь к файлу, содержащему дополнительный сертификат.
Base-64. путь к файлу, который является файлом сертификата X509. cer или. PEM.
...
### -SecondaryKey
Вторичный симметричный ключ доступа, хранящийся в формате Base64.
...
### -StorageRootKey
Корневой ключ хранилища доверенного платформенного модуля для устройства доверенного платформенного модуля.
