...
## КРАТКИй обзор
Получает от клиента учетные записи доверенной политики, которые поставляются в службе аттестации Azure.
...
## NОПИСАНИЕ
Командлет Get-AzAttestationPolicySigners получает входные данные доверенной политики из клиента в аттестации Azure.
...
Получает сведения о доверенных политиках для поставщика аттестации *pshtest* в группе ресурсов *PSH-Test-RG*.  Обратите внимание, что для этого поставщика аттестации не предусмотрены доверенные лица.
...
Получает сведения о доверенных политиках для поставщика аттестации *pshtest2* в группе ресурсов *PSH-Test-RG*.  Обратите внимание, что для поставщика аттестации есть один доверенный подписывающий.
...
Возвращает доверенные политики для поставщика услуг по умолчанию аттестации в расположении *центрального США*.  Обратите внимание на то, что для поставщика по умолчанию для аттестации не предусмотрены доверенные лица.
...
### -Name (имя)
Указывает имя поставщика аттестации.
...
### -ResourceGroupName
Указывает имя группы ресурсов для поставщика аттестации.
...
### Microsoft. Azure. Commands. аттестация. Models. PSPolicySigners
