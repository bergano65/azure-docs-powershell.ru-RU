...
Эта команда обновляет период мягкого удаления и период оперативного кэша для базы данных Kusto "mykustodatabase" в кластере "testnewkustocluster", который находится в группе ресурсов "testrg".
...
### Пример 3: обновление существующей базы данных только для чтения по имени
```powershell
PS C:\> $2ds = New-TimeSpan -Days 2
...
Эта команда обновляет период оперативного кэша базы данных Kusto "mykustodatabase" в кластере "myfollowercluster" в группе ресурсов "testrg".

### Пример 4: обновление существующей базы данных с доступом только для чтения с помощью удостоверения
```powershell
