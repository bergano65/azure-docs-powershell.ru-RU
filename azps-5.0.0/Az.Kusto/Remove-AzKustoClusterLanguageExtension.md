...
### Пример 1: Удаление списка расширений языка из кластера
```powershell
PS C:\> Remove-AzKustoClusterLanguageExtension -ResourceGroupName testrg -ClusterName testnewkustocluster -Value (@{Name="R"})
```

Приведенная выше команда удаляет список расширений языка, которые могут выполняться в запросах KQL.
