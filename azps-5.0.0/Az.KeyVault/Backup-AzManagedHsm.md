...
## КРАТКИй обзор
Полностью архивировать управляемый HSM-объект.
...
## NОПИСАНИЕ
Полностью создайте резервную копию управляемого HSM-устройства для учетной записи хранения.
Используйте `Restore-AzManagedHsm` для восстановления резервной копии.
...
Командлет создаст в контейнере хранилища папку (обычно именуемую `mhsm-{name}-{timestamp}` ), сохраните ее в этой папке и выводите URI для папки.
