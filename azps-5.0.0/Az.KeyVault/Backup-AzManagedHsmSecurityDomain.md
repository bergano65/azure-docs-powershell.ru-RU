...
## КРАТКИй обзор
Резервное копирование данных домена безопасности управляемого HSM-файла для восстановления.
...
## NОПИСАНИЕ
Этот командлет производит резервное копирование данных домена безопасности управляемого HSM-файла для восстановления.
...
Эта команда извлекает управляемый HSM-файл с именем testmhsm и сохраняет резервную копию этого управляемого домена безопасности HSM в указанном выходном файле.
...
### -Force
Укажите, следует ли перезаписать существующий файл.
...
### -InputObject
Объект, представляющий управляемый HSM-аппарат.
...
### -OutputPath
Укажите путь к папке, в которую будут скачаны данные домена безопасности.
...
### -PassThru
Если задано значение, при успешном завершении командлета будет возвращаться логическая переменная.
...
### -Кворум
Минимальное количество общих ресурсов, необходимых для расшифровки домена безопасности для восстановления.
