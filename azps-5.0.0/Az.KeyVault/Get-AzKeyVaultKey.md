...
Эта команда получает ключ с именем ITPfx в хранилище ключей, именуемом contoso.
...
### Пример 8: Загрузка открытого ключа в виде PEM-файла
...
Открытый ключ ключа RSA можно загрузить, указав `-OutFile` параметр.
Это один из этапов импорта ключей, защищенных с защитой от HSM, в Azure Key Vault. Обнаружить https://docs.microsoft.com/en-us/azure/key-vault/keys/hsm-protected-keys
...
### -Файл
Указывает выходной файл, для которого этот командлет сохраняет ключ. Открытый ключ по умолчанию сохраняется в формате PEM.
