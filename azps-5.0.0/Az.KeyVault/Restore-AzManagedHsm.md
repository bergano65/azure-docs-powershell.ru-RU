...
## NОПИСАНИЕ
Полностью восстанавливает управляемый HSM из резервной копии, хранящейся в учетной записи хранения.
Используется `Backup-AzManagedHsm` для резервного копирования.
...
В примере восстанавливается резервная копия, хранящаяся в папке "mhsm-myHsm-2020101308504935" контейнера хранилища "https://{имя_учетной_записи}. BLOB. Core. Windows. NET/{containerName}".

## ПАРАМЕТРЫ

### -BackupFolder
Имя папки резервной копии, например "mhsm- *-2020101309020403". Его также можно вложить, например "резервные копии/mhsm-* -2020101309020403".
...
### -PassThru
Возвращает значение "истина" при восстановлении HSM.
...
### -SasToken
Маркер общего доступа к подписи (SAS) для проверки подлинности учетной записи хранения.
...
### -StorageAccountName
Имя учетной записи хранилища, в которой будет храниться резервная копия.
...
### -StorageContainerUri
Универсальный код ресурса (URI) контейнера хранилища, в котором будет храниться резервная копия.
