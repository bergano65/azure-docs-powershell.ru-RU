...
## NОПИСАНИЕ
Командлет **Set-AzureRmDataLakeStoreItemAcl** изменяет список управления доступом (ACL) для файла или папки в Data Lake Store.
...
Первая команда получает ACL для корневого каталога учетной записи ContosoADL и сохраняет ее в переменной $ACL.
Вторая команда задает для списка ACL для файла Test.txt значение, заданное в $ACL.

### Пример 2: рекурсивный выбор ACL для папки
```
...
Первая команда получает ACL для каталога Folder1 учетной записи ContosoADL и сохраняет его в переменной $ACL.
Вторая команда рекурсивно задает список ACL для Folder2 и его вложенных каталогов и файлов в $ACL.
...
### -ACL
Указывает список ACL для файла или папки.
...
### -Параллелизм
Количество параллельных файлов и каталогов. Необязательно: будет выбрано разумное значение по умолчанию.
...
### -Рекурсия
Указывает, что ACL нужно настроить рекурсивно для дочерних подкаталогов и файлов.
...
### -ShowProgress
После перезагрузки будет выдано состояние "ход выполнения". Применимо только при выполнении рекурсивного множества ACL.
