...
## NОПИСАНИЕ
Командлет **Get-AzureRmDataLakeAnalyticsCatalogItemAclEntry** возвращает список записей (ACE) в списке управления доступом (ACL) для каталога или элемента каталога в Data Lake Analytics.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение ACL для каталога
```powershell
...
Эта команда возвращает ACL для каталога указанной учетной записи Data Lake Analytics.
...
Эта команда возвращает ACL для базы данных указанной учетной записи Data Lake Analytics.
...
### -GroupOwner
Получение записи ACL для каталога для владельца группы
...
### -UserOwner
Получение записи ACL для каталога для владельца пользователя.
