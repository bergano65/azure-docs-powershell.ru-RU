...
### SetCatalogAclEntryForUser (по умолчанию)
```
...
## NОПИСАНИЕ
Командлет **Set-AzureRmDataLakeAnalyticsCatalogItemAclEntry** добавляет или изменяет запись (ACE) в списке управления доступом (ACL) для каталога или элемента каталога в Data Lake Analytics.

## ИЛЛЮСТРИРУЮТ

### Пример 1: изменение разрешений пользователей для каталога
```powershell
...
Эта команда изменяет элемент управления доступом к каталогу для Patti Белова, чтобы иметь разрешения на чтение.

### Пример 2: изменение разрешений пользователей для базы данных
```powershell
...
Эта команда изменяет элемент управления доступом к базе данных для Patti Белова, чтобы иметь разрешения на чтение.

### Пример 3: изменение других разрешений для каталога
```powershell
...
Эта команда изменяет ACE каталога для другого, чтобы иметь разрешения на чтение.

### Пример 4: изменение других разрешений для базы данных
```powershell
...
### Пример 5: изменение разрешений владельца для каталога
```powershell
...
Эта команда задает разрешение владельца учетной записи для чтения.

### Пример 6: изменение разрешений владельца пользователя для базы данных
```powershell
...
Эта команда задает разрешение владельца для чтения базы данных.
...
### -Группа
Настройка элемента ACL каталога для группы.
...
### -GroupOwner
Настройка элемента ACL каталога для владельца группы.
...
### -ObjectId
Удостоверение пользователя, которого нужно установить.
...
### – Другие
Настройка элемента ACL каталога для другого.
...
### -Пользователь
Настройка элемента ACL каталога для пользователя.
...
### -UserOwner
Настройка элемента ACL каталога для владельца пользователя.
