...
## NОПИСАНИЕ
Командлет **Get-AzureRmApiManagementApiRevision** получает подробные сведения обо всех РЕДАКЦИЯх API.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение всех редакций API для API
```powershell
...
Эта команда возвращает все редакции API указанного API для определенного контекста ApiManagement.

## ПАРАМЕТРЫ

### -ApiId
Идентификатор API, для которого нужно создать список исправлений.
