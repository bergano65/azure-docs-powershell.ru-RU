...
## NОПИСАНИЕ
Командлет **Import-AzureRmApiManagementHostnameCertificate** импортирует сертификат в формате PFX для службы управления API.
Этот сертификат используется для настраиваемых конфигураций hostname.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Импорт сертификата узла управления API
```powershell
PS C:\>Import-AzureRmApiManagementHostnameCertificate -Name "ContosoApi" -ResourceGroupName Contoso -HostnameType "Proxy" -PfxPath "C:\proxycert.pfx" -PfxPassword "CertSecret"
```

Эта команда импортирует сертификат для настраиваемого hostname прокси-сервера.
...
### -HostnameType
Указывает тип имени узла, для которого этот командлет загружает сертификат.
Допустимые значения: 
- Посредник
- Узла
...
### -Name (имя)
Указывает имя развертывания управления API, которое этот командлет импортирует.
...
### -PfxPassword
Указывает пароль для PFX-файла сертификата.
...
### -PfxPath
Указывает путь к PFX-файлу сертификата.
