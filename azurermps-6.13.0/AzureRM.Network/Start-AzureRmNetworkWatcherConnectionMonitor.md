...
## NОПИСАНИЕ
Командлет Start-AzureRmNetworkWatcherConnectionMonitor запускает указанный монитор подключения.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Запуск монитора подключений
```
...
В этом примере мы запускаем монитор подключений, заданный по имени и наблюдателю сети.
