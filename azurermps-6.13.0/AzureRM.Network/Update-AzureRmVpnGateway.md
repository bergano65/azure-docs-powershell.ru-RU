...
## КРАТКИй обзор
Update-AzureRmVpnGateway обновляет масштабируемый шлюз VPN до соответствующего состояния цели.
...
## NОПИСАНИЕ
Update-AzureRmVpnGateway обновляет масштабируемый шлюз VPN до соответствующего состояния цели. AzureRmVpnGateway — это программное обеспечение, которое позволяет подключаться к подключениям к сайту в VirtualHub. Этот шлюз изменяет размер и масштабируется в соответствии с единицей масштабирования, заданной пользователем. Подключение можно настроить из ветви или сайта, известного как VPNSite, на масштабируемый шлюз. Каждое соединение состоит из двух туннелей Active-Active
...
После создания шлюза он использует Set-AzureRmVpnGateway для обновления шлюза до 3 единиц масштаба.
