...
## NОПИСАНИЕ
Этот командлет используется в сценариях аварийного восстановления для удаления полученного номера управления обменом из учетной записи интеграции, чтобы соединитель B2B мог обработать сообщение повторно, если включено обнаружение дубликатов номеров.
В редких случаях полученный номер управления обменом может быть зарезервирован вскоре до аварии и до тех пор, пока соединитель B2B не отклоняет обмен как ошибочный.
В таких случаях может потребоваться, чтобы веб-сайт восстановления повторно мог обработать тот же обмен после его устранения.
...
Пытается получить полученный X12ный управляющий номер, который не является допустимым форматом содержимого.
Удаляет полученный X12ный номер управления обменом.
Подтверждение того, что полученный номер элемента управления X12 Interchange был удален, пытаясь получить его еще раз.
...
Пытается получить полученный Edifactный управляющий номер, который не является допустимым форматом содержимого.
Удаляет полученный Edifactный номер управления обменом.
Подтверждение того, что полученный номер элемента управления EDIFACT Interchange был удален, пытаясь получить его еще раз.
