...
## КРАТКИй обзор
Изменение конфигурации пакета учетной записи интеграции.
...
## NОПИСАНИЕ
Командлет **Set-AzIntegrationAccountBatchConfiguration** изменяет конфигурацию пакета учетной записи интеграции.

## ИЛЛЮСТРИРУЮТ

### Пример 1: изменение конфигурации пакета с помощью локального файла
```powershell
...
Изменение пакетной конфигурации с именем "sampleBatchConfig" с помощью локального файла, расположенного в пути к файлу, который хранится в разделе "$batchConfigurationFilePath".

### Пример 2: изменение конфигурации пакета с помощью строки JSON
```powershell
...
Изменение пакетной конфигурации с именем "sampleBatchConfig" с помощью строки JSON, содержащейся в "$batchConfigurationContent".

### Пример 3: изменение конфигурации пакета с помощью параметров
```powershell
...
Измените пакетную конфигурацию с именем "sampleBatchConfig", указав все необходимые параметры вручную.

## ПАРАМЕТРЫ

### -BatchConfigurationDefinition
Определение конфигурации пакета для учетной записи интеграции.
...
Путь к файлу конфигурации пакета учетной записи интеграции.
...
### -BatchGroupName
Имя группы пакетной конфигурации учетной записи для интеграции.
...
### -BatchSize
Размер пакета пакетной конфигурации учетной записи для интеграции.
...
### -InputObject
Настройка пакетной учетной записи для интеграции.
...
### -MessageCount
Число сообщений о конфигурации пакетов учетной записи для интеграции.
...
### -Metadata
Метаданные пакетной конфигурации учетной записи для интеграции.
...
### -Name (имя)
Название пакетной конфигурации учетной записи для интеграции.
...
### -ResourceId
Код ресурса конфигурации для пакетной службы учетной записи интеграции.
...
### -ScheduleFrequency
Периодичность расписания для пакетной настройки учетной записи интеграции.
...
### -ScheduleInterval
Интервал расписания для пакетной настройки учетной записи интеграции.
...
### -ScheduleStartTime
Время начала расписания пакетной конфигурации учетной записи интеграции.
