...
## NОПИСАНИЕ
Командлет **New-AzIntegrationAccountMap** создает карту учетной записи интеграции.
Этот командлет возвращает объект, представляющий карту учетной записи интеграции.
...
### Пример 1: создание карты учетной записи интеграции
```powershell
...
Команда задает определение карты, которое хранится в переменной $MapContent с помощью предыдущей команды.

### Пример 2

Создание карты учетной записи интеграции. (автоматически сформированные)
...
### -ContentType
Указывает тип контента для карты учетной записи интеграции.
...
### -MapDefinition
Указывает объект определения для карты учетной записи интеграции.
Укажите либо этот параметр, либо параметр *MapFilePath* .
...
### -MapFilePath
Задает путь к файлу определения для карты учетной записи интеграции. Укажите либо этот параметр, либо параметр *MapDefinition* .
...
### -MapType
Указывает тип карты учетной записи интеграции.
Этот командлет поддерживает преобразование XSLT в тип карты.
...
### -Metadata
Указывает объект метаданных для карты.
