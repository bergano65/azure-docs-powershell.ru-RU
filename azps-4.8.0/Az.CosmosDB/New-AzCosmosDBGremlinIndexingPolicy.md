...
## КРАТКИй обзор
Создает новый объект CosmosDB IndexingPolicy.
...
## NОПИСАНИЕ
Командлет **New-AzCosmosDBGremlinIndexingPolicy** создает новый объект типа PSIndexingPolicy.
...
### -Автоматический
Логическое значение, определяющее, является ли политика индексации автоматическим
...
### -CompositePath
Массив массив объектов типа Microsoft. Azure. Commands. CosmosDB. PSCompositePath
...
### -ExcludedPath
Массив строк, содержащий excludedPath (указывает путь в документе JSON, который нужно исключить в службе Jet Cosmos DB.)  element.
...
### -IncludedPath
Массив строк, содержащих includedPath (указывает путь в документе JSON для включения в элементы службы Azure Cosmos DB.).
...
Возможные значения: "неоднородный", "ленивый", "нет"
...
### -SpatialSpec
Массив объектов типа Microsoft. Azure. Commands. CosmosDB. PSSpatialSpec
