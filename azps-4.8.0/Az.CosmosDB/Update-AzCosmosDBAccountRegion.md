...
## NОПИСАНИЕ
Обновление регионов учетной записи CosmosDB. Расположение может быть предоставлено как объект типа PSLocation или строки названия расположения, упорядоченного по приоритету отработки отказа.
В параметре LocationObject ожидается список текущих местоположений (включена отработка отказа prioritiies), добавленных новым LocationObjects, соответствующим новым местам, которые нужно добавить.
Для параметра Location ожидается список текущих расположений (приоритет отработки отказа) и новое расположение. Обратите внимание, что поддерживается только добавление областей. Укажите либо расположение, либо LocationObject.
