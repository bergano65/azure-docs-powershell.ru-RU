...
Он должен выполняться в конечном объекте назначения копировать.
...
### Пример 4: Запуск копирования и конвейера для получения состояния копии
```
...
Первая команда запускает копирование BLOB-объектов "ContosoPlanning2015" в "ContosoPlanning2015_copy" и выводит объект destiantion BLOB. Второй командный конвейер для объекта destiantion BLOB для Get-AzStorageBlobCopyState, чтобы получить состояние копирования большого двоичного объекта. 
