...
### Пример 1: получение приложения функции по имени и его удаление.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Remove-AzFunctionApp -Force
```

Эта команда получает приложение функции по имени и удаляет ее.

### Пример 2: Удаление функции приложения по имени.
```powershell
...
### -Force
Принудительно удаляет приложение функции без запроса на подтверждение командлета.
