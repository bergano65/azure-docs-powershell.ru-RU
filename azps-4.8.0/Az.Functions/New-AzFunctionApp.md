...
### Пример 1: создание приложения функции PowerShell для потребления в Центральной США.
```powershell
...
Эта команда создает приложение функции PowerShell для потребления в Центральной США.

### Пример 2: создание приложения функции PowerShell, которое будет размещено в плане служб.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Эта команда создает приложение функции PowerShell, которое будет размещено в плане служб.

### Пример 3: создание приложения-функции с использованием закрытого изображения контроля доступа.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Эта команда создает приложение-функцию с использованием закрытого изображения контроля доступа.

## ПАРАМЕТРЫ

### -ApplicationInsightsKey
Клавиша инструментирования для добавляемого приложения Application Insights.
...
Имя существующего проекта Application Insights, который нужно добавить в приложение Function.
...
### -AppSetting
Функциональные параметры приложения.
...
### -AsJob
Запускает командлет в качестве фонового задания.
...
Отключение возможности создания ресурсов Application Insights во время создания приложения функции.
...
### -DockerImageName
Только для Linux.
Имя контейнера в реестре, например Publisher/Image-Name: Tag.
...
Имя пользователя и пароль в реестре контейнера.
Требуется для закрытых реестров.
...
### -IdentityID
Указывает список удостоверений пользователей, связанных с приложением "функция".
...
### -IdentityType
Указывает тип удостоверения, используемого для приложения функции.
Допустимые значения этого параметра:-SystemAssigned-UserAssigned
...
### -Wait
Запускает операцию и возвращает значение немедленно, прежде чем операция завершится.
Для определения того, успешно ли завершилась операция, используйте какой – то другой механизм.
...
### -OSType
Операционная система, в которой будет размещено приложение "функция".
