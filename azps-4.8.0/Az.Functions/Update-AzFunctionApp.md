...
### Пример 1: Обновление плана размещения приложений функций.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -PlanName NewPlanName 
```

Эта команда обновляет план размещения приложений функций.

### Пример 2: Установка управляемого удостоверения SystemAssigned для приложения функции.
```powershell
...
### Пример 3: обновление функций Application Insights в приложении.
```powershell
...
Эта команда обновляет функции Application Insights приложения.

### Пример 3: Удаление управляемого удостоверения из приложения-функции.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType None 
```

Эта команда удаляет управляемое удостоверение из приложения функции.
...
Тип "нет" приведет к удалению всех удостоверений из приложения "функция".
Допустимые значения этого параметра:-SystemAssigned-UserAssigned-None
