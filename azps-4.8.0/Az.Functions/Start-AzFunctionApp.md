...
### Пример 1: получение приложения-функции по имени и запуску.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Start-AzFunctionApp
```

Эта команда получает приложение с именем и запускает ее.

### Пример 2: запуск функционального приложения по имени.
```powershell
