...
## NОПИСАНИЕ
Командлет **Remove-AzApiManagementSchema** из API.
...
Сценарий удаляет схему `2` из API, `echo-api` если она не указана.

### Пример 2

Сценарий удаляет схему 2 из Echo API API, если на него нет ссылки. (автоматически сформированные)
...
### -ResourceId
ИД ResourceId для ApiSchema. Этот параметр является обязательным.
