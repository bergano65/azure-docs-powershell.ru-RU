...
Командлет **New-AzApiManagementApiRevision** создает редакцию API для существующего API в контексте управления API.
...
Эта команда создает версию API `2` `echo-api` API.
...
Эта команда создает версию API `5` `echo-api` API.

## ПАРАМЕТРЫ

### -ApiId
Идентификатор API, для которого необходимо создать редакцию.
...
### -ServiceUrl
URL веб-службы, предоставляющей API в серверной службе. Этот URL-адрес будет использоваться только для управления API Azure и не будет открыт. Должно содержать от 1 до 2000 символов. Этот параметр является обязательным.
...
### -SourceApiRevision
Идентификатор редакции API исходного API. Этот параметр является необязательным.
