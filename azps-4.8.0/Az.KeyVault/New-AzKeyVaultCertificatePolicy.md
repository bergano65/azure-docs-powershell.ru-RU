...
## NОПИСАНИЕ
Командлет **New-AzKeyVaultCertificatePolicy** создает объект политики сертификата в памяти для хранилища ключей Azure.
...
Создает объект политики сертификата в памяти. (автоматически сформированные)
...
### Пример 3: создание сертификата для альтернативного имени субъекта (или SAN)
...
В этом примере создается сертификат SAN с тремя DNS-именами.

## ПАРАМЕТРЫ

### -CertificateTransparency
Указывает, включена ли прозрачность сертификата для этого сертификата или поставщика; Если не указано, используется значение по умолчанию "true"
...
### -Кривая
Указывает имя эллиптической кривой для ключа сертификата.
...
### -DnsName
Указывает DNS-имена в сертификате. Дополнительные имена субъектов (SAN) можно указывать в DNS-именах.
...
### -KeySize
Задает размер ключа сертификата.
Для этого параметра допустимы следующие значения:
- 2048
- 3072
- 4096
...
> [!NOTE]
> Если необходимо использовать запятую (,) или точку (.) в свойстве в `SubjectName` параметре, необходимо заключить поле свойства в кавычки. Например, вы можете использовать O = "Contoso, Ltd.". в поле Название организации.
...
### System. Collections. Generic. List "1 [[System. Security. Cryptography. X509Certificates. X509KeyUsageFlags, System. Security. Cryptography. X509Certificates, Version = 4.2.1.0, культура = нейтральная, PublicKeyToken = b03f5f7f11d50a3a]]
