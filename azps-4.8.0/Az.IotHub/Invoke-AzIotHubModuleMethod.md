...
## NОПИСАНИЕ
Вызовите метод Edge Module. https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-direct-methodsДля получения дополнительных сведений ознакомьтесь с дополнительными сведениями.
...
### -ConnectionTimeOut
Время (в секундах), по истечении которого соединение будет установлено.
...
### -ModuleId
Идентификатор модуля целевого устройства.
...
### -Name (имя)
Имя метода, который нужно вызвать для этого модуля устройства.
...
### -Полезные данные
Полезные данные для метода, который нужно вызвать для этого модуля устройства.
...
### -ResponseTimeOut
Количество секунд, в течение которого ожидается получение результата из прямого метода.
Значение по умолчанию — 10.
