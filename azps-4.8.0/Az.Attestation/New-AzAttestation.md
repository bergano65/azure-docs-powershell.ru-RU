...
## NОПИСАНИЕ
Командлет New-AzAttestation создает аттестацию в указанной группе ресурсов.
...
Создайте новый экземпляр поставщика аттестации с именем *pshtest4* , используя пару тегов и указав для политики надежной среды выполнения Trust.
...
Создайте новый экземпляр поставщика аттестации с именем *pshtest3* , который использует Isoladed Trust для политики главной настройки надежной среды выполнения, указав набор доверенных ключей подписи с помощью PEM-файла.
...
### -Location
Указывает область Azure, в которой нужно создать поставщика аттестации. Используйте командную Get-AzResourceProvider с параметром ProviderNamespace, чтобы просмотреть варианты выбора.
...
### -Name (имя)
Указывает имя создаваемого экземпляра.
...
Задает набор доверенных ключей подписывания для политики выдачи в одном файле сертификата.
...
### -ResourceGroupName
Указывает имя существующей группы ресурсов, в которой будет создана аттестация.
