...
## КРАТКИй обзор
Задает политику клиента в Azure Attestationn.
...
## NОПИСАНИЕ
Командлет Set-AzAttestationPolicy задает политику из клиента в аттестации Azure.
...
Задает политику, определенную пользователем, для надежной среды выполнения типа *SgxEnclave* для поставщика аттестации *pshtest* с помощью формата текстовой политики (по умолчанию).
...
Задает политику, определенную пользователем, для надежной среды выполнения типа *SgxEnclave* для поставщика аттестации *pshtest* с помощью формата политики JWT.
...
Этот командлет задает политику аттестации для клиента, который указывает этот параметр.
...
### -Policy (политика)
Указывает документ политики, который нужно задать.  Формат политики может быть как текстом, так и веб-токеном JSON (JWT).
...
### -PolicyFormat
Задает формат для политики: Text или JWT (веб-маркер JSON).  Формат политики по умолчанию — текст.
...
### -Надежной среды выполнения
Указывает тип надежной среды выполнения.
Поддерживаются четыре типа среды: SgxEnclave, OpenEnclave, CyResComponent и VBSEnclave.
