...
### Пример 1: создание хранилища конфигурации приложения
```powershell
...
Эта команда создает хранилище конфигурации приложения.

### Пример 2: создание конфигурации приложения с IdentityType, установленным на "UserAssigned"
```powershell
...
Эта команда создает конфигурацию приложения и присваивает ей назначенный пользователю управляемый идентификатор.
`Update-AzAppConfigurationStore`Ниже описаны действия, которые необходимо выполнить, чтобы включить CMK (управляемый ключ cusomer).

### Пример 3: создание конфигурации приложения с IdentityType, установленным на "SystemAssigned" 
```powershell
...
Эта команда создает конфигурацию приложения и включает управляемое системой удостоверение, связанное с ресурсом.
`Update-AzAppConfigurationStore`Ниже описаны действия, которые необходимо выполнить, чтобы включить CMK (управляемый ключ cusomer).

### Пример 4: создание конфигурации приложения с IdentityType, установленным на "SystemAssigned, UserAssigned"
```powershell
PS C:\> $assignedIdentity = New-AzUserAssignedIdentity -ResourceGroupName azpwsh-manual-test -Name assignedIdentity
...
Вы можете включить управляемую системой идентификационную идентификацию и одновременно предоставить пользователям Идентификационные удостоверения.
`Update-AzAppConfigurationStore`Ниже описаны действия, которые необходимо выполнить, чтобы включить CMK (управляемый ключ cusomer).
...
После создания ресурса изменить его нельзя.
