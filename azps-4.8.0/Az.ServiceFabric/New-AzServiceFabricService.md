...
## NОПИСАНИЕ
Этот командлет позволяет создавать службы без сохранения состояния или состояния в указанном приложении. Служба должна выйти из манифеста приложения, а тип должен совпадать с типом в манифесте. Имя приложения должно быть префиксом имени службы.
...
В этом примере создается новая служба без сохранения состояния "testApp ~ testService1" с количеством экземпляров 1 (на всех узлах).
...
В этом примере будет создана новая служба отслеживания состояния "testApp ~ testService2" с целью 5 узлов.
...
### -DefaultMoveCost
Укажите стоимость по умолчанию для перемещения.
Чем выше стоимость, тем меньше вероятность того, что диспетчер ресурсов кластера переместит реплику при попытке сбалансировать кластер.
...
### -InstanceCount
Указание количества экземпляров службы
...
### -MinReplicaSetSize
Указание минимального размера наборов реплик для службы
...
У служб, использующих эту модель, обычно есть данные, которые могут быть сегментированы в рамках ограниченного множества.
Наиболее распространенными примерами полей данных, используемых в качестве именованных ключей секционирования, являются регионы, почтовые индексы, группы клиентов и другие границы бизнеса.
...
Указывает, что служба использует одноэлементную схему секционирования.
Одноэлементные разделы обычно используются, если службе не требуется дополнительная маршрутизация.
...
Указывает, что служба использует схему секционирования UniformInt64.
Это означает, что каждый раздел владеет диапазоном из Int64.
...
Указание длительности ожидания при потере кворума для службы
...
Укажите период ожидания повторного запуска реплики для службы
...
Указание времени ожидания реплики для службы
...
### -Состояние
Использование службы с ведением базы данных
...
### -Без состояния
Использование службы без сохранения состояния
...
Указание размера целевого задания реплики для службы
...
### -Type (тип)
Укажите имя типа службы для приложения, которое должно существовать в манифесте приложения.
