...
## КРАТКИй обзор
Создайте новый управляемый кластер.

## Максимальное

### ClientCertByTp (по умолчанию)
```
...
## NОПИСАНИЕ
Этот командлет создаст ресурс управляемого кластера без типов узлов. Для загрузки кластера необходимо добавить основной тип узла с помощью [New-AzServiceFabricManagedNodeType](./New-AzServiceFabricManagedNodeType.md).
...
Эта команда создает ресурс кластера с базовой конфигурацией по умолчанию.
...
Эта команда создает ресурс кластера в centraluseuap с первоначальным клиентским сертификатом администратора и стандартным SKU.
...
### -ClientCertIsAdmin
Используется, чтобы указать, имеет ли клиентский сертификат уровень администратора.
...
Список отпечаток поставщика для клиентского сертификата.
Используется только в сочетании с ClientCertCommonName.
...
### -CodeVersion
Версия кода фабрики службы кластеров.
Используйте только в том случае, если вы используете режим обновления вручную.
...
Порт, используемый для подключений HTTP к кластеру.
По умолчанию: 19080.
...
### -SKU
SKU кластера, доступны следующие варианты: у него есть не менее 3 начальных узла и разрешено только 1 тип узла и стандарт: у него будет не менее 5 начальных узлов и разрешено использовать несколько типов узлов.
...
### -UpgradeMode
Режим обновления версии кода фабрики службы кластеров.
...
### -UseTestExtension
Если указать кластер, будет использоваться расширение Service Test vmss.
