...
## NОПИСАНИЕ
Команда **Invoke-AzSynapsePipeline** запускает выполнение в указанном конвейере и возвращает идентификатор для этого вызова. Этот GUID можно передать в **Get-AzSynapsePipelineRun** или **Get-AzSynapseActivityRun** , чтобы получить дополнительные сведения об этом запуске.
...
Эта команда запускает выполнение для конвейера с именем ContosoPipeline в рабочей области ContosoWorkspace.
...
Эта команда запускает выполнение для конвейера с именем ContosoPipeline в рабочей области ContosoWorkspace с помощью конвейера.
...
Если для режима восстановления установлено значение true, выполнение указанного конвейера, на который указывает ссылка, и новый прогон будут сгруппированы в том же элементе groupId.
...
Идентификатор запускаемого конвейера для повторного выполнения.
Если указан идентификатор запуска, для создания нового запуска будут использоваться параметры указанного запуска.
...
### -StartActivityName
В режиме восстановления повторный запуск начнется с этого действия.
Если не указано, будут запущены все действия.
