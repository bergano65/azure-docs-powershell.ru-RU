...
### Пример 1: перезапуск сервера MySql с помощью группы ресурсов и имени сервера
```powershell
PS C:\> Restart-AzMySqlServer -ResourceGroupName PowershellMySqlTest -Name mysql-test

```

Этот командлет перезапускает сервер MySql по группе ресурсов и имени сервера.

### Пример 2: перезапуск сервера MySql по удостоверению
```powershell
...
Эти командлеты перезапускают сервер MySql по идентификаторам.
