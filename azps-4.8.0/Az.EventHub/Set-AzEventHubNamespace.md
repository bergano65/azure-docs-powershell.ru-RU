...
## NОПИСАНИЕ
Командлет Set-AzEventHubNamespace обновляет свойства указанного пространства имен концентраторов событий.
...
Обновляет Теги MyNamespaceName пространства имен \` \` для создания.
...
Обновляет состояние пространства имен \` MyNamespaceName \` с AutoInflate = Enabled и MaximumThroughputUnits = 10

### Пример 3

Обновляет указанное пространство имен концентраторов событий. (автоматически сформированные)

```powershell <!-- Aladdin Generated Example --> 
Set-AzEventHubNamespace -Location 'WestUS' -Name MyNamespaceName -ResourceGroupName MyResourceGroupName -SkuName Basic
```

### Пример 5: создание и обновление пространства имен с помощью оснастки «Управление удостоверениями» в кластере 
```powershell
...
### -IdentityUserDefined
Пользовательский идентификатор или None (нет)
...
### -KeyProperty
Список ключевых свойств, @ (@ (KeyName, KeyVaultUri, Keyversion), @ (KeyName, KeyVaultUri, Keyversion))
...
### -State
Отключение и включение пространства имен.
...
### System. Nullable "1 [[Microsoft. Azure. Commands. EventHub. NamespaceState., версия = 1.4.3.0, культура = нейтрально, PublicKeyToken = NULL]]
