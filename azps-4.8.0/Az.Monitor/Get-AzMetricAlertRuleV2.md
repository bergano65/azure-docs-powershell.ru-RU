...
## КРАТКИй обзор
Возвращает правила оповещения о метрике v2 (не классический)
...
## NОПИСАНИЕ
Командлет **Get-AzMetricAlertRuleV2** получает правило оповещения метрик по имени или URI-адресу или все правила оповещения метрик из указанной группы ресурсов или подписки.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение всех правил оповещения о метрике в текущей подписке
```powershell
...
Эта команда получает все правила оповещения о метрике в текущей подписке.

### Пример 2: получение всех правил оповещения метрик в группе ресурсов
...
Эта команда получает все правила оповещения метрик в группе ресурсов с именем metricAlertsRG.

### Пример 3: получение правила оповещения метрики по имени
...
Эта команда получает правило оповещения метрики с именем PS3182019 в группе ресурсов с именем metricAlertsRG.

### Пример 4: получение правила оповещения метрики по ruleID
...
Эта команда возвращает правило оповещения метрики с заданным ИДЕНТИФИКАТОРом ресурса.
...
### -ResourceId
Идентификатор правила для правила оповещения метрики
