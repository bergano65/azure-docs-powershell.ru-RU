...
### -Operator
Оператор, который будет использоваться при сопоставлении Selector, EqualsAny означает, что селектор (все переменные соответствия указанному типу) не выбраны.
...
### -Selector
Шаблон Selector для поиска соответствия с помощью оператора (если оператор не является EqualsAny)
...
### -Variable
Переменная Match. Возможные значения: RequestHeaderNames, RequestCookieNames, QueryStringArgNames, RequestBodyPostArgNames.
Например, QueryStringArgNames является исключением параметров GET, отвечающих селектору с помощью заданного оператора.
