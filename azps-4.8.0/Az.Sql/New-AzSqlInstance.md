...
## NОПИСАНИЕ
Командлет **New-AzSqlInstance** создает экземпляр управляемой базы данных SQL Azure.
...
Эта команда создает новый экземпляр с помощью параметра SkuName.
...
Эта команда создает новый экземпляр с помощью параметров выпусков и ComputeGeneration.

### Пример 3: создание нового экземпляра в пуле экземпляров с помощью объекта пула экземпляров
```powershell
...
Эта команда создает новый экземпляр в пуле экземпляров с помощью объекта пула экземпляров.

### Пример 4: создание нового экземпляра в пуле экземпляров с помощью идентификатора ресурса пула экземпляров
```powershell
...
Эта команда создает новый экземпляр в пуле экземпляров с помощью идентификатора ресурса пула экземпляров.

### Пример 5: создание нового экземпляра в пуле экземпляров
```powershell
...
Эта команда создает новый экземпляр в пуле экземпляров с именем instancePool0

## ПАРАМЕТРЫ

### -AdministratorCredential
Учетные данные проверки подлинности SQL для экземпляра.
...
### -AssignIdentity
Создайте и назначьте удостоверение Azure Active Directory для этого управляемого экземпляра для использования в службах управления ключами, таких как Azure KeyVault.
...
Избыточное резервное хранилище, используемое для хранения резервных копий управляемого экземпляра SQL Azure. Параметры: локальные, Zone и Geo
...
Параметры сортировки управляемого экземпляра Azure SQL, который нужно использовать.
...
### -DnsZonePartner
Идентификатор ресурса сервера, управляемого партнером, для наследования свойства DnsZone из для создания управляемого экземпляра.
...
### -InstancePoolName
Пул экземпляров, в котором нужно разместить этот экземпляр.
...
### -Location
Расположение, в котором нужно создать экземпляр.
...
### -SkuName
Имя SKU для экземпляра, например "GP_Gen4", "BC_Gen4".
...
### -SubnetId
Идентификатор подсети, который будет использоваться для создания экземпляра
...
### -TimezoneId
Идентификатор часового пояса для устанавливаемого экземпляра. Список идентификаторов часовых поясов предоставляется с помощью представления sys.time_zone_info (Transact-SQL).
