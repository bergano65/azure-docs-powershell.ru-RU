...
## NОПИСАНИЕ
Командлет **Get-AzSqlInstanceActiveDirectoryOnlyAuthentication** получает требование проверки подлинности для экземпляра Azure Active Directory (Azure AD) только для управляемых экземпляров AzureSQL в текущей подписке.
...
Эта команда возвращает Azure Active Directory (Azure AD) только требование проверки подлинности для управляемого экземпляра AzureSQL с именем ManagedInstance01, связанного с группой ресурсов с именем ResourceGroup01.
