...
## NОПИСАНИЕ
Командлет **Start-AzSqlInstanceDatabaseLogReplay** запускает службу воспроизведения журнала.
...
Эта команда создаст новую управляемую базу данных и начнет восстанавливать резервные копии из заданного контейнера, пока не будет восстановлен last_backup. bak.
...
Эта команда создаст новую управляемую базу данных и начнет восстанавливать резервные копии из заданного контейнера до тех пор, пока Complete-AzSqlInstanceDatabaseLogReplay не будет вызван с последней резервной копией.

## ПАРАМЕТРЫ

### -AutoCompleteRestore
Индикатор необходимости автоматического завершения восстановления после завершения.
...
Параметры сортировки для базы данных экземпляра, которую нужно использовать.
...
### -LastBackupName
Имя последнего файла резервной копии для восстановления.
