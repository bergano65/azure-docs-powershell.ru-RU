...
Идентификатор приложения Azure Active Directory (AAD) или URI для получения маркера доступа, который будет включен в качестве маркера носителя в запросах на доставку. Применимо только для веб-перехватчика в качестве назначения.
...
Идентификатор клиента Azure Active Directory (AAD) для получения маркера доступа, который будет включен в качестве маркера носителя в запросах на доставку. Применимо только для веб-перехватчика в качестве назначения.
...
### -DeliverySchema
Схема, используемая при доставке событий в назначение. Возможные значения: eventgridschema, CustomInputSchema или cloudeventv01schema. Значение по умолчанию — CustomInputSchema.
...
### -MaxEventsPerBatch
Максимальное количество событий в пакете. Это значение должно находиться в диапазоне от 1 до 5000. Этот параметр является допустимым только в том случае, если тип Endpint — только веб-перехватчик.
...
Предпочтительный размер пакета в килобайтах. Это значение должно находиться в диапазоне от 1 до 1024. Этот параметр является допустимым только в том случае, если тип Endpint — только веб-перехватчик.
