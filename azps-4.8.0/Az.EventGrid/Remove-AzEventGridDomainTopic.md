...
Удаляет раздел элемент1 сетки событий в \` разделе \` Domain1 домена \` \` в группе ресурсов \` MyResourceGroupName \` .
...
### -ResourceId
Идентификатор ресурса, представляющий доменную тему для таблицы событий.
