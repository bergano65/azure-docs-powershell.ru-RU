...
## КРАТКИй обзор
Создайте новую конфигурацию системы управления версиями Kubernetes.
...
### Пример 1: создание configuation для кластера kubernetes
```powershell
...
Эта команда создает configuation для кластера kubernetes.
...
### -ClusterScoped
При переданном параметре задает область конфигурации для кластера (значение по умолчанию — nameSpace).
...
### -EnableHelmOperator
Параметр для включения Helm оператора для этой конфигурации Git.
...
Переопределение значений в диаграмме Helm оператора.
...
Версия Helm оператора на диаграмме.
...
Имя экземпляра оператора, определяющего определенную конфигурацию.
...
### -OperatorNamespace
Пространство имен, в которое установлен этот оператор.
Не более 253 строчных букв, дефисов и периодических знаков.
...
### -OperatorParameters
Любые параметры для экземпляра оператора в строковом формате.
