...
### SubscriptionPropertiesSet (по умолчанию)
```
...
## NОПИСАНИЕ
Командлет **Remove-AzServiceBusSubscription** удаляет подписку на раздел из указанного пространства имен служебной шины.
...
Удаляет подписку `SB-TopicSubscription-Example1` на раздел `SB-Topic_exampl1` в указанном пространстве имен служебной шины `SB-Example1` .
...
### Пример 3: InputObject — использование трубопроводов:
```powershell
...
### Пример 5: ResourceId — использование строкового значения:
```powershell
...
Удаляет подписку, указанную с помощью идентификатора ARM, в $resourceid/String для параметра-ResourceId 
...
### -InputObject
Объект подписки служебной шины
...
### -ResourceId
Идентификатор ресурса подписки на обслуживание служебной шины
