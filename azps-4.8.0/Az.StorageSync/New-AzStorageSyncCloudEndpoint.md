...
## NОПИСАНИЕ
Эта команда создает облачную конечную точку синхронизации файлов Azure. Конечная точка облака — это ссылка на существующую общую файловую службу Azure. Она представляет общую файловую точку и определяет участие в синхронизации всех файлов в группе синхронизации, в которой была создана облачная конечная точка.
...
Облачная конечная точка является неотъемлемой частью группы синхронизации, это пример создания одной из существующих групп синхронизации с именем "mySyncGroupName".
...
### -AzureFileShareName
Имя общего доступа к учетной записи хранения (имя общего файлового файла Azure)
...
### -Name (имя)
Имя конечной точки облака. При создании на портале Azure имя задается именем файла Azure, на который ссылается служба общего доступа.
...
Идентификатор клиента учетной записи хранилища (код каталога организации)
