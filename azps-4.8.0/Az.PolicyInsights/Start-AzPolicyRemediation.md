...
## NОПИСАНИЕ
Командлет **Start-AzPolicyRemediation** создает исправление политики для определенного назначения политики. Все несоответствующие ресурсы на уровне или ниже области исправлений будут исправлены. Исправление поддерживается только для политик с эффектом "deployIfNotExists".
...
### Пример 5: начните исправление, которое будет распознавать ресурсы, не соответствующие требованиям, перед исправление
```
PS C:\> $policyAssignmentId = "/subscriptions/f0710c27-9663-4c05-19f8-1b4be01e86a5/providers/Microsoft.Authorization/policyAssignments/2deae24764b447c29af7c309"
PS C:\> Select-AzSubscription -Subscription "My Subscription"
...
Эта команда создает новое исправление политики в подписке "Моя подписка" для данного назначения политики. Состояние соответствия ресурсов в подписке будет перечислено с учетом назначения политики и несоответствующих ресурсов будут исправлены.
...
В этой статье описано, как задача «исправление» обнаружит ресурсы, которые необходимо исправить.
ReEvaluateCompliance не поддерживается в областях групп управления исправление.
