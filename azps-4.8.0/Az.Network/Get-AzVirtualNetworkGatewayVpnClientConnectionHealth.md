...
## NОПИСАНИЕ
Список всех подключенных подключений VPN-клиентов. Сюда относятся: vpnConnectionId vpnConnectionDuration vpnConnectionTime publicIpAddress privateIpAddress vpnUserName maxBandwidth egressPacketsTransferred egressBytesTransferred ingressPacketsTransferred ingressBytesTransferred maxPacketsPerSecond
...
Для шлюза виртуальной сети Azure с именем gatewayname в группе ресурсов "resourceGroup" выберите подключение VPN-клиента, подключенное в настоящее время с помощью OpenVpn. 

### Пример 2

Получение списка работоспособности подключения VPN-клиента к шлюзу виртуальной сети Azure для подключения VPN-клиента. (автоматически сформированные)
