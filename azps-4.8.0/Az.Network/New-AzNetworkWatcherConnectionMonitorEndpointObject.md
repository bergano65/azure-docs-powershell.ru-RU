...
## NОПИСАНИЕ
New-AzNetworkWatcherConnectionMonitorEndpointObject командлет создает конечную точку монитора подключений.
...
Name (имя): workspaceEndpoint ResourceId:/subscriptions/00000000-0000-0000-0000-000000000000/resourcegroups/myresourceGroup/providers/Microsoft.OperationalInsights/workspaces/myworkspace адрес: Filter: {"Type": "include", "Items": [{"тип": "AgentAddress"; "адрес": "WIN-P0HGNDO2S1B"}]}

## ПАРАМЕТРЫ

### -Address (адрес)
Адрес конечной точки монитора соединений (IP или доменное имя).
...
### -FilterType
Поведение фильтра конечной точки. В настоящее время поддерживается только функция include.
...
### -Name (имя)
Имя конечной точки монитора соединений.
...
### -ResourceId
Идентификатор ресурса конечной точки монитора соединений.
