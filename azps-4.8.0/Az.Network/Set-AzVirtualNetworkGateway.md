...
### Пример 5: Добавление и обновление IpConfigurationBgpPeeringAddresses для существующего шлюза виртуальной сети
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Первая команда получает шлюз виртуальной сети с именем Gateway01, который принадлежит группе ресурсов ResourceGroup001, и сохраняет его в переменной с именем $Gateway Вторая команда присваивает значение ИД IP-адресу шлюза виртуальных сетей Gateway01 в переменную ipconfigurationId1.
...
Пятая команда задали для этого нового созданного PSIpConfigurationBgpPeeringAddress значение IpConfigurationBgpPeeringAddresses и обновить шлюз.
...
### -ASN
ASN шлюза виртуальной сети, используемый для настройки сеансов BGP в туннелях IPsec.
...
Пометка отключения активного активного компонента на шлюзе виртуальной сети
...
Если указан сайт по умолчанию, весь Интернет-трафик из виртуальной сети шлюза пересылается на этот сайт.
...
### -RadiusServerList
P2S несколько внешних RADIUS-серверов.
...
Объект шлюза виртуальной сети, для которого изменяются основные изменения.
...
### -VpnClientAddressPool
Адресное пространство, из которого нужно выделять IP-адреса VPN-клиентов.
...
### -VpnClientRevokedCertificates
Список отозванных сертификатов VPN-клиентов.
VPN-клиент, выступающий сертификат, совпадающий с одним из них, будет указан для выхода.
...
### -VpnClientRootCertificates
Список корневых сертификатов VPN-клиентов, используемых для проверки подлинности клиента VPN.
