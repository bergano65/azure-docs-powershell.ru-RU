...
## NОПИСАНИЕ
Командлет **Update-AzCustomIpPrefix** позволяет пользователю выступающие или списать CustomIpPrefix, а также изменять теги ресурса.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Комиссия CustomIpPrefix
```powershell
PS C:\> Update-AzCustomIpPrefix -Name $prefixName -ResourceGroupName $rgName -Commission
```

Приведенная выше команда запускает процесс комиссий для CustomIpPrefix.

### Пример 2: списание CustomIpPrefix
```powershell
PS C:\> Update-AzCustomIpPrefix -Name $prefixName -ResourceGroupName $rgName -Decommission
```

В приведенной выше команде начинается процесс отмены Комиссии для CustomIpPrefix.

### Пример 3: обновление тегов для CustomIpPrefix
```powershell
PS C:\> Update-AzCustomIpPrefix -Name $prefixName -ResourceGroupName $rgName -Tag $tags
```

В приведенной выше команде будут обновлены теги для CustomIpPrefix.
