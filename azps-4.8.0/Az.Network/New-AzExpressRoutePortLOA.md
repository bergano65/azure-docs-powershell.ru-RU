...
## КРАТКИй обзор
Загрузка документа авторизации письма для порта Express Route.
...
## NОПИСАНИЕ
New-AzExpressRoutePortLOA командлет загружает документ для проверки подлинности в формате PDF для порта Express Route.
...
Скачайте документ для проверки подлинности для порта Express Route "myPort" и сохраните его в файле "loa.pdf".
...
### -Поля
Имя пользователя, которому назначен этот порт Express Route.
...
### -Destination
Выходной путь для хранения письма авторизации.
...
### -ResourceGroupName
Имя группы ресурсов для порта Express Route.
