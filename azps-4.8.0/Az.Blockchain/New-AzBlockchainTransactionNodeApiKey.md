...
### Пример 1: повторное создание ключей API для узла транзакции с помощью имени.
```powershell
...
Эта команда создает ключи API для узла транзакции, используя имя, группу ресурсов.

### Пример 2: повторное создание ключей API для узла транзакции
```powershell
PS C:\> $keyPair = Get-AzBlockchainTransactionNodeApiKey -BlockchainMemberName dolauli001 -ResourceGroupName testgroup -TransactionNodeName tranctionnode001 
...
Эта команда создает ключи API для узла транзакции с помощью idenity.
