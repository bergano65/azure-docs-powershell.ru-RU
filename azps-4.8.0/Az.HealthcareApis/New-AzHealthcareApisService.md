...
## КРАТКИй обзор
Создает метаданные экземпляра службы.
...
### Пример 1: создание новой службы Azure healthcareapis fhir Service с именем MyService в группе ресурсов MyResourceGroup в местоположении westus2 с пропускной способностью = 400
```powershell
...
### -Аудитория
Аудитория службы HealthcareApis Fhir.
...
### -Authority
Центр обслуживания HealthcareApis Fhir.
...
### -CorsMethod
Список служб HealthcareApis Fhir (метод CORS).
...
### -CorsOrigin
HealthcareApis Fhir список служб в источнике CORS. Укажите URL-адреса исходных сайтов, которые могут получать доступ к этому API или используйте * для разрешения доступа с любого сайта.
...
### -Location
Расположение службы HealthcareApis.
...
### -ManagedIdentity
Использовать управляемое удостоверение?
