...
## NОПИСАНИЕ
Команда **Invoke-AzDataFactoryV2Pipeline** запускает выполнение в указанном конвейере и возвращает идентификатор для этого вызова. Этот GUID можно передать в **Get-AzDataFactoryV2PipelineRun** или **Get-AzDataFactoryV2ActivityRun** , чтобы получить дополнительные сведения об этом запуске.
...
Вызывает конвейер, чтобы запустить его для запуска. (автоматически сформированные)
...
### -NORECOVERY
Флаг режима восстановления. Если для режима восстановления установлено значение true, выполнение указанного конвейера, на который указывает ссылка, и новый прогон будут сгруппированы в том же элементе groupId.
...
Идентификатор запускаемого конвейера для повторного выполнения. Если указан идентификатор запуска, для создания нового запуска будут использоваться параметры указанного запуска.
...
### -StartActivityName
В режиме восстановления повторный запуск начнется с этого действия. Если не указано, будут запущены все действия.
...
### -StartFromFailure
Запустите повторный запуск из флага неудачных действий. В режиме восстановления, если оно задано, повторное выполнение начинается с невыполненных действий.
