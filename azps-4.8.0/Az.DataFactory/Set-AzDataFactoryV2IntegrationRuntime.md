...
## NОПИСАНИЕ
Командлет Set-AzDataFactoryV2IntegrationRuntime обновляет среду выполнения интеграции с определенными параметрами.
...
### Пример 2: общий доступ к среде выполнения интеграции с самостоятельным размещением.
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Командлет добавляет файл ADF для использования общей среды выполнения интеграции. При использовании `-SharedIntegrationRuntimeResourceId` параметра `-Type` также необходимо включить. Обратите внимание, что необходимо предоставить фабрике данных разрешение на использование среды выполнения интеграции перед выполнением командлета.

### Пример 3: Настройка Self-Hosted инфракрасной связи в качестве прокси-сервера для службы Azure-SSIS IR в ADF.
```
...
Командлет обновите среду выполнения интеграции Azure-SSIS для использования саморазмещенной среды выполнения интеграции в качестве прокси-сервера данных.
...
### -DataProxyStagingPath
Путь в хранилище промежуточных данных, который будет использоваться при перемещении данных между Self-Hostedми и средами выполнения интеграции Azure-SSIS, используется контейнер по умолчанию, если он не указан
...
### -LicenseType
Тип лицензии, которую вы хотите выбрать для ИК-связи служб SSIS. Существует два типа: LicenseIncluded или BasePrice. Если вы ищете цены на использование гибридной льготы для использования Azure (AHUB), пожалуйста, выберите BasePrice. Если это не так, пожалуйста, выберите LicenseIncluded.
