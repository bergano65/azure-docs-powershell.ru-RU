...
## NОПИСАНИЕ
Используется в сочетании с Update-AzPeering, является операцией с памятью и сохраняется только в `Update-AzPeering` . 

## ИЛЛЮСТРИРУЮТ

### Переход на другую пропускную способность
```powershell
...
Обновляет пропускную способность первого соединения в объекте пиринга в памяти. 

### Обновление адреса сеанса BGP
```powershell
...
Обновляет адрес пиринга для первого соединения в объекте пиринга в памяти. 

### Обновление использования службы пиринга
```powershell
...
Обновляет подключение для использования службой пиринга
