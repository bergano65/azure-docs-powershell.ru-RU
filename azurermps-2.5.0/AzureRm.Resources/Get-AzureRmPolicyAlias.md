...
## КРАТКИй обзор
Get-AzureRmPolicyAlias извлекает и выводит типы ресурсов поставщика Azure, которые имеют определенные псевдонимы и соответствуют указанным значениям параметров. Если не указаны никакие параметры, будут выведены все типы ресурсов поставщика, которые содержат псевдоним.
Параметр-ListAvailable изменяет это поведение, передавая список всех соответствующих типов ресурсов, включая те, у которых нет псевдонимов.
...
## NОПИСАНИЕ
Командлет **Get-AzureRmPolicyAlias** получает список псевдонимов политик.
Псевдонимы политик используются политикой Azure для ссылки на свойства типа ресурса.
Указаны параметры, которые ограничивают элементы в списке, сопоставляя различные свойства типа ресурса или его псевдонимов.
Указанное значение соответствия соответствует условию, что Целевая строка включает в себя сравнение с учетом регистра.
...
Список всех типов ресурсов поставщика с псевдонимом.
...
Список всех типов ресурсов поставщика, включая те, у которых нет псевдонимов.
...
Список всех типов ресурсов поставщика, для которых пространство имен совпадает с "вычисляемым" и содержит псевдоним.
...
Список всех типов ресурсов поставщика, тип ресурсов которых соответствует "Virtual" и содержащих псевдонимы.
...
Список всех типов ресурсов поставщика, тип ресурсов которых соответствует "Virtual", в том числе для тех, у которых нет псевдонимов.
...
Список всех типов ресурсов поставщика, пространство имен которых соответствует "COMPUTE", а тип ресурса — "виртуальный" и содержит псевдоним.
Примечание:-NamespaceMatch и-ResourceTypeMatch предоставляют исключительные совпадения, а другие — включительно.
...
Список всех типов ресурсов поставщика, которые содержат псевдоним, совпадающий с "виртуальным".
...
Выводит список всех типов ресурсов поставщика, которые содержат псевдонимы, соответствующие виртуальному, или псевдоним с путем, соответствующим "Network".
...
Список всех типов ресурсов поставщика с версией Alpha API или псевдонимом с альфа-версией API.

## ПАРАМЕТРЫ

### -AliasMatch
Включает в выходные элементы с псевдонимами, имена которых совпадают с этим значением.
```yaml
Type: String
Parameter Sets: (All)
Aliases: Alias
...
### -ApiVersion
Если этот параметр установлен, указывает версию используемого API поставщика ресурсов. Если не указано, версия API автоматически определяется как самая свежая доступная.
```yaml
Type: String
Parameter Sets: (All)
Aliases:
...
### -ApiVersionMatch
Включает в выходные элементы, для которых типы ресурсов или псевдонимы имеют соответствующую версию API.
```yaml
Type: String
Parameter Sets: (All)
Aliases:
...
### -ListAvailable
Включает в себя элементы, совпадающие с псевдонимами, и без них.
```yaml
...
### -LocationMatch
Включает в выходные элементы, для которых типы ресурсов соответствуют расположениям.
```yaml
Type: String
Parameter Sets: (All)
Aliases: Location
...
### -NamespaceMatch
Ограничивает вывод для элементов, пространство имен которых соответствует этому значению.
```yaml
Type: String
Parameter Sets: (All)
...
### -PathMatch
Включает в выходные элементы с псевдонимами, которые содержат путь, совпадающий с этим значением.
```yaml
Type: String
Parameter Sets: (All)
Aliases: Path
...
### -ResourceTypeMatch
Ограничивает вывод для элементов, тип ресурсов которых соответствует этому значению.
```yaml
Type: String
Parameter Sets: (All)
...
### Microsoft. Azure. Commands. ResourceManager. командлеты. Реализация. PsResourceProviderAlias

## Пуск

* Чтобы развернуть псевдоним или любое другое свойство, переводите его в вертикальный канал `select -ExpandProperty <property>` . Например: `Get-AzureRmPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* Дополнительные свойства доступны в выходных данных и могут отображаться путем передачи выходных данных в поток вывода `Format-List` . Например: `Get-AzureRmPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
