...
## NОПИСАНИЕ
Командлет **New-AzPolicySetDefinition** создает определение политики.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Создание определения политики с метаданными с помощью файла Set политики
```
[
   {
...
Эта команда создает определение политики с именем VMPolicySetDefinition с метаданными, указывающими на ее категорию "виртуальная машина", которая содержит определения политик, указанные в C:\VMPolicy.json. Ниже приведен пример содержимого VMPolicy.json.

### Пример 2: Создание определения параметризованной политики
```
[
   {
      "policyDefinitionId": "/providers/Microsoft.Authorization/policyDefinitions/2a0e14a6-b0a6-4fab-991a-187a4f81c498",
...
Эта команда создает параметризованный набор политик с именем VMPolicySetDefinition, который включает в себя определения политик, указанные в C:\VMPolicy.json. Ниже приведен пример содержимого VMPolicy.json.

### Пример 3: Создание определения политики с помощью групп определения политики
```
[
   {
      "policyDefinitionId": "/providers/Microsoft.Authorization/policyDefinitions/2a0e14a6-b0a6-4fab-991a-187a4f81c498",
...
Эта команда создает определение политики с именем VMPolicySetDefinition и задает группировку определений политик, указанных в C:\VMPolicy.json. Ниже приведен пример содержимого VMPolicy.json.
...
### -DisplayName
Отображаемое имя определения множества политик.
...
### -GroupDefinition
Группы определений политики для нового определения множества политик. Это может быть либо путь к файлу, содержащему группы, либо группам в виде строки JSON.
...
### -ManagementGroupName
Имя группы управления для нового определения набора политик.
...
### -Metadata
Метаданные для определения Set политики. Это может быть либо путь к имени файла, содержащему метаданные, либо метаданные в виде строки JSON.
...
### -Параметр
Объявление параметров для определения задания политики.
Это может быть путь к имени файла, содержащему объявление параметров, или объявление параметров в виде строки JSON.
...
### -PolicyDefinition
Определения политики. Это может быть путь к имени файла, содержащему определения политики, или определениям политики в виде строки JSON.
...
### -SubscriptionId
Идентификатор подписки нового определения набора политики.
