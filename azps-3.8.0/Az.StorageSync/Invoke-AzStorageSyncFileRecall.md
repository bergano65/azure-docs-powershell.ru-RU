...
## NОПИСАНИЕ
При включении уровня облака в конечной точке сервера (определенном месте на зарегистрированном сервере) можно использовать эту команду для отзыва всех многоуровневых файлов на локальном диске. Перед началом отзыва настоятельно рекомендуется отключить уровень облака в этой конечной точке сервера. Если уровень по-прежнему включен, отзыв может привести к другим файлам с повышенными разрешениями, чтобы получить необходимую цель всего содержимого на локальном диске.
...
Эта команда рекурсивно отзывает все многоуровневые файлы, расположенные в корневом пути указанной конечной точки сервера.
...
### -RecallPath
Отзыв пути, который нужно отозвать.
