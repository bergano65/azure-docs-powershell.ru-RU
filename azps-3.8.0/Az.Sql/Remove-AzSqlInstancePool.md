...
## NОПИСАНИЕ
Командлет **Remove-AzSqlInstancePool** удаляет пул экземпляров Azure SQL.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Удаление пула экземпляров
```powershell
PS C:\> Remove-AzSqlInstancePool -ResourceGroup resourcegroup01 -Name instancePool0
```

### Пример 2: Удаление пула экземпляров по его идентификатору ресурса
```powershell
...
### Пример 3: Удаление пула экземпляров с помощью объекта пула экземпляров
```powershell
...
Эта команда удаляет пул экземпляров с именем instancePool0.
...
### -InputObject
Удаляемый объект пула экземпляров.
...
### -ResourceId
Идентификатор ресурса пула экземпляров, который нужно удалить.
