...
## NОПИСАНИЕ
Командлет **Remove-AzSqlServerAudit** удаляет параметры аудита для Azure SQL Server.
Укажите параметры *ResourceGroupName* и *ServerName* для идентификации сервера.

## ИЛЛЮСТРИРУЮТ

### Пример 1: удаление параметров аудита для Azure SQL Server
```
PS C:\>Remove-AzSqlServerAudit -ResourceGroupName "resourcegroup01" -ServerName "server01"
```

### Пример 2: удаление с помощью конвейера параметры аудита для Azure SQL Server
```
