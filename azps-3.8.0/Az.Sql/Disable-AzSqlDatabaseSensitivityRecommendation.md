...
## NОПИСАНИЕ
Командлет Disable-AzSqlDatabaseSensitivityRecommendation отключает (пропуски) рекомендации по пометкам столбцов в базе данных.

## ИЛЛЮСТРИРУЮТ

### Пример 1: отключение рекомендаций по конфиденциальности для определенного столбца в базе данных SQL Azure.
```powershell
...
### Пример 2. Отключите рекомендации по пометкам для столбцов, которые содержат рекомендации по пометкам в базе данных Azure SQL с использованием конвейера.
```powershell
...
### Пример 3. Отключите рекомендации по отметкам для определенного столбца в базе данных Azure SQL с помощью конвейера.
```powershell
