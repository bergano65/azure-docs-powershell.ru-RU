...
### -Destination
Определите относительный URL-адрес, на который будут переписываться указанные выше запросы.
...
### -PreservePath
Следует ли сохранять несовпадающие пути.
...
### -QueryParameter
Параметры запроса, которые нужно включить или исключить (с разделителями-запятыми).
...
### -QueryStringBehavior
Поведение запроса QueryString для запросов
...
### -SourcePattern
Определите шаблон URI запроса, указывающий тип запросов, которые можно переписать. Если значение пусто, все строки совпадают.
