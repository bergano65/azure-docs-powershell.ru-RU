...
## NОПИСАНИЕ
Функция **Get-AzApiManagementDiagnostic** получает подробные сведения о диагностике, настроенном в службе управления API в заданной области.

## ИЛЛЮСТРИРУЮТ

### Пример 1: получение всех диагностических сведений, настроенных в области клиента.
```powershell
...
Эта команда получает все данные о диагностике, настроенные в службе управления API.

### Пример 2: получение всех диагностических сведений, настроенных в области API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Эта команда получает все элементы диагностики, настроенные для `echo-api` области API.

### Пример 3: получение диагностики области API, указанной идентификатором
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Эта команда возвращает `applicationinsights` диагностику, настроенную в API `echo-api` .

## ПАРАМЕТРЫ

### -ApiId
Идентификатор существующего API.
Если указана функция, будет возвращена Диагностика области API.
...
Если задано, будет возвращена политика области продукта.
...
### -ResourceId
Идентификатор ресурса ARM для диагностики или диагностики API. Если задано значение, оно попытается найти диагностику с помощью идентификатора. Этот параметр является обязательным.
