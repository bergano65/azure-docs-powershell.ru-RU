...
## NОПИСАНИЕ
Командлет **Set-AzBatchTask** обновляет свойства задачи в пакетной службе Azure.
Используйте командлет Get-AzBatchTask, чтобы получить объект **PSCloudTask** .
...
Первая команда получает задачу с помощью **Get-AzBatchTask** , а затем сохраняет ее в переменной $Task.
