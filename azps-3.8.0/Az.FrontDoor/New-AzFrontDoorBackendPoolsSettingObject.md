...
## NОПИСАНИЕ
Командлет **New-AzFrontDoorBackendpoolsSettingObject** создает новый объект PSBackendPoolsSettings для создания передней дверцы.

## ИЛЛЮСТРИРУЮТ

### Пример 1: создание объекта BackendPoolsSettings с помощью значений по умолчанию
```powershell
...
Следует ли применять проверку имени сертификата для запросов HTTPS ко всем пулам баз данных.
Не влияет на запросы, не относящиеся к HTTPS.
...
Отправка и получение таймаута для запроса на переадресацию на сервер. При достижении тайм-аута запрос завершается сбоем и возвращает результат.
