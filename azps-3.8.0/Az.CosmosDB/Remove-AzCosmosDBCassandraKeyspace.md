...
## КРАТКИй обзор
Удаление CosmosDB Cassandra keyspace.
...
## NОПИСАНИЕ
Командлет **Remove-AzCosmosDBCassandraKeyspace** удаляет существующий CosmosDB Cassandra keyspace.
...
Командлет возвращает объект типа bool (параметр when-PassThru передается), который является истинным, если удаление прошло успешно.
