...
### -Подписавшего
Указывает веб-токен JSON RFC7519, содержащий утверждение "консультанты-policyCertificate", значение которого является веб-ключом JSON RFC7517, который содержит доверенный ключ подписи, который нужно удалить.
