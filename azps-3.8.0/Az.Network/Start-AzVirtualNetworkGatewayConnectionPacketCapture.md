...
```
### Пример 3
Пример захвата пакетов для захвата всех внутренних и внешних пакетов
```powershell
...
### -FilterData
Параметры фильтра для запуска захвата пакетов на подключении к шлюзу виртуальной сети.
...
### -InputObject
Объект подключения к шлюзу виртуальной сети, для которого необходимо запустить сбор пакетов.
...
### -Name (имя)
Имя подключения к шлюзу виртуальной сети, в котором нужно запустить сбор пакетов.
...
### -ResourceId
Идентификатор ресурса Azure VirtualNetworkGatewayConnection, в котором запускается захват пакетов.
