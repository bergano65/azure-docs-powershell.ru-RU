...
Этот пример создает группу ресурсов, виртуальную глобальную сеть, виртуальный концентратор, масштабируемый шлюз ExpressRoute в центральной стране США и сразу же удаляет его. Чтобы отключить вывод запроса при удалении виртуального шлюза, используйте флаг-force.
Это приведет к удалению ExpressRouteGateway и всех ExpressRouteConnections, прикрепленных к нему.
...
В этом примере создается группа ресурсов, виртуальная сеть глобальной сети, виртуальный концентратор, масштабируемый шлюз ExpressRoute и сразу же удаляются. Это удаление выполняется с помощью оболочки PowerShell, в которой используется объект ExpressRouteGateway, возвращенный командой "Get-AzExpressRouteGateway".
...
### -ResourceId
Идентификатор ресурса Azure для ExpressRouteGateway, который нужно удалить.
