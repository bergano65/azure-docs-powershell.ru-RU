...
## NОПИСАНИЕ
Командлет **Set-AzVirtualNetworkGatewayConnection** настраивает подключение к шлюзу виртуальной сети.
...
### Пример 2: Добавление и обновление тегов в существующем VirtualNetworkGatewayConnection
```
$conn = Get-AzVirtualNetworkGatewayConnection -Name 1 -ResourceGroupName myRG
...
### -EnableBgp
Необходимость использования сеанса BGP через туннель VPN S2S
...
### -DpdTimeoutInSeconds
Таймаут обнаружения однорангового элемента для соединения в секундах
...
Следует ли использовать PrivateIP для S2S-соединения
...
Следует ли использовать селекторы трафика на основе политики для S2S-соединения
...
Указывает объект PSVirtualNetworkGatewayConnection, используемый этим командлетом для изменения подключения к шлюзу виртуальной сети.
