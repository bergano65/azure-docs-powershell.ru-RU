...
## NОПИСАНИЕ
Командлет **New-AzContainerNicConfigIpConfig** создает новую конфигурацию IP-интерфейса сети контейнера. 
...
Первые две команды создают и инициализируют виртуальную сеть и подсеть. Третья команда создает контейнерный профиль IP NIC конфигурации, указывающий на созданную подсеть. Четвертая команда создает контейнер конфигурации сетевого интерфейса, на котором указан профиль конфигурации IP, созданный в предыдущей команде. Наконец, пятая команда создает сетевой профиль, инициализируемый с помощью конфигурации контейнера сетевого интерфейса, хранящейся в $containerNicConfig.
...
### -Name (имя)
Имя профиля IP-конфигурации сетевого интерфейса контейнера.
