...
## NОПИСАНИЕ
Командлет **New-AzApplicationGatewayRewriteRule** создает правило повторной записи для шлюза приложений Azure.

## ИЛЛЮСТРИРУЮТ

### Пример 1: Создание правила повторной записи для шлюза приложения
```powershell
PS C:\> $rule = New-AzApplicationGatewayRewriteRule -Name rule1 -ActionSet $action -RuleSequence 101 -Condition $condition
```

Эта команда создает правило перезаписи с именем rule1 и сохраняет результат в переменной с именем $rule.

## ПАРАМЕТРЫ

### -В качестве действия
Набор макрокоманд для правила перезаписи
...
### -Условие
Условие для правила перезаписи для выполнения
...
### -RuleSequence
Порядок правил для этого правила перезаписи в наборе правил перезаписи
