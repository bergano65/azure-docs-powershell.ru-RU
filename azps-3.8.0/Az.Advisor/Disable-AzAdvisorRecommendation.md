...
## КРАТКИй обзор
Отключите рекомендации по Azure Advisor.
...
## NОПИСАНИЕ
Создание подавления для рекомендаций (s) это позволяет отложить определенные рекомендации для определенной длительности или бесконечности.
...
Создавайте подавление для указанного имени рекомендации, указав значение по умолчанию (SuppressionName) и дни по умолчанию как-1.
...
Для данного ИД рекомендации создается подавление.
...
Создание подавления от Get-AzAdvisorRecommendation до отключения-AzAdvisorRecommendation.
...
### -RecommendationName
ResourceName (значение) рекомендации
...
### -ResourceId
Идентификатор рекомендации, которое нужно отключить.
...
### Microsoft. Azure. Commands. Advisor. командлеты. Models. PsAzureAdvisorSuppressionContract
