...
## NОПИСАНИЕ
Командлет **Set-AzVMDiskEncryptionExtension** позволяет выполнять шифрование на работающей инфраструктуре в качестве виртуальной машины службы (IaaS) в Azure.  Это обеспечивает шифрование, устанавливая расширение шифрования диска на виртуальной машине. 
...
Linux: параметр **VolumeType** требуется при шифровании виртуальных машин Linux, для которого необходимо установить значение ("OS", "Data" или "ALL"), которое поддерживается дистрибутивом Linux. 

Windows: параметр **VolumeType** можно опустить, в противном случае операция по умолчанию имеет значение ALL; Если параметр VolumeType присутствует для виртуальной машины Windows, для него должно быть задано значение ALL или OS.
...
В этом примере включается шифрование на виртуальной машине без указания учетных данных Active Directory.

### Пример 2: Включение шифрования с помощью конвейерного ввода
```
...
Этот пример отправляет параметры с помощью конвейера ввода для включения шифрования на виртуальной машине без указания учетных данных Active Directory.

### Пример 3: Включите шифрование с помощью идентификатора клиента Azure AD и секретного клиента
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
В этом примере идентификатор клиента Azure AD и секрет клиента используются для включения шифрования на виртуальной машине.

### Пример 4: Включение шифрования с помощью идентификатора клиента Azure AD и отпечатка сертификата клиента
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
В этом примере для включения шифрования на виртуальной машине используется идентификатор клиента Azure AD и отпечатки сертификата клиента.

### Пример 5: Включите шифрование с помощью идентификатора клиента Azure AD, секрета клиента и ключа шифрования диска с помощью ключа шифрования ключа.
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
...
В этом примере идентификатор клиента Azure AD и секрет клиента используются для включения шифрования на виртуальной машине и переноса ключа шифрования диска с помощью ключа шифрования ключа.

### Пример 6. Включите шифрование с помощью идентификатора клиента Azure Active Directory, отпечатка сертификата клиента и обтекания диска EncryptionKey с помощью ключа шифрования ключей.
```
$RGName = "MyResourceGroup"
$VMName = "MyTestVM"
#The KeyVault must have enabledForDiskEncryption property set on it
...
В этом примере используется идентификатор клиента Azure AD и отпечаток сертификата клиента, чтобы включить шифрование на виртуальной машине, а также переключить ключ шифрования диска с помощью ключа шифрования ключа.
...
### -Name (имя)
Указывает имя ресурса диспетчера ресурсов Azure, представляющего расширение. Если параметр *Name* не указан, установленное расширение будет называться AzureDiskEncryption на виртуальных машинах Windows и AzureDiskEncryptionForLinux на виртуальных машинах Linux.
...
### -VolumeType
Указывает тип томов виртуальных машин, для которых требуется выполнить операцию шифрования: ОС, данные или все. 
